...
fn impl_struct(input: Struct) -> TokenStream {
    ...
    let content = if input.attrs.transparent.is_some() {
        ...
        quote! {
            ...
            let error: bsmr_error::Error = #arg_token.#member.into();
            error.tag(tags)
        }
        ...
        quote! {
            ...
            let source_location = bsmr_error::source_location::SourceLocation::new(core::file!(), core::line!()).with_type_name(#source_location_type_name);
            let root_error = bsmr_error::Error::new(format!("{}", #arg_token), tags[0], source_location, None);
            root_error.tag(tags)
        }
    };
    ...
    quote! {
        #[allow(unused_qualifications)]
        impl #impl_generics From<#ty #ty_generics> for bsmr_error::Error #error_where_clause
        {
            #[cold]
            #[allow(unused_assignments, unused_variables, deprecated)]
            fn from(#arg_token: #ty #ty_generics) -> bsmr_error::Error {
                #content
            }
        }
    ...
    let from_impl_arms = input.variants.iter().map(|variant| {
        ...
        let content = if variant.attrs.transparent.is_some() {
            ...
            quote! {
                ...
                let error: bsmr_error::Error = #only_field.into();
                error.tag(tags)
            }
            ...
            quote! {
                ...
                let error = bsmr_error::Error::from(#member);
                let error = error.tag(tags);
            ...
            quote! {
                ...
                let source_location = bsmr_error::source_location::SourceLocation::new(core::file!(), core::line!()).with_type_name(#source_location_type_name);
                let root_error = bsmr_error::Error::new(err_msg, tags[0], source_location, None);
                root_error.tag(tags)
            }
        };
...
/// Generates the from implementation to bsmr_error for a provided error type
fn get_tags(attrs: &Attrs) -> TokenStream {
    ...
    if let Some(tags_expr) = tags_expr {
        quote! {
            let mut tags: Vec<bsmr_error::ErrorTag> = #tags_expr;
