    ...
    fn validate(&self) -> Result<()> {
        ...
        if self.attrs.tags.is_empty() && self.attrs.tags_expr.is_none() {
            return Err(Error::new(
                self.ident.span(),
                "missing #[bsmr(tag = ...)] attribute. All 'bsmr_error' must have a tag (Common choices are 'Input', 'Tier0', and 'Environment').
                Full list: https://www.internalfb.com/code/fbsource/[4e97d95ed7f3145bf6828827e779eecaeb0c5712]/fbcode/bsmr/app/bsmr_data/error.proto?lines=42",
            ));
        }
    ...
    fn validate(&self) -> Result<()> {
        ...
        for variant in &self.variants {
                ...
                && self.attrs.tags_expr.is_none()
            {
                return Err(Error::new_spanned(
                    variant.original,
                    "missing #[bsmr(tag = ...)] attribute. All 'bsmr_error' must have a tag (Default choices are 'Input', 'Tier0', and 'Environment').
                    Full list: https://www.internalfb.com/code/fbsource/[4e97d95ed7f3145bf6828827e779eecaeb0c5712]/fbcode/bsmr/app/bsmr_data/error.proto?lines=42",
                ));
            }
        }
...
fn check_field_attrs(fields: &[Field]) -> Result<()> {
    ...
    for field in fields {
        ...
        if let Some(style) = field.attrs.tags.first() {
            return Err(Error::new(
                style.span(),
                "not expected here; the #[bsmr(...)] attribute belongs on top of a struct or an enum variant",
            ));
        }
    }
