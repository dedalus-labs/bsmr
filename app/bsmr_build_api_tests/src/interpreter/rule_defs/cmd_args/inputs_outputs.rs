...
#[test]
fn test_inputs_outputs() -> bsmr_error::Result<()> {
...
#[test]
fn test_ignore_artifacts() -> bsmr_error::Result<()> {
    let mut tester = tester()?;
    tester.run_starlark_bzl_test(indoc!(
        r#"
        def norm(xs):
        ...
        def test():
            ...
            assert_eq_ignore_hash(["bsmr-out/default/art/root/<HASH>/__dep2__/dir/quz.h"], get_args(cli))
        "#
    ))?;
    Ok(())
}
...
#[test]
fn test_frozen_inputs_outputs() -> bsmr_error::Result<()> {
    let mut tester = tester()?;
