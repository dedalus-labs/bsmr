...
use bsmr_build_api::interpreter::rule_defs::provider::collection::tester::collection_creator;
...
fn provider_collection_tester() -> bsmr_error::Result<Tester> {
...
#[test]
fn provider_collection_constructs_properly() -> bsmr_error::Result<()> {
...
#[test]
fn provider_collection_fails_to_construct_on_bad_data() -> bsmr_error::Result<()> {
    let mut tester = provider_collection_tester()?;
...
#[test]
fn returns_default_info() -> bsmr_error::Result<()> {
    let mut tester = provider_collection_tester()?;
    tester.run_starlark_bzl_test(indoc!(
        r#"
...
#[test]
fn provider_collection_contains_methods_and_in_operator() -> bsmr_error::Result<()> {
    let mut tester = provider_collection_tester()?;
    tester.add_import(
...
#[test]
fn provider_collection_get() -> bsmr_error::Result<()> {
    let mut tester = provider_collection_tester()?;
    tester.add_import(
        &ImportPath::testing_new("root//providers:defs.bzl"),
        indoc!(
