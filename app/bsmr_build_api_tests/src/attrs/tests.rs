...
use bsmr_interpreter_for_build::attrs::coerce::testing::to_value;
use bsmr_interpreter_for_build::interpreter::selector::register_select;
use bsmr_node::attrs::attr_type::AttrType;
use bsmr_node::attrs::coerced_deps_collector::CoercedDepsCollector;
use bsmr_node::attrs::configurable::AttrIsConfigurable;
use bsmr_node::attrs::configuration_context::AttrConfigurationContext;
use bsmr_node::attrs::configured_attr_info_for_tests::ConfiguredAttrInfoForTests;
use bsmr_node::attrs::display::AttrDisplayWithContextExt;
use bsmr_node::attrs::fmt_context::AttrFmtContext;
use bsmr_node::attrs::testing::configuration_ctx;
...
#[test]
fn test_string() -> bsmr_error::Result<()> {
    Module::with_temp_heap(|env| {
        let globals = GlobalsBuilder::standard().with(register_select).build();
...
#[test]
fn test_invalid_concat_coercion_into_one_of() -> bsmr_error::Result<()> {
    Module::with_temp_heap(|env| {
        let globals = GlobalsBuilder::standard().with(register_select).build();
...
#[test]
fn test_option() -> bsmr_error::Result<()> {
    Heap::temp(|heap| {
...
#[test]
fn test_dict() -> bsmr_error::Result<()> {
    Module::with_temp_heap(|env| {
        let globals = GlobalsBuilder::standard().with(register_select).build();
...
#[test]
fn test_one_of() -> bsmr_error::Result<()> {
    Heap::temp(|heap| {
...
#[test]
fn test_label() -> bsmr_error::Result<()> {
    Heap::temp(|heap| {
        let value = heap.alloc(vec!["//some:target", "cell1//named:target[foo]"]);
...
#[test]
fn test_coerced_deps() -> bsmr_error::Result<()> {
    Module::with_temp_heap(|env| {
        let globals = GlobalsBuilder::standard().with(register_select).build();
...
#[test]
fn test_configured_deps() -> bsmr_error::Result<()> {
    Module::with_temp_heap(|env| {
        let globals = GlobalsBuilder::standard().with(register_select).build();
...
#[test]
fn test_resolved_deps() -> bsmr_error::Result<()> {
    Module::with_temp_heap(|env| {
        let globals = GlobalsBuilder::standard()
            .with(register_select)
            .with(bsmr_build_api::interpreter::rule_defs::register_rule_defs)
            .build();
...
#[test]
fn test_dep_requires_providers() -> bsmr_error::Result<()> {
    Module::with_temp_heap(|env| {
...
#[test]
fn test_source_label() -> bsmr_error::Result<()> {
    Heap::temp(|heap| {
...
#[test]
fn test_source_label_deps() -> bsmr_error::Result<()> {
    Module::with_temp_heap(|env| {
        let globals = GlobalsBuilder::standard().with(register_select).build();
...
#[test]
fn test_source_label_resolution() -> bsmr_error::Result<()> {
    fn resolve_and_test(
        content: &str,
        test_content: &str,
        files: &[&str],
...
#[test]
fn test_single_source_label_fails_if_multiple_returned() -> bsmr_error::Result<()> {
    Module::with_temp_heap(|env| {
        Heap::temp(|heap| {
...
#[test]
fn test_bool() -> bsmr_error::Result<()> {
    Module::with_temp_heap(|env| {
        let globals = GlobalsBuilder::standard().with(register_select).build();
...
#[test]
fn test_user_placeholders() -> bsmr_error::Result<()> {
    Module::with_temp_heap(|env| {
        let globals = GlobalsBuilder::standard()
            .with(register_select)
            .with(register_builtin_providers)
...
#[test]
fn test_select_incompatible_configure() -> bsmr_error::Result<()> {
    let attr = AttrType::string();
    let coerced = bsmr_node::attrs::coerced_attr::CoercedAttr::SelectIncompatible(
        bsmr_util::arc_str::ArcStr::from("not supported on this platform"),
    );
