...
use bsmr_build_api::actions::calculation::command_details;
use bsmr_build_api::actions::execute::dice_data::CommandExecutorResponse;
use bsmr_build_api::actions::execute::dice_data::HasCommandExecutor;
use bsmr_build_api::actions::execute::dice_data::SetCommandExecutor;
use bsmr_build_api::actions::execute::dice_data::SetInvalidationTrackingConfig;
...
use bsmr_build_api::actions::registry::RecordedActions;
...
use bsmr_execute::execute::testing_dry_run::DryRunEntry;
...
fn mock_analysis_for_action_resolution(
    ...
    dice_builder = dice_builder.mock_and_return(
        ...
        bsmr_error::Ok(MaybeCompatible::Compatible(AnalysisResult::new(
...
) -> bsmr_error::Result<DiceTransaction> {
        ...
        fn get_command_executor(
            ...
            Ok(CommandExecutorResponse {
                ...
                output_trees_download_config:
                    bsmr_execute::re::output_trees_download_config::OutputTreesDownloadConfig::new(
                        None, true,
                    ),
            })
        }
...
#[tokio::test]
async fn test_get_action_for_artifact() -> bsmr_error::Result<()> {
...
#[tokio::test]
async fn test_build_action() -> bsmr_error::Result<()> {
    bsmr_certs::certs::maybe_setup_cryptography();
...
#[tokio::test]
async fn test_build_artifact() -> bsmr_error::Result<()> {
    bsmr_certs::certs::maybe_setup_cryptography();
    let temp_fs = ProjectRootTemp::new()?;
    let build_artifact = create_test_build_artifact();
    let registered_action = registered_action(
...
#[tokio::test]
async fn test_ensure_artifact_build_artifact() -> bsmr_error::Result<()> {
    bsmr_certs::certs::maybe_setup_cryptography();
    let temp_fs = ProjectRootTemp::new()?;
    let build_artifact = create_test_build_artifact();
    let registered_action = registered_action(
...
#[tokio::test]
async fn test_ensure_artifact_source_artifact() -> bsmr_error::Result<()> {
    bsmr_certs::certs::maybe_setup_cryptography();
...
#[tokio::test]
async fn test_ensure_artifact_external_symlink() -> bsmr_error::Result<()> {
    bsmr_certs::certs::maybe_setup_cryptography();
    let path = CellPath::new(
        CellName::testing_new("cell"),
