...
struct Validate<'v>(#[trace(unsafe_ignore)] fn(Value<'v>) -> bsmr_error::Result<()>);
...
impl<'v> StarlarkPromise<'v> {
    ...
    pub fn map(
        ...
        f: StarlarkCallable<'v, (Value<'v>,), Value<'v>>,
        eval: &mut Evaluator<'v, '_, '_>,
    ) -> bsmr_error::Result<ValueTyped<'v, StarlarkPromise<'v>>> {
        match x.get() {
            Some(x) => Ok(eval
                .heap()
    ...
    pub fn validate(&self, f: fn(Value<'v>) -> bsmr_error::Result<()>) -> bsmr_error::Result<()> {
        match self.get() {
            Some(x) => f(x),
            _ => {
...
mod tests {
        ...
        fn promise_validate<'v>(
            ...
            promise.validate(|x| {
                if x.unpack_str() == Some("ok") {
                    ...
                    Err(bsmr_error!(bsmr_error::ErrorTag::Tier0, "VALIDATE_FAILED"))
                }
            })?;
            Ok(NoneType)
        }
    ...
    fn assert_promise<'v>(modu: &Module<'v>, content: &str) -> bsmr_error::Result<Value<'v>> {
        alloc_promises(modu);
    ...
    fn assert_promise_err<'v>(modu: &Module<'v>, content: &str, err: &str) -> bsmr_error::Error {
