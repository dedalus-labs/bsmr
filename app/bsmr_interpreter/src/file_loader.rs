...
fn to_diagnostic(err: &bsmr_error::Error, id: &str) -> bsmr_error::Error {
...
impl InterpreterFileLoader {
    ...
    fn find_module(&self, id: StarlarkModulePath) -> bsmr_error::Result<&FrozenModule> {
...
mod tests {
    ...
    use super::*;
    use crate::testing::BsmrTestHeapName;
    ...
    impl LoadResolver for TestLoadResolver {
        fn resolve_load(
            ...
            match path {
                ...
                _ => Err(bsmr_error!(bsmr_error::ErrorTag::Tier0, "error")),
            }
        }
    }
    ...
    fn env(name: StarlarkModulePath) -> FrozenModule {
        ...
        Module::with_temp_heap(|m| {
            ...
            m.freeze_named(BsmrTestHeapName::frozen_heap_name())
        })
        .unwrap()
    }
    ...
    #[test]
    fn no_resolution() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn missing_in_loaded_modules() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn valid_load() -> bsmr_error::Result<()> {
        let path = "cell1//next/package:import.bzl".to_owned();
        let resolver = resolver();
    ...
    #[test]
    fn valid_find() -> bsmr_error::Result<()> {
        let path = "cell1//next/package:import.bzl".to_owned();
        let resolver = resolver();
