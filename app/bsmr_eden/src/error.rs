...
pub enum IsMountReadyError {
    ...
    #[bsmr(tag = IoEdenMountDoesNotExist)]
    MountDoesNotExist { mount: AbsPathBuf },
    #[error(transparent)]
    #[bsmr(tag = IoEdenRequestError)]
...
pub enum ConnectAndRequestError<E> {
    #[error(transparent)]
    #[bsmr(tag = IoEdenConnectionError)]
    ConnectionError(bsmr_error::Error),
