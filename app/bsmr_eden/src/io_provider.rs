...
use bsmr_common::io::ReadDirOutcome;
use bsmr_common::io::fs::FsIoProvider;
use bsmr_common::io::fs::ReadUncheckedOptions;
use bsmr_core;
...
use crate::semaphore::bsmr_default;
...
impl EdenIoProvider {
    pub async fn new(
        ...
        let eden_semaphore = bsmr_default();
        ...
        if let Some(eden_version) = &eden_version {
            if eden_version.as_str() < min_eden_version {
                tracing::warn!(
                    ...
                    Update Eden then restart Bessemer to use Eden I/O.",
                    eden_version,
                    min_eden_version
                );
                return Ok(None);
    ...
    ) -> bsmr_error::Result<PathMetadataResult> {
    ...
    async fn read_dir_impl(
        &self,
        path: ProjectRelativePathBuf,
    ) -> bsmr_error::Result<ReadDirOutcome> {
    ...
    async fn read_file_if_exists_impl(
        ...
        match res {
            Ok(res) => match res.blob {
                        ...
                        EdenError::PosixError { error, code } if code == libc::EFBIG => {
                            ...
                            soft_error!(
                                ...
                                bsmr_error::bsmr_error!(
                                    ...
                                    Set env var `BSMR_DISABLE_EDEN_THRIFT_READ=true` if this is constantly an issue: {:#}",
                                    error
                                ),
                            )
                            .ok();
                ...
                ScmBlobOrError::UnknownField(code) => Err(bsmr_error::bsmr_error!(
                    bsmr_error::ErrorTag::IoEdenUnknownField,
...
impl IoProvider for EdenIoProvider {
    async fn read_path_metadata_if_exists_impl(
        &self,
        path: ProjectRelativePathBuf,
    ) -> bsmr_error::Result<Option<RawPathMetadata<ProjectRelativePathBuf>>> {
    ...
    async fn read_file_if_exists_impl(
        ...
        if bsmr_env!("BSMR_ENABLE_EDEN_THRIFT_READ", bool).unwrap_or(false)
    ...
    async fn settle(&self) -> bsmr_error::Result<()> {
    ...
    async fn eden_version(&self) -> bsmr_error::Result<Option<String>> {
