...
pub mod semaphore {
    ...
    /// A bsmr-specific semaphore that is used to limit the number of outstanding requests to
    /// EdenFS. Reads bsmr specific environment variable "BSMR_EDEN_SEMAPHORE" to determine the
    /// number of permits.
    pub fn bsmr_default() -> Semaphore {
        let permits =
            bsmr_env!("BSMR_EDEN_SEMAPHORE", type=usize, default=DEFAULT_MAX_OUTSTANDING_REQUESTS, applicability=internal)
