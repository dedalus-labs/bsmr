...
impl EdenConnectionManager {
    ...
    fn get_eden_connector(
    ...
    ) -> bsmr_error::Result<EdenConnector> {
        ...
        if cfg!(windows) {
            ...
            let config: EdenConfig = toml::from_str(&config_contents)
                .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::IoEdenConfigError))?;
    ...
    pub async fn get_eden_version(&self) -> bsmr_error::Result<Option<String>> {
        let fb303 = self.connector.connect_fb303()?;
        let values = fb303
            .getRegexExportedValues("^build_.*")
            .await
            .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::IoEdenVersionError))?;
                ...
                Ok(res) => {
                    ...
                    if retries > 0 {
                        soft_error!(
                            ...
                            bsmr_error!(bsmr_error::ErrorTag::Input, "Eden IO retried {} times", retries),
                            quiet: true
                        ).ok();
                    }
                    break Ok(res);
...
type EdenClientFuture =
    Shared<BoxFuture<'static, bsmr_error::Result<Arc<dyn EdenService + Send + Sync>>>>;
...
fn thrift_builder(
    fb: FacebookInit,
    socket: &AbsPathBuf,
) -> bsmr_error::Result<::thriftclient::ThriftChannelBuilder> {
...
impl EdenConnector {
    ...
    fn connect_fb303(&self) -> bsmr_error::Result<Arc<dyn BaseService + Send + Sync>> {
