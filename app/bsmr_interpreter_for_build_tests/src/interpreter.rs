...
use bsmr_interpreter_for_build::interpreter::testing::CellsData;
use bsmr_interpreter_for_build::interpreter::testing::Tester;
use bsmr_interpreter_for_build::interpreter::testing::run_simple_starlark_test;
use bsmr_node::attrs::inspect_options::AttrInspectOptions;
use bsmr_node::nodes::unconfigured::testing::targets_to_json;
use dupe::Dupe;
...
fn test_root_import() {
    let mut tester = Tester::with_cells(
        bsmr_interpreter_for_build::interpreter::testing::cells(Some(indoc!(
            r#"
            [buildfile]
...
#[test]
fn prelude_is_included() -> bsmr_error::Result<()> {
...
#[test]
fn test_package_import() -> bsmr_error::Result<()> {
    let mut tester = Tester::with_cells(bsmr_interpreter_for_build::interpreter::testing::cells(
        Some(indoc!(
            r#"
            [buildfile]
...
fn eval() -> bsmr_error::Result<()> {
    ...
    let content = indoc!(
        r#"
            ...
            def test():
                assert_eq("some/package", __bsmr_builtins__.package_name())
                assert_eq("@root", __bsmr_builtins__.repository_name())

                assert_eq(package_name(), __bsmr_builtins__.package_name())
                assert_eq(repository_name(), __bsmr_builtins__.repository_name())
...
#[test]
fn test_builtins() -> bsmr_error::Result<()> {
    // Test that most things end up on __bsmr_builtins__
    run_simple_starlark_test(indoc!(
        r#"
            def test():
                assert_eq(__bsmr_builtins__.json.encode({}), "{}")
            "#
    ))?;
    ...
    tester.run_starlark_test_expecting_error(
        indoc!(
            r#"
            def test():
                __bsmr_builtins__.bsmr_fail("message")
            "#
        ),
        "The attribute `bsmr_fail` is not available",
    );
    Ok(())
}

#[test]
fn test_oncall() -> bsmr_error::Result<()> {
    let mut tester = Tester::new().unwrap();
