...
use bsmr_interpreter_for_build::nodes::attr_spec::AttributeSpecExt;
...
#[test]
fn rule_creates_callable() -> bsmr_error::Result<()> {
...
#[test]
fn udr_is_recorded() -> bsmr_error::Result<()> {
    let content = indoc!(
        r#"
        def impl(ctx):
            pass
...
#[test]
fn option_allows_none() -> bsmr_error::Result<()> {
    let mut tester = rule_tester();
...
#[test]
fn returns_documentation() -> bsmr_error::Result<()> {
    let bzl = indoc::indoc!(
        r#"def impl(ctx):
            pass
