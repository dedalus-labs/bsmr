...
use bsmr_build_api::build::ConfiguredBuildTargetResult;
...
mod proto {
    pub(crate) use bsmr_cli_proto::BuildTarget;
    pub(crate) use bsmr_cli_proto::build_target::BuildOutput;
    pub(crate) use bsmr_cli_proto::build_target::build_output::BuildOutputProviders;
}

/// Simple container for multiple [`bsmr_error::Error`]s
pub(crate) struct BuildErrors {
    pub(crate) errors: Vec<bsmr_error::Error>,
}
...
/// Collects build results into a Result<Vec<proto::BuildTarget>, bsmr_error::Errors>. If any targets
...
impl<'a> ResultReporter<'a> {
    ...
    ) -> bsmr_error::Result<BuildTargetsAndErrors> {
        let mut out = Self {
            artifact_fs,
            options,
            results: Vec::new(),
