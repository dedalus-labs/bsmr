...
use bsmr_build_api::actions::calculation::get_target_rule_type_name;
...
use bsmr_build_api::interpreter::rule_defs::provider::builtin::install_info::FrozenInstallInfo;
...
use bsmr_build_api::materialize::materialize_and_upload_artifact_group;
use bsmr_build_api::validation::validation_impl::VALIDATION_IMPL;
use bsmr_cli_proto::InstallRequest;
use bsmr_cli_proto::InstallResponse;
...
use bsmr_install_proto::ShutdownRequest;
use bsmr_install_proto::installer_client::InstallerClient;
...
use bsmr_util::future::try_join_all;
...
pub(crate) async fn install_command(
    ...
    req: InstallRequest,
) -> bsmr_error::Result<InstallResponse> {
...
impl ServerCommandTemplate for InstallServerCommand {
    type StartEvent = bsmr_data::InstallCommandStart;
    type EndEvent = bsmr_data::InstallCommandEnd;
    type Response = InstallResponse;
    type PartialResult = NoPartialResult;

    fn end_event(&self, _response: &bsmr_error::Result<Self::Response>) -> Self::EndEvent {
        bsmr_data::InstallCommandEnd {
...
async fn collect_install_request_data<'a>(
    ...
    request: &InstallRequest,
) -> bsmr_error::Result<impl IntoIterator<Item = InstallRequestData<'a>> + use<'a>> {
...
fn get_random_tcp_port() -> bsmr_error::Result<u16> {
...
fn get_timestamp_as_string() -> bsmr_error::Result<String> {
...
impl<'a> ConnectedInstaller<'a> {
    async fn connect(
        ...
        let timeout =
            Duration::from_secs(bsmr_env!("BSMR_INSTALLER_TIMEOUT_S", type=u64)?.unwrap_or(120));
        let send_timeout = Duration::from_secs(
            bsmr_env!("BSMR_INSTALLER_SEND_TIMEOUT_S", type=u64)?
                .unwrap_or_else(|| parse_install_timeout(installer_run_args)),
        );

        let client: bsmr_error::Result<InstallerClient<Channel>> = span_async_simple(
            bsmr_data::ConnectToInstallerStart {
            ...
            async {
                ...
                tokio::select! {
                    ...
                    exit_status = installer_child.wait() => {
                        match exit_status {
                            Ok(status) => Err(bsmr_error::bsmr_error!(
            ...
            bsmr_data::ConnectToInstallerEnd {},
        )
        .await;
    ...
    fn install_result(self, result: bsmr_error::Result<()>) -> InstallResult {
        InstallResult {
    ...
    async fn send_install_info(&mut self) -> bsmr_error::Result<()> {
        for (installed_target, install_files) in &self.install_request_data.installed_targets {
            ...
            if install_info_response.install_id != install_id {
                ...
                return Err(bsmr_error::bsmr_error!(
                    bsmr_error::ErrorTag::InstallIdMismatch,
    ...
    async fn send_shutdown_command(&self) -> bsmr_error::Result<()> {
    ...
    async fn send_file(&self, file: FileResult) -> bsmr_error::Result<()> {
        ...
        span_async(start, async {
            let mut outcome: bsmr_error::Result<()> = Ok(());
            ...
            if let Some(error_detail) = response.error_detail {
                let mut error: bsmr_error::Error = InstallError::ProcessingFileReadyFailure {
                    install_id: install_id.to_owned(),
                    artifact: name.to_owned(),
                    path: path.to_owned(),
                ...
                let category_tag = if let Ok(category) =
                    bsmr_install_proto::ErrorCategory::try_from(error_detail.category)
                {
                    match category {
                        bsmr_install_proto::ErrorCategory::Unspecified => {
                            ErrorTag::InstallerUnknown
                        }
                        bsmr_install_proto::ErrorCategory::Tier0 => ErrorTag::InstallerTier0,
                        bsmr_install_proto::ErrorCategory::Input => ErrorTag::InstallerInput,
                        bsmr_install_proto::ErrorCategory::Environment => {
...
async fn handle_install_request(
        ...
        .try_compute2(
            ...
            |ctx| {
                async move {
                    ...
                    // 1. bsmr passes a temp file for a tcp port output.
                    // 2. installer app choose unused tcp port and writes it into the passed file.
                    // 3. bsmr reads tcp port from file and use it to connect to the installer app. (`connect_to_installer` function)
                    ...
                    bsmr_error::Ok(installer.install(files_rx).await)
                }
                .boxed()
            },
        )
    ...
    let (install_duration, device_metadata, mut result) = match compute_result {
        Ok((artifacts_ready, install_result)) => {
            ...
            let device_metadata: Vec<bsmr_data::DeviceMetadata> = device_metadata
                .lock()
                .await
                .iter()
                .map(|metadata| bsmr_data::DeviceMetadata {
                    entry: metadata
                        .entry
                        .iter()
                        .map(|e| bsmr_data::device_metadata::Entry {
                            key: e.key.clone(),
                            value: e.value.clone(),
                        })
                        .collect(),
    ...
    get_dispatcher().instant_event(bsmr_data::InstallFinished {
...
async fn build_launch_installer(
    ...
    stderr_log_path: &str,
) -> bsmr_error::Result<tokio::process::Child> {
