...
use bsmr_build_api::build::BuildProviderType;
...
use bsmr_build_api::build::build_report::initialize_streaming_build_report;
use bsmr_build_api::build::build_report::stream_build_report;
...
use bsmr_build_api::build::detailed_aggregated_metrics::types::ActionGraphSketchResult;
use bsmr_build_api::build::detailed_aggregated_metrics::types::ArtifactPathSketchResult;
use bsmr_build_api::build::detailed_aggregated_metrics::types::DetailedAggregatedMetrics;
use bsmr_build_api::build::graph_properties::GraphPropertiesOptions;
...
impl ServerCommandTemplate for BuildServerCommand {
    type StartEvent = bsmr_data::BuildCommandStart;
    type EndEvent = bsmr_data::BuildCommandEnd;
    type Response = bsmr_cli_proto::BuildResponse;
    type PartialResult = NoPartialResult;

    fn end_event(&self, _response: &bsmr_error::Result<Self::Response>) -> Self::EndEvent {
        bsmr_data::BuildCommandEnd {
...
fn expect_build_opts(req: &bsmr_cli_proto::BuildRequest) -> &CommonBuildOptions {
...
async fn build(
    ...
    request: &bsmr_cli_proto::BuildRequest,
) -> bsmr_error::Result<bsmr_cli_proto::BuildResponse> {
...
async fn process_streaming_build_result(
    ...
    let cwd: &bsmr_core::fs::project_rel_path::ProjectRelativePath = server_ctx.working_dir();
    let cell_resolver = ctx.get_cell_resolver().await?;
    let artifact_fs = ctx.get_artifact_fs().await?;
...
async fn maybe_stream_build_reports(
    build_future: impl std::future::Future<Output = bsmr_error::Result<BuildTargetResult>>,
...
) -> bsmr_error::Result<BuildTargetResult> {
    if build_opts
        .unstable_streaming_build_report_filename
        .is_empty()
    {
...
async fn process_build_result(
    ...
    if should_create_unhashed_links.unwrap_or(false) {
        span_async(bsmr_data::CreateOutputSymlinksStart {}, async {
            let lock = ctx
            ...
            (res, bsmr_data::CreateOutputSymlinksEnd { created })
        })
        .await?;
    }
    ...
    Ok(bsmr_cli_proto::BuildResponse {
        build_targets,
        project_root,
...
async fn build_targets_in_universe(
    ...
    provider_labels
        .into_iter()
        .map(|p| {
            bsmr_util::async_move_clone!(providers_to_build, {
