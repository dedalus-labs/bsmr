...
use bsmr_action_metadata_proto::REMOTE_DEP_FILE_KEY;
...
fn error_on_cache_upload() -> bsmr_error::Result<bool> {
    bsmr_env!(
        "BSMR_TEST_ERROR_ON_CACHE_UPLOAD",
        bool,
...
impl CacheUploader {
    ...
    async fn upload_local_outputs(
    ...
    ) -> bsmr_error::Result<(CacheUploadOutcome, Option<TActionResult2>)> {
        ...
        let (outcome, action_result_for_dep_file) = span_async(
            ...
            async {
                ...
                let cache_upload_end_event = bsmr_data::CacheUploadEnd {
                    key: Some(info.target.as_proto_action_key()),
                    name: Some(info.target.as_proto_action_name()),
                    action_digest: digest_str.clone(),
    ...
    async fn upload_dep_file(
        ...
        error_on_cache_upload: bool,
    ) -> bsmr_error::Result<CacheUploadOutcome> {
        let remote_dep_file_key = remote_dep_file_action.action.to_string();
        span_async(
            bsmr_data::DepFileUploadStart {
                key: Some(info.target.as_proto_action_key()),
                name: Some(info.target.as_proto_action_name()),
            ...
            async {
                ...
                let end_event = bsmr_data::DepFileUploadEnd {
                    key: Some(info.target.as_proto_action_key()),
                    name: Some(info.target.as_proto_action_name()),
                    remote_dep_file_key: remote_dep_file_key.clone(),
                    success: outcome.uploaded(),
    ...
    async fn check_upload_permission(
        &self,
        info: &CacheUploadInfo<'_>,
    ) -> bsmr_error::Result<Result<(), CacheUploadOutcome>> {
    ...
    ) -> bsmr_error::Result<Result<TActionResult2, CacheUploadOutcome>> {
...
fn systemtime_to_ttimestamp(time: SystemTime) -> bsmr_error::Result<TTimestamp> {
...
mod tests {
    ...
    fn test_cache_upload_result_maps_rejections() {
        ...
        assert_eq!(
            bsmr_data::UploadResult::RejectedSymlinkOutput,
    ...
    fn test_cache_upload_result_maps_failures() {
        let mk_error = || bsmr_error::bsmr_error!(bsmr_error::ErrorTag::Tier0, "boom");
        ...
        assert_eq!(
            bsmr_data::UploadResult::FailedUploadOutputs,
        ...
        assert_eq!(
            bsmr_data::UploadResult::FailedOther,
