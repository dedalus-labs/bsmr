...
use bsmr_execute::execute::kind::RemoteCommandExecutionDetails;
...
use bsmr_execute::execute::result::CommandExecutionErrorType;
...
use bsmr_execute::re::action_identity::ReActionIdentity;
use bsmr_execute::re::client::CancellationReason;
use bsmr_execute::re::client::ExecuteResponseOrCancelled;
use bsmr_execute::re::error::RemoteExecutionError;
use bsmr_execute::re::error::get_re_error_tag;
...
impl ReExecutor {
    async fn upload(
        ...
        let upload_response = span_async(bsmr_data::ReUploadStart {}, async move {
            let res = re_client
                .upload(
                    &self.project_fs,
                    &self.materializer,
            ...
            match res {
                ...
                Err(e) => (Err(e), bsmr_data::ReUploadEnd::default()),
            }
        })
        .await;
    ...
    ) -> ControlFlow<CommandExecutionResult, (CommandExecutionManager, ExecuteResponseWithQueueStats)>
    {
        ...
        let execute_response =
            if let Some(timeout) = bsmr_common::self_test_timeout::maybe_cap_timeout(None) {
    ...
    async fn exec_cmd(
        ...
        let res = download_action_results(
            ...
            *digest_config,
            ...
            &identity,
            bsmr_data::ReStage {
                stage: Some(bsmr_data::ReDownload {}.into()),
            }
            .into(),
            request.paths(),
            request.outputs(),
...
#[bsmr(tag = get_re_error_tag(&inner.code))]
...
fn is_re_queue_full(e: &bsmr_error::Error) -> bool {
    #[cfg(all(fbcode_build, target_os = "linux"))]
    let enabled = justknobs::eval(
        "bsmr/packages/rust/remote_execution:re_queue_full_as_cancelled",
        None,
        None,
    )
    .unwrap_or(false);
