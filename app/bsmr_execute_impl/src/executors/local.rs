...
use bsmr_execute::entry::HashingInfo;
...
use bsmr_execute::execute::inputs_directory::inputs_directory;
...
use bsmr_execute::execute::request::CommandExecutionOutputRef;
...
use bsmr_execute_local::maybe_absolutize_exe;
use bsmr_execute_local::spawn_command_and_stream_events;
use bsmr_execute_local::status_decoder::DefaultStatusDecoder;
...
use bsmr_resource_control::action_scene::ActionCgroupSession;
use bsmr_resource_control::memory_tracker::MemoryTrackerHandle;
use bsmr_resource_control::path::CgroupPathBuf;
...
pub enum ForkserverAccess {
    None,
    #[cfg(unix)]
    Client(bsmr_forkserver::client::ForkserverClient),
}
...
impl LocalExecutor {
    ...
    ) -> impl futures::future::Future<Output = bsmr_error::Result<CommandResult>> + Send + 'a {
        async move {
            ...
            if !result.orphan_processes.is_empty() {
                bsmr_events::dispatch::instant_event(bsmr_data::OrphanProcessesKilled {
                    orphan_processes: result
                        .orphan_processes
                        .iter()
                        .map(|o| bsmr_data::orphan_processes_killed::OrphanProcess {
                            pid: o.pid,
                            comm: o.comm.clone(),
                        })
                        .collect(),
    ...
    > {
        if let Err(e) = executor_stage_async(
            bsmr_data::LocalStage {
                stage: Some(bsmr_data::LocalPrepareOutputDirs {}.into()),
            },
            async {
                tokio::try_join!(
                    create_output_dirs(
        ...
        let (time_span, start_time, res) = executor_stage_async(
            {
                let env = env
                    .iter()
                    .copied()
                    .map(|(k, v)| bsmr_data::EnvironmentEntry {
                        key: k.to_owned(),
                ...
                let stage = match worker {
                    None => bsmr_data::LocalExecute {
                        command: Some(bsmr_data::LocalCommand {
                    ...
                    .into(),
                    Some(_) => bsmr_data::WorkerExecute {
                        command: Some(bsmr_data::WorkerCommand {
                            action_digest: action_digest.to_string(),
                ...
                bsmr_data::LocalStage { stage: Some(stage) }
            },
            async move {
    ...
    > {
        ...
        let mut res = loop {
            ...
            let kill_observer = if let Some(kill_future) = kill_future {
                let kill_awaiter = bsmr_util::async_move_clone!(retry_future, {
    ...
    async fn exec_request(
        ...
        let executor_stage_result = executor_stage_async(
            bsmr_data::LocalStage {
                stage: Some(bsmr_data::LocalMaterializeInputs {}.into()),
            },
            async {
                ...
                bsmr_error::Ok((scratch_path, Instant::now() - start))
            },
        )
        .boxed()
        .await;
        ...
        let dispatcher = match get_dispatcher_opt() {
            ...
            None => {
                return manager.error(
                    "no_dispatcher",
                    bsmr_error!(
                        bsmr_error::ErrorTag::DispatcherUnavailable,
        ...
        let result = match status {
            GatherOutputStatus::Finished {
                ...
                let mut execution_stats =
                    execution_stats.map(|s| bsmr_data::CommandExecutionStats {
    ...
    async fn calculate_and_declare_output_values(
        ...
        bsmr_util::future::try_join_all(output_path_to_content_based_path_copies.into_iter().map(
            ...
            if let Some(broker) = &worker_pool.get_worker_broker(worker_spec) {
                Some(
                    executor_stage_async(
                        bsmr_data::LocalStage {
                            stage: Some(bsmr_data::WorkerQueued {}.into()),
                        },
    ...
    > {
            ...
            (request.worker(), self.worker_pool.dupe(), &self.forkserver)
        {
            ...
            let stage = if new_worker {
                bsmr_data::LocalStage {
                    stage: Some(
                        bsmr_data::WorkerInit {
                            command: Some(bsmr_data::WorkerInitCommand {
                ...
                bsmr_data::LocalStage {
                    stage: Some(bsmr_data::WorkerWait {}.into()),
                }
            };
...
impl PreparedCommandExecutor for LocalExecutor {
    async fn exec_cmd(
        ...
        let local_resource_holders = executor_stage_async(
            bsmr_data::LocalStage {
                stage: Some(bsmr_data::AcquireLocalResource {}.into()),
            },
            async move {
                let mut holders = vec![];
        ...
        let _permit = executor_stage_async(
            bsmr_data::LocalStage {
                stage: Some(bsmr_data::LocalQueued {}.into()),
            },
...
pub async fn materialize_inputs(
    ...
    request: &CommandExecutionRequest,
    digest_config: DigestConfig,
) -> bsmr_error::Result<MaterializedInputPaths> {
    let mut paths = vec![];
...
mod unix {
    ...
    pub async fn exec_via_forkserver(
        forkserver: &bsmr_forkserver::client::ForkserverClient,
        ...
        let mut req = bsmr_forkserver_proto::CommandRequest {
            exe: exe.as_bytes().to_vec(),
            argv: args
                .into_iter()
                .map(|s| s.as_ref().as_bytes().to_vec())
                .collect(),
            cwd: Some(bsmr_forkserver_proto::WorkingDirectory {
    ...
    trait CommandRequestExt {
        ...
        where
            D: Into<bsmr_forkserver_proto::env_directive::Data>;
    }

    impl CommandRequestExt for bsmr_forkserver_proto::CommandRequest {
        fn push_env_directive<D>(&mut self, directive: D)
        where
            D: Into<bsmr_forkserver_proto::env_directive::Data>,
        {
            self.env.push(bsmr_forkserver_proto::EnvDirective {
                data: Some(directive.into()),
            });
        }
    }

    impl EnvironmentBuilder for bsmr_forkserver_proto::CommandRequest {
        fn clear(&mut self) {
            self.push_env_directive(bsmr_forkserver_proto::EnvClear {});
        }
        ...
        where
            ...
            V: AsRef<OsStr>,
        {
            self.push_env_directive(bsmr_forkserver_proto::EnvSet {
        ...
        where
            K: AsRef<OsStr>,
        {
            self.push_env_directive(bsmr_forkserver_proto::EnvRemove {
                key: key.as_ref().as_bytes().to_vec(),
            })
        }
    }
...
mod tests {
    ...
    fn test_executor() -> bsmr_error::Result<(LocalExecutor, AbsNormPathBuf, ProjectRootTemp)> {
    ...
    #[tokio::test]
    async fn test_exec_cmd_environment() -> bsmr_error::Result<()> {
    ...
    #[cfg(fbcode_build)]
    #[tokio::test]
    async fn test_exec_cmd_timeout() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_exec_cmd_environment_filtering() -> bsmr_error::Result<()> {
        use bsmr_execute::execute::environment_inheritance::EnvironmentInheritance;
