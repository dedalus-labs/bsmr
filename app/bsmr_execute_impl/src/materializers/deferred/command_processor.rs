...
struct CommandStream<T: 'static> {
    ...
    clean_stale_fut: Option<BoxFuture<'static, bsmr_error::Result<CleanResult>>>,
}
...
impl<T: IoHandler> DeferredMaterializerCommandProcessor<T> {
                ...
                Op::CleanStaleRequest => {
                    if let Some(config) = clean_stale_config.as_ref() {
                        ...
                        let _unused = soft_error!(
                            ...
                            bsmr_error!(bsmr_error::ErrorTag::Tier0, "clean scheduled without being configured"),
                            quiet: true
                        );
                    }
                }
        ...
        match command {
            ...
            MaterializerCommand::Declare(
                ...
                self.maybe_log_command(&event_dispatcher, || {
                    bsmr_data::materializer_command::Data::Declare(
                        bsmr_data::materializer_command::Declare {
                            path: path.to_string(),
                        },
                    )
                });
                ...
                if self.subscriptions.should_materialize_eagerly(&path) {
                        ...
                        .should_materialize_eagerly(eager_path)
                    {
                        ...
                        self.maybe_log_command(&event_dispatcher, || {
                            bsmr_data::materializer_command::Data::EagerDispatchOnDeclare(
                                bsmr_data::materializer_command::EagerDispatchOnDeclare {
                                    path: path.to_string(),
                                },
                            )
                        });
            ...
            MaterializerCommand::InvalidateFilePaths(
                ...
                maybe_proxy_current_span(parent_id, || {
                    ...
                    self.maybe_log_command(&event_dispatcher, || {
                        bsmr_data::materializer_command::Data::InvalidateFilePaths(
                            bsmr_data::materializer_command::InvalidateFilePaths {
            ...
            MaterializerCommand::Ensure(paths, event_dispatcher, parent_id, fut_sender) => {
                maybe_proxy_current_span(parent_id, || {
                    self.maybe_log_command(&event_dispatcher, || {
                        bsmr_data::materializer_command::Data::Ensure(
                            bsmr_data::materializer_command::Ensure {
                                paths: paths.iter().map(|p| p.to_string()).collect::<Vec<_>>(),
                            },
                        )
                    });
            ...
            MaterializerCommand::RegisterEagerPaths(paths, event_dispatcher, sender) => {
                self.maybe_log_command(&event_dispatcher, || {
                    bsmr_data::materializer_command::Data::RegisterEagerPaths(
                        bsmr_data::materializer_command::RegisterEagerPaths {
                            paths: paths.iter().map(|p| p.to_string()).collect::<Vec<_>>(),
                        },
                    )
                });
        ...
        self.ttl_refresh_instance = match self.ttl_refresh_instance.take() {
            Some(mut curr) => match curr.try_recv() {
                ...
                Err(TryRecvError::Closed) => {
                    ...
                    self.ttl_refresh_history.push(TtlRefreshHistoryEntry {
                        at: Utc::now(),
                        outcome: Some(Err(bsmr_error!(bsmr_error::ErrorTag::Tier0, "Shutdown"))),
                    });
                    None
                }
            },
    ...
    pub(super) fn flush_access_times(&mut self) -> String {
        ...
        "Access time updates are disabled. Consider removing `update_access_times = false` from your .bsmr".to_owned()
    }
        ...
        if let Some(data) = self.tree.prefix_get_mut(&mut path_iter) {
            match &data.stage {
                ArtifactMaterializationStage::Materialized {
                    ...
                    let force_mismatch = bsmr_env!(
                        "BSMR_TEST_FORCE_DECLARE_MISMATCH",
                        bool,
    ...
    where
        F: FnOnce() -> bsmr_data::materializer_command::Data,
    {
        if self.verbose_materializer_log {
            let data = Some(f());
            event_dispatcher.instant_event(bsmr_data::MaterializerCommand { data });
        }
    }
}
...
fn clean_path<T: IoHandler>(
    ...
    .map(|r| match r {
        Ok(r) => r,
        Err(e) => Err(e.into()), // Turn the JoinError into a bsmr_error::Error.
    })
    .boxed()
    .shared()
}
...
struct ExistingFutures(bsmr_error::Result<Vec<(ProjectRelativePathBuf, ProcessingFuture)>>);
...
impl ExistingFutures {
    ...
    fn into_result(self) -> bsmr_error::Result<Vec<(ProjectRelativePathBuf, ProcessingFuture)>> {
        self.0
    }
