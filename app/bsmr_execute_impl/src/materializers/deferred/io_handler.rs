...
use bsmr_execute::materialize::http::http_download;
...
impl IoHandler for DefaultIoHandler {
    ...
    async fn materialize_entry(
        ...
        let materialization_start = bsmr_data::MaterializationStart {
...
/// This is used for testing to ingest digests (via BSMR_TEST_TOMBSTONED_DIGESTS).
fn maybe_tombstone_digest(digest: &FileDigest) -> bsmr_error::Result<&FileDigest> {
        ...
        for (use_case, digests_to_refresh) in digests_to_refresh {
            ...
            for chunk in digests_to_refresh.as_slice().chunks(REFRESH_CHUNK_SIZE) {
                ...
                let mut digests_expires = digests_expires.into_try_map(|(digest, expires)| {
                    bsmr_error::Ok((FileDigest::from_re(&digest, digest_config)?, expires))
                })?;
...
impl WriteIoRequest {
    fn execute_inner(&self, project_fs: &ProjectRoot) -> bsmr_error::Result<()> {
