...
pub struct CleanStaleArtifactsExtensionCommand {
    ...
    pub sender: Sender<BoxFuture<'static, bsmr_error::Result<CleanResult>>>,
}
...
enum PendingCleanResult {
    ...
    Pending(BoxFuture<'static, bsmr_error::Result<CleanResult>>),
}
...
impl From<CleanResult> for bsmr_cli_proto::CleanStaleResponse {
    fn from(result: CleanResult) -> Self {
        let message = match result.kind {
            ...
            CleanStaleResultKind::SkippedDeferWriteDisabled => {
                Some("Skipping clean, set bsmr.defer_write_actions to use clean --stale")
            }
            CleanStaleResultKind::SkippedSqliteDisabled => {
                Some("Skipping clean, set bsmr.sqlite_materializer_state to use clean --stale")
            }
...
fn create_result(
    result: Result<CleanResult, bsmr_error::Error>,
    trace_id: Option<TraceId>,
    daemon_id: &DaemonId,
    total_duration_s: u64,
) -> bsmr_data::CleanStaleResult {
...
impl CleanStaleArtifactsCommand {
    pub(super) fn create_clean_fut<T: IoHandler>(
        ...
        trace_id: Option<TraceId>,
        daemon_id: DaemonId,
    ) -> BoxFuture<'static, bsmr_error::Result<CleanResult>> {
        ...
        async move {
            ...
            let result: Result<CleanResult, bsmr_error::Error> = result;
            let result_event: bsmr_data::CleanStaleResult = create_result(
                result.clone(),
                trace_id,
                &daemon_id,
    ...
    fn create_pending_clean_result<T: IoHandler>(
        &self,
        processor: &mut DeferredMaterializerCommandProcessor<T>,
    ) -> bsmr_error::Result<PendingCleanResult> {
    ...
    fn scan_and_create_clean_fut<T: IoHandler>(
        ...
        for (path, file_type) in found_paths
            ...
            .take(2000)
        {
            self.dispatcher.instant_event(bsmr_data::UntrackedFile {
                path: path.to_string(),
...
#[derive(Debug, Clone, bsmr_error::Error)]
...
pub(crate) struct CleanStaleError {
    db_size: usize,
    stats: bsmr_data::CleanStaleStats,
}

fn stats_for_paths(paths: &Vec<FoundPath>) -> bsmr_data::CleanStaleStats {
    let mut stats = bsmr_data::CleanStaleStats::default();
    for path in paths {
        match path {
...
fn create_clean_fut<T: IoHandler>(
    ...
    cancellations: &'static CancellationContext,
    liveliness_observer: Arc<dyn LivelinessObserverSync>,
) -> bsmr_error::Result<BoxFuture<'static, bsmr_error::Result<CleanResult>>> {
    let io = io.dupe();
    ...
    let fut = async move {
        ...
        let res = bsmr_util::future::try_join_all(
            found_paths
                .into_iter()
                .filter_map(|x| match x {
...
impl IoRequest for CleanInvalidatedPathRequest {
    fn execute(self: Box<Self>, project_fs: &ProjectRoot) -> bsmr_error::Result<()> {
        if !self.liveliness_observer.is_alive_sync() {
            return Err(bsmr_error!(ErrorTag::CleanInterrupt, "Interrupt"));
        }
...
pub fn get_size(path: &AbsNormPath) -> bsmr_error::Result<u64> {
    let mut result = 0;
    if path.is_dir() {
    ...
    fn mixed_inputs_only_touch_retained() {
        let mut paths = vec![
            FoundPath::Untracked(
                ...
                bsmr_common::file_ops::metadata::FileType::File,
                42,
            ),
            FoundPath::Tracked {
