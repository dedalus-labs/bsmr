...
#[test]
fn test_find_artifacts() -> bsmr_error::Result<()> {
...
mod state_machine {
    ...
    use bsmr_fs::paths::forward_rel_path::ForwardRelativePath;
    use bsmr_util::threads::ignore_stack_overflow_checks_for_future;
    ...
    impl IoHandler for StubIoHandler {
        ...
        async fn immediate_write<'a>(
            self: &Arc<Self>,
            _gen: Box<dyn FnOnce() -> bsmr_error::Result<Vec<WriteRequest>> + Send + 'a>,
        ...
        fn clean_path<'a>(
            ...
            command_sender: Arc<MaterializerSender<Self>>,
            _cancellations: &'a CancellationContext,
        ) -> BoxFuture<'a, Result<(), bsmr_error::Error>> {
    ...
    fn make_processor_for_io(
        ...
        let (daemon_dispatcher_events, daemon_dispatcher_sink) =
            bsmr_events::create_source_sink_pair();
    ...
    async fn make_materializer(
        ...
        let command_thread = thread_spawn("bsmr-dm", {
            move || {
    ...
    #[tokio::test]
    async fn test_allocative_profiles_artifact_tree() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_declare_reuse() -> bsmr_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
    ...
    fn make_artifact_value_with_symlink_dep(
        ...
        digest_config: DigestConfig,
    ) -> bsmr_error::Result<ArtifactValue> {
    ...
    #[tokio::test]
    async fn test_materialize_symlink_and_target() -> bsmr_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
    ...
    #[tokio::test]
    async fn test_materialize_symlink_first_then_target() -> bsmr_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
    ...
    #[tokio::test]
    async fn test_subscription_subscribe_also_materializes() -> bsmr_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let (mut dm, mut channel) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
            let value = ArtifactValue::file(digest_config.empty_file());
    ...
    #[tokio::test]
    async fn test_invalidate_error() -> bsmr_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async{
            let (mut dm, _) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
    ...
    #[tokio::test]
    async fn test_materialize_dep_error() -> bsmr_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            // Construct a tree with a symlink and its target, materialize both at once
            let symlink_path = make_path("foo/bar_symlink");
            let target_path = make_path("foo/bar_target");
            ...
            dm.materialize_artifact(&symlink_path, EventDispatcher::null())
                .ok_or_else(|| internal_error!("Expected a future"))?
                .await
                .map_err(|err| bsmr_error!(bsmr_error::ErrorTag::MaterializationError, "error materializing {:?}", err))?;
            assert_eq!(
                dm.io.take_log(),
                &[
            ...
            dm.materialize_artifact(&symlink_path, EventDispatcher::null())
                .ok_or_else(|| internal_error!("Expected a future"))?
                .await
                .map_err(|err| bsmr_error!(bsmr_error::ErrorTag::MaterializationError, "error materializing 2 {:?}", err))?;
    ...
    #[tokio::test]
    async fn test_retry() -> bsmr_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let (mut dm, mut channel) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
    ...
    async fn test_clean_stale_schedule() -> bsmr_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            ...
            while i < 5 {
                ...
                if let bsmr_data::CleanStaleStats {
    ...
    #[tokio::test]
    async fn test_has_artifact_at() -> bsmr_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let (mut dm, _) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
    ...
    #[tokio::test]
    async fn test_get_artifact_entries_for_materialized_paths() -> bsmr_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let (mut dm, _) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
    ...
    #[tokio::test]
    async fn test_get_artifact_entries_for_projected_paths() -> bsmr_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let (mut dm, _) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
    ...
    #[tokio::test]
    async fn test_priority_promotion_propagates_to_symlink_deps() -> bsmr_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
