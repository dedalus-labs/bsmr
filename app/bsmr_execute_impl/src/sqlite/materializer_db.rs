...
use bsmr_common::sqlite::sqlite_db::SqliteTable;
use bsmr_common::sqlite::sqlite_db::SqliteTables;
...
impl MaterializerStateSqliteDb {
    ...
    /// or the DB has a different set of versions than the versions this bsmr expects, we
    ...
    ) -> bsmr_error::Result<(Self, bsmr_error::Result<MaterializerState>)> {
        io_executor
            .execute_io_inline(|| {
...
pub(crate) fn testing_materializer_state_sqlite_db(
    ...
    MaterializerStateSqliteDb::initialize_materializer_sqlite_db(
        fs.resolve(ProjectRelativePath::unchecked_new(
            "bsmr-out/default/cache/materializer_state",
        )),
        versions,
        metadata,
...
mod tests {
    ...
    use bsmr_common::cas_digest::TrackedCasDigest;
    ...
    use bsmr_execute::directory::new_symlink;
    ...
    fn test_materializer_state_sqlite_table() {
        ...
        let mut artifacts: StdBsmrHashMap<_, _> =
    ...
    #[test]
    fn test_initialize_sqlite_db() -> bsmr_error::Result<()> {
