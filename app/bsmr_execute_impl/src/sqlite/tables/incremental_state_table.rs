...
mod tests {
    ...
    #[test]
    fn test_insert_and_read_single_action() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_insert_and_read_multiple_actions() -> bsmr_error::Result<()> {
        let conn = Connection::open_in_memory()?;
        let table = IncrementalStateSqliteTable::new(Arc::new(Mutex::new(conn)));
        table.create_table()?;
    ...
    #[test]
    fn test_delete_action() -> bsmr_error::Result<()> {
        let conn = Connection::open_in_memory()?;
        let table = IncrementalStateSqliteTable::new(Arc::new(Mutex::new(conn)));
        table.create_table()?;
