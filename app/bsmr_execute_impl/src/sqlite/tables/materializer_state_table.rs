...
fn convert_non_directory_sqlite_entry_to_materializer_state_entry(
    ...
    digest_config: DigestConfig,
) -> bsmr_error::Result<MaterializerStateEntry> {
...
fn convert_non_directory_sqlite_entry_to_action_directory_member(
    sqlite_entry: SqliteEntry,
    digest_config: DigestConfig,
) -> bsmr_error::Result<ActionDirectoryMember> {
...
impl MaterializerStateSqliteTable {
    ...
    pub(crate) fn create_table(&self) -> bsmr_error::Result<()> {
        let sql = format!(
    ...
    fn create_parent_path_index(&self) -> bsmr_error::Result<()> {
        let sql = format!(
    ...
    fn read_all_entries(&self) -> bsmr_error::Result<Vec<SqliteEntry<'_>>> {
        static SQL: LazyLock<String> = LazyLock::new(|| {
            format!(
...
mod tests {
    ...
    #[test]
    fn test_delete_many() -> bsmr_error::Result<()> {
