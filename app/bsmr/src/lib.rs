...
use bsmr_client::commands::build::BuildCommand;
use bsmr_client::commands::bxl::BxlCommand;
...
use bsmr_client::commands::kill::KillCommand;
use bsmr_client::commands::killall::KillallCommand;
use bsmr_client::commands::lsp::LspCommand;
use bsmr_client::commands::profile::ProfileCommand;
use bsmr_client::commands::query::aquery::AqueryCommand;
use bsmr_client::commands::query::cquery::CqueryCommand;
use bsmr_client::commands::query::uquery::UqueryCommand;
use bsmr_client::commands::root::RootCommand;
use bsmr_client::commands::run::RunCommand;
use bsmr_client::commands::server::ServerCommand;
use bsmr_client::commands::status::StatusCommand;
use bsmr_client::commands::subscribe::SubscribeCommand;
use bsmr_client::commands::targets::TargetsCommand;
use bsmr_client::commands::test::TestCommand;
...
use bsmr_cmd_starlark_client::StarlarkCommand;
use bsmr_common::argv::Argv;
use bsmr_common::invocation_paths_result::InvocationPathsResult;
use bsmr_common::invocation_roots::get_invocation_paths_result;
...
pub fn exec(process: ProcessContext<'_>) -> ExitResult {
    ...
    let arg0_override = bsmr_env!("BSMR_ARG0")?;
    ...
    let matches = match clap.try_get_matches_from(argv.expanded_argv.args()) {
        ...
        Err(e) => {
            ...
            return if e.exit_code() == 0 {
                ...
                let e = bsmr_error::Error::from(e).tag([ErrorTag::ClapMatch]);
    ...
    // is an end user: https://fburl.com/scuba/bsmr_builds/n4klo51d
...
impl ParsedArgv {
    fn parse(argv: Argv, matches: clap::ArgMatches) -> bsmr_error::Result<Self> {
...
pub(crate) enum CommandKind {
    #[cfg(not(client_only))]
    #[clap(hide = true)]
    Daemon(bsmr_daemon::daemon::DaemonCommand),
    #[cfg(not(client_only))]
    #[clap(hide = true)]
    ...
    Debug(DebugCommand),
    #[clap(hide = true)]
    Complete(bsmr_cmd_completion_client::complete::CompleteCommand),
    Completion(bsmr_cmd_completion_client::completion::CompletionCommand),
    Docs(bsmr_cmd_docs_client::DocsCommand),
    #[clap(subcommand)]
        ...
        match self {
            ...
            CommandKind::Query(cmd) => {
                bsmr_client_ctx::eprintln!(
                    "WARNING: \"bsmr query\" is an alias for \"bsmr uquery\". Consider using \"bsmr cquery\" or \"bsmr uquery\" explicitly."
                )?;
