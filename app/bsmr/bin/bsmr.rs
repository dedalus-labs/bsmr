...
use bsmr::error_reporting;
use bsmr::exec;
use bsmr::process_context::ClientRuntime;
use bsmr::process_context::ProcessContext;
use bsmr::process_context::SharedProcessContext;
use bsmr_build_info::BSMR_BUILD_INFO;
use bsmr_build_info::BsmrBuildInfo;
use bsmr_client_ctx::events_ctx::EventsCtx;
use bsmr_client_ctx::exit_result::ExitResult;
use bsmr_client_ctx::restarter::Restarter;
use bsmr_client_ctx::stdio;
use bsmr_client_ctx::subscribers::recorder::InvocationRecorder;
use bsmr_core::bsmr_env;
use bsmr_core::logging::LogConfigurationReloadHandle;
use bsmr_core::logging::init_tracing_for_writer;
use bsmr_core::logging::log_file::TracingLogFile;
...
#[global_allocator]
#[cfg(all(target_os = "windows", not(bsmr_memfrag)))]
static ALLOC: mimalloc::MiMalloc = mimalloc::MiMalloc;

// Opt-in fragmentation profiler, enabled with `-c bsmr_dev.memfrag=true`. It wraps
...
// would. It stays a passthrough unless BSMR_MEMFRAG_SHIFT is set at process
...
fn print_retry() -> bsmr_error::Result<()> {
    bsmr_client_ctx::eprintln!("============================================================")?;
    bsmr_client_ctx::eprintln!("|| Bessemer has detected that it needs to restart to proceed ||")?;
    bsmr_client_ctx::eprintln!("|| Your command will now restart.                         ||")?;
    bsmr_client_ctx::eprintln!("============================================================")?;
    bsmr_client_ctx::eprintln!()?;
    Ok(())
}
...
fn exec_with_logging(
    ...
    shared: bsmr_error::Result<SharedProcessContext>,
...
fn main() -> ! {
    ...
    // bsmr_daemon — threads do not survive the daemonizing fork.
    #[cfg(bsmr_memfrag)]
    mem_frag::init();

    bsmr_core::client_only::CLIENT_ONLY_VAL.init(cfg!(client_only));
    #[cfg(not(client_only))]
    {
        bsmr_analysis::init_late_bindings();
        bsmr_anon_target::init_late_bindings();
        bsmr_action_impl::init_late_bindings();
        bsmr_cmd_audit_server::init_late_bindings();
        bsmr_build_api::init_late_bindings();
        bsmr_cmd_docs_server::init_late_bindings();
        bsmr_external_cells::init_late_bindings();
        bsmr_transition::init_late_bindings();
        bsmr_build_signals_impl::init_late_bindings();
        bsmr_bxl::init_late_bindings();
        bsmr_cfg_constructor::init_late_bindings();
        bsmr_configured::init_late_bindings();
        bsmr_query_impls::init_late_bindings();
        bsmr_interpreter_for_build::init_late_bindings();
        bsmr_server_commands::init_late_bindings();
        bsmr_cmd_targets_server::init_late_bindings();
        bsmr_cmd_query_server::init_late_bindings();
        bsmr_cmd_starlark_server::init_late_bindings();
        bsmr_test::init_late_bindings();
        bsmr_validation::init_late_bindings();
        bsmr_events::init_late_bindings();
    }
    BSMR_BUILD_INFO.init(BsmrBuildInfo {
        revision: std::option_env!("BSMR_SET_EXPLICIT_VERSION"),
        win_internal_version: std::option_env!("BSMR_WIN_INTERNAL_VERSION"),
        release_timestamp: std::option_env!("BSMR_RELEASE_TIMESTAMP"),
    });

    // Set up crypto impl once per process
    bsmr_certs::certs::setup_cryptography_or_fail();

    fn init_shared_context() -> bsmr_error::Result<SharedProcessContext> {
        ...
        let stdin_buffer_size = bsmr_env!(
            "BSMR_TEST_STDIN_BUFFER_SIZE",
            type=usize,
        ...
        Ok(SharedProcessContext {
            ...
            force_want_restart: bsmr_env!("FORCE_WANT_RESTART", bool)?,
        })
    }
