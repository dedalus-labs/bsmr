...
/// Internal test runner implementation for Bessemer.
///
/// This is a basic test runner intended to be used by the open-source Bessemer build
/// if no external test runner is provided. This ensures that `bsmr test` works
/// out-of-the-box for open-source users.
///
/// **This is intended for open-source use only.**
pub struct BsmrTestRunner {
...
impl BsmrTestRunner {
    ...
    pub async fn run_all_tests(&self) -> bsmr_error::Result<()> {
        let receiver;
        {
        ...
        let run_verdict = receiver
            ...
            // Use an arbitrarily large buffer -- execution throttling will be handled by the Bessemer
    ...
    async fn report_test_result(&self, test_result: TestResult) -> bsmr_error::Result<()> {
        self.orchestrator_client
