...
function __bsmr_takes_target
    set -l cmd (commandline --current-process --tokenize --cut-at-cursor)
    if contains -- -- $cmd[..-1]
        return 1
    end
    set -l subcommand (__bsmr_subcommand $cmd)
...
function __bsmr_add_target_completions
    set -l cur (commandline --current-token)

    string match --quiet -- '-*' $cur && return

    bsmr complete --target="$cur" 2>/dev/null
end
