...
impl PathSanitizer {
    ...
    pub(crate) fn sanitize(&self, given: &str) -> bsmr_error::Result<SanitizedPath> {
    ...
    fn sanitize_relative_path(
        ...
        given: &str,
        path_str: &str,
    ) -> Result<SanitizedPath, bsmr_error::Error> {
    ...
    fn cell_abs_path(&self, cell: CellName) -> bsmr_error::Result<AbsNormPathBuf> {
    ...
    fn resolve_alias(&self, dir: &str) -> bsmr_error::Result<CellName> {
        self.alias_resolver.resolve(dir)
    }

    fn resolve_cell(&self, path: &AbsNormPath) -> bsmr_error::Result<CellName> {
    ...
    fn relative_to_cell(&self, dir: &AbsNormPath) -> bsmr_error::Result<CellRelativePathBuf> {
        Ok(self
            .cell_resolver
    ...
    fn relative_to_project<'a>(
        &'a self,
        dir: &'a AbsNormPath,
    ) -> bsmr_error::Result<Cow<'a, ProjectRelativePath>> {
...
mod tests {
    ...
    fn paths_to_test_data() -> &'static [&'static str] {
        &[
            "fbcode/bsmr/app/bsmr_cmd_completion_client/test_data",
            "app/bsmr_cmd_completion_client/test_data",
            "test_data",
        ]
    }

    fn in_dir(d: &str) -> bsmr_error::Result<AbsWorkingDir> {
    ...
    fn in_root() -> bsmr_error::Result<AbsWorkingDir> {
        let cwd = AbsNormPathBuf::new(std::env::current_dir().unwrap())?;
    ...
    fn abs_path_from_root(relative: &str) -> bsmr_error::Result<AbsNormPathBuf> {
        let root = in_root()?;
        root.into_abs_norm_path_buf().join_normalized(relative)
    }

    fn abs_str_from_root(relative: &str) -> bsmr_error::Result<String> {
    ...
    macro_rules! testy {
        ($test_name:ident($in_dir:expr, $partial:expr) -> {
            ...
            paste! {
                #[tokio::test]
                async fn [<test_ $test_name _verify_abs_path>]() -> bsmr_error::Result<()> {
                    let cwd = $in_dir;
                ...
                #[tokio::test]
                async fn [<test_ $test_name _verify_cell_name>]() -> bsmr_error::Result<()> {
                    let cwd = $in_dir;
                    let uut = PathSanitizer::new(&cell_configs(&cwd)?, &cwd, &find_invocation_roots(&cwd)?).await?;
                ...
                #[tokio::test]
                async fn [<test_ $test_name _verify_given>]() -> bsmr_error::Result<()> {
                    let cwd = $in_dir;
                    let uut = PathSanitizer::new(&cell_configs(&cwd)?, &cwd, &find_invocation_roots(&cwd)?).await?;
    ...
    #[tokio::test]
    async fn test_can_create_from_a_canonical_path() -> bsmr_error::Result<()> {
        let cwd = in_root()?;
        let uut =
    ...
    #[tokio::test]
    async fn test_can_create_from_a_str_relative_path() -> bsmr_error::Result<()> {
        let cwd = in_root()?;
        let uut =
            PathSanitizer::new(&cell_configs(&cwd)?, &cwd, &find_invocation_roots(&cwd)?).await?;
    ...
    testy!(canonical_cell_path_from_root(in_root()?, "cell1//bsmr") -> {
        abs_path: from_root("cell1/bsmr"),
    ...
    testy!(corrects_malformed_cross_cell_forward_path_from_root(in_root()?, "root//cell1/bsmr") -> {
        abs_path: from_root("cell1/bsmr"),
        canonical: "cell1//bsmr",
        cell_name: "cell1",
        cell_path: "bsmr",
    ...
    #[tokio::test]
    async fn test_root_dir_as_empty_string_is_ready_for_subdirs() -> bsmr_error::Result<()> {
        let cwd = in_root()?;
        let uut =
            PathSanitizer::new(&cell_configs(&cwd)?, &cwd, &find_invocation_roots(&cwd)?).await?;
    ...
    #[tokio::test]
    async fn test_slash_terminated_dir_is_ready_for_subdirs() -> bsmr_error::Result<()> {
        let cwd = in_root()?;
        let uut =
            PathSanitizer::new(&cell_configs(&cwd)?, &cwd, &find_invocation_roots(&cwd)?).await?;
    ...
    #[tokio::test]
    async fn test_partial_with_no_slash_is_not_ready_for_subdirs() -> bsmr_error::Result<()> {
        let cwd = in_root()?;
        let uut =
            PathSanitizer::new(&cell_configs(&cwd)?, &cwd, &find_invocation_roots(&cwd)?).await?;
    ...
    #[tokio::test]
    async fn test_fully_qualified_cell_is_ready_for_subdirs() -> bsmr_error::Result<()> {
        let cwd = in_root()?;
        let uut =
            PathSanitizer::new(&cell_configs(&cwd)?, &cwd, &find_invocation_roots(&cwd)?).await?;
    ...
    #[tokio::test]
    async fn test_bails_on_nonexistent_cell() -> bsmr_error::Result<()> {
        let cwd = in_root()?;
        let uut =
            PathSanitizer::new(&cell_configs(&cwd)?, &cwd, &find_invocation_roots(&cwd)?).await?;
    ...
    testy!(absolute_path_in_subcell(in_dir("cell1")?, &abs_str_from_root("cell1/bsmr")?) -> {
        abs_path: from_root("cell1/bsmr"),
        canonical: "cell1//bsmr",
        cell_name: "cell1",
        cell_path: "bsmr",
    ...
    testy!(aliased_cell(in_dir("cell1/bsmr/prelude")?, "cell1_alias//bsmr") -> {
        abs_path: from_root("cell1/bsmr"),
        canonical: "cell1//bsmr",
        cell_name: "cell1",
        cell_path: "bsmr",
        given: "cell1_alias//bsmr",
        to_string: "cell1_alias//bsmr",
    });

    #[tokio::test]
    async fn test_creation_returns_error_on_non_local_alias() -> bsmr_error::Result<()> {
        let cwd = in_dir("cell1/bsmr")?;
        let uut =
            PathSanitizer::new(&cell_configs(&cwd)?, &cwd, &find_invocation_roots(&cwd)?).await?;

        assert!(uut.sanitize("cell1_alias//bsmr").is_err());
