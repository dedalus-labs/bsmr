...
/// and cannot rely on bsmr's own parsing logic which (generally) requires
/// its targets to be well-formed.
///
/// Its goal is to create a label that bsmr commands will accept. These may,
...
/// 3. When corrections are necessary to create a label that bsmr will accept,
...
impl CompleteCommand {
    pub fn exec(
        ...
        let lockfile = bsmr_env!("COMPLETION_VERIFY_LOCKFILE", applicability = testing)?
            .map(AbsPath::new)
            .transpose()?;
