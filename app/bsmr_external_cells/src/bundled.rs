...
use bsmr_common::io::fs::is_executable;
...
use bsmr_core::directory_digest::DirectoryDigest;
...
use bsmr_external_cells_bundled::BundledCell;
use bsmr_external_cells_bundled::BundledFile;
use bsmr_external_cells_bundled::get_bundled_data;
...
impl BundledFileOpsDelegate {
    ...
    async fn read_dir(&self, path: &CellRelativePath) -> bsmr_error::Result<Arc<[RawDirEntry]>> {
    ...
    fn read_file_if_exists(
        &self,
        path: &CellRelativePath,
    ) -> bsmr_error::Result<Option<&'static str>> {
...
) -> bsmr_error::Result<BundledFileOpsDelegate> {
...
pub(crate) async fn get_file_ops_delegate(
    ctx: &mut DiceComputations<'_>,
    cell_name: CellName,
) -> bsmr_error::Result<Arc<BundledFileOpsDelegate>> {
    #[derive(
        dupe::Dupe,
        Clone,
        Copy,
    ...
    impl Key for BundledFileOpsDelegateKey {
        type Value = bsmr_error::Result<Arc<BundledFileOpsDelegate>>;
