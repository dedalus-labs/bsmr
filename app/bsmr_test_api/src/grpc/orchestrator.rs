...
use bsmr_downward_api_proto::ConsoleRequest;
use bsmr_downward_api_proto::ExternalEventRequest;
...
use bsmr_test_proto::ExecuteResponse2;
use bsmr_test_proto::PrepareForLocalExecutionResponse;
use bsmr_test_proto::ReportTestResultRequest;
use bsmr_test_proto::ReportTestSessionRequest;
use bsmr_test_proto::ReportTestsDiscoveredRequest;
use bsmr_test_proto::Testing;
use bsmr_test_proto::UploadFileToCasRequest;
use bsmr_test_proto::UploadFileToCasResponse;
use bsmr_test_proto::test_orchestrator_client;
use bsmr_test_proto::test_orchestrator_server;
use dupe::Dupe;
...
/// Test runner client to bsmr test orchestrator.
///
/// When running `bsmr test`, bsmr starts a gRPC server
/// and spawns a test runner which connects to bsmr process using this client.
...
impl TestOrchestratorClient {
        ...
        let response = match response.ok_or_else(|| internal_error!("Missing `response`"))? {
            ...
            bsmr_test_proto::execute_response2::Response::Cancelled(
                ...
                let reason = match reason {
                    Some(reason) => bsmr_test_proto::CancellationReason::try_from(reason)
                        .map(|proto_reason| match proto_reason {
                            bsmr_test_proto::CancellationReason::NotSpecified => {
                                crate::data::CancellationReason::NotSpecified
                            }
                            bsmr_test_proto::CancellationReason::ReQueueTimeout => {
    ...
    pub async fn end_of_test_results(&self, exit_code: i32) -> bsmr_error::Result<()> {
        self.test_orchestrator_client
            .clone()
    ...
    pub async fn upload_to_cas(
        ...
        let UploadFileToCasResponse { digest } = self
            ...
            .upload_file_to_cas(UploadFileToCasRequest {
                local_path,
                ttl_config: Some(bsmr_test_proto::TtlConfig {
                    ttl_seconds,
                    use_case,
                }),
            })
    ...
    T: TestOrchestrator + Send + Sync + 'static,
{
    async fn execute2(
        &self,
        request: tonic::Request<bsmr_test_proto::ExecuteRequest2>,
            ...
            let response = match response {
                ...
                ExecuteResponse::Cancelled(reason) => {
                    let reason = if let Some(reason) = reason {
                        match reason {
                            crate::data::CancellationReason::NotSpecified => {
                                Some(bsmr_test_proto::CancellationReason::NotSpecified.into())
                            }
                            crate::data::CancellationReason::ReQueueTimeout => {
                                Some(bsmr_test_proto::CancellationReason::ReQueueTimeout.into())
                            }
                        }
                    } else {
                        None
    ...
    async fn prepare_for_local_execution(
        &self,
        request: tonic::Request<bsmr_test_proto::PrepareForLocalExecutionRequest>,
    ) -> Result<tonic::Response<PrepareForLocalExecutionResponse>, tonic::Status> {
        to_tonic(async move {
            let bsmr_test_proto::PrepareForLocalExecutionRequest {
                test_executable,
                required_local_resources,
            } = request.into_inner();
    ...
    T: DownwardApi + Send + Sync + 'static,
{
    ...
    ) -> Result<tonic::Response<bsmr_downward_api_proto::Empty>, tonic::Status> {
        to_tonic(async move {
