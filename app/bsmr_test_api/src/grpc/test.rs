...
impl TestExecutor for MockExecutor {
    async fn external_runner_spec(&self, _: ExternalRunnerSpec) -> bsmr_error::Result<()> {
        Ok(())
    }
...
#[tokio::test]
async fn test_basic() -> bsmr_error::Result<()> {
...
#[tokio::test]
async fn test_client_disconnect() -> bsmr_error::Result<()> {
    let (client_io, server_io) = tokio::io::duplex(64);
    let mut server =
