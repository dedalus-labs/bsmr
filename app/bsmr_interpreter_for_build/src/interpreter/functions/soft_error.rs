...
fn validate_category(category: &str) -> starlark::Result<()> {
    if !category.starts_with("starlark_") {
        return Err(
            bsmr_error::Error::from(SoftErrorError::InvalidCategory(category.to_owned())).into(),
        );
    }
...
pub(crate) fn register_soft_error(builder: &mut GlobalsBuilder) {
    ...
    /// In the open source version of Bessemer this function always results in an error
