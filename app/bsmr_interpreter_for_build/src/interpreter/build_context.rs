...
impl PerFileTypeContext {
    ...
    pub(crate) fn into_build(self) -> bsmr_error::Result<ModuleInternals> {
        match self {
            PerFileTypeContext::Build(internals) => Ok(internals),
    ...
    pub(crate) fn require_package_file(
        &self,
        function_name: &str,
    ) -> bsmr_error::Result<&PackageFileEvalCtx> {
        match self {
    ...
    pub(crate) fn into_package_file(self) -> bsmr_error::Result<PackageFileEvalCtx> {
        match self {
            PerFileTypeContext::Package(ctx) => Ok(ctx),
...
impl<'a> BuildContext<'a> {
    ...
    ) -> bsmr_error::Result<&'a1 BuildContext<'a>> {
    ...
    pub fn require_package(&self) -> bsmr_error::Result<PackageLabel> {
    ...
    pub(crate) fn base_path(&self) -> bsmr_error::Result<CellPath> {
