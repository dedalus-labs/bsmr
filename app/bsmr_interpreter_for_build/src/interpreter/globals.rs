...
use bsmr_interpreter::downstream_crate_starlark_defs::REGISTER_BSMR_ACTION_IMPL_GLOBALS;
use bsmr_interpreter::downstream_crate_starlark_defs::REGISTER_BSMR_ANON_TARGETS_GLOBALS;
use bsmr_interpreter::downstream_crate_starlark_defs::REGISTER_BSMR_BUILD_API_GLOBALS;
use bsmr_interpreter::downstream_crate_starlark_defs::REGISTER_BSMR_BUILD_API_INTERNALS;
use bsmr_interpreter::downstream_crate_starlark_defs::REGISTER_BSMR_BXL_GLOBALS;
use bsmr_interpreter::downstream_crate_starlark_defs::REGISTER_BSMR_CFG_CONSTRUCTOR_GLOBALS;
use bsmr_interpreter::downstream_crate_starlark_defs::REGISTER_BSMR_TRANSITION_GLOBALS;
use bsmr_interpreter::starlark_promise::register_promise;
use bsmr_interpreter::types::cell_path::register_cell_path;
use bsmr_interpreter::types::cell_root::register_cell_root;
...
// and `starlark_library_extensions_for_bsmr` are all the same, since all symbols are available
...
pub fn register_load_natives(builder: &mut GlobalsBuilder) {
    from_late_binding(&REGISTER_BSMR_CFG_CONSTRUCTOR_GLOBALS, builder);
    from_late_binding(&REGISTER_BSMR_TRANSITION_GLOBALS, builder);
...
pub fn register_analysis_natives(builder: &mut GlobalsBuilder) {
    from_late_binding(&REGISTER_BSMR_ACTION_IMPL_GLOBALS, builder);
    from_late_binding(&REGISTER_BSMR_BUILD_API_GLOBALS, builder);
    register_promise(builder);
    from_late_binding(&REGISTER_BSMR_ANON_TARGETS_GLOBALS, builder);
}

pub fn register_bxl_natives(builder: &mut GlobalsBuilder) {
    from_late_binding(&REGISTER_BSMR_BXL_GLOBALS, builder);
}

pub fn starlark_library_extensions_for_bsmr() -> &'static [LibraryExtension] {
    &[
...
fn register_all_natives(builder: &mut GlobalsBuilder) {
    ...
    for ext in starlark_library_extensions_for_bsmr() {
        ext.add(builder);
    }
}
...
fn register_all_internals(builder: &mut GlobalsBuilder) {
    ...
    from_late_binding(&REGISTER_BSMR_BUILD_API_INTERNALS, builder);
...
pub(crate) fn base_globals() -> GlobalsBuilder {
    ...
    global_env.namespace("__bsmr_builtins__", |x| {
