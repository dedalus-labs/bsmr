...
use bsmr_node::nodes::eval_result::EvaluationResultWithStats;
use bsmr_node::super_package::SuperPackage;
use bsmr_util::per_thread_instruction_counter::PerThreadInstructionCounter;
...
pub type ParseResult = Result<ParseData, bsmr_error::Error>;
...
impl InterpreterForDir {
    fn verbose_gc() -> bsmr_error::Result<bool> {
        match std::env::var_os("BSMR_STARLARK_VERBOSE_GC") {
            Some(val) => Ok(!val.is_empty()),
            None => Ok(false),
        }
    }

    fn is_ignore_attrs_for_profiling() -> bsmr_error::Result<bool> {
        // If unsure, feel free to break this code or just delete it.
        // It is intended only for profiling of very specific use cases.
        let ignore_attrs_for_profiling = match std::env::var_os("BSMR_IGNORE_ATTRS_FOR_PROFILING") {
    ...
    fn create_env<'v>(
        ...
        .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::Interpreter))?;
    ...
    pub(crate) fn parse(
        ...
        let ast = match AstModule::parse(
            ...
            Ok(ast) => ast,
            Err(e) => {
                return Ok(Err(bsmr_error::Error::from(e).context(format!(
                    "Error parsing: `{}`",
    ...
    ) -> bsmr_error::Result<(FinishedStarlarkEvaluation, EvalResult)> {
