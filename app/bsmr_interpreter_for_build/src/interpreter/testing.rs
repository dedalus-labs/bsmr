...
/// ignore the targets
pub fn run_simple_starlark_test(content: &str) -> bsmr_error::Result<()> {
...
pub fn cells(extra_root_config: Option<&str>) -> bsmr_error::Result<CellsData> {
...
pub fn expect_error<T>(result: bsmr_error::Result<T>, content: &str, expected: &str) {
    match result {
        Ok(_) => {
...
impl Tester {
    ...
    pub fn with_cells(cells_data: CellsData) -> bsmr_error::Result<Self> {
        let (
            cell_alias_resolver,
            cell_resolver,
            root_config,
    ...
    fn interpreter(&self) -> bsmr_error::Result<Arc<InterpreterForDir>> {
    ...
    ) -> bsmr_error::Result<EvaluationResult> {
    ...
    pub fn run_starlark_test(&mut self, content: &str) -> bsmr_error::Result<TargetsMap> {
    ...
    /// environment.
    pub fn run_starlark_bzl_test(&mut self, content: &str) -> bsmr_error::Result<()> {
        let import_path = ImportPath::testing_new("root//some/package:defs.bzl");
        let template = indoc!(
            r#"
