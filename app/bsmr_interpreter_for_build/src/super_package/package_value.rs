...
use bsmr_node::metadata::key::MetadataKey;
use bsmr_node::metadata::key::MetadataKeyRef;
use bsmr_node::metadata::super_package_values::SuperPackageValues;
use dupe::Dupe;
use pagable::Pagable;
...
impl SuperPackageValuesImpl {
    pub fn get(values: &dyn SuperPackageValues) -> bsmr_error::Result<&SuperPackageValuesImpl> {
        values
            .as_any()
    ...
    ) -> bsmr_error::Result<Arc<dyn SuperPackageValues>> {
...
impl SuperPackageValues for SuperPackageValuesImpl {
    ...
    fn package_values_json(&self) -> bsmr_error::Result<SmallMap<MetadataKey, serde_json::Value>> {
...
impl OwnedFrozenStarlarkPackageValue {
    ...
    pub(crate) fn to_json_value(&self) -> bsmr_error::Result<serde_json::Value> {
        self.0
            .value()
            .to_json_value()
...
pub(crate) fn write_package_value_impl<'v>(
    ...
    let key = MetadataKeyRef::new(key).map_err(bsmr_error::Error::from)?;
    ...
    if package_file_extra.package_values.borrow().contains_key(key) {
        return Err(
            bsmr_error::Error::from(PackageValueError::KeyAlreadySetInThisFile(key.to_owned()))
                .into(),
        );
    }
    ...
    if !overwrite {
        if package_ctx.parent.package_values().contains_key(key) {
            return Err(
                bsmr_error::Error::from(PackageValueError::KeySetInParentFile(key.to_owned()))
                    .into(),
            );
        }
    }
