...
use bsmr_node::attrs::traversal::CoercedAttrTraversal;
...
pub(crate) fn check_within_view(
    ...
    impl<'x> WithinViewCheckTraversal<'x> {
        fn check_dep_within_view(&self, dep: &TargetLabel) -> bsmr_error::Result<()> {
            if self.pkg == dep.pkg()
    ...
    impl<'a, 'x> CoercedAttrTraversal<'a> for WithinViewCheckTraversal<'x> {
        ...
        fn input(&mut self, _input: SourcePathRef) -> bsmr_error::Result<()> {
            Ok(())
        }
    }
