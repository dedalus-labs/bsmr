...
use bsmr_node::attrs::attr_type::arg::ArgAttrType;
use bsmr_node::attrs::attr_type::arg::MacroBase;
use bsmr_node::attrs::attr_type::arg::MacroDepKind;
use bsmr_node::attrs::attr_type::arg::QueryExpansion;
use bsmr_node::attrs::attr_type::arg::StringWithMacrosPart;
use bsmr_node::attrs::attr_type::arg::UnconfiguredMacro;
use bsmr_node::attrs::attr_type::arg::UnconfiguredStringWithMacros;
use bsmr_node::attrs::attr_type::arg::UnrecognizedMacro;
use bsmr_node::attrs::attr_type::arg::parser;
use bsmr_node::attrs::attr_type::arg::parser::ParsedMacro;
use bsmr_node::attrs::attr_type::arg::parser::parse_macros;
use bsmr_node::attrs::attr_type::query::QueryAttrType;
use bsmr_node::attrs::attr_type::query::QueryMacroBase;
...
fn get_single_target_arg(
    args: Vec<String>,
    ctx: &dyn AttrCoercionContext,
) -> bsmr_error::Result<ProvidersLabel> {
    if args.len() != 1 {
...
pub trait UnconfiguredMacroExt {
    ...
    ) -> bsmr_error::Result<UnconfiguredMacro> {
...
mod tests {
    ...
    trait GetMacroDeps {
        type DepsType;
        fn get_deps(&self) -> bsmr_error::Result<Vec<Self::DepsType>>;
    }
    ...
    #[test]
    fn test_concat() -> bsmr_error::Result<()> {
        // patternlint-disable-next-line bsmr-no-starlark-module: Test
    ...
    #[test]
    fn test_location() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_location_exec() -> bsmr_error::Result<()> {
        let ctx = coercion_ctx();
        let location = UnconfiguredMacro::new_location(
            &ctx,
            vec!["//some:target".to_owned()],
    ...
    #[test]
    fn test_location_toolchain() -> bsmr_error::Result<()> {
        let ctx = coercion_ctx();
        let location = UnconfiguredMacro::new_location(
            &ctx,
            vec!["//some:target".to_owned()],
    ...
    fn test_exe() -> bsmr_error::Result<()> {
        ...
        if let MacroBase::Exe { label, .. } = &configured {
            ...
            return Err(bsmr_error!(bsmr_error::ErrorTag::Input, "Expected Exe"));
        }
    ...
    #[test]
    fn test_exe_target() -> bsmr_error::Result<()> {
        let ctx = coercion_ctx();
