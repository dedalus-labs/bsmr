...
pub(crate) trait AttributeExt {
    ...
    fn attr<'v>(
        ...
        doc: &str,
        coercer: AttrType,
    ) -> bsmr_error::Result<StarlarkAttribute>;
}
...
) -> bsmr_error::Result<BuildAttrCoercionContext> {
...
fn dep_like_attr_handle_providers_arg(providers: Vec<Value>) -> bsmr_error::Result<ProviderIdSet> {
    Ok(ProviderIdSet::from(providers.try_map(|v| {
        match v.as_provider_callable() {
            Some(callable) => bsmr_error::Ok(callable.id()?.dupe()),
            None => Err(
...
fn attr_module(registry: &mut GlobalsBuilder) {
    ...
    fn option<'v>(
        ...
        match attr.default() {
            Some(default) if !default.may_return_none() => Err(bsmr_error::Error::from(
    ...
    fn default_only<'v>(
        ...
        let Some(default) = inner.default().duped() else {
            return Err(bsmr_error::Error::from(AttrError::DefaultOnlyMustHaveDefault).into());
        };
