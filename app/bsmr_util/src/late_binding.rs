...
///
/// bsmr compilation speed is very important for us, so we try to make smaller crates
/// with as few dependencies between them as possible. So when we change one line of code,
/// we recompile as little as possible.
///
/// # Example
///
/// We have a crate `bsmr_node` that defines target graph data types
...
/// `bsmr_interpreter_for_build`, but code that needs to compute a target node,
/// calls the function defined in `bsmr_node` crate without compile time dependency
/// on `bsmr_interpreter_for_build` or `starlark` crates.
///
/// The best example of this pattern is probably `bsmr_bxl` crate:
/// the only target depending on `bsmr_bxl` is the final binary with `bsmr.rs` file.
/// So when `bsmr_bxl` changes, we recompile only `bsmr_bxl` and `bsmr.rs`.
/// At the moment of writing this comment, `bsmr_bxl` had only one public function,
...
/// * in the file `app/bsmr/bin/bsmr.rs` call `init_late_bindings()` of the corresponding crate
...
impl<T> LateBinding<T> {
    ...
    #[inline]
    pub fn get(&self) -> bsmr_error::Result<&T> {
        self.symbol
            .get()
