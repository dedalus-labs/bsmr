...
pub fn get_last_command_execution_kind(
    ...
    if let Some(command_kind) = last_command_kind {
        ...
        match command_kind.command.as_ref() {
            ...
            Some(Command::RemoteCommand(bsmr_data::RemoteCommand {
                cache_hit: true,
                cache_hit_type,
                ..
            })) => {
                match bsmr_data::CacheHitType::try_from(*cache_hit_type).unwrap() {
                    // ActionCache is 0, so this should be backwards compatible
                    bsmr_data::CacheHitType::ActionCache => LastCommandExecutionKind::Cached,
                    bsmr_data::CacheHitType::RemoteDepFileCache => {
                        LastCommandExecutionKind::RemoteDepFileCached
                    }
                    bsmr_data::CacheHitType::Executed => LastCommandExecutionKind::Remote,
                }
            }
            Some(Command::RemoteCommand(bsmr_data::RemoteCommand {
                cache_hit: false, ..
...
pub fn get_last_command_execution_time(action: &bsmr_data::ActionExecutionEnd) -> ExecTime {
