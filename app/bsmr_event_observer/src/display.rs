...
use bsmr_data::AnonTarget;
use bsmr_data::BxlFunctionKey;
use bsmr_data::BxlFunctionLabel;
use bsmr_data::ConfiguredTargetLabel;
...
pub fn display_anon_target(ctl: &AnonTarget) -> bsmr_error::Result<String> {
    if let AnonTarget {
...
pub fn display_analysis_target(
    target: &bsmr_data::analysis_start::Target,
    opts: TargetDisplayOptions,
) -> bsmr_error::Result<String> {
    use bsmr_data::analysis_start::Target;
    match target {
        ...
        Target::DynamicLambda(dynamic) => {
            use bsmr_data::dynamic_lambda_owner::Owner;
            match dynamic
                .owner
                .as_ref()
...
pub fn display_bxl_key(ctl: &BxlFunctionKey) -> bsmr_error::Result<String> {
...
pub fn display_event(event: &BuckEvent, opts: TargetDisplayOptions) -> bsmr_error::Result<String> {
    let res: bsmr_error::Result<_> = try {
        ...
        let res: bsmr_error::Result<_> = match data {
            ...
            Data::FinalMaterialization(materialization) => {
                let build = &materialization
                    .artifact
                    .as_ref()
                    .ok_or_else(|| bsmr_error::Error::from(ParseEventError::MissingArtifact))?;

                let key = display_action_key(
                    build.key.as_ref().ok_or_else(|| {
                        bsmr_error::Error::from(ParseEventError::MissingActionKey)
                    })?,
                    opts,
                )?;
                let path = {
                    if build.path.is_empty() {
                        Err(bsmr_error::Error::from(
            ...
            Data::MatchDepFiles(bsmr_data::MatchDepFilesStart {
            ...
            Data::SharedTask(bsmr_data::SharedTaskStart { owner_trace_id }) => Ok(format!(
            ...
            Data::DiceSynchronizeSection(..) => Ok("Synchronizing bsmr internal state".to_owned()),
            Data::DiceCleanup(..) => Ok("Cleaning up graph state".to_owned()),
            Data::ExclusiveCommandWait(bsmr_data::ExclusiveCommandWaitStart { command_name }) => {
            ...
            Data::DeferredPreparationStage(prep) => {
                use bsmr_data::deferred_preparation_stage_start::Stage;
                match prep
                    .stage
                    .as_ref()
            ...
            Data::DynamicLambda(lambda) => {
                use bsmr_data::dynamic_lambda_start::Owner;
            ...
            Data::ConnectToInstaller(bsmr_data::ConnectToInstallerStart { tcp_port }) => {
...
fn display_file_watcher(provider: i32) -> &'static str {
    match bsmr_data::FileWatcherProvider::try_from(provider) {
        Ok(bsmr_data::FileWatcherProvider::Watchman) => "Watchman",
        Ok(bsmr_data::FileWatcherProvider::RustNotify) => "notify",
        Ok(bsmr_data::FileWatcherProvider::FsHashCrawler) => "fs_hash_crawler",
        Ok(bsmr_data::FileWatcherProvider::EdenFs) => "EdenFS",
        Err(_) => "unknown mechanism",
    }
}

pub fn display_analysis_stage(stage: &bsmr_data::analysis_stage_start::Stage) -> &'static str {
    use bsmr_data::analysis_stage_start::Stage;
...
pub fn display_file_watcher_end(file_watcher_end: &bsmr_data::FileWatcherEnd) -> Vec<String> {
...
pub fn display_executor_stage(
    stage: &bsmr_data::executor_stage_start::Stage,
) -> Option<&'static str> {
    use bsmr_data::executor_stage_start::Stage;

    let label = match stage {
        Stage::Prepare(..) => "prepare",
        Stage::CacheQuery(cache_query) => {
            match bsmr_data::CacheType::try_from(cache_query.cache_type).unwrap() {
                bsmr_data::CacheType::ActionCache => "re_action_cache",
                bsmr_data::CacheType::RemoteDepFileCache => "re_dep_file_cache",
            }
        }
        Stage::CacheHit(..) => "re_download",
        Stage::Re(re) => {
            use bsmr_data::re_stage::Stage;
        ...
        Stage::Local(local) => {
            use bsmr_data::local_stage::Stage;
...
pub fn format_test_result(
    ...
    verbosity: Verbosity,
) -> bsmr_error::Result<Option<Lines>> {
    let bsmr_data::TestResult {
        name,
        status,
        duration,
        details,
...
pub struct ActionErrorDisplay<'a> {
    ...
    pub reason: String,
    pub command: Option<&'a bsmr_data::CommandExecutionDetails>,
    pub error_diagnostics: Option<&'a bsmr_data::ActionErrorDiagnostics>,
}
    ...
    fn simple_format_inner(
        ...
        if let Some(error_diagnostics) = self.error_diagnostics {
            match error_diagnostics.data.as_ref().unwrap() {
                bsmr_data::action_error_diagnostics::Data::SubErrors(sub_errors) => {
                ...
                bsmr_data::action_error_diagnostics::Data::HandlerInvocationError(error) => {
...
pub fn get_action_error_reason(error: &bsmr_data::ActionError) -> bsmr_error::Result<String> {
    use bsmr_data::action_error::Error;
    ...
    Ok(
            ...
            .ok_or_else(|| internal_error!("Internal error: Missing error in action error"))?
        {
            ...
            Error::CommandExecutionError(bsmr_data::CommandExecutionError {}) => {
...
pub fn display_action_error(
    error: &bsmr_data::ActionError,
    opts: TargetDisplayOptions,
) -> bsmr_error::Result<ActionErrorDisplay<'_>> {
...
fn failure_reason_for_command_execution(
    command_execution: &bsmr_data::CommandExecution,
) -> bsmr_error::Result<String> {
    use bsmr_data::command_execution::Cancelled;
    use bsmr_data::command_execution::Error;
    use bsmr_data::command_execution::Failure;
    use bsmr_data::command_execution::Status;
    use bsmr_data::command_execution::Success;
    use bsmr_data::command_execution::Timeout;
    use bsmr_data::command_execution::WorkerFailure;
...
impl<'a> CriticalPathEntryDisplay<'a> {
    ...
    pub fn from_entry(
        entry: &'a bsmr_data::CriticalPathEntry2,
        opts: TargetDisplayOptions,
    ) -> bsmr_error::Result<Option<Self>> {
        use bsmr_data::critical_path_entry2::Entry;
        ...
        let (kind, name, category, identifier, execution_kind) = match entry_data {
            Entry::Analysis(analysis) => {
                use bsmr_data::critical_path_entry2::analysis::Target;
            ...
            Entry::DynamicAnalysis(analysis) => {
                use bsmr_data::critical_path_entry2::dynamic_analysis::Target;
            ...
            Entry::ActionExecution(action_execution) => {
                use bsmr_data::critical_path_entry2::action_execution::Owner;
                ...
                let execution_kind = Some(
                    bsmr_data::ActionExecutionKind::try_from(action_execution.execution_kind)
                        .unwrap_or(bsmr_data::ActionExecutionKind::NotSet)
                        .as_str_name(),
                );
            ...
            Entry::FinalMaterialization(materialization) => {
                use bsmr_data::critical_path_entry2::final_materialization::Owner;
