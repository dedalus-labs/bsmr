...
use bsmr_data::re_platform::Property;
...
impl<'a> WhatRanOptionsRegex<'a> {
    pub fn from_options(options: &'a WhatRanOptions) -> bsmr_error::Result<Self> {
...
pub enum WhatRanRelevantAction {
    ActionExecution(bsmr_data::ActionExecutionStart),
    TestDiscovery(bsmr_data::TestDiscoveryStart),
    TestRun(bsmr_data::TestRunStart),
    SetupLocalResources(bsmr_data::SetupLocalResourcesStart),
}
    ...
    pub fn from_bsmr_data(data: &bsmr_data::bsmr_event::Data) -> Option<Self> {
        match data {
            bsmr_data::bsmr_event::Data::SpanStart(span) => match &span.data {
                ...
                Some(bsmr_data::span_start_event::Data::TestDiscovery(suite)) => {
                    Some(Self::TestDiscovery(suite.clone()))
                }
                Some(bsmr_data::span_start_event::Data::TestRun(test)) => {
                    Some(Self::TestRun(test.clone()))
                }
                Some(bsmr_data::span_start_event::Data::LocalResources(setup)) => {
...
pub trait WhatRanOutputWriter {
    fn emit_command(&mut self, command: WhatRanOutputCommand<'_>) -> bsmr_error::Result<()>;
}
...
pub enum CommandReproducer {
    CacheQuery(bsmr_data::CacheQuery),
    CacheHit(bsmr_data::CacheHit),
    LocalDepFileCacheHit,
    ReExecute(bsmr_data::ReExecute),
    LocalExecute(bsmr_data::LocalExecute),
    WorkerExecute(bsmr_data::WorkerExecute),
    WorkerInit(bsmr_data::WorkerInit),
}
    ...
    pub fn executor(&self) -> String {
        match self {
            ...
            Self::CacheHit(cache) => {
                ...
                match bsmr_data::CacheHitType::try_from(cache_type) {
    ...
    pub fn from_bsmr_data(
                ...
                &span.data
        {
            match &executor_stage.stage {
                Some(bsmr_data::executor_stage_start::Stage::CacheQuery(cache_hit))
                    if options.emit_cache_queries =>
                {
                    return Some(CommandReproducer::CacheQuery(cache_hit.clone()));
                }
                Some(bsmr_data::executor_stage_start::Stage::CacheHit(cache_hit))
                    if !options.skip_cache_hits =>
                {
                    return Some(CommandReproducer::CacheHit(cache_hit.clone()));
                }
                Some(bsmr_data::executor_stage_start::Stage::Re(re_stage))
                    if !options.skip_remote_executions =>
                {
                    if let Some(bsmr_data::re_stage::Stage::Execute(execute)) = &re_stage.stage {
                ...
                Some(bsmr_data::executor_stage_start::Stage::Local(local_stage)) => {
                    if !options.skip_local_executions {
                        match &local_stage.stage {
                            Some(bsmr_data::local_stage::Stage::Execute(local_execute)) => {
                                return Some(CommandReproducer::LocalExecute(
                                    local_execute.clone(),
                                ));
                            }
                            Some(bsmr_data::local_stage::Stage::WorkerExecute(worker_execute)) => {
                                return Some(CommandReproducer::WorkerExecute(
                                    worker_execute.clone(),
                                ));
                            }
                            Some(bsmr_data::local_stage::Stage::WorkerInit(worker_init)) => {
...
pub struct Command<'a> {
    env: &'a Vec<bsmr_data::EnvironmentEntry>,
    argv: &'a Vec<String>,
}

impl<'a> From<&'a bsmr_data::LocalCommand> for Command<'a> {
    fn from(command: &'a bsmr_data::LocalCommand) -> Self {
        Command {
            env: &command.env,
            argv: &command.argv,
        }
...
impl<'a> From<&'a bsmr_data::WorkerCommand> for Command<'a> {
    fn from(command: &'a bsmr_data::WorkerCommand) -> Self {
        Command {
            env: &command.env,
            argv: &command.argv,
        }
...
impl<'a> From<&'a bsmr_data::WorkerInitCommand> for Command<'a> {
    fn from(command: &'a bsmr_data::WorkerInitCommand) -> Self {
        Command {
            env: &command.env,
            argv: &command.argv,
        }
...
pub fn worker_command_as_fallback_to_string(command: &bsmr_data::WorkerCommand) -> String {
...
pub fn command_to_string<'a>(command: impl Into<Command<'a>>) -> String {
    ...
    let mut cmd = "env -C \"$(bsmr root --kind project)\" --".to_owned();
...
fn executor_with_platform(execute: &bsmr_data::ReExecute) -> String {
...
mod tests {
    ...
    fn test_executor_with_platform_no_platform() {
        let execute = bsmr_data::ReExecute::default();
        let result = executor_with_platform(&execute);
