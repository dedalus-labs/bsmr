...
impl<T: SpanTrackable + Dupe> Span<T> {
    ...
    fn remove_child(&mut self, child: Span<T>, roots: &mut Roots<T>) -> bsmr_error::Result<()> {
...
impl<T: SpanTrackable + Dupe> SpanTracker<T> {
    ...
    pub fn start_at(&mut self, event: &T) -> bsmr_error::Result<()> {
        if !event.is_shown() {
            return Ok(());
        }
    ...
    fn end(&mut self, event: &T) -> bsmr_error::Result<()> {
        let span_id = event
            .span_id()
            .ok_or_else(|| SpanTrackerError::NonSpanEvent(event.dupe()))?;
...
mod tests {
    ...
    #[test]
    fn test_boring_via_self() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_boring_via_child() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_iter_roots_len() -> bsmr_error::Result<()> {
        let e1 = TestSpan::new();
    ...
    #[test]
    fn test_dice_counts() -> bsmr_error::Result<()> {
