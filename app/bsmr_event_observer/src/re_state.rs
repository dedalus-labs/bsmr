...
impl ReState {
    ...
    pub fn add_re_session(&mut self, session: &bsmr_data::RemoteExecutionSessionCreated) {
        self.session_id = Some(session.session_id.clone());
    }

    pub fn update(&mut self, snapshot: &bsmr_data::Snapshot) {
    ...
    fn render_detailed(&self, two_snapshots: &TwoSnapshots) -> bsmr_error::Result<Vec<Line>> {
        let mut r = Vec::new();
        if let (Some(first), Some((_, last))) = (&self.first_snapshot, &two_snapshots.last) {
