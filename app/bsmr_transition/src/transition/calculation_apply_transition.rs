...
use bsmr_build_api::actions::query::CONFIGURED_ATTR_TO_VALUE;
use bsmr_build_api::actions::query::PackageLabelOption;
use bsmr_build_api::analysis::calculation::RuleAnalysisCalculation;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::platform_info::PlatformInfo;
...
use bsmr_build_api::transition::TransitionCalculation;
use bsmr_core::configuration::cfg_diff::cfg_diff;
...
) -> bsmr_error::Result<TransitionApplied> {
    let mut args = vec![(
        "platform",
        eval.heap()
...
impl TransitionCalculation for TransitionCalculationImpl {
    async fn apply_transition(
        ...
        impl Key for TransitionKey {
            type Value = bsmr_error::Result<Arc<TransitionApplied>>;
