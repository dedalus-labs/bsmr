...
use bsmr_interpreter::types::transition::TransitionValue;
...
pub(crate) struct FrozenTransition {
    ...
    // Mixed: keys are starlark, `ProvidersLabel` is pagable-only (`bsmr_core`
...
fn register_transition_function(builder: &mut GlobalsBuilder) {
    fn transition<'v>(
        ...
        let path: ImportPath = (*starlark_path_from_build_context(eval)?
            .unpack_load_file()
            .ok_or(bsmr_error::Error::from(TransitionError::OnlyBzl))?)
        .clone();
...
pub(crate) fn init_register_transition() {
    REGISTER_BSMR_TRANSITION_GLOBALS.init(register_transition_function);
