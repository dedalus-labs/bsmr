...
use bsmr_build_api::transition::TransitionAttrProvider;
...
pub(crate) trait FetchTransition {
    ...
    async fn fetch_transition(&mut self, id: &TransitionId) -> bsmr_error::Result<TransitionData>;
}
        ...
        match id {
            TransitionId::MagicObject { path, name } => {
                ...
                let transition = module
                    ...
                    .map_err(|_| {
                        bsmr_error::Error::from(FetchTransitionError::NotFound(id.clone()))
                    })?
                    .0;
...
impl Key for TransitionAttrsKey {
    type Value = bsmr_error::Result<Option<Arc<[String]>>>;
