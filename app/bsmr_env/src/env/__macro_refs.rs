...
pub fn convert_from_str<T>(v: &str) -> bsmr_error::Result<T>
where
    T: FromStr,
...
/// This macro is used to register environment variables that are used by Bessemer.
///
...
///    for self-testing of bsmr
///
/// The macro expands to an expression of type `bsmr_error::Result<Type>` if a default is set, and
/// `bsmr_error::Result<Option<Type>` otherwise.
pub macro bsmr_env {
    ($var:expr, bool $(, $($rest:tt)*)?) => {{
        let v: bsmr_error::Result<bool> = $crate::env::__macro_refs::bsmr_env!($var, type=bool, default=false, converter = |s| {
            match s.to_lowercase().as_str() {
                "1" | "true" => Ok(true),
                "0" | "false" => Ok(false),
                _ => Err(bsmr_error::bsmr_error!(bsmr_error::ErrorTag::Tier0, "Invalid bool value: {}", s).into()),
            }
        }, $($($rest)*)?);
        v
    }},
...
/// Register env name to be shown in `bsmr help-env`.
pub macro bsmr_env_name($var:expr) {{
...
#[allow(unused_macros)]
/// `parser` is `&str -> bsmr_error::Result<$stored_type>`, `processor` is `Option<& $stored_type> -> $output_type`
///
...
macro expand(
    ...
    let v: bsmr_error::Result<$output_ty> = ENV_HELPER.get().map($processor);
    v
}}
