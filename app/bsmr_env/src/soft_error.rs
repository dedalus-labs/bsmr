...
use crate::env::bsmr_env;

type StructuredErrorHandler = Box<
    dyn for<'a> Fn(&'a str, &bsmr_error::Error, (&'a str, u32, u32), StructuredErrorOptions)
        + Send
        + Sync
        + 'static,
>;
...
pub fn bsmr_hard_error_env() -> bsmr_error::Result<Option<&'static str>> {
    bsmr_env!("BSMR_HARD_ERROR")
}
...
pub fn bsmr_show_soft_errors_env() -> bsmr_error::Result<Option<&'static str>> {
    bsmr_env!("BSMR_SHOW_SOFT_ERRORS")
}
...
/// * The error is a `bsmr_error::Error`.
///
/// Soft errors from Meta internal runs can be viewed
/// [in logview](https://www.internalfb.com/logview/overview/bsmr).
///
...
fn hard_error_config() -> bsmr_error::Result<Arc<HardErrorConfig>> {
    ...
    let config = bsmr_hard_error_env()?.unwrap_or_default();
...
pub fn reload_hard_error_config(var_value: &str) -> bsmr_error::Result<()> {
...
pub fn handle_soft_error(
...
) -> Result<bsmr_error::Error, bsmr_error::Error> {
    ...
    if hard_error_config()?.should_panic(category) {
        panic!(
            "Upgraded warning to panic via $BSMR_HARD_ERROR\n {category}: {:?}",
            err
        );
    }
    if hard_error_config()?.should_hard_error(category) {
        return Err(err.context("Upgraded warning to failure via $BSMR_HARD_ERROR"));
    }
...
pub fn initialize(handler: StructuredErrorHandler) -> bsmr_error::Result<()> {
    hard_error_config()?;
...
impl HardErrorConfigHolder {
    fn reload_hard_error_config(&self, var_value: &str) -> bsmr_error::Result<()> {
...
pub fn validate_logview_category(category: &str) -> bsmr_error::Result<()> {
...
pub(crate) mod tests {
    ...
    #[test]
    fn test_hard_error() -> bsmr_error::Result<()> {
