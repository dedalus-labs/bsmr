...
pub(crate) fn analysis_actions_methods_unsorted(builder: &mut MethodsBuilder) {
    ...
    fn declare_output<'v>(
        ...
        let has_content_based_path = has_content_based_path.unwrap_or(
            *bsmr_build_api::interpreter::rule_defs::context::DECLARE_OUTPUT_HAS_CONTENT_BASED_PATH_DEFAULT
                .get()
                .unwrap_or(&false),
        );
    ...
    /// 3. **Subsequent builds**: Bessemer only triggers rebuilds when files listed in the depfile change
    ///
    /// ### Usage Pattern
    ///
    ...
    /// ```
    ///
    /// This tells Bessemer that `output.o` depends on these specific files. Bessemer reads this file
    ...
    /// for equality, allowing Bessemer to match tagged inputs with their corresponding depfiles.
    ///
    /// ### See Also
    ///
