...
use bsmr_artifact::artifact::artifact_type::ArtifactErrors;
...
use bsmr_build_api::interpreter::rule_defs::cmd_args::StarlarkCommandLineValueUnpack;
use bsmr_build_api::interpreter::rule_defs::cmd_args::value_as::ValueAsCommandLineLike;
use bsmr_build_api::interpreter::rule_defs::command_executor_config::parse_custom_re_image;
use bsmr_build_api::interpreter::rule_defs::command_executor_config::parse_meta_internal_extra_params;
use bsmr_build_api::interpreter::rule_defs::context::AnalysisActions;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::run_info::RunInfo;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::worker_run_info::WorkerRunInfo;
...
pub(crate) fn analysis_actions_methods_run(methods: &mut MethodsBuilder) {
    ...
    /// * `category`: category and identifier - when used together, identify the action in Bessemer's
    ...
    /// * `no_outputs_cleanup`: if this flag is set then Bessemer won't clean the outputs of a previous
    ...
    ///     * Metadata contains the path relative to the Bessemer project root and hash digest for
    ///       every action input (this excludes symlinks as they could be resolved by a user script
    ///       if needed). The resolved path relative to the Bessemer project for the metadata file will
    ...
    ///     * After execution, Bessemer reads the depfile and only tracks changes to inputs listed in it,
    ...
    ///     * When `true`, action outputs are cached during trace builds (via `bsmr debug trace-io`)
    ...
    fn run<'v>(
        ...
        #[starlark(require = named, default = NoneOr::None)] allow_local_cache_upload: NoneOr<bool>,
        ...
        impl<'v> CommandLineArtifactVisitor<'v> for RunCommandArtifactVisitor<'v> {
            ...
            fn push_frame(&mut self) -> bsmr_error::Result<()> {
                self.depth += 1;
                if self.depth > 1000 {
        ...
        let weight = match (weight, weight_percentage) {
            ...
            (Some(v), None) => {
                if v == 0 {
                    return Err(bsmr_error::Error::from(RunActionError::InvalidWeight(v)).into());
                }
            ...
            (Some(..), Some(..)) => {
                return Err(
                    bsmr_error::Error::from(RunActionError::DuplicateWeightsSpecified).into(),
                );
            }
        };
        ...
        if let Some(frozen) = { artifacts.frozen_outputs }.pop() {
            return Err(bsmr_error::Error::from(ArtifactErrors::DuplicateBind(frozen)).into());
        }
        ...
        if let Some(dep_files) = dep_files {
            for (key, tag) in dep_files {
                ...
                if count != 1 {
                    return Err(
                        bsmr_error::Error::from(RunActionError::InvalidDepFileOutputs {
                            key: (*key).to_owned(),
                            count,
                        })
                        .into(),
                ...
                match dep_files_configuration.labels.entry(tag.dupe()) {
                    ...
                    small_map::Entry::Occupied(o) => {
                        return Err(
                            bsmr_error::Error::from(RunActionError::ConflictingDepFiles {
        ...
        if let Some((input, conflicting_tags)) = inputs_with_multiple_tags_for_dep_files.first() {
            return Err(
                bsmr_error::Error::from(RunActionError::ConflictingDepFileInputTags {
                    input: input.dupe(),
        ...
        let metadata_param = match (metadata_env_var, metadata_path) {
            (Some(env_var), Some(path)) => {
                ...
                bsmr_error::Ok(Some(Box::new(MetadataParameter {
                    env_var,
                    path,
        ...
        if artifacts.declared_outputs.is_empty() {
            return Err(bsmr_error::Error::from(RunActionError::NoOutputsSpecified).into());
        }
        let heap = eval.heap();
        ...
        let re_dependencies = remote_execution_dependencies
            ...
            .collect::<bsmr_error::Result<ThinBoxSlice<RemoteExecutorDependency>>>()?;

        let re_gang_workers = re_gang_workers
            .into_iter()
            .map(ReGangWorker::parse)
            .collect::<bsmr_error::Result<ThinBoxSlice<ReGangWorker>>>()?;
        ...
        let timeout = match timeout_seconds.into_option() {
            Some(t) => {
                if t == 0 {
                    return Err(bsmr_error::Error::from(RunActionError::InvalidTimeout(t)).into());
                }
        ...
        let action = UnregisteredRunAction {
            ...
            allow_local_cache_upload: allow_local_cache_upload.into_option(),
        ...
        if expect_eligible_for_dedupe {
            ...
            for i in artifacts.inputs.iter() {
                if i.is_eligible_for_dedupe(target_platform)
                    == bsmr_data::EligibleForDedupe::IneligibleInput
                {
                    return Err(bsmr_error::Error::from(
