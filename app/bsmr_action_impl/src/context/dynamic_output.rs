...
use bsmr_artifact::artifact::artifact_type::BoundBuildArtifact;
...
impl DynamicActionsOutputArtifactBinder {
    ...
    ) -> bsmr_error::Result<BoundBuildArtifact> {
...
) -> bsmr_error::Result<Box<[ValueTyped<'v, StarlarkOutputArtifact<'v>>]>> {
...
pub(crate) fn analysis_actions_methods_dynamic_output(methods: &mut MethodsBuilder) {
    ...
    fn dynamic_output<'v>(
        ...
        if outputs.items.is_empty() {
            return Err(bsmr_error::Error::from(DynamicOutputError::EmptyOutput).into());
        }
    ...
    /// overview of dynamic dependencies in bsmr.
    ///
