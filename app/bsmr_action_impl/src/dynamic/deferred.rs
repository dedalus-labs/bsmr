...
use bsmr_build_api::dynamic::calculation::dynamic_lambda_result;
...
) -> bsmr_error::Result<ProviderCollection<'v>> {
    let pos;
    let named;
...
) -> bsmr_error::Result<(FinishedStarlarkEvaluation, AnalysisRegistry<'v>)> {
...
async fn execute_lambda(
    ...
    if let BaseDeferredKey::BxlLabel(key) = self_key.owner().dupe() {
        ...
        let r = async {
            ...
            bsmr_error::Ok((artifact_fs, eval_provider))
        }
        .await;
        ...
        let start_event = bsmr_data::AnalysisStart {
            target: Some(bsmr_data::analysis_start::Target::DynamicLambda(
        ...
        let (res, spans) = record_root_spans(|| {
            span(start_event, || {
                ...
                (
                    output,
                    bsmr_data::AnalysisEnd {
                        target: Some(bsmr_data::analysis_end::Target::DynamicLambda(
                            self_key.owner().to_proto().into(),
                        )),
                        rule: proto_rule,
                        profile: None,
    ...
    span_async_simple(
        ...
        async move {
            ...
            let (input_artifacts_materialized, resolved_dynamic_values) = span_async_simple(
                bsmr_data::DeferredPreparationStageStart {
                    stage: Some(bsmr_data::MaterializedArtifacts {}.into()),
                },
                ctx.try_compute2(
                ...
                bsmr_data::DeferredPreparationStageEnd {},
            )
            .await?;
...
async fn ensure_artifacts_built(
    ...
    ctx: &mut DiceComputations<'_>,
) -> bsmr_error::Result<Vec<ArtifactGroupValues>> {
        ...
        .try_compute_join(dynamic_values, |ctx, dynamic_value| {
            Box::pin(async {
                let result = dynamic_lambda_result(ctx, &dynamic_value.dynamic_lambda_results_key)
                    ...
                    .provider_collection()?
                    .to_owned();
                bsmr_error::Ok((dynamic_value.dupe(), result))
            })
        })
        .await?;
...
pub fn dynamic_lambda_ctx_data<'v>(
    ...
    digest_config: DigestConfig,
    env: &Module<'v>,
) -> bsmr_error::Result<DynamicLambdaCtxData<'v>> {
