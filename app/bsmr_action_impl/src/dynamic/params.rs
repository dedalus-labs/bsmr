...
use bsmr_build_api::interpreter::rule_defs::plugins::FrozenAnalysisPlugins;
...
pub(crate) struct DynamicLambdaStaticFields {
    ...
    /// Execution platform inherited from the owner to use for actionsfbcode/bsmr/app/bsmr_action_impl/src/dynamic/deferred.rs
...
impl FrozenDynamicLambdaParams {
    pub(crate) fn attributes<'v>(
        &'v self,
    ) -> bsmr_error::Result<Option<ValueOfUnchecked<'v, StructRef<'static>>>> {
    ...
    pub(crate) fn plugins<'v>(
        &'v self,
    ) -> bsmr_error::Result<Option<ValueTypedComplex<'v, AnalysisPlugins<'v>>>> {
