...
use bsmr_build_api::deferred::calculation::lookup_deferred_holder;
use bsmr_build_api::dynamic::calculation::DYNAMIC_LAMBDA_CALCULATION_IMPL;
use bsmr_build_api::dynamic::calculation::DynamicLambdaCalculation;
use bsmr_build_api::dynamic::calculation::DynamicLambdaResult;
...
impl DynamicLambdaCalculation for DynamicLambdaCalculationImpl {
    ...
    ) -> bsmr_error::Result<Arc<DynamicLambdaResult>> {
...
impl Key for DynamicLambdaDiceKey {
    type Value = bsmr_error::Result<Arc<DynamicLambdaResult>>;
...
impl BuildSignalsNodeKeyImpl for DynamicLambdaDiceKey {
    fn critical_path_entry_proto(&self) -> Option<bsmr_data::critical_path_entry2::Entry> {
        let entry = bsmr_data::critical_path_entry2::Entry::DynamicAnalysis(
            bsmr_data::critical_path_entry2::DynamicAnalysis {
                target: match self.0.holder_key() {
                    DeferredHolderKey::Base(BaseDeferredKey::TargetLabel(target)) => Some(
                        bsmr_data::critical_path_entry2::dynamic_analysis::Target::StandardTarget(
                            target.as_proto(),
                        ),
                    ),
                    _ => None,
