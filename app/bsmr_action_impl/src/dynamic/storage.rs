...
use bsmr_build_api::analysis::registry::AnalysisValueStorage;
use bsmr_build_api::analysis::registry::FrozenAnalysisValueStorage;
use bsmr_build_api::dynamic::storage::DYNAMIC_LAMBDA_PARAMS_STORAGES;
use bsmr_build_api::dynamic::storage::DynamicLambdaParamStorages;
use bsmr_build_api::dynamic::storage::DynamicLambdaParamsStorage;
use bsmr_build_api::dynamic::storage::FrozenDynamicLambdaParamsStorage;
use bsmr_core::deferred::dynamic::DynamicLambdaIndex;
...
pub(crate) struct FrozenDynamicLambdaParamsStorageImpl {
    // Mixed: `DynamicLambdaResultsKey` is pagable-only (`bsmr_core` cannot
...
impl<'v> DynamicLambdaParamsStorageImpl<'v> {
    ...
    ) -> bsmr_error::Result<&'a mut DynamicLambdaParamsStorageImpl<'v>> {
        storage
            .lambda_params
            .as_any_mut()
            .downcast_mut()
    ...
    pub fn next_dynamic_actions_key(&self) -> bsmr_error::Result<DynamicLambdaResultsKey> {
...
impl FrozenDynamicLambdaParamsStorageImpl {
    pub(crate) fn lookup_lambda<'f>(
        ...
        key: &DynamicLambdaResultsKey,
    ) -> bsmr_error::Result<OwnedRefFrozenRef<'f, FrozenDynamicLambdaParams>> {
