...
impl<'v> DynamicAttrValue<Value<'v>> {
    fn bind(&self, bind: &mut DynamicActionsOutputArtifactBinder) -> bsmr_error::Result<()> {
        match self {
...
impl<'v> DynamicAttrValues<Value<'v>> {
    pub(crate) fn bind(&self, key: &DynamicLambdaResultsKey) -> bsmr_error::Result<()> {
...
impl DynamicAttrType {
    ...
    pub(crate) fn coerce<'v>(
        &self,
        value: Value<'v>,
    ) -> bsmr_error::Result<DynamicAttrValue<Value<'v>>> {
        match self {
