...
use bsmr_artifact::artifact::artifact_type::BaseArtifactKind;
...
use bsmr_build_api::actions::execute::action_execution_target::ActionExecutionTarget;
...
use bsmr_build_api::actions::impls::expanded_command_line::ExpandedCommandLine;
use bsmr_build_api::actions::impls::expanded_command_line::ExpandedCommandLineDigest;
use bsmr_build_api::actions::impls::expanded_command_line::ExpandedCommandLineFingerprinter;
...
use bsmr_build_api::interpreter::rule_defs::artifact::starlark_artifact_value::StarlarkArtifactValue;
use bsmr_build_api::interpreter::rule_defs::artifact::starlark_output_artifact::FrozenStarlarkOutputArtifact;
use bsmr_build_api::interpreter::rule_defs::artifact::starlark_output_artifact::StarlarkOutputArtifact;
...
use bsmr_build_api::interpreter::rule_defs::cmd_args::CommandLineSink;
use bsmr_build_api::interpreter::rule_defs::cmd_args::FrozenStarlarkCmdArgs;
...
use bsmr_build_api::interpreter::rule_defs::cmd_args::StarlarkCmdArgs;
use bsmr_build_api::interpreter::rule_defs::cmd_args::path_format;
use bsmr_build_api::interpreter::rule_defs::cmd_args::value_as::ValueAsCommandLineLike;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::worker_info::FrozenWorkerInfo;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::worker_info::WorkerInfo;
use bsmr_build_signals::env::WaitingCategory;
...
use bsmr_execute::execute::request::ActionMetadataBlob;
...
use bsmr_execute::execute::request::RemoteWorkerSpec;
...
impl FrozenStarlarkRunActionValues {
    pub(crate) fn worker<'v>(
        &'v self,
    ) -> bsmr_error::Result<Option<ValueOf<'v, &'v WorkerInfo<'v>>>> {
...
impl RunAction {
    ...
    ) -> bsmr_error::Result<UnpackedRunActionValues<'v>> {
    ...
    fn expand_command_line_and_worker<'v>(
            ...
            let worker_key = if worker.supports_bazel_remote_persistent_worker_protocol {
                    ...
                    && !worker_visitor.frozen_outputs.is_empty()
                {
                    ...
                    return Err(bsmr_error!(
                        bsmr_error::ErrorTag::ActionMismatchedOutputs,
        ...
        let remote_worker = if let Some(remote_worker) = values.remote_worker {
            ...
            let remote_worker_env: bsmr_error::Result<SortedVectorMap<_, _>> = remote_worker
                .env
                .into_iter()
                .map(|(k, v)| {
        ...
        let cli_env: bsmr_error::Result<SortedVectorMap<_, _>> = values
            .env
            .into_iter()
            .map(|(k, v)| {
                v.visit_artifacts(artifact_visitor)?;
    ...
    ) -> bsmr_error::Result<ControlFlow<CommandExecutionResult, ()>> {
    ...
    async fn output_paths_as_inputs(
        &self,
        ctx: &dyn ActionExecutionCtx,
    ) -> bsmr_error::Result<Vec<CommandExecutionInput>> {
        let executor_fs = ctx.executor_fs();
        let fs = executor_fs.fs();
        let output_paths = {
    ...
    fn outputs_for_error_handler(&self) -> bsmr_error::Result<Vec<BuildArtifactPath>> {
        self.starlark_values
            ...
            .map(|artifact| {
                ...
                match a.as_parts().0 {
                    BaseArtifactKind::Source(s) => Err(bsmr_error::bsmr_error!(
...
impl RunAction {
    ...
    async fn execute_for_offline(
        &self,
        ctx: &mut dyn ActionExecutionCtx,
    ) -> bsmr_error::Result<Option<(ActionOutputs, ActionExecutionMetadata)>> {
...
impl Action for RunAction {
    ...
    ) -> bsmr_error::Result<ValueOfUnchecked<'v, DictType<StarlarkArtifact, StarlarkArtifactValue>>>
    {
    ...
    async fn execute(
        ...
        let incremental_kind = match (
            ...
            (true, true) => bsmr_data::IncrementalKind::IncrementalLocalAndRemote,
            (false, true) => bsmr_data::IncrementalKind::IncrementalRemote,
            (true, false) => bsmr_data::IncrementalKind::IncrementalLocal,
            (false, false) => bsmr_data::IncrementalKind::NonIncremental,
        };
                ...
                || force_cache_upload()?)
        {
            ...
            result.cache_upload_result = bsmr_data::UploadResult::ActionNotSuccessful;
        } else if result.was_served_by_remote_dep_file_cache() {
            result.cache_upload_result = bsmr_data::UploadResult::RemoteDepFileCacheHit;
        } else if !allow_cache_upload {
            result.cache_upload_result = bsmr_data::UploadResult::ActionUploadNotAllowed;
        }
