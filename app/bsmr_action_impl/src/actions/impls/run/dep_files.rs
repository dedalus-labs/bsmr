...
use bsmr_action_metadata_proto::DepFileInputs;
...
use bsmr_directory::directory::directory_hasher::DirectoryDigester;
use bsmr_directory::directory::directory_ref::DirectoryRef;
use bsmr_directory::directory::directory_selector::DirectorySelector;
...
use bsmr_execute::directory::ActionFingerprintedDirectoryRef;
...
use bsmr_execute::directory::ReDirectorySerializer;
use bsmr_execute::directory::expand_selector_for_dependencies;
...
use bsmr_file_watcher::dep_files::FLUSH_DEP_FILES;
use bsmr_file_watcher::dep_files::FLUSH_NON_LOCAL_DEP_FILES;
use bsmr_fs::fs_util;
...
fn keep_directories() -> bsmr_error::Result<bool> {
    bsmr_env!("BSMR_KEEP_DEP_FILE_DIRECTORIES", bool)
}
...
pub(crate) enum StoredFingerprints {
    ...
    /// Store digests + dirs. We allow this via BSMR_KEEP_DEP_FILE_DIRECTORIES because it gives
    /// more debuggability.
...
impl DepFileState {
    ...
    pub(crate) fn locked_compute_fingerprints<'a>(
        ...
        fs: &ArtifactFs,
    ) -> bsmr_error::Result<MappedMutexGuard<'a, StoredFingerprints>> {
...
impl DepFileBundle {
    ...
    ) -> bsmr_error::Result<(Option<(ActionOutputs, ActionExecutionMetadata)>, bool)> {
    ...
    pub(crate) async fn check_remote_dep_file_entry(
        ...
        let computed_filtered_fingerprints = self
            .shared_declared_inputs
            .clone()
            .ok_or_else(|| bsmr_error::internal_error!("Dep files should have been declared"))?
            .unshare()
            .filter(dep_files, fs)?
...
) -> bsmr_error::Result<DepFileBundle> {
...
pub(crate) async fn match_if_identical_action(
    ...
    declared_outputs: &[BuildArtifact],
    declared_dep_files: &DeclaredDepFiles,
) -> bsmr_error::Result<(Option<ActionOutputs>, bool)> {
...
pub(crate) async fn match_or_clear_dep_file(
    ...
    declared_outputs: &[BuildArtifact],
    declared_dep_files: &DeclaredDepFiles,
) -> bsmr_error::Result<Option<ActionOutputs>> {
    let previous_state = match get_dep_files(key) {
        Some(d) => d.dupe(),
        None => return Ok(None),
    };
...
async fn outputs_match(
    ...
    let output_matches = previous_state
        ...
        .collect::<bsmr_error::Result<Vec<(ProjectRelativePathBuf, ArtifactValue)>>>()?;
...
fn compute_fingerprints(
    ...
    keep_directories: bool,
    fs: &ArtifactFs,
) -> bsmr_error::Result<StoredFingerprints> {
...
impl PartitionedInputs<Vec<ArtifactGroup>> {
    ...
    fn to_directories(
        &self,
        ctx: &dyn ActionExecutionCtx,
    ) -> bsmr_error::Result<PartitionedInputs<ActionDirectoryBuilder>> {
        fn reduce(
            ctx: &dyn ActionExecutionCtx,
            inputs: &[ArtifactGroup],
        ) -> bsmr_error::Result<ActionDirectoryBuilder> {
        ...
        Ok(PartitionedInputs {
            ...
            tagged: self
                .tagged
                .iter()
                .map(|(tag, inputs)| bsmr_error::Ok((tag.dupe(), reduce(ctx, inputs)?)))
                .collect::<Result<_, _>>()?,
        })
    }
}
...
impl DeclaredDepFiles {
    ...
    fn read(
        ...
        for declared_dep_file in self.tagged.values() {
            ...
            let read_dep_file: bsmr_error::Result<()> = try {
...
impl ConcreteDepFiles {
    ...
    ) -> bsmr_error::Result<Option<DirectorySelector>> {
