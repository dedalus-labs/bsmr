...
pub trait QueryEnvironment: Send + Sync {
    ...
    /// Evaluates a literal target pattern. See bsmr_common::pattern
    ...
    async fn testsof_with_default_target_platform(
        &self,
        targets: &TargetSet<Self::Target>,
    ) -> bsmr_error::Result<Vec<MaybeCompatible<Self::Target>>> {
        let target_tests = targets
            .iter()
            .map(|target| {
                let tests = target
...
{
    async fn get(&self, label: &<Q::Target as LabeledNode>::Key) -> bsmr_error::Result<Q::Target> {
