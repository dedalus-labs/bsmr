...
use bsmr_query_parser::SpannedExpr;
...
use bsmr_query::query::syntax::simple::functions::docs::MarkdownOptions;
    ...
    pub fn description(self) -> Option<&'static str> {
        match self {
            ...
            QueryArgType::TargetSet => Some(
                ...
                queries in CLI commands (like `bsmr query`), literals can be relative to the current working dir (like `some:target` \
                or `...`).",
            ),
...
// Helper for bsmr_query_proc_macro implementations. Evaluates an arg at an index and tries to convert it to the QueryFunctionArg type, providing decent errors on failures.
