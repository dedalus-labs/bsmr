...
use bsmr_query_derive::query_module;
use bsmr_query_parser::BinaryOp;
...
pub trait QueryLiteralVisitor<'a> {
    fn target_pattern(&mut self, pattern: &'a str) -> bsmr_error::Result<()>;
}
...
impl<Env: QueryEnvironment> DefaultQueryFunctionsModule<Env> {
    ...
    /// For example:
    /// ```text
    /// $ bsmr uquery "allpaths('//foo:bar', '//foo/bar/lib:baz')"
    /// ```
    ...
    /// ```text
    /// $ bsmr uquery "allpaths(kind(java_library, '//...'), '//foo:bar')"
    /// ```
    ...
    /// For example:
    /// ```text
    /// $ bsmr uquery "nattrfilter(deps, '//foo:bar', '//...')"
    /// ```
    ...
    /// and
    /// ```text
    /// $ bsmr uquery "buildfile(owner(app/bsmr_action_impl_tests/src/context.rs))"
    /// ```
    /// both return `bsmr/app/bsmr_action_impl_tests/TARGETS`.
    ...
    /// For example:
    /// ```text
    /// $ bsmr query "kind('java.*', deps('//foo:bar'))"
    /// ```
    ...
    ///
    /// Example:
    /// `bsmr cquery "deps('//foo:bar', 1, first_order_deps())"` is equivalent to `bsmr cquery "deps('//foo:bar', 1)"`
    ...
    ///
    /// Example:
    /// `bsmr cquery "deps('//foo:bar', 1, target_deps())"`
    ...
    ///
    /// Example:
    /// `bsmr cquery "deps('//foo:bar', 1, exec_deps())"`
    ...
    ///
    /// Example:
    /// `bsmr cquery "deps('//foo:bar', 1, configuration_deps())"`
    ...
    ///
    /// Example:
    /// `bsmr cquery "deps('//foo:bar', 1, toolchain_deps())"`
    ...
    ///
    /// Example:
    /// `bsmr aquery "deps('//foo:bar') intersect deps('//baz:lib')"` is the same as
    /// `bsmr aquery "deps('//foo:bar') ^ deps('//baz:lib')"`
    ...
    ///
    /// Example:
    /// `bsmr aquery "deps('//foo:bar') except deps('//baz:lib')"` is the same as
    /// `bsmr aquery "deps('//foo:bar') - deps('//baz:lib')"`
    ...
    ///
    /// Example:
    /// `bsmr aquery "deps('//foo:bar') union deps('//baz:lib')"` is the same as
    /// `bsmr aquery "deps('//foo:bar') + deps('//baz:lib')"`
...
impl<Env: QueryEnvironment> DefaultQueryFunctions<Env> {
    ...
    pub fn attrfilter(
        ...
        value: &str,
        targets: &TargetSet<Env::Target>,
    ) -> bsmr_error::Result<TargetSet<Env::Target>> {
    ...
    pub fn filter_file_set(&self, regex: &str, files: &FileSet) -> bsmr_error::Result<FileSet> {
        files.filter_name(regex)
    }

    pub fn inputs(&self, targets: &TargetSet<Env::Target>) -> bsmr_error::Result<FileSet> {
        targets.inputs()
    }
    ...
    pub async fn testsof_with_default_target_platform(
        ...
        env: &Env,
        targets: &TargetSet<Env::Target>,
    ) -> bsmr_error::Result<Vec<MaybeCompatible<Env::Target>>> {
    ...
    where
        ...
        Ff: Fn(&FileSet, &FileSet) -> bsmr_error::Result<FileSet>,
    {
        match (left, right) {
