...
impl<T: QueryTarget> TargetSet<T> {
    ...
    pub(crate) fn filter<F: Fn(&T) -> bsmr_error::Result<bool>>(
        &self,
        filter: F,
...
impl<T: QueryTarget> TargetSet<T> {
    ...
    pub fn attrregexfilter(
        ...
        let filter = move |s: &'_ str| -> bsmr_error::Result<bool> { Ok(regex.is_match(s)?) };
        self.attrfilter(attribute, &filter)
    }

    /// Filter targets by fully qualified name using regex partial match.
    pub fn filter_name(&self, regex: &str) -> bsmr_error::Result<TargetSet<T>> {
    ...
    pub fn kind(&self, regex: &str) -> bsmr_error::Result<TargetSet<T>> {
        let re = Regex::new(regex)?;
        self.filter(|node| Ok(re.is_match(&node.rule_type())?))
    }

    pub fn intersect(&self, right: &TargetSet<T>) -> bsmr_error::Result<TargetSet<T>> {
        self.filter(|node| Ok(right.contains(node.node_key())))
    }

    pub fn difference(&self, right: &TargetSet<T>) -> bsmr_error::Result<TargetSet<T>> {
        self.filter(|node| Ok(!right.contains(node.node_key())))
    }
}
