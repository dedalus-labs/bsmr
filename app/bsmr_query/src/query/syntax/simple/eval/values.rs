...
impl<T: QueryTarget> QueryEvaluationResult<T> {
    ...
    pub fn targets(&self) -> impl Iterator<Item = bsmr_error::Result<&T>> {
        match self {
...
impl<T: QueryTarget> QueryEvaluationValue<T> {
    pub fn try_into_targets(self) -> bsmr_error::Result<TargetSet<T>> {
        match self {
    ...
    pub(crate) fn targets(&self) -> impl Iterator<Item = bsmr_error::Result<&T>> {
        match self {
...
pub trait QueryResultExt<T> {
    fn into_bsmr_error(self, query: &str) -> bsmr_error::Result<T>;
}
