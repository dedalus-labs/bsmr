...
impl FileSet {
    ...
    pub(crate) fn filter_name(&self, regex: &str) -> bsmr_error::Result<Self> {
        let re = Regex::new(regex)?;
        self.filter(|node| Ok(re.is_match(&node.0.to_string())?))
    }

    fn filter<F: Fn(&FileNode) -> bsmr_error::Result<bool>>(
        &self,
        filter: F,
    ...
    pub fn intersect(&self, right: &FileSet) -> bsmr_error::Result<FileSet> {
        self.filter(|file| Ok(right.contains(file)))
    }

    pub fn difference(&self, right: &FileSet) -> bsmr_error::Result<FileSet> {
        self.filter(|file| Ok(!right.contains(file)))
    }
}
