...
pub enum QueryError {
    ...
    #[error("Argument `{1}` to `{0}` is not yet supported in bsmr")]
...
impl From<bsmr_error::Error> for QueryError {
    fn from(err: bsmr_error::Error) -> Self {
...
impl QueryError {
    pub fn drop_spans(err: Spanned<Self>) -> bsmr_error::Error {
        match err.value {
    ...
    pub fn convert_error(err: Spanned<Self>, input: &str) -> bsmr_error::Error {
        ...
        match err.value {
            ...
            e => {
                // TODO(cjhopman): This is going to drop the backtrace attached to the error, we should figure
                // out how to keep that.
                bsmr_error::bsmr_error!(bsmr_error::ErrorTag::Input, "{}:{}", e, context)
            }
        }
    }
}
