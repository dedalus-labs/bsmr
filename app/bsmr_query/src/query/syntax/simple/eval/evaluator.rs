...
impl<'e, Env: QueryEnvironment> QueryEvaluator<'e, Env> {
    ...
    async fn resolve_literal(&self, literal: &str) -> bsmr_error::Result<TargetSet<Env::Target>> {
        ...
        match expr {
            ...
            Expr::BinaryOpSequence(left, exprs) => {
                let (left, rights) = futures::future::try_join(
                    self.eval(left),
                    bsmr_util::future::try_join_all(exprs.iter().map(|(op, expr)| async move {
    ...
    pub async fn eval_query(
        &self,
        query: &str,
    ) -> bsmr_error::Result<QueryEvaluationValue<Env::Target>> {
        ...
        self.eval(expr)
            ...
            .async_into_map_res(|value| async move {
                match value {
                    ...
                    // `bsmr query //lib/...` to resolve to the corresponding targets.
