...
pub trait ChildVisitor<T: LabeledNode>: Send {
    fn visit(&mut self, node: &T::Key) -> bsmr_error::Result<()>;
}

impl<F, T: LabeledNode> ChildVisitor<T> for F
where
    F: FnMut(&T::Key) -> bsmr_error::Result<()>,
    F: Send,
{
    fn visit(&mut self, node: &T::Key) -> bsmr_error::Result<()> {
        self(node)
...
pub trait NodeLookup<T: LabeledNode> {
    ...
    fn get(&self, label: &T::Key) -> bsmr_error::Result<T>;
}

#[async_trait]
pub trait AsyncNodeLookup<T: LabeledNode>: Send + Sync {
    async fn get(&self, label: &T::Key) -> bsmr_error::Result<T>;
}
...
impl<T: LabeledNode<Key = T>> NodeLookup<T> for NodeLookupId {
    fn get(&self, key: &T::Key) -> bsmr_error::Result<T> {
        Ok(key.dupe())
    }
}
...
impl<T: LabeledNode<Key = T>> AsyncNodeLookup<T> for NodeLookupId {
    async fn get(&self, key: &T::Key) -> bsmr_error::Result<T> {
        Ok(key.dupe())
    }
}
...
>(
    ...
    mut visit: impl FnMut(T) -> bsmr_error::Result<()>,
) -> bsmr_error::Result<()> {
...
mod tests {
    ...
    impl QueryTarget for Node {
        ...
        fn attr_any_matches(
            _attr: &Self::Attr<'_>,
            _filter: &dyn Fn(&str) -> bsmr_error::Result<bool>,
    ...
    impl Graph {
        ...
        fn async_node_lookup(&self) -> impl AsyncNodeLookup<Node> + '_ {
            ...
            impl<'a> AsyncNodeLookup<Node> for Lookup<'a> {
                async fn get(&self, label: &Ref) -> bsmr_error::Result<Node> {
                    self.0.get(label)
                }
            }
    ...
    impl NodeLookup<Node> for Graph {
        fn get(&self, label: &Ref) -> bsmr_error::Result<Node> {
            self.0
                .get(label)
                .ok_or_else(|| bsmr_error::bsmr_error!(bsmr_error::ErrorTag::Tier0, "missing node"))
                .map(|v| v.dupe())
        }
    }
    ...
    #[tokio::test]
    async fn test_async_depth_first_postorder_traversal() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_async_depth_limited_traversal() -> bsmr_error::Result<()> {
        let graph = make_graph(&[
            (0, &[1, 2]),
            (1, &[2, 3, 4]),
            (2, &[3, 4]),
    ...
    #[tokio::test]
    async fn test_async_fast_depth_first_postorder_traversal() -> bsmr_error::Result<()> {
        let graph = make_graph(&[
            (0, &[1, 2]),
            (1, &[2, 3, 4]),
            (2, &[3, 4]),
