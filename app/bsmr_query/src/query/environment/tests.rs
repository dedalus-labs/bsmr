...
impl NodeLookup<TestTarget> for TestEnv {
    fn get(&self, label: &<TestTarget as LabeledNode>::Key) -> bsmr_error::Result<TestTarget> {
        self.graph
            .get(label)
            .duped()
...
impl QueryEnvironment for TestEnv {
    ...
    async fn eval_file_literal(&self, _literal: &str) -> bsmr_error::Result<FileSet> {
        unimplemented!()
    }
...
#[tokio::test]
async fn test_one_path() -> bsmr_error::Result<()> {
...
#[tokio::test]
async fn test_many_paths() -> bsmr_error::Result<()> {
    let mut env = TestEnvBuilder::default();
    env.edge(1, 2);
    env.edge(2, 3);
    env.edge(1, 10);
...
#[tokio::test]
async fn test_distinct_paths() -> bsmr_error::Result<()> {
    let mut env = TestEnvBuilder::default();
    env.edge(1, 10);
    env.edge(10, 100);
    env.edge(2, 20);
...
#[tokio::test]
async fn test_no_path() -> bsmr_error::Result<()> {
    let mut env = TestEnvBuilder::default();
    env.edge(1, 10);
    env.edge(2, 20);
    let env = env.build();
...
#[tokio::test]
async fn test_nested_paths() -> bsmr_error::Result<()> {
    let mut env = TestEnvBuilder::default();
    env.edge(1, 2);
    env.edge(2, 3);
    env.edge(3, 4);
...
#[tokio::test]
async fn test_paths_with_cycles_present() -> bsmr_error::Result<()> {
    let mut env = TestEnvBuilder::default();
    env.edge(1, 2);
    env.edge(2, 3);
    env.edge(3, 4);
...
#[tokio::test]
async fn test_rdeps() -> bsmr_error::Result<()> {
    let mut env = TestEnvBuilder::default();
    env.edge(1, 2);
    env.edge(2, 3);
