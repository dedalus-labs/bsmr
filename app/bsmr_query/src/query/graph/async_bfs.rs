...
impl<N: LabeledNode + 'static> BfsVisited<N> {
    fn take_path(mut self, last: &N::Key, mut item: impl FnMut(N)) -> bsmr_error::Result<()> {
        let node = self
            .visited
            .remove(last)
...
) -> bsmr_error::Result<Option<Vec<N>>> {
    let lookup = &lookup;
...
mod tests {
    ...
    impl TestGraph {
        async fn bfs_find_path(
            ...
            target: u32,
        ) -> bsmr_error::Result<Option<Vec<u32>>> {
    ...
    impl AsyncNodeLookup<TestNode> for TestGraph {
        async fn get(&self, label: &TestNodeKey) -> bsmr_error::Result<TestNode> {
