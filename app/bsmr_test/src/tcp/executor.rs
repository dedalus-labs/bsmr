...
pub(crate) async fn spawn(
    ...
    args: Vec<String>,
    tpx_args: Vec<String>,
) -> bsmr_error::Result<(ExecutorFuture, TcpStream, TcpStream)> {
    ...
    let conns = async {
        ...
        bsmr_error::Ok((orchestrator_tcp_stream, executor_tcp_stream))
    };
    ...
    match futures::future::select(exec, conns).await {
        Either::Left((output, _)) => Err(bsmr_error::internal_error!(
...
async fn create_tcp_listener() -> bsmr_error::Result<(String, TcpListener)> {
