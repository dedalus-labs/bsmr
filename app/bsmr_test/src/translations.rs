...
use bsmr_test_api::data::RemoteObject;
...
) -> bsmr_error::Result<ConfiguredTarget> {
...
pub(crate) fn convert_test_result(
    test_result: bsmr_test_api::data::TestResult,
    session: &TestSession,
) -> bsmr_error::Result<bsmr_data::TestResult> {
    let bsmr_test_api::data::TestResult {
        name,
        status,
        msg,
        duration,
...
fn convert_digest(digest: &FileDigest) -> bsmr_test_api::data::CasDigest {
    let hash = format!("{}", digest.raw_digest());
    bsmr_test_api::data::CasDigest {
        hash,
