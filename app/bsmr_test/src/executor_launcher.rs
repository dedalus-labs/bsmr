...
pub struct ExecutorFuture {
    fut: BoxFuture<'static, bsmr_error::Result<ExecutorOutput>>,
}
...
impl Future for ExecutorFuture {
    type Output = bsmr_error::Result<ExecutorOutput>;
...
pub trait ExecutorLauncher: Send + Sync {
    async fn launch(&self, tpx_args: Vec<String>) -> bsmr_error::Result<ExecutorLaunch>;
}
...
impl ExecutorLauncher for OutOfProcessTestExecutor {
    async fn launch(&self, tpx_args: Vec<String>) -> bsmr_error::Result<ExecutorLaunch> {
        // Declare outside of `cfg(unix)` so `bsmr help-env` would include it on Windows
        // even if it is no-op on Windows.
        let use_tcp = bsmr_env!("BSMR_TEST_TPX_USE_TCP", bool)?;
