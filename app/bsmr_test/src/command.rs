...
use bsmr_build_api::build::AsyncBuildTargetResultBuilder;
use bsmr_build_api::build::BuildConfiguredLabelOptions;
use bsmr_build_api::build::BuildEvent;
use bsmr_build_api::build::BuildEventConsumer;
use bsmr_build_api::build::BuildTargetResult;
use bsmr_build_api::build::BuildTargetResultBuilder;
use bsmr_build_api::build::ConfiguredBuildEventVariant;
use bsmr_build_api::build::ProvidersToBuild;
use bsmr_build_api::build::build_configured_label;
use bsmr_build_api::build::build_report::build_report_opts;
use bsmr_build_api::build::build_report::write_build_report;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::internal_runner_test_info::FrozenInternalRunnerTestInfo;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::run_info::FrozenRunInfo;
use bsmr_build_api::interpreter::rule_defs::provider::collection::FrozenProviderCollectionValue;
use bsmr_build_api::interpreter::rule_defs::provider::test_provider::TestProvider;
use bsmr_build_api::interpreter::rule_defs::provider::test_provider::build_external_runner_spec;
use bsmr_build_api::materialize::MaterializationAndUploadContext;
use bsmr_cli_proto::HasClientContext;
use bsmr_cli_proto::TestRequest;
use bsmr_cli_proto::TestResponse;
use bsmr_cli_proto::representative_config_flag;
...
use bsmr_common::tenting::HasTentingAclProvider;
use bsmr_common::tenting::TentingStatus;
...
use bsmr_core::pattern::pattern::Modifiers;
use bsmr_core::pattern::pattern::ModifiersError;
use bsmr_core::pattern::pattern::PackageSpec;
...
use bsmr_server_ctx::commands::send_target_cfg_event;
...
use bsmr_server_ctx::test_command::TEST_COMMAND;
use bsmr_server_ctx::tpx_experiment_util::get_tpx_experiments;
...
struct TestOutcome {
    errors: Vec<bsmr_data::ErrorReport>,
...
impl TestOutcome {
    fn exit_code(&self) -> bsmr_error::Result<i32> {
...
impl CounterWithExamples {
    ...
    fn into_cli_proto_counter(self) -> bsmr_cli_proto::CounterWithExamples {
        bsmr_cli_proto::CounterWithExamples {
            count: self.count,
            max: self.max,
...
#[derive(Debug, bsmr_error_derive::Error)]
#[bsmr(tag = TestDeadlineExpired)]
...
async fn test_command(
    ...
    req: TestRequest,
) -> bsmr_error::Result<TestResponse> {
...
impl ServerCommandTemplate for TestServerCommand {
    type StartEvent = bsmr_data::TestCommandStart;
    type EndEvent = bsmr_data::TestCommandEnd;
    type Response = bsmr_cli_proto::TestResponse;
    type PartialResult = NoPartialResult;

    fn build_result(&self, response: &Self::Response) -> Option<BuildResult> {
        let build_completed =
            if let Some(bsmr_cli_proto::test_response::TestStatuses { build_errors, .. }) =
...
async fn test(
    ...
    let test_statuses = bsmr_cli_proto::test_response::TestStatuses {
        passed: Some(
            test_outcome
                .executor_report
                .statuses
...
async fn test_targets(
    ...
    let test_server = tokio::spawn({
        ...
        with_dispatcher_async(
            ...
            async move {
                ...
                bsmr_error::Ok((build_target_result, test_statuses))
            },
        )
    });
    ...
    let _ignored = test_status_sender.unbounded_send(Err(bsmr_error::bsmr_error!(
        ErrorTag::TestExecutor,
    ...
    let mut errors = convert_error(&build_target_result)
        .iter()
        .map(bsmr_data::ErrorReport::from)
        .unique_by(|e| e.message.clone())
        .collect::<Vec<_>>();

    if let Some(timeout_observer) = timeout_observer {
        if !timeout_observer.is_alive().await {
            errors.push(bsmr_data::ErrorReport::from(&DeadlineExpired.into()));
        }
    }
...
struct TestDriverState<'a, 'e> {
    ...
    internal_test_status_sender: &'a mpsc::UnboundedSender<bsmr_error::Result<ExecutorMessage>>,
                        ...
                        MissingTargetBehavior::Fail => {
                            ...
                            let events = vec![BuildEvent::OtherError {
                                label: Some(ProvidersLabel::new(
                                    ...
                                    bsmr_core::provider::label::ProvidersName::Default,
                                )),
                                err: err.into(),
                            }];
                ...
                let labels =
                    targets
                        ...
                        .map(|((target_name, providers_pattern), target_node)| {
                            ...
                            instant_event(bsmr_data::TargetRuleTypeName {
...
async fn test_target<'a, 'e>(
    ...
    oncall: Option<String>,
    tenting_acl_names: TentingStatus,
) -> bsmr_error::Result<Option<ConfiguredProvidersLabel>> {
...
fn convert_error(build_result: &BuildTargetResult) -> Vec<bsmr_error::Error> {
...
fn run_tests<'a, 'b>(
    ...
    oncall: Option<String>,
    tenting_acl_names: TentingStatus,
) -> BoxFuture<'a, bsmr_error::Result<ConfiguredProvidersLabel>> {
...
fn generate_config_entry_args(
    ...
    representative_config_flags: &[bsmr_cli_proto::RepresentativeConfigFlag],
) {
...
fn post_process_test_executor(s: &str) -> bsmr_error::Result<PathBuf> {
    match s.split_once("$BSMR_BINARY_DIR/") {
        Some(("", rest)) => {
            ...
            let exe_dir = exe
                .parent()
                .ok_or_else(|| internal_error!("Bessemer executable directory has no parent"))?;

            Ok(exe_dir.join(rest).to_path_buf())
        }
        Some(..) => Err(bsmr_error::bsmr_error!(
...
fn ai_agent_tpx_args(agent_context: &[bsmr_data::AgentContextEntry]) -> Vec<String> {
...
#[cfg(test)]
mod tests {
    use bsmr_cli_proto::RepresentativeConfigFlag;
    use bsmr_cli_proto::representative_config_flag;
    use bsmr_data::AgentContextEntry;
