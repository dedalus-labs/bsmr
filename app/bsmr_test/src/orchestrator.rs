...
//! Implementation of the `TestOrchestrator` from `bsmr_test_api`.
...
use bsmr_build_api::actions::execute::dice_data::CommandExecutorResponse;
use bsmr_build_api::actions::execute::dice_data::DiceHasCommandExecutor;
use bsmr_build_api::actions::execute::dice_data::GetReClient;
use bsmr_build_api::actions::impls::run_action_knobs::HasRunActionKnobs;
...
use bsmr_build_api::artifact_groups::calculation::ArtifactGroupCalculation;
use bsmr_build_api::build_signals::HasBuildSignals;
use bsmr_build_api::context::HasBuildContextData;
use bsmr_build_api::interpreter::rule_defs::cmd_args::ArtifactPathMapperImpl;
...
use bsmr_build_api::interpreter::rule_defs::cmd_args::SingletonCommandLineSink;
use bsmr_build_api::interpreter::rule_defs::command_executor_config::StarlarkCommandExecutorConfig;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::external_runner_test_info::FrozenExternalRunnerTestInfo;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::external_runner_test_info::TestCommandMember;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::internal_runner_test_info::FrozenInternalRunnerTestInfo;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::local_resource_info::FrozenLocalResourceInfo;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::worker_info::WorkerInfo;
use bsmr_build_api::interpreter::rule_defs::required_test_local_resource::StarlarkRequiredTestLocalResource;
use bsmr_build_api::keep_going::KeepGoing;
...
use bsmr_core::execution_types::executor_config::Executor;
use bsmr_core::execution_types::executor_config::LocalExecutorOptions;
use bsmr_core::execution_types::executor_config::MetaInternalExtraParams;
...
use bsmr_data::EndOfTestResults;
use bsmr_data::SetupLocalResourcesEnd;
use bsmr_data::SetupLocalResourcesStart;
use bsmr_data::TestDiscovery;
use bsmr_data::TestDiscoveryEnd;
use bsmr_data::TestDiscoveryStart;
use bsmr_data::TestRunEnd;
use bsmr_data::TestRunStart;
...
use bsmr_execute::execute::environment_inheritance::EnvironmentInheritance;
...
use bsmr_execute::execute::request::OutputCreationBehavior;
use bsmr_execute::execute::request::WorkerId;
use bsmr_execute::execute::request::WorkerSpec;
use bsmr_execute::execute::result::CommandCancellationReason;
...
use bsmr_execute_impl::executors::local::EnvironmentBuilder;
use bsmr_execute_impl::executors::local::apply_local_execution_environment;
use bsmr_execute_impl::executors::local::create_output_dirs;
use bsmr_execute_impl::executors::local::materialize_inputs;
...
use bsmr_test_api::data::CasDigest;
use bsmr_test_api::data::ConfiguredTargetHandle;
use bsmr_test_api::data::DeclaredOutput;
use bsmr_test_api::data::ExecuteResponse;
use bsmr_test_api::data::ExecutionDetails;
use bsmr_test_api::data::ExecutionResult2;
use bsmr_test_api::data::ExecutionStatus;
use bsmr_test_api::data::ExecutionStream;
use bsmr_test_api::data::ExecutorConfigOverride;
use bsmr_test_api::data::ExternalRunnerSpecValue;
use bsmr_test_api::data::LocalExecutionCommand;
use bsmr_test_api::data::Output;
use bsmr_test_api::data::PrepareForLocalExecutionResult;
use bsmr_test_api::data::RequiredLocalResources;
use bsmr_test_api::data::TestResult;
use bsmr_test_api::data::TestStage;
use bsmr_test_api::data::convert::host_sharing_requirements_to_grpc;
use bsmr_test_api::protocol::TestOrchestrator;
...
async fn prepare_and_execute_dice(
    ...
    ctx.compute(key).await.map_err(bsmr_error::Error::from)?
}
...
impl TestOrchestrator for BsmrTestOrchestrator<'_> {
            ...
            Err(ExecuteError::Cancelled(cancelled)) => {
                Ok(ExecuteResponse::Cancelled(match cancelled.reason {
                    Some(CancellationReason::NotSpecified) => {
                        Some(bsmr_test_api::data::CancellationReason::NotSpecified)
                    }
                    Some(CancellationReason::ReQueueTimeout) => {
                        Some(bsmr_test_api::data::CancellationReason::ReQueueTimeout)
                    }
                    None => None,
                }))
            }
    ...
    async fn report_test_result(&self, r: TestResult) -> bsmr_error::Result<()> {
        let event = bsmr_data::instant_event::Data::TestResult(translations::convert_test_result(
            r.clone(),
            &self.session,
        )?);
        ...
        self.results_channel
            ...
            .map_err(|_| {
                bsmr_error::internal_error!("Test result was received after end-of-tests")
            })?;
        Ok(())
    }
    ...
    async fn report_tests_discovered(
        ...
        self.events.instant_event(TestDiscovery {
            data: Some(bsmr_data::test_discovery::Data::Tests(TestSuite {
                suite_name: suite,
                test_names: names,
    ...
    async fn report_test_session(
        ...
        self.events.instant_event(TestDiscovery {
            data: Some(bsmr_data::test_discovery::Data::Session(TestSessionInfo {
                info: session_info,
                test_session_id,
            })),
        });
    ...
    async fn end_of_test_results(&self, exit_code: i32) -> bsmr_error::Result<()> {
        self.events.instant_event(EndOfTestResults { exit_code });
        self.results_channel
            .unbounded_send(Ok(ExecutorMessage::ExitCode(exit_code)))
            .map_err(|_| bsmr_error::internal_error!("end_of_tests was received twice"))?;
    ...
    async fn prepare_for_local_execution(
        ...
        pre_create_dirs: Vec<DeclaredOutput>,
        required_local_resources: RequiredLocalResources,
    ) -> bsmr_error::Result<PrepareForLocalExecutionResult> {
        let test_target = self.session.get(test_target)?;
    ...
    async fn attach_info_message(&self, message: String) -> bsmr_error::Result<()> {
        self.results_channel
            .unbounded_send(Ok(ExecutorMessage::InfoMessage(message)))
            .map_err(|_| bsmr_error::internal_error!("Message received after end-of-tests"))?;
        Ok(())
    }
...
impl BsmrTestOrchestrator<'_> {
    ...
    ) -> bsmr_error::Result<ExecutorPreference> {
    ...
    fn executor_config_with_remote_cache_override<'a>(
        ...
        stage: &TestStage,
        supports_test_execution_caching: bool,
    ) -> bsmr_error::Result<Cow<'a, CommandExecutorConfig>> {
    ...
    async fn get_command_executor(
        ...
        stage: &TestStage,
        supports_test_execution_caching: bool,
    ) -> bsmr_error::Result<CommandExecutor> {
    ...
    async fn get_test_info(
        ...
        test_target: &ConfiguredProvidersLabel,
        internal_runner_config: &InternalRunnerConfig,
    ) -> bsmr_error::Result<OwnedTestInfo> {
        let providers = dice
    ...
    async fn get_test_executor(
        ...
        stage: &TestStage,
        supports_test_execution_caching: bool,
    ) -> bsmr_error::Result<TestExecutor> {
        ...
        let (expanded_cmd, expanded_env, ensured_inputs, expanded_worker) = {
            ...
            let ensured_inputs = KeepGoing::try_compute_join_all(dice, inputs, |dice, input| {
                async move {
                    ...
                    bsmr_error::Ok((input, artifact_group_value))
                }
                .boxed()
            })
            .await?;
    ...
    ) -> bsmr_error::Result<CommandExecutionRequest> {
    ...
    async fn setup_local_resources(
        ...
        let result: bsmr_error::Result<Vec<_>> = required_targets
            .iter()
    ...
    async fn prepare_local_resource(
        ...
        executor_fs: &ExecutorFs<'_>,
        default_timeout: Duration,
    ) -> bsmr_error::Result<PreparedLocalResourceSetupContext> {
        let digest_config = dice.global_data().get_digest_config();
    ...
    fn expand(
        ...
        let expanded_env = env
            ...
            .map(|(k, v)| {
                ...
                fmt.pop_scope();
                bsmr_error::Ok((k.to_owned(), curr_env.finalize()?))
            })
        ...
        let expanded_worker = match test_info.worker() {
            Some(worker) => {
                ...
                let worker_env: bsmr_error::Result<SortedVectorMap<_, _>> = worker
                    .env()
                    .into_iter()
                    .map(|(k, v)| {
...
impl CommandExecutionTarget for TestTarget<'_> {
    ...
    fn as_proto_action_key(&self) -> bsmr_data::ActionKey {
        bsmr_data::ActionKey {
            id: Default::default(),
            owner: Some(bsmr_data::action_key::Owner::TestTargetLabel(
...
impl CommandExecutionTarget for LocalResourceTarget<'_> {
    ...
    fn as_proto_action_key(&self) -> bsmr_data::ActionKey {
        bsmr_data::ActionKey {
            id: Default::default(),
            owner: Some(bsmr_data::action_key::Owner::LocalResourceSetup(
                self.target.as_proto(),
            )),
            key: Default::default(),
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn orchestrator_results() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn orchestrator_attach_info_messages() -> bsmr_error::Result<()> {
        let (orchestrator, channel) = make().await?;
    ...
    #[tokio::test]
    async fn test_orchestrator_channel_drop() -> bsmr_error::Result<()> {
        let (orchestrator, channel) = make().await?;
        drop(orchestrator);
    ...
    #[tokio::test]
    async fn test_orchestrator_closes_channel() -> bsmr_error::Result<()> {
        let (orchestrator, channel) = make().await?;
