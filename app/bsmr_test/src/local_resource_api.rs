...
use bsmr_common::local_resource_state::EnvironmentVariable;
    ...
    pub(crate) fn into_state(
        ...
        fn make_resource(
            ...
            let env_vars = env_var_to_alias
                .iter()
                .map(|(env_var, alias)| {
                    let value = alias_to_value.get(alias).ok_or_else(|| bsmr_error::bsmr_error!(
                ...
                .collect::<Result<_, bsmr_error::Error>>()?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_into_state() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_missing_value() -> bsmr_error::Result<()> {
        let setup_result = LocalResourcesSetupResult {
            pid: Some(42),
            resources: vec![
