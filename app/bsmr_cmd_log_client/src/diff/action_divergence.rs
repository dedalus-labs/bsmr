...
use bsmr_event_observer::display::display_action_identity;
use futures::Stream;
...
) -> bsmr_error::Result<LinkedHashMap<ActionKey, ActionExecutionData>> {
...
fn print_divergence_msg(
    ...
    bsmr_client_ctx::println!("{}", output.join("\n"))?;
...
impl BuckSubcommand for ActionDivergenceCommand {
    ...
    async fn exec_impl(
        ...
        if !divergence_found {
            bsmr_client_ctx::println!("No divergent actions found.")?;
        }
        ExitResult::success()
    }
}
