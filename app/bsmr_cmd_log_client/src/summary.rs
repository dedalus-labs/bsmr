...
use bsmr_client_ctx::subscribers::recorder::process_memory;
...
use bsmr_event_observer::humanized::HumanizedBytesPerSecond;
    ...
    fn update_with_event(&mut self, event: &bsmr_data::BuckEvent) {
        match &event.data {
            Some(bsmr_data::buck_event::Data::SpanEnd(end)) => match end.data.as_ref() {
                ...
                Some(bsmr_data::span_end_event::Data::Materialization(data)) => {
                    self.total_files_materialized += data.file_count;
                    self.total_bytes_materialized += data.total_bytes;
                }
                Some(bsmr_data::span_end_event::Data::ActionExecution(data)) => {
                ...
                Some(bsmr_data::span_end_event::Data::Analysis(_)) => {
                    self.total_targets_analysed += 1;
                }
                Some(bsmr_data::span_end_event::Data::Command(_command)) => {
            ...
            Some(bsmr_data::buck_event::Data::Instant(instant_event)) => {
                match instant_event.data.as_ref() {
                    ...
                    Some(bsmr_data::instant_event::Data::SystemInfo(system_info)) => {
                    ...
                    Some(bsmr_data::instant_event::Data::VersionControlRevision(vcs)) => {
...
impl BuckSubcommand for SummaryCommand {
    ...
    async fn exec_impl(
        ...
        bsmr_client_ctx::println!("build ID: {}", invocation.trace_id)?;
