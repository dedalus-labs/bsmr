...
use bsmr_event_observer::what_ran;
...
use bsmr_event_observer::what_ran::WhatRanOutputCommandExtra;
...
/// (if unsure, use `bsmr root --kind project` to find it), then run the command. The command is
...
pub struct WhatRanCommand {
    ...
    /// That is command were running if bsmr process was killed,
    /// or command currently running if bsmr is running build now.
    #[clap(long)]
    pub incomplete: bool,
        ...
        if let Some(data) = event.data {
                ...
                && should_emit_finished_action(&span.data, options)
            {
                ...
                let (execution_kind, std_err, duration, scheduling_mode) =
                    match &span.data {
                        Some(bsmr_data::span_end_event::Data::ActionExecution(action_exec)) => (
                ...
                if execution_kind == Some(bsmr_data::ActionExecutionKind::LocalDepFile as i32) {
                    entry
                        .reproducers
...
fn should_emit_finished_action(
    data: &Option<bsmr_data::span_end_event::Data>,
    options: &WhatRanCommandOptions,
) -> bool {
    ...
    match data {
        Some(bsmr_data::span_end_event::Data::ActionExecution(action)) => {
        ...
        match &mut self.format {
            ...
            LogCommandOutputFormatWithWriter::Json(w) => {
                let reproducer = match &command.repro {
                    ...
                    CommandReproducer::CacheHit(cache_hit) => match cache_hit.cache_type() {
                        bsmr_data::CacheType::ActionCache => JsonReproducer::Cache {
                            digest: &cache_hit.action_digest,
                            action_key: cache_hit.action_key.as_deref(),
                        },
                        bsmr_data::CacheType::RemoteDepFileCache => {
            ...
            LogCommandOutputFormatWithWriter::Csv(writer) => {
                ...
                writer
                    ...
                    .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::LogCmd))?;
                Ok(())
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn serialize_what_ran_command_no_extr() -> bsmr_error::Result<()> {
  ...
  "duration": "1"
    ...
    #[test]
    fn serialize_what_ran_command_with_extra() -> bsmr_error::Result<()> {
  ...
  "extra": {
    ...
    #[test]
    fn serialize_what_ran_command_in_re() -> bsmr_error::Result<()> {
