...
use bsmr_client_ctx::events_ctx::DaemonEventsCtx;
use bsmr_client_ctx::events_ctx::EventsCtx;
use bsmr_client_ctx::exit_result::ExitResult;
use bsmr_client_ctx::signal_handler::with_simple_sigint_handler;
use bsmr_client_ctx::subscribers::superconsole::timekeeper::Clock;
use bsmr_client_ctx::subscribers::superconsole::timekeeper::Timekeeper;
use bsmr_client_ctx::subscribers::superconsole::timekeeper::duration_between_timestamps;
use bsmr_client_ctx::ticker::Tick;
...
impl BsmrSubcommand for ReplayCommand {
    ...
    async fn exec_impl(
        ...
        if !speed.is_finite() || speed <= 0.0 {
            return ExitResult::from(bsmr_error::Error::from(ReplayError::InvalidSpeed(speed)));
        }

        let seek = if let Some(seek) = seek {
            if !seek.is_finite() || seek < 0.0 {
                return ExitResult::from(bsmr_error::Error::from(ReplayError::InvalidSeek(seek)));
            }
            Seek::Relative(Duration::from_secs(1).mul_f64(seek))
        } else if let Some(seek_absolute) = seek_absolute {
            Seek::Absolute(bsmr_event_log::utils::timestamp::parse(seek_absolute.as_str())?.into())
        } else {
        ...
        let work = async {
            ...
            let res = res??;
            for e in &res.errors {
                bsmr_client_ctx::eprintln!("{}", e.message)?;
            }
...
impl TryFrom<bsmr_cli_proto::command_result::Result> for ReplayResult {
    type Error = bsmr_cli_proto::command_result::Result;

    fn try_from(v: bsmr_cli_proto::command_result::Result) -> Result<Self, Self::Error> {
        use bsmr_cli_proto::command_result::Result;
...
async fn make_replayer(
    ...
    impl Stream<Item = bsmr_error::Result<StreamValue>> + Unpin,
    Invocation,
    Timekeeper,
)> {
...
async fn replay_events_into(
    sink: UnboundedSender<bsmr_error::Result<StreamValue>>,
    events: impl Stream<Item = bsmr_error::Result<StreamValue>>,
    syncher: Syncher,
    mut speed_update_requests: mpsc::Receiver<SpeedUpdateRequest>,
    first_event: bsmr_error::Result<StreamValue>,
...
async fn find_next_event_with_delay(
    sink: &UnboundedSender<bsmr_error::Result<StreamValue>>,
    events: &mut (impl Stream<Item = bsmr_error::Result<StreamValue>> + Unpin),
