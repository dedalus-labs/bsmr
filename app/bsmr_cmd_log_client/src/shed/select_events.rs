...
mod tests {
    ...
    fn span_start_action() -> Data {
        Data::SpanStart(SpanStartEvent {
            data: Some(bsmr_data::span_start_event::Data::ActionExecution(
    ...
    fn span_end_action() -> Data {
        Data::SpanEnd(SpanEndEvent {
            data: Some(bsmr_data::span_end_event::Data::ActionExecution(
                Box::default(),
            )),
            ..Default::default()
        })
    ...
    fn instant_snapshot() -> Data {
        Data::Instant(bsmr_data::InstantEvent {
            data: Some(bsmr_data::instant_event::Data::Snapshot(Box::default())),
        })
    }
    ...
    async fn build_input_log(
        ...
        let invocation = bsmr_data::Invocation {
            command_line_args: vec!["bsmr".to_owned()],
            expanded_command_line_args: vec!["bsmr".to_owned()],
        ...
        for e in events {
            let cp = bsmr_cli_proto::CommandProgress {
                progress: Some(bsmr_cli_proto::command_progress::Progress::Event(e.into())),
            };
    ...
    #[tokio::test]
    async fn round_trip_no_filter_preserves_events() -> bsmr_error::Result<()> {
        let tmp = TempDir::new()?;
    ...
    #[tokio::test]
    async fn exclude_action_execution_drops_both_halves() -> bsmr_error::Result<()> {
        let tmp = TempDir::new()?;
        let input = AbsPathBuf::try_from(tmp.path().join("in.pb.zst")).unwrap();
        let output = AbsPathBuf::try_from(tmp.path().join("out.pb.zst")).unwrap();
    ...
    #[tokio::test]
    async fn extension_mismatch_errors() -> bsmr_error::Result<()> {
        let tmp = TempDir::new()?;
        let input = AbsPathBuf::try_from(tmp.path().join("in.pb.zst")).unwrap();
