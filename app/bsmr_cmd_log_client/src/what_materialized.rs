...
fn get_record(materialization: &bsmr_data::MaterializationEnd) -> Record {
    let method = match materialization
        .method
        .and_then(|v| bsmr_data::MaterializationMethod::try_from(v).ok())
    {
        Some(bsmr_data::MaterializationMethod::CasDownload) => "cas",
        Some(bsmr_data::MaterializationMethod::LocalCopy) => "copy",
        Some(bsmr_data::MaterializationMethod::HttpDownload) => "http",
        Some(bsmr_data::MaterializationMethod::Write) => "write",
        _ => "<unknown>",
    };
    Record {
