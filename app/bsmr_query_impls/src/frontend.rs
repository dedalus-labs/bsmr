...
use bsmr_build_api::query::oneshot::QueryFrontend;
...
impl QueryFrontend for QueryFrontendImpl {
    async fn eval_uquery(
        ...
        query: &str,
        query_args: &[String],
    ) -> bsmr_error::Result<QueryEvaluationResult<TargetNode>> {
        Ok(ctx
    ...
    async fn eval_cquery(
        ...
        Ok(ctx
            .with_linear_recompute(|ctx| async move {
                ...
                //   ```
                //   bsmr cquery --target-universe android//:binary 'deps("some//:lib (<arm32>)")'
                //   ```
                eval_cquery(
                    dice_query_delegate,
                    query,
    ...
    async fn eval_aquery(
        ...
        query_args: &[String],
        global_cfg_options: GlobalCfgOptions,
    ) -> bsmr_error::Result<QueryEvaluationResult<ActionQueryNode>> {
        Ok(ctx
            .with_linear_recompute(|ctx| async move {
...
async fn universe_from_literals(
    ...
    literals: &[String],
    global_cfg_options: GlobalCfgOptions,
) -> bsmr_error::Result<CqueryUniverse> {
