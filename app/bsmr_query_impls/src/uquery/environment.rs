...
impl<T: QueryTarget> PreresolvedQueryLiterals<T> {
    ...
    pub(crate) fn literals(&self) -> bsmr_error::Result<TargetSet<T>> {
...
impl<'c> UqueryEnvironment<'c> {
    ...
    async fn get_node(&self, target: &TargetLabel) -> bsmr_error::Result<TargetNode> {
        let package = self
            .delegate
            .ctx()
...
impl QueryEnvironment for UqueryEnvironment<'_> {
    ...
    async fn get_node(&self, node_ref: &TargetLabel) -> bsmr_error::Result<Self::Target> {
    ...
    async fn eval_literals(&self, literals: &[&str]) -> bsmr_error::Result<TargetSet<TargetNode>> {
        self.literals
    ...
    async fn dfs_postorder(
        ...
        visit: impl FnMut(TargetNode) -> bsmr_error::Result<()> + Send,
    ) -> bsmr_error::Result<()> {
...
async fn split_universe_files(
    ...
    delegate: &dyn UqueryDelegate,
) -> bsmr_error::Result<(Vec<ArcCellPath>, Vec<ArcCellPath>)> {
