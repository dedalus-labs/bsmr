...
use bsmr_common::init::ResourceControlInit;
use bsmr_common::init::ResourceControlStatus;
...
async fn get_daemon_spawner(init: &ResourceControlInit) -> bsmr_error::Result<DaemonSpawner> {
    match &init {
...
///
/// Returns the command, followed by a list of additional flags to pass to `bsmr daemon`.
pub async fn create_daemon_spawn_command(
    config: &ResourceControlConfig,
    program: impl AsRef<OsStr>,
    unit_name: String,
    working_directory: &AbsNormPath,
) -> bsmr_error::Result<(std::process::Command, Vec<String>)> {
    let daemon_spawner = {
        if config.status == ResourceControlStatus::Off
            || bsmr_core::bsmr_env!(
                "BSMR_TEST_DISABLE_DAEMON_CGROUP",
                type = bool,
...
async fn systemd_check_available() -> bsmr_error::Result<()> {
...
mod tests {
    ...
    async fn test_always_unavailable_on_nonlinux() {
        let _error = bsmr_error::Error::from(SystemdNotAvailableReason::UnsupportedPlatform);
        assert!(matches!(
