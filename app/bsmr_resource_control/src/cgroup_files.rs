...
impl CgroupFile {
    ...
    async fn read_to_short_buf(&self) -> bsmr_error::Result<([u8; 32], usize)> {
        let file = self.0.dupe();
        tokio::task::spawn_blocking(move || {
            let mut data = [0u8; 32];
            let mut filled = 0;
    ...
    pub(crate) async fn read_int(&self) -> bsmr_error::Result<u64> {
        let (data, filled) = self.read_to_short_buf().await?;
        let data = &data[..filled];
        std::str::from_utf8(data)
            .ok()
    ...
    pub(crate) async fn read_resource_pressure(&self) -> bsmr_error::Result<ResourcePressure> {
...
impl MemoryStat {
    fn parse(content: &str) -> bsmr_error::Result<Self> {
...
impl ResourcePressure {
    fn parse(s: &str) -> bsmr_error::Result<Self> {
        Self::parse_inner(s).ok_or_else(|| {
            bsmr_error::Error::from(CgroupFileError::UnexpectedFormat(
