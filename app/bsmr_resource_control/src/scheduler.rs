...
fn suspend_scene(
    cgroup: RunningScene,
    now: Instant,
) -> (SuspendedScene, bsmr_data::ResourceControlEventKind) {
    let (wake_implementation, event_kind) = match cgroup.suspend_implementation {
        SuspendImplementation::CgroupFreeze(kill_sender, freeze_sender) => {
            ...
            (
                ...
                bsmr_data::ResourceControlEventKind::SuspendFreeze,
            )
        }
        SuspendImplementation::KillAndRetry(kill_sender) => {
            ...
            (
                ...
                bsmr_data::ResourceControlEventKind::SuspendKill,
            )
        }
    };
    (
...
fn wake_scene(
    cgroup: Scene,
    wake_implementation: WakeImplementation,
) -> (RunningScene, bsmr_data::ResourceControlEventKind) {
    let (retry_tx, event_kind) = match wake_implementation {
        WakeImplementation::CgroupUnfreeze {
            ...
            return (
                ...
                bsmr_data::ResourceControlEventKind::WakeUnfreeze,
            );
        }
        WakeImplementation::KillAndRetry(retry_tx) => {
            (retry_tx, bsmr_data::ResourceControlEventKind::WakeRetry)
        }
        WakeImplementation::UnblockStart(retry_tx) => (
            retry_tx,
            bsmr_data::ResourceControlEventKind::WakeDelayedStart,
        ),
    };
...
mod tests {
    ...
    #[test]
    fn test_peak_memory_and_swap() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_freeze() -> bsmr_error::Result<()> {
        let (mut scheduler, timeline) = TestTimeline::new();
