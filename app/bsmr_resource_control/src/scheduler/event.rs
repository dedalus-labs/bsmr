...
impl EventSenderState {
    pub(crate) fn new(daemon_id: &DaemonId, estimated_memory_cap: u64) -> Self {
        Self {
            metadata: bsmr_events::metadata::collect(daemon_id),
    ...
    pub(crate) fn maybe_send_scheduled_event(
            ...
            .is_none_or(|last| now.duration_since(last) >= freq)
        {
            ...
            self.send_event(
                bsmr_data::ResourceControlEventKind::Scheduled,
                None,
                actions_running,
                actions_suspended,
            );
...
impl ResourceControlEventMostly {
    pub(crate) fn complete(self, uuid: &TraceId) -> bsmr_data::ResourceControlEvent {
        bsmr_data::ResourceControlEvent {
            uuid: uuid.to_string(),
            kind: self.kind.into(),
            ...
            ancestor_cgroup_constraints: Some(bsmr_data::AncestorCgroupConstraints {
