...
fn parse_procfs_cgroup_output(out: &str) -> bsmr_error::Result<CgroupPathBuf> {
...
/// Read the cgroup path of the bsmr daemon process based on its pid from the client side
pub fn read_cgroup_path_of_bsmr_daemon(daemon_pid: i64) -> bsmr_error::Result<Option<String>> {
...
impl PreppedBsmrCgroups {
    ...
    ///  2. Bessemer may manage the child cgroups by itself without interference from outside things;
...
pub struct BsmrCgroupTree {
    ...
    daemon: CgroupLeaf,
    /// The resource constraints imposed by the ancestors of the bsmr cgroup tree
    ///
...
impl BsmrCgroupTree {
    /// Finishes setting up bsmr's cgroups from the prepped ones
    pub async fn set_up(
        prepped: PreppedBsmrCgroups,
    ...
    /// The parent cgroup that contains all other cgroups bsmr manages as descendants
...
mod tests {
    ...
    use crate::cgroup_tree::parse_procfs_cgroup_output;
    use crate::cgroup_tree::resolve_memory_restriction_value;

    #[test]
    fn test_cgroup_info_parse() {
        let cgroup = "\
0::/user.slice/user-532497.slice/user@532497.service/bsmr.cg\n";
        assert_eq!(
            "/sys/fs/cgroup/user.slice/user-532497.slice/user@532497.service/bsmr.cg",
...
0::/user.slice/user-532497.slice/user@532497.service/bsmr.cg\n";
    ...
    async fn test_drain_to_child_moves_orphan_to_daemon() {
        ...
        let p = PreppedBsmrCgroups::testing_new_in(root).await;
        let t = BsmrCgroupTree::set_up(
            p,
