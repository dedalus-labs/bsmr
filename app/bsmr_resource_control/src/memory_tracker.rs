...
pub fn spawn_memory_reporter(
    ...
    tokio::task::spawn(bsmr_util::async_move_clone!(cancel, {
...
pub async fn create_memory_tracker(
    ...
    let action_cgroups = Scheduler::init(
        ...
        bsmr_util::system_stats::system_memory_stats(),
        Instant::now(),
    );
...
impl MemoryTracker {
    pub(crate) async fn spawn(self, duration: Duration) -> bsmr_error::Result<()> {
...
mod tests {
    ...
    async fn test_current_memory_changes_tracked() -> bsmr_error::Result<()> {
        ...
        let assert_max_over = |f: fn(&bsmr_data::ResourceControlEvent) -> u64, val, name: &str| {
