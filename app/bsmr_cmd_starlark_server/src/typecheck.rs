...
use bsmr_cmd_starlark_client::typecheck::StarlarkTypecheckCommand;
...
impl Cache<'_> {
    async fn typecheck(&mut self, path: OwnedStarlarkPath) -> bsmr_error::Result<()> {
        self.run(path).await?;
        Ok(())
    }
    ...
    async fn get(&mut self, path: OwnedStarlarkModulePath) -> bsmr_error::Result<Interface> {
    ...
    #[async_recursion]
    async fn run(&mut self, path: OwnedStarlarkPath) -> bsmr_error::Result<Interface> {
