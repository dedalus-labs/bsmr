    ...
    let root_certs = if let Some(path) = find_root_ca_certs() {
        ...
        if !native_certs_results.certs.is_empty() {
            ...
            let native_certs_error = native_certs_results
                .errors
                .pop()
                .map(bsmr_error::Error::from)
                .unwrap_or(bsmr_error!(
                    bsmr_error::ErrorTag::NoValidCerts,
            ...
            if cfg!(fbcode_build) {
                ...
                let context = format!(
                    ...
                    Please try `getchef -reason 'chef broken'`{windows_message}, `Fix My <OS>` via the f-menu, then `bsmr killall`.
...
async fn load_key<P: AsRef<Path>>(key: P) -> bsmr_error::Result<PrivateKeyDer<'static>> {
    let key = key.as_ref();
...
async fn load_cert_pair<P: AsRef<Path>>(
    cert: P,
    key: P,
) -> bsmr_error::Result<(Vec<CertificateDer<'static>>, PrivateKeyDer<'static>)> {
...
pub async fn tls_config_with_system_roots() -> bsmr_error::Result<ClientConfig> {
...
pub async fn tls_config_with_single_cert<P: AsRef<Path>>(
    cert_path: P,
    key_path: P,
) -> bsmr_error::Result<ClientConfig> {
    let system_roots = load_system_root_certs().await?;
...
pub(crate) async fn load_certs<P: AsRef<Path>>(
    cert_path: P,
) -> bsmr_error::Result<Vec<CertificateDer<'static>>> {
