...
#[bsmr(environment, tag = NoValidCerts)]
...
async fn verify(path: &OsString) -> bsmr_error::Result<()> {
...
pub async fn validate_certs() -> bsmr_error::Result<()> {
...
pub async fn check_cert_state(cert_state: CertState) -> Option<bsmr_error::Error> {
