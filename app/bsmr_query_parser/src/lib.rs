...
//! # quoting targets "@upstream//some:target" or common regexes ".*" or filenames "Foo.java".
//! WORD ::=
//!          '\'' not_single_quote * '\''
//!        | '"' not_double_quote * '"'
//!        | "a-zA-Z0-9*/@._:$#%-" *
...
pub fn parse_expr(input: &str) -> bsmr_error::Result<SpannedExpr<'_>> {
...
mod tests {
    ...
    #[test]
    fn test_expr() -> bsmr_error::Result<()> {
        run_tests(
            expr,
            &[
                "f",
    ...
    #[test]
    fn test_function() -> bsmr_error::Result<()> {
        run_tests(
            expr_function,
    ...
    #[test]
    fn test_word() -> bsmr_error::Result<()> {
        run_tests(
            expr_word,
            &[
                "@cell//package:target",
    ...
    #[test]
    fn test_integer() -> bsmr_error::Result<()> {
        run_tests(expr_int, &["0", "1234"], &["w123", ".1", ""], &["0123"]);
        Ok(())
    }

    #[test]
    fn test_trailing_infix() -> bsmr_error::Result<()> {
        run_tests(
            trailing_infix,
            &[
                " + b",
