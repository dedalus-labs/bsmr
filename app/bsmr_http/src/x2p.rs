...
mod imp {
    ...
    pub(super) fn find_http_proxy() -> bsmr_error::Result<Option<Proxy>> {
...
#[cfg(fbcode_build)]
pub fn find_proxy() -> bsmr_error::Result<Option<Proxy>> {
    #[cfg(unix)]
...
impl From<bsmr_error::Error> for X2PAgentError {
...
impl X2PAgentError {
    pub fn from_headers(uri: &Uri, headers: &HeaderMap) -> Option<Self> {
        ...
        match (auth_decision, error_type, error_msg) {
            ...
            (_, _, Some(message)) => Some(Self::Error(bsmr_error!(
