...
where
    Exec: Fn() -> F,
    E: IntoBsmrError + HttpErrorForRetry + std::fmt::Display + Send + Sync + 'static,
        ...
        if err.is_retryable() {
            if let Some(b) = backoff.peek() {
                ...
                tracing::warn!(
                    ...
                    b.as_secs(),
                    // Print as a bsmr_error to make sure we get the source
                    err.into_bsmr_error()
                );
                continue;
            }
...
mod tests {
    ...
    impl IntoBsmrError for HttpTestError {
        fn into_bsmr_error(self) -> bsmr_error::Error {
    ...
    fn ok_response() -> Result<String, HttpTestError> {
        Ok("Success".to_owned()).map_err(|_: bsmr_error::Error| test_error(StatusCode::IM_A_TEAPOT))
    }
