...
impl ProviderCodegen {
        ...
        if let Some(ref custom_methods) = self.args.methods_func {
            ...
            Ok(syn::parse_quote_spanned! {self.span=>
                fn documentation(&self) -> starlark::docs::DocItem {
                    ...
                    bsmr_build_api::interpreter::rule_defs::provider::doc::provider_callable_documentation(
                        Some(#create_func),
                        bsmr_build_api::interpreter::rule_defs::provider::doc::ProviderMembersSource::FromMethods(#custom_methods),
            ...
            Ok(syn::parse_quote_spanned! {self.span=>
                fn documentation(&self) -> starlark::docs::DocItem {
                    ...
                    bsmr_build_api::interpreter::rule_defs::provider::doc::provider_callable_documentation(
                        Some(#create_func),
                        bsmr_build_api::interpreter::rule_defs::provider::doc::ProviderMembersSource::FromFields {
                            fields: &field_names,
    ...
    fn builtin_provider_ty(&self) -> syn::Result<syn::Item> {
        ...
        Ok(syn::parse_quote_spanned! { self.span =>
            static BUILTIN_PROVIDER_TY: bsmr_build_api::interpreter::rule_defs::provider::builtin::ty::BuiltinProviderTy<
                    #gen_name<starlark::values::Value>,
                    #callable_name,
            > =
                bsmr_build_api::interpreter::rule_defs::provider::builtin::ty::BuiltinProviderTy::new();
        })
    }
        ...
        Ok(syn::parse_quote_spanned! { self.span=>
            impl<V: starlark::values::ValueLifetimeless> std::fmt::Display for #gen_name<V> {
                fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                    bsmr_build_api::__derive_refs::display_container::fmt_keyed_container(
                        f,
                        &format!("{}(", #name_str),
                        ")",
                        "=",
                    ...
                    Self: starlark::any::ProvidesStaticType<'v>,
                {
                    ...
                    fn provide(&'v self, demand: &mut starlark::values::Demand<'_, 'v>) {
                        demand.provide_value::<
                            &dyn bsmr_build_api::interpreter::rule_defs::provider::ProviderLike>(self);
                    }
    ...
    fn impl_serializable_value(&self) -> syn::Result<syn::Item> {
        ...
        Ok(syn::parse_quote_spanned! { self.span=>
            impl<'v, V: starlark::values::ValueLike<'v>> bsmr_build_api::__derive_refs::serde::Serialize
                for #gen_name<V>
            {
                fn serialize<S>(&self, s: S) -> Result<S::Ok, S::Error> where S : bsmr_build_api::__derive_refs::serde::Serializer {
                    use bsmr_build_api::__derive_refs::serde::ser::SerializeMap;
    ...
    fn impl_provider_like(&self) -> syn::Result<syn::Item> {
        ...
        let callable_name = self.callable_name()?;
        Ok(syn::parse_quote_spanned! { self.span=>
            impl<'v, V: starlark::values::ValueLike<'v>> bsmr_build_api::interpreter::rule_defs::provider::ProviderLike<'v> for #gen_name<V>
            where
                Self: std::fmt::Debug,
            {
                fn id(&self) -> &std::sync::Arc<bsmr_core::provider::id::ProviderId> {
    ...
    fn impl_frozen_builtin_provider(&self) -> syn::Result<syn::Item> {
        ...
        let callable_name = self.callable_name()?;
        Ok(syn::parse_quote_spanned! { self.span=>
            impl bsmr_build_api::interpreter::rule_defs::provider::FrozenBuiltinProviderLike for #frozen_name {
                fn builtin_provider_id() -> &'static std::sync::Arc<bsmr_core::provider::id::ProviderId> {
                    #callable_name::provider_id()
                }
            }
        })
        ...
        Ok(vec![
            ...
            syn::parse_quote_spanned! {self.span=>
                ...
                impl<'v> starlark::values::StarlarkValue<'v> for #callable_name
                {
                    ...
                    fn provide(&'v self, demand: &mut starlark::values::Demand<'_, 'v>) {
                        demand.provide_value::<
                            &dyn bsmr_interpreter::types::provider::callable::ProviderCallableLike>(self);
                    }
    ...
    fn callable_impl_provider_callable_like(&self) -> syn::Result<syn::Item> {
        let callable_name = self.callable_name()?;
        Ok(syn::parse_quote_spanned! { self.span=>
            impl bsmr_interpreter::types::provider::callable::ProviderCallableLike for #callable_name {
                fn id(&self) -> bsmr_error::Result<&std::sync::Arc<bsmr_core::provider::id::ProviderId>> {
    ...
    fn callable_impl(&self) -> syn::Result<syn::Item> {
        ...
        Ok(syn::parse_quote_spanned! { self.span=>
            impl #callable_name {
                #vis fn provider_id()
                -> &'static std::sync::Arc<bsmr_core::provider::id::ProviderId> {
                    Self::provider_id_t().id()
                }

                #vis fn provider_id_t() -> &'static std::sync::Arc<
                    bsmr_core::provider::id::ProviderIdWithType<#frozen_name>,
                > {
                    ...
                    PROVIDER_ID_T.get_or_init(|| {
                        std::sync::Arc::new(
                            bsmr_core::provider::id::ProviderIdWithType::new(
                                None,
                                #name_str.to_owned(),
                            ),
                        )
    ...
    fn inventory(&self) -> syn::Result<syn::Item> {
        Ok(syn::parse_quote_spanned! { self.span=>
            bsmr_build_api::__derive_refs::inventory::submit! {
                bsmr_build_api::interpreter::rule_defs::provider::registration::ProviderRegistration {
