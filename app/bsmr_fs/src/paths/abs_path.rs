...
impl AbsPath {
    pub fn new<P: AsRef<Path> + ?Sized>(path: &P) -> bsmr_error::Result<&AbsPath> {
        // Wrapper function to make sure the lifetimes are right
        fn inner(path: &Path) -> bsmr_error::Result<&AbsPath> {
            if path.is_absolute() {
    ...
    pub fn to_str(&self) -> bsmr_error::Result<&str> {
        match self.0.to_str() {
            Some(s) => Ok(s),
    ...
    pub fn strip_prefix<P: AsRef<AbsPath>>(&self, prefix: P) -> bsmr_error::Result<&Path> {
    ...
    pub fn as_maybe_relativized_str(&self) -> bsmr_error::Result<&str> {
        Ok(cwd::maybe_relativize_str(self.to_str()?))
    }
}

impl AbsPathBuf {
    pub fn new<P: AsRef<Path>>(path: P) -> bsmr_error::Result<Self> {
    ...
    pub fn into_string(self) -> bsmr_error::Result<String> {
        self.into_os_string()
            .into_string()
...
impl FromStr for AbsPathBuf {
    ...
    fn from_str(s: &str) -> bsmr_error::Result<AbsPathBuf> {
