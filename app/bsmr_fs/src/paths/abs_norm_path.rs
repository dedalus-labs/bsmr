...
impl AbsNormPath {
    ...
    /// }
    /// ```
    pub fn new<P: ?Sized + AsRef<Path>>(p: &P) -> bsmr_error::Result<&AbsNormPath> {
    ...
    fn strip_prefix_impl(&self, base: &AbsNormPath) -> bsmr_error::Result<&Path> {
        if self.windows_prefix()? == base.windows_prefix()? {
            ...
            Err(bsmr_error::bsmr_error!(
                bsmr_error::ErrorTag::InvalidAbsPath,
    ...
    /// # bsmr_error::Ok(())
    /// ```
    pub fn windows_prefix(&self) -> bsmr_error::Result<OsString> {
        use std::os::windows::ffi::OsStringExt;
        use std::path::Prefix;

        match self.0.components().next().ok_or_else(|| {
            bsmr_error::bsmr_error!(bsmr_error::ErrorTag::InvalidAbsPath, "AbsPath is empty.")
        })? {
            std::path::Component::Prefix(prefix_component) => match prefix_component.kind() {
                ...
                prefix => Err(bsmr_error::bsmr_error!(
                    bsmr_error::ErrorTag::InvalidAbsPath,
                    "Unknown prefix kind: {:?}.",
                    prefix
                )),
    ...
    /// # bsmr_error::Ok(())
    /// ```
    pub fn strip_windows_prefix(&self) -> bsmr_error::Result<&Path> {
...
impl AbsNormPathBuf {
    pub fn new(path: PathBuf) -> bsmr_error::Result<AbsNormPathBuf> {
    ...
    pub fn from(s: String) -> bsmr_error::Result<Self> {
...
impl TryFrom<String> for AbsNormPathBuf {
    ...
    /// }
    /// ```
    fn try_from(s: String) -> bsmr_error::Result<AbsNormPathBuf> {
...
impl TryFrom<OsString> for AbsNormPathBuf {
    ...
    // no allocation
    fn try_from(s: OsString) -> bsmr_error::Result<AbsNormPathBuf> {
...
impl TryFrom<PathBuf> for AbsNormPathBuf {
    ...
    /// }
    /// ```
    fn try_from(p: PathBuf) -> bsmr_error::Result<AbsNormPathBuf> {
...
impl FromStr for AbsNormPathBuf {
    type Err = bsmr_error::Error;

    fn from_str(s: &str) -> bsmr_error::Result<Self> {
...
fn verify_abs_path(path: &AbsPath) -> bsmr_error::Result<()> {
...
mod tests {
    ...
    #[test]
    fn abs_path_is_comparable() -> bsmr_error::Result<()> {
        let foo_string = make_absolute("/foo");
        let bar_string = make_absolute("/bar");
    ...
    #[cfg(not(windows))]
    #[test]
    fn absolute_path_display_is_readable() -> bsmr_error::Result<()> {
