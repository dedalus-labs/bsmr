...
fn verify_file_name(file_name: &str) -> bsmr_error::Result<()> {
...
impl FileName {
    ...
    /// ```
    #[inline]
    pub fn new<S: ?Sized + AsRef<str>>(s: &S) -> bsmr_error::Result<&Self> {
        verify_file_name(s.as_ref())?;
        Ok(Self::unchecked_new(s.as_ref()))
    }

    pub fn from_os_string(file_name: &OsString) -> bsmr_error::Result<&FileName> {
...
impl<'a> TryFrom<&'a str> for &'a FileName {
    ...
    #[inline]
    fn try_from(value: &'a str) -> bsmr_error::Result<&'a FileName> {
        FileName::new(value)
    }
}
...
impl TryFrom<String> for FileNameBuf {
    ...
    #[inline]
    fn try_from(value: String) -> bsmr_error::Result<FileNameBuf> {
...
impl TryFrom<CompactString> for FileNameBuf {
    ...
    #[inline]
    fn try_from(value: CompactString) -> bsmr_error::Result<FileNameBuf> {
        verify_file_name(value.as_str())?;
