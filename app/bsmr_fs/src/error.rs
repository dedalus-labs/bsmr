...
impl IoError {
    /// Create an error from an io::Error or bsmr_error::Error.
    ...
    pub fn internal(e: bsmr_error::Error) -> Self {
        Self::new(IoErrorSource::Internal(e))
    }

    pub fn from_join(e: JoinError) -> Self {
        Self::internal(bsmr_error::Error::from(e))
    }
    ...
    fn convert_to_bsmr_error(self) -> bsmr_error::Error {
        ...
        let source_error: bsmr_error::Error = match self.source {
    ...
    /// Convert to a bsmr_error::Error and tag as an input error if a path is missing.
    /// This should be used if the path was user provided,
    /// from a source file, configuration or CLI argument.
    pub fn categorize_input(mut self) -> bsmr_error::Error {
        self.is_input_path = Some(true);
        self.convert_to_bsmr_error()
    }

    /// Convert to a bsmr_error::Error and tag as a tier0 error if a path is missing.
    /// This should be used if the path was not user provided,
    /// generated by bsmr or internal infra.
    ...
    pub fn categorize_internal(mut self) -> bsmr_error::Error {
        self.is_input_path = Some(false);
        self.convert_to_bsmr_error()
    }

    /// Convert to a bsmr_error::Error with no tags, new code should only use this in tests
    pub fn uncategorized(self) -> bsmr_error::Error {
        self.convert_to_bsmr_error()
    }
}
...
pub trait IoResultExt<T> {
    fn categorize_input(self) -> bsmr_error::Result<T>;
    fn categorize_internal(self) -> bsmr_error::Result<T>;
    fn uncategorized(self) -> bsmr_error::Result<T>;
}
