...
impl<T: WorkingDirectoryImpl> WorkingDirectoryGen<T> {
    pub fn open(path: AbsNormPathBuf) -> bsmr_error::Result<Self> {
        let imp = T::open(&path)?;
        Ok(Self { path, imp })
    }
    ...
    pub fn is_stale(&self) -> bsmr_error::Result<bool> {
...
pub trait WorkingDirectoryImpl: Sized {
    fn open(path: &AbsNormPath) -> bsmr_error::Result<Self>;
    fn chdir(&self, _path: &AbsNormPath) -> bsmr_error::Result<()>;
...
impl WorkingDirectoryImpl for PortableWorkingDirectoryImpl {
    fn open(_path: &AbsNormPath) -> bsmr_error::Result<Self> {
        Ok(Self)
    }

    fn chdir(&self, path: &AbsNormPath) -> bsmr_error::Result<()> {
        fs_util::set_current_dir(path).categorize_internal()?;
        Ok(())
    }

    /// Not supported for PortableWorkingDirectoryImpl
    fn is_stale(&self, _path: &AbsNormPath) -> bsmr_error::Result<bool> {
        Ok(false)
    }
}
...
fn cwd_will_not_change(cwd: &AbsNormPath) -> bsmr_error::Result<()> {
...
pub(crate) fn assert_cwd_is_not_set() -> bsmr_error::Result<()> {
