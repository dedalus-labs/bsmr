...
use bsmr_util::process_stats::process_cpu_time_us;
...
pub(crate) fn maybe_schedule_termination() -> bsmr_error::Result<()> {
    if let Some(duration) =
        bsmr_core::bsmr_env!("BSMR_TERMINATE_AFTER", type=u64, applicability=testing)?
            .map(Duration::from_secs)
            .or_else(bsmr_common::self_test_timeout::until_post_test_shutdown)
    {
        thread_spawn("bsmr-terminate-after", move || {
