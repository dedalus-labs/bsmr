...
impl MacosQosClass {
    ...
    fn from_config(s: &str) -> bsmr_error::Result<Self> {
        match s {
            ...
            other => Err(bsmr_error::bsmr_error!(
...
/// bsmr daemon spawns processes like compilers which do not really need high priority.
...
#[cfg(target_os = "macos")]
fn do_lower_priority(qos_class: MacosQosClass) -> bsmr_error::Result<()> {
    use std::env;
    use std::ffi::CString;
    use std::io;
    use std::iter;
    ...
    impl Spawnattr {
        fn new() -> bsmr_error::Result<Spawnattr> {
            unsafe {
    ...
    let mut argv: Vec<CString> = env::args()
        .map(|s| CString::new(s).map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::Tier0)))
        .chain(iter::once(Ok(CString::new("--skip-macos-qos").map_err(
            |e| from_any_with_tag(e, bsmr_error::ErrorTag::Tier0),
        )?)))
