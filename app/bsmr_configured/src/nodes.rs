...
use bsmr_build_api::interpreter::rule_defs::provider::builtin::dep_only_incompatible_info::DepOnlyIncompatibleCustomSoftErrors;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::dep_only_incompatible_info::FrozenDepOnlyIncompatibleInfo;
use bsmr_build_api::transition::TRANSITION_ATTRS_PROVIDER;
use bsmr_build_api::transition::TRANSITION_CALCULATION;
...
use bsmr_core::configuration::compatibility::ResultMaybeCompatibleValueSerialize;
use bsmr_core::configuration::data::ConfigurationData;
use bsmr_core::configuration::pair::ConfigurationNoExec;
use bsmr_core::configuration::pair::ConfigurationWithExec;
...
use bsmr_core::plugins::PluginListElemKind;
use bsmr_core::plugins::PluginLists;
...
use bsmr_node::attrs::configuration_context::AttrConfigurationContextImpl;
use bsmr_node::attrs::configuration_context::PlatformConfigurationError;
use bsmr_node::attrs::configured_attr::ConfiguredAttr;
use bsmr_node::attrs::configured_traversal::ConfiguredAttrTraversal;
use bsmr_node::attrs::display::AttrDisplayWithContextExt;
use bsmr_node::attrs::inspect_options::AttrInspectOptions;
use bsmr_node::attrs::spec::AttributeId;
use bsmr_node::attrs::spec::internal::INCOMING_TRANSITION_ATTRIBUTE;
use bsmr_node::attrs::spec::internal::LEGACY_TARGET_COMPATIBLE_WITH_ATTRIBUTE;
use bsmr_node::attrs::spec::internal::TARGET_COMPATIBLE_WITH_ATTRIBUTE;
use bsmr_node::configuration::calculation::CellNameForConfigurationResolution;
use bsmr_node::configuration::resolved::MatchedConfigurationSettingKeys;
...
use bsmr_node::nodes::configured_frontend::CONFIGURED_TARGET_NODE_CALCULATION;
use bsmr_node::nodes::configured_frontend::ConfiguredTargetNodeCalculation;
use bsmr_node::nodes::configured_frontend::ConfiguredTargetNodeCalculationImpl;
...
use bsmr_node::rule::RuleIncomingTransition;
...
enum NodeCalculationError {
    ...
    #[error(
        "Target {0} configuration transition is not idempotent
         ...
         Use `bsmr audit configurations {1} {2} {3}` to see the configurations."
    )]
...
fn unpack_target_compatible_with_attr(
    ...
    attr_id: AttributeId,
) -> bsmr_error::Result<Option<ConfiguredAttr>> {
    ...
    impl AttrConfigurationContext for AttrConfigurationContextToResolveCompatibleWith<'_> {
        ...
        fn base_exec_cfg(&self) -> bsmr_error::Result<ConfigurationNoExec> {
            Err(internal_error!(
        ...
        fn platform_cfg(&self, _label: &TargetLabel) -> bsmr_error::Result<ConfigurationData> {
            unreachable!(
        ...
        fn resolved_transitions(
            &self,
        ) -> bsmr_error::Result<&OrderedMap<Arc<TransitionId>, Arc<TransitionApplied>>> {
            Err(internal_error!(
...
fn check_compatible(
    ...
    target_node: TargetNodeRef,
    resolved_cfg: &MatchedConfigurationSettingKeysWithCfg,
) -> bsmr_error::Result<MaybeCompatible<()>> {
    ...
    let check_compatibility = |attr| -> bsmr_error::Result<(Vec<_>, Vec<_>)> {
...
pub(crate) async fn gather_deps(
    ...
    impl ConfiguredAttrTraversal for Traversal {
        ...
        fn plugin_dep(&mut self, dep: &TargetLabel, kind: &PluginKind) -> bsmr_error::Result<()> {
            self.plugin_lists
...
async fn resolve_transition_input_attrs<'a>(
    ...
    impl AttrConfigurationContext for AttrConfigurationContextToResolveTransitionAttrs<'_> {
        ...
        fn platform_cfg(&self, label: &TargetLabel) -> bsmr_error::Result<ConfigurationData> {
...
impl bsmr_error::TypedContext for LookingUpConfiguredNodeContext {
    fn eq(&self, other: &dyn bsmr_error::TypedContext) -> bool {
...
pagable::static_str!(SECTION_BSMR = "bsmr");
...
async fn check_target_enabled_for_config(
    ...
    impl Key for ConfigPatternCalculation {
        type Value = bsmr_error::Result<Arc<Vec<ParsedPattern<TargetPatternExtra>>>>;
...
async fn get_dep_only_incompatible_custom_soft_error(
    ctx: &mut DiceComputations<'_>,
    target_label: &TargetLabel,
) -> bsmr_error::Result<Option<Vec<ArcStr>>> {
    #[derive(Clone, Dupe, Display, Debug, Eq, Hash, PartialEq, Allocative, Pagable)]
    #[pagable_typetag(dice::DiceKeyDyn)]
    ...
    impl Key for GetDepOnlyIncompatibleInfo {
        type Value = bsmr_error::Result<Option<Arc<DepOnlyIncompatibleCustomSoftErrors>>>;
