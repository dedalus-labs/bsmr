...
use bsmr_build_api::actions::execute::dice_data::HasFallbackExecutorConfig;
use bsmr_build_api::analysis::calculation::RuleAnalysisCalculation;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::constraint_value_info::FrozenConstraintValueInfo;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::execution_platform_registration_info::FrozenExecutionPlatformRegistrationInfo;
...
use bsmr_core::execution_types::execution::ExecutionPlatformError;
use bsmr_core::execution_types::execution::ExecutionPlatformIncompatibleReason;
...
use bsmr_core::execution_types::execution_platforms::ExecutionPlatformFallback;
use bsmr_core::execution_types::execution_platforms::ExecutionPlatforms;
use bsmr_core::execution_types::execution_platforms::ExecutionPlatformsData;
...
use bsmr_node::attrs::spec::internal::EXEC_COMPATIBLE_WITH_ATTRIBUTE;
use bsmr_node::configuration::calculation::CellNameForConfigurationResolution;
use bsmr_node::configuration::resolved::ConfigurationSettingKey;
...
use bsmr_core::configuration::pair::Configuration;
...
) -> bsmr_error::Result<ExecutionPlatform> {
...
impl ToolchainExecutionPlatformCompatibilityKey {
    async fn compute_impl(
        &self,
        ctx: &mut DiceComputations<'_>,
    ) -> bsmr_error::Result<Result<(), ExecutionPlatformIncompatibleReason>> {
...
impl Key for ToolchainExecutionPlatformCompatibilityKey {
    type Value = bsmr_error::Result<Result<(), ExecutionPlatformIncompatibleReason>>;
    async fn compute(
        &self,
...
) -> bsmr_error::Result<MaybeCompatible<ExecutionPlatformResolutionPartial>> {
...
async fn get_execution_platforms_enabled(
    ctx: &mut DiceComputations<'_>,
) -> bsmr_error::Result<ExecutionPlatforms> {
...
impl Key for ExecutionPlatformResolutionKey {
    type Value = bsmr_error::Result<ExecutionPlatformResolutionPartial>;
...
impl Key for ExecutionPlatformsKey {
    type Value = bsmr_error::Result<Option<ExecutionPlatforms>>;
    async fn compute(
        &self,
        ctx: &mut DiceComputations,
        _cancellation: &CancellationContext,
