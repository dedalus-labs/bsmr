    ...
    fn handle_event(&mut self, event: &BuckEvent) -> bsmr_error::Result<()> {
        match event.data() {
            ...
            bsmr_data::buck_event::Data::SpanEnd(end) => {
                ...
                } else if let Some(bsmr_data::span_end_event::Data::Load(_)) = end.data.as_ref() {
                    if end
                        .duration
                        .as_ref()
                ...
                } else if let Some(bsmr_data::span_end_event::Data::LoadPackage(_)) =
                    end.data.as_ref()
                {
                    if end
                        .duration
            ...
            bsmr_data::buck_event::Data::Instant(instant) => {
                    ...
                    instant.data.as_ref()
                {
                    self.critical_path_span_ids = info
                        ...
                        .collect()
                } else if let Some(bsmr_data::instant_event::Data::CommandOptions(options)) =
                    instant.data.as_ref()
                {
...
impl ChromeTraceInstant {
    fn into_json(self) -> bsmr_error::Result<serde_json::Value> {
        let mut js = json!(
            {
                "name": self.name,
        ...
        let obj = js
            .as_object_mut()
            .ok_or(bsmr_error::internal_error!("expected a mutable object"))?;
...
impl TrackId {
    fn as_u64(&self) -> bsmr_error::Result<u64> {
        ...
        + Serialize,
{
    ...
    fn process_timestamp(&mut self, timestamp: SystemTime) -> bsmr_error::Result<()> {
    ...
    fn set(&mut self, timestamp: SystemTime, key: &str, amount: T) -> bsmr_error::Result<()> {
    ...
    fn bump(&mut self, timestamp: SystemTime, key: &str, amount: T) -> bsmr_error::Result<()> {
        self.process_timestamp(timestamp)?;
        let entry = self.counter_entry(key);
        entry.value += amount;
        Ok(())
    ...
    fn subtract(&mut self, timestamp: SystemTime, key: &str, amount: T) -> bsmr_error::Result<()> {
        self.process_timestamp(timestamp)?;
        let entry = self.counter_entry(key);
        entry.value -= amount;
        Ok(())
    ...
    pub fn flush_all_to(&mut self, output: &mut Vec<serde_json::Value>) -> bsmr_error::Result<()> {
        self.flush()?;
...
impl ChromeTraceWriter {
    ...
    pub fn into_writer<W>(mut self, file: W) -> bsmr_error::Result<()>
    where
        W: Write,
    {
        self.span_counters
        ...
        match event.data() {
            bsmr_data::buck_event::Data::SpanStart(bsmr_data::SpanStartEvent {
                ...
                let categorization = match start_data {
                    bsmr_data::span_start_event::Data::Command(_command) => Categorization::Show {
                    ...
                    bsmr_data::span_start_event::Data::LoadPackage(load_package) => {
                        let category = if on_critical_path {
                            Some(SpanCategorization::CriticalPath)
                        } else if self
                            .first_pass
                    ...
                    bsmr_data::span_start_event::Data::ReUpload(_) => {
                        let name = "re_upload";
                        self.span_counters.bump_counter_while_span(event, name, 1)?;
                        Categorization::ShowIfParent { name: name.into() }
                    }
                    bsmr_data::span_start_event::Data::FinalMaterialization(..) => {
                        if on_critical_path {
                            Categorization::Show {
                    ...
                    bsmr_data::span_start_event::Data::FileWatcher(_file_watcher) => {
                        Categorization::Show {
                            category: SpanCategorization::CriticalPath,
            ...
            bsmr_data::buck_event::Data::Instant(bsmr_data::InstantEvent {
                ...
                bsmr_data::instant_event::Data::Snapshot(snapshot) => {
                    self.process_memory_counters.set(
                        event.timestamp(),
                        "max_rss_gigabyte",
                        (snapshot.bsmr_max_rss) as f64 / Self::BYTES_PER_GIGABYTE,
                    )?;
                    ...
                    self.rate_of_change_counters
                        .set_average_rate_of_change_per_s(
                            ...
                            snapshot.bsmr_user_cpu_us,
                        )?;
                    self.rate_of_change_counters
                        .set_average_rate_of_change_per_s(
                            event.timestamp(),
                            "average_system_cpu_in_usecs_per_s",
                            snapshot.bsmr_system_cpu_us,
                        )?;
                ...
                bsmr_data::instant_event::Data::ResourceControlEvent(events) => {
                    self.snapshot_counters.set(
                        event.timestamp(),
                ...
                bsmr_data::instant_event::Data::CommandPreempted(..) => {
    ...
    fn write_instant_events(&mut self, events: Vec<ChromeTraceInstant>) -> bsmr_error::Result<()> {
    ...
    fn write_thread_names(&mut self) -> bsmr_error::Result<()> {
    ...
    fn write_critical_path(
        &mut self,
        track_key: SpanCategorization,
        critical_path: &[bsmr_data::CriticalPathEntry2],
    ) -> bsmr_error::Result<()> {
    ...
    fn write_critical_path_hierarchical(
        ...
        let mut pending_waiting: Vec<&bsmr_data::CriticalPathEntry2> = Vec::new();
        ...
        if !pending_waiting.is_empty() {
            ...
            let synthetic_parent = bsmr_data::CriticalPathEntry2 {
                span_ids: Vec::new(),
    ...
    fn write_critical_path_group(
        &mut self,
        name: SpanCategorization,
        waiting_entries: &[&bsmr_data::CriticalPathEntry2],
        main_entry: &bsmr_data::CriticalPathEntry2,
        target_display_options: TargetDisplayOptions,
        track: u64,
    ) -> bsmr_error::Result<()> {
    ...
    fn write_critical_path_child_entry(
        &mut self,
        entry: &bsmr_data::CriticalPathEntry2,
        target_display_options: TargetDisplayOptions,
    ...
    fn handle_event_end(
        ...
        match end.data.as_ref() {
            Some(bsmr_data::span_end_event::Data::Materialization(materialization)) => {
...
impl BuckSubcommand for ChromeTraceCommand {
    ...
    async fn exec_impl(
        ...
        if !instant_remainder.is_empty() {
            return ExitResult::err(bsmr_error::internal_error!(
...
impl ChromeTraceCommand {
    fn parse_marker_arg(time: &str, name: &str) -> bsmr_error::Result<ChromeTraceInstant> {
        // Look for integer or float times. Note that we don't parse f64 in order to maintain the tv_nsec values
        let datetime = bsmr_event_log::utils::timestamp::parse(time)?;
    ...
    ) -> bsmr_error::Result<ChromeTraceWriter> {
