...
impl TransitiveValidationKey {
    ...
    async fn validate_current_node(
        ...
        let artifacts = info
            ...
            .collect::<bsmr_error::Result<Vec<Artifact>>>()?;
...
impl Key for TransitiveValidationKey {
    type Value = bsmr_error::Result<CachedValidationResult>;
...
enum TreatValidationFailureAsError {
    ...
    Transient(bsmr_error::Error),
}

impl From<bsmr_error::Error> for TreatValidationFailureAsError {
...
impl From<DiceError> for TreatValidationFailureAsError {
    fn from(value: DiceError) -> Self {
        TreatValidationFailureAsError::Transient(bsmr_error::Error::from(value))
    }
}
...
fn tighten_cached_validation_result(
    result: bsmr_error::Result<CachedValidationResult>,
) -> Result<(), TreatValidationFailureAsError> {
    match result {
