...
use bsmr_cli_proto::BuildTarget;
...
use bsmr_client_ctx::common::build::CommonOutputOptions;
...
pub trait MaterializationsToProto {
    fn to_proto(&self) -> bsmr_cli_proto::build_request::Materializations;
}
impl MaterializationsToProto for Option<FinalArtifactMaterializations> {
    fn to_proto(&self) -> bsmr_cli_proto::build_request::Materializations {
        match self {
            Some(FinalArtifactMaterializations::All) => {
                bsmr_cli_proto::build_request::Materializations::Materialize
            }
            Some(FinalArtifactMaterializations::None) => {
                bsmr_cli_proto::build_request::Materializations::Skip
            }
            None => bsmr_cli_proto::build_request::Materializations::Default,
        }
    }
}
...
pub trait UploadsToProto {
    fn to_proto(&self) -> bsmr_cli_proto::build_request::Uploads;
}
impl UploadsToProto for Option<FinalArtifactUploads> {
    fn to_proto(&self) -> bsmr_cli_proto::build_request::Uploads {
        match self {
            Some(FinalArtifactUploads::Always) => bsmr_cli_proto::build_request::Uploads::Always,
            Some(FinalArtifactUploads::Never) => bsmr_cli_proto::build_request::Uploads::Never,
            None => bsmr_cli_proto::build_request::Uploads::Never,
        }
    }
}
...
impl StreamingCommand for BuildCommand {
    ...
    async fn exec_impl(
        ...
        if bsmr_env!("BSMR_TEST_BUILD_ERROR", bool, applicability = testing)? {
            return bsmr_error!(
                bsmr_error::ErrorTag::TestOnly,
...
pub(crate) fn print_build_failed(console: &FinalConsole) -> bsmr_error::Result<()> {
...
fn print_build_rating(
    ...
    let url = format!("https://www.internalfb.com/bsmr/{}?rbs", ctx.trace_id);
...
mod tests {
    ...
    fn parse(args: &[&str]) -> bsmr_error::Result<BuildCommand> {
    ...
    #[test]
    fn infos_default() -> bsmr_error::Result<()> {
        let opts = parse(&[])?;
    ...
    #[test]
    fn infos_configure() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn infos_validation() -> bsmr_error::Result<()> {
        // Test duplicate args
        assert_matches!(
    ...
    #[test]
    fn rating_sample_works_with_real_trace_id() -> bsmr_error::Result<()> {
        use std::str::FromStr;
