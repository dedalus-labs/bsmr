...
/// Open a subscription channel to the Bessemer daemon. This allows you to interact with the Bessemer
...
/// `bsmr_subscription_proto/subscription.proto`. The client writes `SubscriptionRequest` and
...
#[clap(about = "Subscribe to updates from the Bessemer daemon")]
    ...
    async fn exec_impl(
        ...
        let stream = FramedRead::new(ctx.stdin(), ProtobufSplitter)
            ...
            .map(|res| match res {
                ...
                Err(e) => {
                    ...
                    let _ignored = bsmr_client_ctx::eprintln!("{}", reason);
                    SubscriptionRequest {
                        request: Some(
                            bsmr_subscription_proto::Disconnect { reason, ok: false }.into(),
                        ),
                    }
                }
            });
        ...
        let stream = if self.active_commands {
            futures::stream::once(futures::future::ready(SubscriptionRequest {
                request: Some(bsmr_subscription_proto::SubscribeToActiveCommands {}.into()),
            }))
            .chain(stream)
            .left_stream()
        } else {
        ...
        let stream = stream.map(|request| bsmr_cli_proto::SubscriptionRequestWrapper {
...

        {
            ...
            reborrow_stream_for_static(
                ...
                || {
                    Some(bsmr_cli_proto::SubscriptionRequestWrapper {
                        request: Some(SubscriptionRequest {
                            request: Some(
                                bsmr_subscription_proto::Disconnect {
        ...
        if partial_result_handler.ok {
            ...
            bsmr_error::bsmr_error!(bsmr_error::ErrorTag::Tier0, "Subscribe command failed").into()
        }
    }
...
impl PartialResultHandler for SubscriptionPartialResultHandler {
    type PartialResult = bsmr_cli_proto::SubscriptionResponseWrapper;
    ...
    async fn handle_partial_result(
        ...
        if let Some(bsmr_subscription_proto::subscription_response::Response::Goodbye(goodbye)) =
            &response.response
        {
