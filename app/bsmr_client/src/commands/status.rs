...
#[clap(about = "Show Bessemer daemon status")]
    ...
    pub fn exec(
        ...
        ctx.with_runtime(|ctx| async move {
            ...
            if self.all {
                ...
                for entry in walker {
                    let entry =
                        entry.map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::Tier0))?;
...
fn timestamp_to_string(seconds: u64, nanos: u32) -> bsmr_error::Result<String> {
...
pub(crate) fn process_status(status: StatusResponse) -> bsmr_error::Result<serde_json::Value> {
