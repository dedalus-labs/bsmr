...
/// Confirms native setup owns the generated IR while preserving the legacy init migration.
#[test]
fn writes_owned_cross_host_toolchain_configuration() {
    ...
    fs::write(
        root.path().join("toolchains/BUCK"),
        crate::commands::go_toolchain::LEGACY_TOOLCHAINS_BUCK,
    )
    .expect("legacy toolchains");
    ...
    let buck = fs::read_to_string(root.path().join("toolchains/BUCK")).expect("BUCK");
    let definition = fs::read_to_string(root.path().join("toolchains/bsmr_go_toolchain.bzl"))
        .expect("toolchain definition");
    assert!(buck.contains("system_demo_toolchains(include_go = False)"));
    assert!(buck.contains("bsmr_go_toolchains()"));
...
fn refuses_user_owned_toolchain_configuration() {
    let root = tempfile::tempdir().expect("temporary repository");
    fs::create_dir(root.path().join("toolchains")).expect("toolchains directory");
    fs::write(root.path().join("toolchains/BUCK"), "custom_toolchain()\n")
