...
use bsmr_client_ctx::query_args::CommonAttributeArgs;
...
impl Compression {
    fn to_proto(&self) -> bsmr_cli_proto::targets_request::Compression {
        match self {
            Compression::None => bsmr_cli_proto::targets_request::Compression::Uncompressed,
            Compression::Gzip => bsmr_cli_proto::targets_request::Compression::Gzip,
            Compression::Zstd => bsmr_cli_proto::targets_request::Compression::Zstd,
        }
    }
}
...
impl TargetsCommand {
    ...
    fn output_format(&self) -> bsmr_error::Result<OutputFormat> {
        if self.json {
    ...
    fn package_values_as_regexes(&self) -> bsmr_error::Result<Vec<String>> {
...
impl StreamingCommand for TargetsCommand {
    ...
    async fn exec_impl(
        ...
        let target_hash_use_fast_hash = match self.target_hash_function {
            TargetHashFunction::Sha1 | TargetHashFunction::Sha256 => {
                bsmr_client_ctx::eprintln!(
                    "bsmr only supports \"fast\" and \"strong\" target hash functions. Using the \"strong\" hash."
                )?;
                false
            }
            TargetHashFunction::Murmur_Hash3 => {
                bsmr_client_ctx::eprintln!(
                    "bsmr only supports \"fast\" and \"strong\" target hash functions. Using the \"fast\" hash."
                )?;
                true
            }
        ...
        let target_request = TargetsRequest {
            ...
            concurrency: self
                .num_threads
                .map(|num| bsmr_cli_proto::Concurrency { concurrency: num }),
...
async fn targets_show_outputs(
    ...
    bsmr_client_ctx::stdio::print_with_writer::<ClientIoError, _>(async move |w| {
...
async fn targets(
    ...
    if !response.serialized_targets_output.is_empty() {
        bsmr_client_ctx::print!("{}", response.serialized_targets_output)?;
    }
    ExitResult::success()
}
