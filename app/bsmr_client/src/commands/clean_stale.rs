...
) -> bsmr_error::Result<Option<KeepSinceArg>> {
    let arg = match (stale, keep_since_time) {
        (Some(Some(human_duration)), None) => {
            let duration = chrono::Duration::from_std(human_duration.into())
                .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::InvalidDuration))?;
...
fn format_result_stats(stats: bsmr_data::CleanStaleStats) -> String {
...
impl StreamingCommand for CleanStaleCommand {
    ...
    async fn exec_impl(
        ...
        if let Some(message) = response.message {
            bsmr_client_ctx::eprintln!("{}", message)?;
        }
        if let Some(stats) = response.stats {
            bsmr_client_ctx::eprintln!("{}", format_result_stats(stats))?;
        }
        ExitResult::success()
    }
