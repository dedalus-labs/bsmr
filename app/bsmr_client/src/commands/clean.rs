...
async fn clean(
    ...
    let paths_to_clean = if background {
        ...
        console.print_stderr("You can run other bsmr commands while this completes.")?;
...
fn clean_buck_out(path: &AbsNormPathBuf, console_type: ConsoleType) -> bsmr_error::Result<()> {
    ...
    // Use the same console_builder() as other bsmr commands to ensure consistent behavior.
