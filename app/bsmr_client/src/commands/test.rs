...
use bsmr_cli_proto::CounterWithExamples;
...
use bsmr_client_ctx::stdio::eprint_line;
...
#[bsmr(tag = TestExecutor)]
enum ExecutorError {
    #[bsmr(tag = TestRunnerInternal)]
    ...
    #[bsmr(tag = TestRunnerUnknownExitCode)]
...
impl ExecutorError {
    fn new(
        exit_code: i32,
        test_statuses: &bsmr_cli_proto::test_response::TestStatuses,
    ) -> Option<Self> {
...
impl TestStatusError {
    fn new(test_statuses: &bsmr_cli_proto::test_response::TestStatuses) -> Self {
...
fn test_executor_error(
    ...
    if let Some(error) = ExecutorError::new(executor_exit_code, test_statuses) {
        ...
        let mut error = bsmr_error::Error::from(error);
...
impl StreamingCommand for TestCommand {
    ...
    async fn exec_impl(
            ...
            should_warn_about_flag_position(&self.patterns, &self.include, &self.exclude)
        {
            ...
            if !suspicious_labels.is_empty() {
                writeln!(
                    ...
                    hint: For example: bsmr test //foo --include mylabel",
        ...
        if let Some(build_report) = response.serialized_build_report {
            bsmr_client_ctx::println!("{}", build_report)?;
        }
