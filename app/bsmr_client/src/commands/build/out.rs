...
use bsmr_cli_proto::build_target::BuildOutput;
        ...
        let single_default_output = match default_outputs.len() {
            0 => {
                return Err(bsmr_error!(
                    bsmr_error::ErrorTag::CopyOutputs,
...
async fn copy_directory<P, Q>(src: P, dst: Q, context: &CopyContext) -> bsmr_error::Result<()>
where
...
async fn copy_file(src: &Path, dst: &Path) -> bsmr_error::Result<()> {
...
fn convert_broken_pipe_error(e: io::Error) -> bsmr_error::Error {
    bsmr_error::Error::from(ClientIoError::from(e)).context("Error writing build artifact to --out")
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_copy_directory() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_copy_directory_with_symlink_pointing_externally() -> bsmr_error::Result<()> {
        let src_dir = tempfile::tempdir()?;
    ...
    #[tokio::test]
    async fn test_copy_symlink() -> bsmr_error::Result<()> {
    ...
    fn test_copy_symlink_parametrized(
        prepare_dst_dir: &dyn Fn(&TempDir) -> bsmr_error::Result<()>,
    ...
    mod unix {
        ...
        #[tokio::test]
        async fn test_copy_file() -> bsmr_error::Result<()> {
            let dir = tempfile::tempdir()?;
            let out = dir.path().join("sleep");

            let res = bsmr_util::process::async_background_command("cp")
            ...
            let mut proc = bsmr_util::process::async_background_command(&out)
                .arg("10000")
                .kill_on_drop(true)
                .spawn()
