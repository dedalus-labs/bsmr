...
impl ConfiguredAttr {
    ...
    fn concat_not_supported(&self, attr_ty: &'static str) -> bsmr_error::Error {
        bsmr_error!(
    ...
    fn unpack_oneof(self) -> bsmr_error::Result<(Self, u32)> {
        match self {
    ...
    fn unpack_oneof_i(self, expected_i: u32, oneof: &OneOfAttrType) -> bsmr_error::Result<Self> {
    ...
    fn concat_oneof(
        self,
        items: &mut dyn Iterator<Item = bsmr_error::Result<Self>>,
        oneof: &OneOfAttrType,
    ...
    pub(crate) fn try_into_configuration_dep(self) -> bsmr_error::Result<ProvidersLabel> {
        match self {
    ...
    pub(crate) fn try_into_list(self) -> bsmr_error::Result<Vec<ConfiguredAttr>> {
        match self {
