...
use bsmr_common::file_ops::trait_::DiceFileOps;
use bsmr_common::pattern::package_roots::collect_package_roots;
...
async fn resolve_patterns_and_load_buildfiles<'c, T: PatternType>(
    ...
    ResolvedPattern<T>,
    impl Stream<Item = (PackageLabel, bsmr_error::Result<Arc<EvaluationResult>>)> + use<'c, T>,
)> {
...
pub struct LoadedPatterns<T: PatternType> {
    results: BTreeMap<PackageLabelWithModifiers, bsmr_error::Result<PackageLoadedPatterns<T>>>,
}
...
impl<T: PatternType> LoadedPatterns<T> {
    pub fn iter(
        ...
        Item = (
            ...
            &bsmr_error::Result<PackageLoadedPatterns<T>>,
        ),
    > {
    ...
    pub fn iter_loaded_targets(
        &self,
    ) -> impl Iterator<Item = bsmr_error::Result<TargetNodeRef<'_>>> {
        self.results
            .values()
    ...
    pub fn iter_loaded_targets_by_package(
        ...
        Item = (
            PackageLabelWithModifiers,
            bsmr_error::Result<Vec<TargetNode>>,
        ),
    > + '_ {
...
fn apply_spec<T: PatternType>(
    ...
    load_results: BTreeMap<PackageLabel, bsmr_error::Result<Arc<EvaluationResult>>>,
    skip_missing_targets: MissingTargetBehavior,
) -> bsmr_error::Result<LoadedPatterns<T>> {
    let mut all_targets: BTreeMap<_, bsmr_error::Result<PackageLoadedPatterns<T>>> =
        BTreeMap::new();
...
pub async fn load_patterns<T: PatternType>(
    ...
    ctx.with_linear_recompute(|ctx| async move {
        ...
        let mut results: BTreeMap<PackageLabel, bsmr_error::Result<Arc<EvaluationResult>>> =
            BTreeMap::new();
