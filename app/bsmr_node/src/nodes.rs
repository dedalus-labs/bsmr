...
/// Attributes on target nodes that are generated by bsmr, not provided by users.
...
pub mod attributes {
    ...
    pub const DEPS: &'static str = "bsmr.deps";

    /// The oncall for this node.
    pub const ONCALL: &'static str = "bsmr.oncall";

    /// The package or the buildfile path.
    /// The package in `targets` command, but the buildfile path in `*query` commands.
    pub const PACKAGE: &'static str = "bsmr.package";

    /// A string representation of the target's rule type.
    pub const TYPE: &'static str = "bsmr.type";

    /// The target hash of this target.
    pub const TARGET_HASH: &'static str = "bsmr.target_hash";

    /// The callstack for this target.
    pub const TARGET_CALL_STACK: &'static str = "bsmr.target_call_stack";

    /// The configuration deps, deps that appear as conditions in selects.
    pub const CONFIGURATION_DEPS: &'static str = "bsmr.configuration_deps";

    /// The resolved execution platform for this node.
    pub const EXECUTION_PLATFORM: &'static str = "bsmr.execution_platform";

    /// The resolved target configuration for this node.
    pub const TARGET_CONFIGURATION: &'static str = "bsmr.target_configuration";

    /// The input source files/directories that this node uses.
    pub const INPUTS: &'static str = "bsmr.inputs";

    /// The package values for the package this target belongs to.
    pub const PACKAGE_VALUES: &'static str = "bsmr.package_values";

    /// The plugin lists on the node. This includes all plugins, regardless of whether they're
    /// propagated or actually used.
    pub const PLUGINS: &'static str = "bsmr.plugins";

    /// The package values for the package this target belongs to.
    pub const PACKAGE_CFG_MODIFIERS: &'static str = "bsmr.tree_modifiers";
}
