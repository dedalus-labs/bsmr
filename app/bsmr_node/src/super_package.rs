...
impl SuperPackageData {
    pub fn new(
        ...
        cfg_constructor: Option<Arc<dyn CfgConstructorImpl>>,
        test_config_unification_rollout: bool,
    ) -> bsmr_error::Result<SuperPackageData> {
        let modifier_key =
            MetadataKeyRef::new(MODIFIER_METADATA_KEY).map_err(bsmr_error::Error::from)?;
...
impl SuperPackage {
    ...
    pub fn empty<T: SuperPackageValues + Default>() -> bsmr_error::Result<SuperPackage> {
        SuperPackage::new(
            Arc::new(T::default()),
