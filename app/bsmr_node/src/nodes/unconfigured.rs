...
impl TargetNode {
    ...
    ///
    /// "special" attributes are generated by bsmr, not provided by users.
    ...
    pub fn visibility(&self) -> bsmr_error::Result<&VisibilitySpecification> {
    ...
    pub fn is_visible_to(&self, target: &TargetLabel) -> bsmr_error::Result<bool> {
    ...
    pub fn attr<'a>(
        ...
        key: &str,
        opts: AttrInspectOptions,
    ) -> bsmr_error::Result<Option<CoercedAttrFull<'a>>> {
    ...
    pub fn tests(&self) -> impl Iterator<Item = &ProvidersLabel> {
        ...
        impl<'a> CoercedAttrTraversal<'a> for TestCollector<'a> {
            ...
            fn dep(&mut self, _dep: &ProvidersLabel) -> bsmr_error::Result<()> {
                Ok(())
            }

            fn label(&mut self, label: &'a ProvidersLabel) -> bsmr_error::Result<()> {
    ...
    #[inline]
    pub fn metadata(&self) -> bsmr_error::Result<Option<&MetadataMap>> {
...
impl<'a> TargetNodeRef<'a> {
    ...
    pub fn metadata(self) -> bsmr_error::Result<Option<&'a MetadataMap>> {
    ...
    pub fn target_modifiers(self) -> bsmr_error::Result<Option<&'a TargetModifiersValue>> {
        ...
        let map: Map<String, Value> = target
            ...
            .map(|(target_name, values)| {
                let mut json_values: Map<String, Value> = values
                    ...
                    .collect::<bsmr_error::Result<Map<String, Value>>>()?;
                json_values.insert(
                    "__type__".to_owned(),
