...
impl TargetNodeOrForward {
    ...
    fn is_visible_to(&self, target: &TargetLabel) -> bsmr_error::Result<bool> {
        match self {
...
impl ConfiguredTargetNode {
    ...
    pub fn new_forward(
        ...
        Ok(ConfiguredTargetNode(triomphe::Arc::new(Hashed::new(
            ConfiguredTargetNodeData {
                ...
                // so we can call `bsmr test` on Forward.
    ...
    pub fn attr_as_target_compatible_with(
        attr: ConfiguredAttr,
    ) -> impl Iterator<Item = bsmr_error::Result<ConfigurationSettingKey>> {
    ...
    pub fn tests(&self) -> impl Iterator<Item = ConfiguredProvidersLabel> + use<> {
        ...
        impl ConfiguredAttrTraversal for TestCollector {
            ...
            fn label(&mut self, label: &ConfiguredProvidersLabel) -> bsmr_error::Result<()> {
    ...
    #[inline]
    pub fn execution_platform(&self) -> bsmr_error::Result<&ExecutionPlatform> {
