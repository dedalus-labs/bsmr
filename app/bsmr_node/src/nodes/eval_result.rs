...
use bsmr_common::starlark_profiler::StarlarkProfileDataAndStatsDyn;
...
impl Display for AllTargetsDisplay {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if self.total_count > self.targets.len() {
            writeln!(
                f,
                "\n\nTo see all {} targets, run:\n  bsmr uquery --reuse-current-config {}:\n",
...
impl EvaluationResult {
    ...
    pub fn resolve_target<'a>(
        &'a self,
        path: &TargetNameRef,
    ) -> bsmr_error::Result<TargetNodeRef<'a>> {
