...
use bsmr_core::pattern::package::PackagePattern;
...
use bsmr_core::target::name_glob::TargetNameGlob;
use bsmr_error::internal_error;
use bsmr_util::arc_str::ThinArcSlice;
use dupe::Dupe;
use pagable::Pagable;
use serde::Serialize;
...
pub enum VisibilityError {
    #[error(
        "`{0}` is not visible to `{1}` (run `bsmr uquery --output-attribute visibility {0}` to check the visibility)"
    )]
    #[bsmr(input, tag = Visibility)]
...
impl StarlarkTargetNameGlob {
    ...
    pub fn try_new(name_globs: Vec<String>, within: Vec<String>) -> bsmr_error::Result<Self> {
    ...
    pub fn coerce(
        &self,
        parse_within: impl Fn(&str) -> bsmr_error::Result<ParsedPattern<TargetPatternExtra>>,
    ) -> bsmr_error::Result<TargetNameGlobRecord> {
        let within = self
            .within
            .iter()
            .map(|within_str| {
            ...
            .collect::<bsmr_error::Result<ThinArcSlice<PackagePattern>>>()?;
...
impl TargetNameGlobRecord {
    ...
    pub fn matches(&self, target: &TargetLabel) -> bsmr_error::Result<bool> {
        let within_matches =
...
impl VisibilityPatternList {
    ...
    ) -> bsmr_error::Result<VisibilityPatternList> {
        match (self, other) {
    ...
    pub fn matches_target(&self, target: &TargetLabel) -> bsmr_error::Result<bool> {
        match self {
...
impl AnyMatches for VisibilityPatternList {
    fn any_matches(
        &self,
        filter: &dyn Fn(&str) -> bsmr_error::Result<bool>,
...
impl VisibilitySpecification {
    ...
    ) -> bsmr_error::Result<VisibilitySpecification> {
...
impl WithinViewSpecification {
    ...
    ) -> bsmr_error::Result<WithinViewSpecification> {
