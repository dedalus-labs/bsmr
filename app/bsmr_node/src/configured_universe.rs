...
use bsmr_query::query::syntax::simple::eval::label_indexed::LabelIndexed;
use bsmr_query::query::syntax::simple::eval::set::TargetSet;
use bsmr_util::late_binding::LateBinding;
use bsmr_util::self_ref::RefData;
use bsmr_util::self_ref::SelfRef;
...
pub static UNIVERSE_FROM_LITERALS: LateBinding<
    for<'c> fn(
        ...
        -> Pin<Box<dyn Future<Output = bsmr_error::Result<CqueryUniverse>> + Send + 'c>>,
...
impl<'a> CqueryUniverseInner<'a> {
    ...
    ) -> bsmr_error::Result<CqueryUniverseInner<'a>> {
...
impl CqueryUniverse {
    ...
    pub fn build(universe: &TargetSet<ConfiguredTargetNode>) -> bsmr_error::Result<CqueryUniverse> {
        span(bsmr_data::CqueryUniverseBuildStart {}, || {
            let r = SelfRef::try_new(universe.clone(), |universe| {
                CqueryUniverseInner::build_inner(universe)
            })
            .map(|data| CqueryUniverse { data });
            (r, bsmr_data::CqueryUniverseBuildEnd {})
        })
    }
    ...
    pub fn owners(&self, path: &CellPath) -> bsmr_error::Result<Vec<ConfiguredTargetNode>> {
...
mod tests {
    use bsmr_common::pattern::resolve::ResolvedPattern;
    use bsmr_core::configuration::bound_label::BoundConfigurationLabel;
    use bsmr_core::configuration::data::ConfigurationData;
    use bsmr_core::configuration::hash::ConfigurationHash;
    ...
    use bsmr_core::pattern::pattern::PackageSpec;
    use bsmr_core::pattern::pattern_type::ConfigurationPredicate;
    ...
    async fn test_get_from_package_by_configured_provider_pattern() {
        fn providers_name() -> ProvidersName {
            ProvidersName::NonDefault(triomphe::Arc::new(NonDefaultProvidersName::Named(
                bsmr_util::arc_str::ArcSlice::new([ProviderName::new("P".to_owned()).unwrap()]),
            )))
        }
