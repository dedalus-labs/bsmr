...
use bsmr_common::init::SystemWarningConfig;
...
use bsmr_execute_impl::materializers::deferred::TtlRefreshConfiguration;
use bsmr_execute_impl::materializers::deferred::clean_stale::CleanStaleConfig;
use bsmr_execute_impl::re::paranoid_download::ParanoidDownloader;
...
pub struct DaemonStateData {
    ...
    /// What bsmr state to store on disk, ex. materializer state on sqlite
...
impl DaemonStateData {
    pub fn dice_dump(&self, path: &Path, format: DiceDumpFormat) -> bsmr_error::Result<()> {
...
impl DaemonState {
    ...
    ) -> bsmr_error::Result<Arc<dyn Materializer>> {
    ...
    pub async fn prepare_events(
        &self,
        trace_id: TraceId,
    ) -> bsmr_error::Result<(ChannelEventSource, EventDispatcher)> {
        let (events, sink) = bsmr_events::create_source_sink_pair();
    ...
    pub async fn prepare_command(
    ...
    ) -> bsmr_error::Result<BaseServerCommandContext> {
        let data = self.data();

        dispatcher.instant_event(bsmr_data::RestartConfiguration {
        ...
        dispatcher.instant_event(bsmr_data::IoProviderInfo { eden_version });
    ...
    pub fn validate_cwd(&self) -> bsmr_error::Result<()> {
...
fn convert_algorithm_kind(kind: DigestAlgorithmFamily) -> bsmr_error::Result<DigestAlgorithm> {
    bsmr_error::Ok(match kind {
...
mod tests {
    ...
    use bsmr_common::legacy_configs::configs::testing::parse;
    use indoc::indoc;

    use super::*;

    #[tokio::test]
    async fn test_from_startup_config_defaults_internal() -> bsmr_error::Result<()> {
        ...
        assert_eq!(
            ...
            bsmr_certs::certs::supports_vpnless()
        );
        assert_eq!(
    ...
    #[tokio::test]
    async fn test_from_startup_config_overrides() -> bsmr_error::Result<()> {
        bsmr_certs::certs::maybe_setup_cryptography();
        let config = parse(
            &[(
                "config",
    ...
    #[tokio::test]
    async fn test_from_startup_config_zero_for_unset() -> bsmr_error::Result<()> {
        bsmr_certs::certs::maybe_setup_cryptography();
        let config = parse(
            &[(
                "config",
