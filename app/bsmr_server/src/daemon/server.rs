...
use bsmr_core::error::reload_hard_error_config;
use bsmr_core::error::reload_show_soft_error_config;
use bsmr_core::error::reset_soft_error_counters;
...
use bsmr_server_ctx::late_bindings::QUERY_SERVER_COMMANDS;
use bsmr_server_ctx::late_bindings::STARLARK_SERVER_COMMAND;
use bsmr_server_ctx::late_bindings::TARGETS_SERVER_COMMANDS;
...
use bsmr_server_starlark_debug::run::run_dap_server_command;
use bsmr_test::executor_launcher::get_all_test_executors;
use bsmr_util::system_stats::num_cores;
use bsmr_util::system_stats::system_memory_stats;
...
impl DaemonShutdown {
    ...
    fn start_shutdown(&self, reason: bsmr_data::DaemonShutdown, timeout: Option<Duration>) {
...
impl BuckdServer {
    ...
    pub async fn run(
        ...
        #[cfg(fbcode_build)]
        {
            ...
            if !bsmr_env!("BSMR_DISABLE_EDEN_HEALTH_CHECK", bool)?
        ...
        if let Some(sleep_secs) = bsmr_env!(
            "BSMR_TEST_INIT_DATA_SLEEP_SECS",
            type = u64,
    ...
    where
        F: for<'a> FnOnce(
            ...
            ) -> BoxFuture<'a, bsmr_error::Result<Res>>
            + Send
            + 'static,
        Req: TryFrom<StreamingRequest, Error = bsmr_error::Error> + Send + Sync + 'static,
        ...
        PartialRes: Into<partial_result::PartialResult> + Send + 'static,
    {
        ...
        let init_request = match req.message().await? {
            Some(
                m @ StreamingRequest {
                    request: Some(bsmr_cli_proto::streaming_request::Request::Context(_)),
                },
            ) => Ok(m),
    ...
    async fn run_streaming_fallible<Req, Res, PartialRes, F>(
        ...
        func: F,
    ) -> bsmr_error::Result<Response<ResponseStream>>
    where
        F: for<'a> FnOnce(
                &'a ServerCommandContext,
                PartialResultDispatcher<PartialRes>,
        ...
        PartialRes: Into<partial_result::PartialResult> + Send + 'static,
    {
        ...
        if bsmr_env!("BSMR_TEST_FAIL_STREAMING", bool, applicability = testing).unwrap() {
        ...
        reload_hard_error_config(&client_ctx.bsmr_hard_error)?;
        reload_show_soft_error_config(&client_ctx.bsmr_show_soft_errors);
        ...
        dispatch.instant_event(bsmr_data::SystemInfo {
        ...
        let memory_reporter = daemon_state.data.memory_tracker.as_ref().map(|t| {
            bsmr_resource_control::memory_tracker::spawn_memory_reporter(dispatch.dupe(), t.dupe())
        });
        ...
        let resp = streaming(
            ...
            move |req, cancellations| {
                async move {
                    let result: bsmr_error::Result<Res> = try {
                        let base_context =
    ...
    async fn oneshot<
        ...
        Fut: Future<Output = bsmr_error::Result<Res>> + Send,
        F: FnOnce(Req) -> Fut,
    >(
        &self,
        req: Request<Req>,
...
fn error_to_command_result(e: bsmr_error::Error) -> CommandResult {
    CommandResult {
        result: Some(command_result::Result::Error(bsmr_data::ErrorReport::from(
            &e,
        ))),
    }
}
...
fn error_to_command_progress(e: bsmr_error::Error) -> CommandProgress {
    CommandProgress {
...
fn error_to_response_stream(e: bsmr_error::Error) -> Response<ResponseStream> {
    tonic::Response::new(Box::pin(stream::once(future::ready(Ok(
        bsmr_cli_proto::MultiCommandProgress {
...
fn pump_events(
    ...
    output_send: tokio::sync::mpsc::UnboundedSender<
        Result<bsmr_cli_proto::CommandProgress, tonic::Status>,
    >,
) {
...
fn streaming<Req, F>(
    ...
    daemon_shutdown_channel: oneshot::Receiver<bsmr_data::DaemonShutdown>,
    func: F,
    rt: &Handle,
    ...
    F: for<'a> FnOnce(Req, &'a CancellationContext) -> BoxFuture<'a, ()> + Send + 'static,
{
    ...
    if let Err(e) = merge_task {
        return error_to_response_stream(
            bsmr_error::Error::from(e).context("failed to spawn pump-events"),
        );
    };
...
impl<Req> StreamingCommandOptions<Req> for QueryCommandOptions {
    fn starlark_profiler_instrumentation_override(
        ...
        match self.profile_mode {
            ...
            Some(mode) => {
                let mode = bsmr_cli_proto::ProfileMode::try_from(mode)
...
impl DaemonApi for BuckdServer {
    async fn kill(&self, req: Request<KillRequest>) -> Result<Response<CommandResult>, Status> {
        ...
        self.oneshot(req, KillRunCommandOptions, move |req| async move {
            ...
            let reason = bsmr_data::DaemonShutdown {
                reason: req.reason,
                callers: req.callers,
            };
    ...
    async fn status(&self, req: Request<StatusRequest>) -> Result<Response<CommandResult>, Status> {
        ...
        self.oneshot(req, DefaultCommandOptions, move |req| async move {
            ...
            let extra_constraints = bsmr_cli_proto::ExtraDaemonConstraints {
    ...
    async fn flush_dep_files(
        ...
        self.oneshot(req, DefaultCommandOptions, move |req| async move {
            ...
            if retain_locally_produced_dep_files {
                bsmr_file_watcher::dep_files::flush_non_local_dep_files();
            } else {
                bsmr_file_watcher::dep_files::flush_dep_files();
            }
            Ok(GenericResponse {})
        })
        .await
    ...
    async fn unstable_flush_pgo_profile(
        ...
        self.check_if_accepting_requests()?;
        let pgo_active = bsmr_util::pgo::flush_pgo_profile();
            ...
            |ctx, partial_result_dispatcher, req| {
                Box::pin(async {
                    match req.profile_opts.as_ref().expect("Missing profile opts") {
                        bsmr_cli_proto::profile_request::ProfileOpts::TargetProfile(_) => {
                            profile_command(ctx, partial_result_dispatcher, req).await
                        }
                        bsmr_cli_proto::profile_request::ProfileOpts::BxlProfile(_) => {
...
mod eden_health {
    ...
    use bsmr_eden::error::ErrorFromHangingMount;
    ...
    pub(crate) async fn edenfs_health_check(fb: fbinit::FacebookInit, root: ProjectRoot) {
        tokio::task::spawn(async move {
            ...
            loop {
                ...
                match EdenConnectionManager::new(fb, &root, Some(semaphore::bsmr_default())) {
                    Ok(Some(conn)) => {
                        let info = conn
                            .with_eden(|eden| {
                        ...
                        match info {
                            Err(e) if e.is_caused_by_hanging_mount() => {
                                ...
                                soft_error!(
                                    ...
                                    bsmr_error!(bsmr_error::ErrorTag::Input, "check failed with: {:#}", e),
                                    quiet: true
                                )
                                .ok();
                            }
                    ...
                    Err(e) => {
                        ...
                        soft_error!(
                            ...
                            bsmr_error!(bsmr_error::ErrorTag::Input, "client creation failed with: {:#}", e),
                            quiet: true
                        )
                        .ok();
                    }
