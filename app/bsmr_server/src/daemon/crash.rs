...
pub(crate) async fn crash(req: UnstableCrashRequest) -> bsmr_error::Result<GenericResponse> {
    let crash_type = CrashType::try_from(req.crash_type).map_err(|_| {
        bsmr_error::bsmr_error!(bsmr_error::ErrorTag::CrashRequested, "{}", "bad request")
    })?;
    match crash_type {
        CrashType::Panic => {
...
#[cfg(unix)]
fn query_page_size() -> bsmr_error::Result<usize> {
    bsmr_util::os::unix_like::sc_page_size::sc_page_size()
}

#[cfg(windows)]
fn query_page_size() -> bsmr_error::Result<usize> {
    bsmr_util::os::win::page_size::page_size()
}

async fn allocate_memory(bytes: u64) -> bsmr_error::Result<GenericResponse> {
