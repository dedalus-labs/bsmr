...
impl SnapshotCollector {
    ...
    pub async fn create_snapshot(&self) -> bsmr_data::Snapshot {
        let mut snapshot = bsmr_data::Snapshot::default();
    ...
    pub async fn create_snapshot_with_page_in_metrics(&self) -> bsmr_data::Snapshot {
        let mut snapshot = self.create_snapshot().await;
        self.add_dice_page_in_metrics(&mut snapshot);
        snapshot
    }

    fn add_daemon_metrics(&self, snapshot: &mut bsmr_data::Snapshot) {
        snapshot.blocking_executor_io_queue_size =
            self.daemon.blocking_executor.queue_size() as u64;
    }

    fn add_io_metrics(&self, snapshot: &mut bsmr_data::Snapshot) {
    ...
    fn add_re_metrics(&self, snapshot: &mut bsmr_data::Snapshot) {
        fn inner(
            snapshot: &mut bsmr_data::Snapshot,
    ...
    fn add_http_metrics(&self, snapshot: &mut bsmr_data::Snapshot) {
        snapshot.http_download_bytes = self.daemon.http_client.stats().get_downloaded_bytes();
    }

    fn add_dice_metrics(&self, snapshot: &mut bsmr_data::Snapshot) {
    ...
    fn add_dice_page_in_metrics(&self, snapshot: &mut bsmr_data::Snapshot) {
        snapshot.dice_page_in_by_key_type =
            compute_page_in_delta(&self.page_in_baseline, &page_in_proto_map(&self.daemon));
    }

    fn add_materializer_metrics(&self, snapshot: &mut bsmr_data::Snapshot) {
        self.daemon.materializer.add_snapshot_stats(snapshot);
    }

    fn add_net_io_metrics(&self, snapshot: &mut bsmr_data::Snapshot) {
        if let Ok(Some(net_io_counters_per_nic)) = self.net_io_collector.collect() {
            snapshot.network_interface_stats = net_io_counters_per_nic
                ...
                .map(|(nic, counters)| {
                    (
                        nic,
                        bsmr_data::NetworkInterfaceStats {
                            tx_bytes: counters.bytes_sent,
                            rx_bytes: counters.bytes_recv,
                            network_kind: match counters.network_kind {
                                NetworkKind::WiFi => bsmr_data::NetworkKind::WiFi.into(),
                                NetworkKind::Ethernet => bsmr_data::NetworkKind::Ethernet.into(),
                                NetworkKind::Unknown => {
                                    bsmr_data::NetworkKind::UnknownNetKind.into()
                                }
                            },
                        },
                    )
    ...
    fn add_system_metrics(&self, snapshot: &mut bsmr_data::Snapshot) {
        ...
        {
            snapshot.unix_system_stats = Some(bsmr_data::UnixSystemStats {
                load1,
                load5,
                load15,
            });
    ...
    fn add_tokio_runtime_stats(&self, snapshot: &mut bsmr_data::Snapshot) {
        let metrics = self.runtime.metrics();
        ...
        *prev_counters_guard = Some(curr_counters);
        ...
        let workers = (0..num_workers)
            .map(|i| {
                ...
                bsmr_data::tokio_runtime_stats::Worker {
        ...
        *prev_runtime_guard = Some(curr_runtime);
        ...
        snapshot.tokio_runtime_stats = Some(bsmr_data::TokioRuntimeStats {
    ...
    fn add_cpu_usage(&self, snapshot: &mut bsmr_data::Snapshot) {
    ...
    async fn add_memory_metrics(&self, snapshot: &mut bsmr_data::Snapshot) {
        #[cfg(not(unix))]
        {
        ...
        #[cfg(unix)]
        {
            use bsmr_resource_control::cgroup_files::MemoryStat;
            ...
            ) -> bsmr_data::UnixCgroupMemoryStats {
                bsmr_data::UnixCgroupMemoryStats {
                    anon: stats.anon,
                    file: stats.file,
                    kernel: stats.kernel,
            ...
            // Try to read Bessemer daemon memory information from cgroup
    ...
    daemon
        ...
        .map(|(&key_type, stats)| {
            (
                ...
                bsmr_data::DicePageInKeyTypeStats {
                    count: stats.count,
...
fn compute_page_in_delta(
    ...
    current
        ...
        .filter_map(|(key_type, c)| {
            ...
            let delta = bsmr_data::DicePageInKeyTypeStats {
...
mod tokio_unstable_smoke {
    ...
    //! `tokio_unstable` rustflag in `fbcode/bsmr/.cargo/config.toml` is
...
mod compute_page_in_delta_tests {
    ...
    fn delta_subtracts_baseline_and_drops_unchanged() {
        let stat = |count, fetch_us, deser_us, bytes| bsmr_data::DicePageInKeyTypeStats {
            count,
            fetch_us,
            deser_us,
            bytes,
