...
use bsmr_build_api::actions::impls::run_action_knobs::RunActionKnobs;
use bsmr_build_api::build::HasCreateUnhashedSymlinkLock;
use bsmr_build_api::build::detailed_aggregated_metrics::dice::HasDetailedAggregatedMetrics;
use bsmr_build_api::build::detailed_aggregated_metrics::dice::SetDetailedAggregatedMetricsEventsHolder;
use bsmr_build_api::build_signals::BuildSignalsInstaller;
use bsmr_build_api::build_signals::SetBuildSignals;
use bsmr_build_api::build_signals::create_build_signals;
...
use bsmr_build_signals::env::HasCriticalPathBackend;
...
use bsmr_directory::directory::dashmap_directory_interner::DashMapDirectoryInterner;
use bsmr_events::dispatch::EventDispatcher;
use bsmr_events::metadata;
use bsmr_events::schedule_type::SandcastleScheduleType;
use bsmr_execute::execute::blocking::SetBlockingExecutor;
use bsmr_execute::knobs::ExecutorGlobalKnobs;
use bsmr_execute::materialize::materializer::Materializer;
use bsmr_execute::materialize::materializer::SetMaterializer;
use bsmr_execute::re::client::RemoteExecutionClient;
...
use bsmr_interpreter::factory::SetProfileEventListener;
...
use bsmr_interpreter_for_build::interpreter::cycles::LoadCycleDescriptor;
use bsmr_interpreter_for_build::interpreter::interpreter_setup::setup_interpreter;
use bsmr_resource_control::HasResourceControl;
use bsmr_server_ctx::bxl::InitBxlStreamingTracker;
use bsmr_server_ctx::concurrency::DiceUpdater;
use bsmr_server_ctx::ctx::DiceAccessor;
use bsmr_server_ctx::ctx::LockedPreviousCommandData;
use bsmr_server_ctx::ctx::PrivateStruct;
...
use bsmr_util::truncate::truncate_container;
use bsmr_validation::enabled_optional_validations_key::SetEnabledOptionalValidations;
...
fn parse_concurrency(requested: u32) -> Option<usize> {
    let ret: usize = requested
        .try_into()
        .expect("Bessemer isn't built for 16 bit systems");
...
pub struct ServerCommandContext<'a> {
    ...
    pub(crate) agent_context: Vec<bsmr_data::AgentContextEntry>,
...
impl<'a> ServerCommandContext<'a> {
    pub fn new(
        ...
        let working_dir_project_relative = working_dir
            ...
            .map_err(|_| {
                Into::<bsmr_error::Error>::into(DaemonCommunicationError::InvalidWorkingDirectory(
        ...
        impl ReConnectionObserver for Observer {
            fn session_created(&self, client: &RemoteExecutionClient) {
                ...
                self.events
                    .instant_event(bsmr_data::RemoteExecutionSessionCreated {
    ...
    ) -> bsmr_error::Result<DiceCommandUpdater<'s, 'a>> {
    ...
    pub(crate) async fn finalize(mut self) -> bsmr_error::Result<()> {
...
impl DiceUpdater for DiceCommandUpdater<'_, '_> {
    ...
    ) -> bsmr_error::Result<(DiceTransactionUpdater, UserComputationData)> {
...
impl DiceCommandUpdater<'_, '_> {
    fn make_user_computation_data(
        ...
        if let Some(max_lines) = root_config.parse(BsmrconfigKeyRef {
            ...
            self.cmd_ctx
                .events()
                .instant_event(bsmr_data::ConsolePreferences { max_lines });
        }
        ...
        // They currently use either a usecase specified in actions (cas_artifact), or a global default (bsmr.default_remote_execution_use_case).
        ...
        self.cmd_ctx
            .events()
            .instant_event(bsmr_data::TagEvent { tags });

        self.cmd_ctx
            .events()
            .instant_event(bsmr_data::CommandOptions {
                configured_parallelism: concurrency as _,
                available_parallelism: bsmr_util::threads::available_parallelism() as _,
            });
...
fn collect_config_metadata_into(config: &LegacyBsmrConfig, data: &mut UserComputationData) {
    ...
    // ```
    //
    // TODO(swgillespie) - This only covers the normals since Bessemer's event protocol only allows for string
        ...
        soft_error!(
            ...
            bsmr_error::bsmr_error!(
                ...
                "Setting `client.id` via config (`-c|--config client.id={}`) is deprecated \
                 ...
                 This will become a hard error in a future Bessemer release. \
                 For more information, see: https://internalfb.com/intern/staticdocs/bsmr/docs/rule_authors/client_metadata/",
                client_id,
                client_id
            ),
            quiet: false,
...
impl ServerCommandContextTrait for ServerCommandContext<'_> {
    ...
    ) -> bsmr_error::Result<DiceAccessor<'s>> {
    ...
    fn stderr(&self) -> bsmr_error::Result<StderrOutputGuard<'_>> {
    ...
    async fn config_metadata(
        ...
        ctx.per_transaction_data()
            ...
            .map_err(|_| bsmr_error::internal_error!("Config metadata not set"))
    }
    ...
    fn log_target_pattern(
        ...
        let patterns = providers_patterns.map(|pat| bsmr_data::TargetPattern {
            value: format!("{pat}"),
        });

        self.events()
            .instant_event(bsmr_data::ParsedTargetPatterns {
    ...
    fn log_target_pattern_with_modifiers(
        ...
        let patterns = seen_values
            .into_iter()
            .map(|pat| bsmr_data::TargetPattern { value: pat })
            .collect();
