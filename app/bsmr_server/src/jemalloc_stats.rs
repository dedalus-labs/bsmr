...
use bsmr_common::memory::allocator_stats;

// TODO(raulgarcia4): Consider moving out this file to bsmr_common,
// similarly to what was done with bsmr_common::process_stats.
pub struct AllocatorStats {
    pub bytes_active: Option<u64>,
    pub bytes_allocated: Option<u64>,
}

pub fn get_allocator_stats() -> bsmr_error::Result<AllocatorStats> {
    fn set(stats: &serde_json::Value, val: &str, to: &mut Option<u64>) -> bsmr_error::Result<()> {
        if let serde_json::Value::Number(stat) = &stats["jemalloc"]["stats"][val] {
            *to = stat.as_u64();
            Ok(())
        } else {
            Err(bsmr_error::bsmr_error!(
                bsmr_error::ErrorTag::MallocStats,
...
#[cfg(test)]
mod tests {
    use bsmr_common::memory::has_jemalloc_stats;

    use crate::jemalloc_stats::get_allocator_stats;

    #[test]
    fn test_get_allocator_stats() -> bsmr_error::Result<()> {
