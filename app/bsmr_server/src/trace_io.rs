...
pub(crate) async fn trace_io_command(
    ...
    req: bsmr_cli_proto::TraceIoRequest,
) -> bsmr_error::Result<bsmr_cli_proto::TraceIoResponse> {
    let start_event = context
        .command_start_event(bsmr_data::TraceIoCommandStart {}.into())
        .await?;
    span_async(start_event, async move {
        ...
        let result = match (tracing_provider, respond_with_trace) {
            ...
            (Some(_), false) => Ok(bsmr_cli_proto::TraceIoResponse {
                enabled: true,
                trace: Vec::new(),
            ...
            (None, _) => Ok(bsmr_cli_proto::TraceIoResponse {
                enabled: false,
                trace: Vec::new(),
                external_entries: Vec::new(),
                relative_symlinks: Vec::new(),
        ...
        let end_event = command_end(&result, bsmr_data::TraceIoCommandEnd {});
        (result, end_event)
    })
    .await
}
...
async fn build_response_with_trace(
    ...
    Ok(bsmr_cli_proto::TraceIoResponse {
        enabled: true,
        trace,
        external_entries,
        relative_symlinks,
