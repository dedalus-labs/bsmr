...
pub(crate) async fn file_status_command(
    ...
    req: bsmr_cli_proto::FileStatusRequest,
...
impl ServerCommandTemplate for FileStatusServerCommand {
    type StartEvent = bsmr_data::FileStatusCommandStart;
    type EndEvent = bsmr_data::FileStatusCommandEnd;
    type Response = bsmr_cli_proto::GenericResponse;
    ...
    async fn command(
        ...
        if result.bad != 0 {
            ...
            Ok(bsmr_cli_proto::GenericResponse {})
        }
    }
}
    ...
    match (fs_metadata, dice_metadata) {
        ...
        (RawPathMetadata::Directory, RawPathMetadata::Directory) => {
            ...
            let mut fs_names = fs_read_dir
                .iter()
                .map(|f| bsmr_error::Ok(FileName::new(&f.file_name)?.to_owned()))
