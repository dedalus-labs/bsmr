...
/// Default bsmr isolation dir. Must match the `--isolation-dir` clap
/// `default_value` in `app/bsmr/src/lib.rs`; the default-isolation golden test
...
fn denial_message(
    ...
    let run_command = if isolation == DEFAULT_ISOLATION_DIR {
        "bsmr kill && bsmr server".to_owned()
    } else {
        format!("bsmr --isolation-dir {isolation} kill && bsmr --isolation-dir {isolation} server")
    };

    let mut message = format!(
        "Running bsmr with bsmr daemon spawned from a coding agent's sandbox on certain hosts can make builds extremely slow. This daemon is in that situation on host `{hostname}`, so its builds are blocked for now. We are working on a long-term fix to remove bsmr daemon from sandbox.
...
This restarts the bsmr daemon outside the sandbox, after which all bsmr builds (including ones triggered by a coding agent) will work again."
    );
...
pub(crate) fn check_agent_host_guard(
    ...
    let cgroup_override = bsmr_env!(
        "BSMR_TEST_DAEMON_ORIGINATING_CGROUP",
...
fn build_hostname_globset(spec: &str) -> bsmr_error::Result<GlobSet> {
