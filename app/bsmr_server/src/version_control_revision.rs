...
use bsmr_data::VersionControlRevision;
...
pub(crate) fn spawn_version_control_collector(
    ...
    bsmr_core::execution_types::revision::clear_revision();
        ...
        while let Some(event) = tasks.next().await {
            if let Some(error) = &event.command_error {
                soft_error!(
                    ...
                    bsmr_error::bsmr_error!(bsmr_error::ErrorTag::Input, "{}", error),
                    quiet: true
                )
                .ok();
            }
...
) -> bsmr_data::VersionControlRevision {
    let mut revision = bsmr_data::VersionControlRevision::default();
...
async fn create_hg_data(
    revision: &mut bsmr_data::VersionControlRevision,
    revision_type: RevisionDataType,
    repo_root: &AbsNormPathBuf,
) {
    match revision_type {
...
async fn get_hg_status(revision: &mut bsmr_data::VersionControlRevision) {
...
async fn repo_type(repo_root: &AbsNormPathBuf) -> bsmr_error::Result<&'static RepoVcs> {
    static REPO_TYPE: OnceCell<bsmr_error::Result<RepoVcs>> = OnceCell::const_new();
    async fn repo_type_impl(repo_root: &AbsNormPathBuf) -> bsmr_error::Result<RepoVcs> {
...
mod tests {
    ...
    fn temp_repo_root(temp_dir: &tempfile::TempDir) -> bsmr_error::Result<AbsNormPathBuf> {
    ...
    #[tokio::test]
    async fn test_get_git_revision_matches_head() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_get_git_status_clean() -> bsmr_error::Result<()> {
        let temp_dir = tempfile::tempdir()?;
        let repo_root = temp_repo_root(&temp_dir)?;
        init_git_repo(&repo_root).await;
        write_file(&repo_root, "file.txt", "hello");
