...
use bsmr_event_observer::dice_state::DiceState;
use bsmr_event_observer::pending_estimate::pending_estimate;
...
pub fn broadcast_instant_event<E: Into<bsmr_data::instant_event::Data> + Clone>(event: &E) -> bool {
...
pub fn broadcast_shutdown(shutdown: &bsmr_data::DaemonShutdown) {
    for cmd in ACTIVE_COMMANDS.lock().values() {
...
pub struct ActiveCommandHandle {
    ...
    daemon_shutdown_channel: Arc<Mutex<Option<oneshot::Sender<bsmr_data::DaemonShutdown>>>>,
...
impl ActiveCommandHandle {
    fn notify_shutdown(&self, shutdown: bsmr_data::DaemonShutdown) {
...
impl ActiveCommandStateWriter {
    ...
    pub fn peek_event(&mut self, buck_event: &BuckEvent) {
        ...
        match buck_event.data() {
            ...
            Instant(instant) => {
                use bsmr_data::instant_event::Data::*;
...
pub struct ActiveCommand {
    ...
    pub daemon_shutdown_channel: oneshot::Receiver<bsmr_data::DaemonShutdown>,
}
...
impl ActiveCommand {
    pub fn new(event_dispatcher: &EventDispatcher, sanitized_argv: Vec<String>) -> Self {
        ...
        if let Some(commands) = result {
            ...
            event_dispatcher.instant_event(bsmr_data::ConcurrentCommands {
                trace_ids: commands.keys().map(|cmd| cmd.to_string()).collect(),
            });

            // Notify other commands that they are concurrent with ours.
            for cmd in commands.values() {
                cmd.dispatcher.instant_event(bsmr_data::ConcurrentCommands {
...
mod tests {
    ...
    fn test_active_command_state() {
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            Some(root),
            None,
            bsmr_data::SpanStartEvent {
                data: Some(bsmr_data::AnalysisStart::default().into()),
            }
            .into(),
        ));
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            Some(root),
            bsmr_data::SpanStartEvent {
                data: Some(bsmr_data::AnalysisStageStart::default().into()),
            }
            .into(),
        ));
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            Some(child),
            Some(root),
            bsmr_data::SpanEndEvent {
                data: Some(bsmr_data::AnalysisStageEnd::default().into()),
                ..Default::default()
            }
            .into(),
        ));
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            None,
            bsmr_data::SpanEndEvent {
                data: Some(bsmr_data::AnalysisEnd::default().into()),
                ..Default::default()
            }
            .into(),
        ));
