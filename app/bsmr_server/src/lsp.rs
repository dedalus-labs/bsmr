...
use bsmr_interpreter_for_build::interpreter::dice_calculation_delegate::HasCalculationDelegate;
use bsmr_interpreter_for_build::interpreter::global_interpreter_state::HasGlobalInterpreterState;
use bsmr_interpreter_for_build::interpreter::interpreter_for_dir::ParseData;
...
impl DocsCacheManager {
    async fn new(fs: ProjectRoot, mut dice_ctx: DiceTransaction) -> bsmr_error::Result<Self> {
        Ok(Self {
    ...
    ) -> bsmr_error::Result<MutexGuard<'_, DocsCache>> {
...
async fn get_builtin_globals_docs(dice_ctx: &mut DiceTransaction) -> bsmr_error::Result<DocModule> {
    Ok(dice_ctx
...
impl DocsCache {
    async fn get_prelude_uri(
        ...
        Uri::from_file_path(&abs_path)
            ...
            .try_into()
            .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::Lsp))
    }
    ...
    async fn new_with_lookup<
        ...
        Fut: Future<Output = bsmr_error::Result<LspUri>>,
    >(
...
impl<'a> BsmrLspContext<'a> {
    ...
    async fn with_dice_ctx<F, Fut, R>(&self, exec: F) -> bsmr_error::Result<R>
    where
        F: FnOnce(DiceTransaction) -> Fut + Send,
        Fut: Future<Output = bsmr_error::Result<R>> + Send,
        R: Send,
    {
        self.server_ctx
    ...
    async fn import_path(&self, path: &Path) -> bsmr_error::Result<OwnedStarlarkModulePath> {
    ...
    async fn parse_file_from_contents_and_handle_diagnostic(
        ...
        uri: &LspUri,
        content: String,
    ) -> bsmr_error::Result<LspEvalResult> {
    ...
    async fn parse_file_from_string(
        ...
        literal: &str,
    ) -> bsmr_error::Result<Option<StringLiteralResult>> {
                        ...
                        Ok(DicePackageListingResolver(&mut dice_ctx)
                            ...
                            .and_then(|listing| {
                                ...
                                match uri.try_into() {
                                    ...
                                    Err(e) => Err(from_any_with_tag(e, bsmr_error::ErrorTag::Lsp)),
                                }
                            }))
                    })
                    .await?;
    ...
    fn resolve_string_literal(
        ...
        self.runtime
            .block_on(with_dispatcher_async(dispatcher, async {
                ...
                //
                // bsmr_error::Error sort of obscures the root causes, so we just have
        ...
        self.runtime
            .block_on(with_dispatcher_async(dispatcher, async {
                let docs_cache = self
                    ...
                    .await?;
                bsmr_error::Ok(docs_cache.uri_for_symbol(symbol).cloned())
            }))
            .map_err(|e| format!("{:#}", e))
    }
...
pub(crate) async fn run_lsp_server_command(
    ...
    partial_result_dispatcher: PartialResultDispatcher<bsmr_cli_proto::LspMessage>,
    req: StreamingRequestHandler<bsmr_cli_proto::LspRequest>,
) -> bsmr_error::Result<bsmr_cli_proto::LspResponse> {
    let start_event = ctx
        .command_start_event(bsmr_data::LspCommandStart {}.into())
        .await?;
    span_async(start_event, async move {
        let result = run_lsp_server(ctx, partial_result_dispatcher, req).await;
        let end_event = command_end(&result, bsmr_data::LspCommandEnd {});
        (result, end_event)
    })
    .await
}
...
async fn run_lsp_server(
    ctx: &dyn ServerCommandContextTrait,
    mut partial_result_dispatcher: PartialResultDispatcher<bsmr_cli_proto::LspMessage>,
    mut req: StreamingRequestHandler<LspRequest>,
) -> bsmr_error::Result<LspResponse> {
...
async fn recv_from_lsp(
    ...
    mut event_sender: UnboundedSender<bsmr_cli_proto::LspMessage>,
) -> bsmr_error::Result<()> {
    loop {
        let msg = to_client
            .recv()
        ...
        let res = bsmr_cli_proto::LspMessage { lsp_json };
        match event_sender.send(res).await {
            Ok(_) => {}
            Err(e) if e.is_disconnected() => break Ok(()),
            Err(e) => break Err(from_any_with_tag(e, bsmr_error::ErrorTag::Lsp)),
        }
    }
}
...
fn handle_incoming_lsp_message(
    ...
    message: bsmr_error::Result<LspRequest>,
) -> bsmr_error::Result<Option<LspResponse>> {
    if let Ok(m) = message {
...
fn handle_outgoing_lsp_message(
    partial_result_dispatcher: &mut PartialResultDispatcher<bsmr_cli_proto::LspMessage>,
    event: Option<bsmr_cli_proto::LspMessage>,
) -> Option<LspResponse> {
    match event {
        Some(event) => {
...
mod tests {
    ...
    #[test]
    fn cache_builds() -> bsmr_error::Result<()> {
        ...
        async fn lookup_function(location: &ImportPath) -> bsmr_error::Result<LspUri> {
