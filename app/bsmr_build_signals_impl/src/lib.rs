...
use bsmr_analysis::analysis::calculation::AnalysisKey;
use bsmr_analysis::analysis::calculation::AnalysisKeyActivationData;
use bsmr_analysis::analysis::calculation::AnalysisWithExtraData;
use bsmr_artifact::actions::key::ActionKey;
use bsmr_artifact::artifact::build_artifact::BuildArtifact;
use bsmr_build_api::actions::RegisteredAction;
use bsmr_build_api::actions::calculation::ActionExtraData;
use bsmr_build_api::actions::calculation::ActionWithExtraData;
use bsmr_build_api::actions::calculation::BuildKey;
use bsmr_build_api::actions::calculation::BuildKeyActivationData;
use bsmr_build_api::artifact_groups::ArtifactGroup;
use bsmr_build_api::artifact_groups::ResolvedArtifactGroupBuildSignalsKey;
use bsmr_build_api::artifact_groups::calculation::EnsureProjectedArtifactKey;
use bsmr_build_api::artifact_groups::calculation::EnsureTransitiveSetProjectionKey;
use bsmr_build_api::artifact_groups::calculation::EnsureTransitiveSetProjectionKeyActivationData;
use bsmr_build_api::build_signals::BuildSignals;
use bsmr_build_api::build_signals::BuildSignalsInstaller;
use bsmr_build_api::build_signals::CREATE_BUILD_SIGNALS;
use bsmr_build_signals::env::BuildSignalsContext;
use bsmr_build_signals::env::CriticalPathBackendName;
use bsmr_build_signals::env::DeferredBuildSignals;
use bsmr_build_signals::env::EarlyCommandTiming;
use bsmr_build_signals::env::FinishBuildSignals;
...
use bsmr_build_signals::node_key::BuildSignalsNodeKey;
use bsmr_build_signals::node_key::BuildSignalsNodeKeyImpl;
use bsmr_common::package_listing::dice::PackageListingKey;
use bsmr_common::package_listing::dice::PackageListingKeyActivationData;
...
impl NodeKey {
    ...
    ) -> bsmr_data::critical_path_entry2::Entry {
        match self {
            NodeKey::BuildKey(ref key) => {
                ...
                match extra_data {
                    NodeExtraData::Action(ActionNodeData {
                        ...
                        action_digest,
                        invalidation_info,
                    }) => bsmr_data::critical_path_entry2::ActionExecution {
                        owner: Some(owner),
            ...
            NodeKey::AnalysisKey(key) => bsmr_data::critical_path_entry2::Analysis {
            ...
            NodeKey::FinalMaterialization(key) => {
                ...
                bsmr_data::critical_path_entry2::FinalMaterialization {
                    owner: Some(owner),
                    path: key.get_path().path().to_string(),
                }
                .into()
            }
            NodeKey::InterpreterResultsKey(key) => bsmr_data::critical_path_entry2::Load {
                package: key.0.to_string(),
            }
            .into(),

            NodeKey::PackageListingKey(key) => bsmr_data::critical_path_entry2::Listing {
                package: key.0.to_string(),
            }
            .into(),
            ...
            NodeKey::EnsureTransitiveSetProjectionKey(_key) => {
                bsmr_data::critical_path_entry2::EnsureTransitiveSetProjection {}.into()
            }
            NodeKey::Dyn(_, ref d) => match d.critical_path_entry_proto() {
                Some(mut entry) => {
                    ...
                    if let (
                        bsmr_data::critical_path_entry2::Entry::AnonAnalysis(anon),
            ...
            NodeKey::TestExecution(t) => bsmr_data::critical_path_entry2::TestExecution {
            ...
            NodeKey::TestListing(t) => bsmr_data::critical_path_entry2::TestListing {
                target_label: Some(t.target.as_proto()),
                suite: t.suite.to_string(),
            }
            .into(),
    ...
    fn into_generic_entry(self) -> bsmr_data::critical_path_entry2::Entry {
        bsmr_data::critical_path_entry2::GenericEntry {
            kind: match self {
            ...
            NodeKey::Dyn(name, dyn_key) if *name == "AnonTargetKey" => {
                ...
                let anon_target_proto = dyn_key
                    ...
                    .and_then(|entry| match entry {
                        bsmr_data::critical_path_entry2::Entry::AnonAnalysis(a) => a.anon_target,
                        _ => None,
                    })
                    .unwrap_or_default();
                NodeKey::Dyn(
    ...
    T: BuildListenerBackend,
{
    ...
    pub(crate) async fn run_and_log(
        ...
        let top_level_targets =
            top_level_targets.map(|(key, duration)| bsmr_data::TopLevelTargetCriticalPath {
        ...
        instant_event(bsmr_data::BuildGraphExecutionInfo {
            critical_path2,
            slowest_path,
...
impl DetailedCriticalPath {
    ...
    fn create_proto_entries_for_early_timings(
        ...
        let generic_entry = |kind: &str| -> bsmr_data::critical_path_entry2::Entry {
            bsmr_data::critical_path_entry2::GenericEntry {
                kind: kind.to_owned(),
            }
            .into()
    ...
    fn into_critical_path_proto(
    ...
    ) -> Vec<bsmr_data::CriticalPathEntry2> {
        ...
        enhancer.add_simple_entry(
            ...
            bsmr_data::critical_path_entry2::ComputeCriticalPath {}.into(),
...
struct ActionNodeData {
    ...
    execution_kind: bsmr_data::ActionExecutionKind,
    target_rule_type_name: Option<String>,
    action_digest: Option<String>,
    invalidation_info: Option<bsmr_data::CommandInvalidationInfo>,
}
...
impl BuildSignalsNodeKeyImpl for AnalysisFinishNodeKey {
    fn critical_path_entry_proto(&self) -> Option<bsmr_data::critical_path_entry2::Entry> {
        Some(
            bsmr_data::critical_path_entry2::Analysis {
                target: Some(
                    bsmr_data::critical_path_entry2::analysis::Target::StandardTarget(
...
struct AnonTargetFinishNodeKey {
    ...
    anon_target_proto: bsmr_data::AnonTarget,
}
...
impl BuildSignalsNodeKeyImpl for AnonTargetFinishNodeKey {
    fn critical_path_entry_proto(&self) -> Option<bsmr_data::critical_path_entry2::Entry> {
        Some(
            bsmr_data::critical_path_entry2::AnonAnalysis {
