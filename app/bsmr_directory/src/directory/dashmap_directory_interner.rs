...
where
    H: DirectoryDigest,
{
    inner: Arc<DashMap<H, Weak<SharedDirectoryInner<L, H>>, BsmrHasherBuilder>>,
}
    ...
    H: DirectoryDigest,
{
    pub fn new() -> Self {
        Self {
            inner: Arc::new(DashMap::with_hasher(BsmrHasherBuilder)),
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_directory_interner() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_directory_interner_deep() -> bsmr_error::Result<()> {
        let interner = DashMapDirectoryInterner::new();
