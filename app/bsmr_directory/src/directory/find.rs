...
struct PrefixLookupContainer<T> {
    leaf: T,
    path: bsmr_fs::paths::forward_rel_path::ForwardRelativePathBuf,
}
...
pub(crate) fn find_prefix<'a, 'b, D: DirectoryRef<'a>>(
    ...
    Option<(
        ...
        // Remaining path.
        bsmr_fs::paths::forward_rel_path::ForwardRelativePathBuf,
    )>,
    DirectoryFindError,
> {
    let mut path = path.into_iter();
    ...
    let path_needle = match path.next() {
        ...
        None => {
            return Ok(Some((
                ...
                bsmr_fs::paths::forward_rel_path::ForwardRelativePathBuf::default(),
            )));
        }
    };
...
mod tests {
    ...
    #[test]
    fn test_find() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_find_prefix() -> bsmr_error::Result<()> {
        let mut a = TestDirectoryBuilder::empty();
        a.insert(path("a/b/c"), DirectoryEntry::Leaf(NopEntry))?;
