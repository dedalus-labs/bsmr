...
use bsmr_cli_proto::new_generic::DocsOutputFormat;
use bsmr_cli_proto::new_generic::DocsRequest;
use bsmr_cli_proto::new_generic::DocsStarlarkRequest;
...
impl StreamingCommand for DocsStarlarkCommand {
    ...
    async fn exec_impl(
        ...
        let response = buckd
            ...
            .new_generic(
                client_context,
                bsmr_cli_proto::new_generic::NewGenericRequest::Docs(DocsRequest::Starlark(
                    DocsStarlarkRequest {
        ...
        let bsmr_cli_proto::new_generic::NewGenericResponse::Docs(response) = response else {
            return bsmr_error!(
        ...
        if let Some(json_output) = response.json_output {
            bsmr_client_ctx::println!("{}", json_output.trim_end())?;
        }
