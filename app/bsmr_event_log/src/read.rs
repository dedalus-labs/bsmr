...
fn is_truncated_stream_error(error: &bsmr_error::Error) -> bool {
...
fn tolerant_of_truncation<T>(
    stream: impl Stream<Item = bsmr_error::Result<T>>,
) -> impl Stream<Item = bsmr_error::Result<T>> {
...
impl EventLogPathBuf {
    pub fn infer(path: AbsPathBuf) -> bsmr_error::Result<Self> {
    ...
    fn file_name(path: &AbsPathBuf) -> bsmr_error::Result<&str> {
        let name = path
            .file_name()
    ...
    pub fn uuid_from_filename(&self) -> bsmr_error::Result<TraceId> {
    ...
    pub fn command_from_filename(&self) -> bsmr_error::Result<&str> {
        ...
        match file_name.split('_').nth(1) {
            ...
            None => Err(bsmr_error::bsmr_error!(
    ...
    pub(crate) fn infer_opt(path: &AbsPathBuf) -> bsmr_error::Result<Option<Self>> {
    ...
    ) -> bsmr_error::Result<(Invocation, BoxStream<'a, bsmr_error::Result<StreamValue>>)> {
    ...
    async fn unpack_stream_protobuf<'a>(
        ...
        let events = stream.and_then(|data| async move {
            ...
            match val.progress {
                ...
                None => Err(bsmr_error::bsmr_error!(
                    bsmr_error::ErrorTag::InvalidEvent,
    ...
    async fn unpack_stream_inner<'a>(
        ...
        Invocation,
        impl Stream<Item = bsmr_error::Result<StreamValue>> + use<'a>,
    )> {
    ...
    pub async fn unpack_stream(
        ...
        Invocation,
        impl Stream<Item = bsmr_error::Result<StreamValue>> + 'static + use<>,
    )> {
    ...
    async fn open<'a>(
        &self,
        stats: Option<&'a ReaderStats>,
    ) -> bsmr_error::Result<EventLogReader<'a>> {
        tracing::info!(
            "Open {} using encoding {:?}",
            self.path.display(),
            self.encoding
