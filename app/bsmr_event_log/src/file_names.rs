...
fn get_files_in_log_dir(logdir: &AbsNormPath) -> bsmr_error::Result<Vec<AbsNormPathBuf>> {
...
pub fn get_local_logs(logdir: &AbsNormPath) -> bsmr_error::Result<Vec<EventLogPathBuf>> {
...
fn sort_logs(dir: bsmr_fs::fs_util::ReadDir) -> Vec<AbsNormPathBuf> {
...
pub fn find_log_by_trace_id(
    log_dir: &AbsNormPath,
    trace_id: &TraceId,
) -> bsmr_error::Result<Option<EventLogPathBuf>> {
...
pub fn retrieve_all_logs(paths: &InvocationPaths) -> bsmr_error::Result<Vec<EventLogPathBuf>> {
    let log_dir = paths.log_dir();
...
mod tests {
    ...
    #[tokio::test]
    async fn test_remove_old_logs_with_mix_of_files_and_folders() -> bsmr_error::Result<()> {
