...
impl WriteEventLog {
    ...
    async fn log_invocation(&mut self, trace_id: TraceId) -> bsmr_error::Result<()> {
    ...
    async fn write_ln<'b, T, I>(&'b mut self, events: I) -> bsmr_error::Result<()>
    where
        T: SerializeForLog + 'b,
...
) -> bsmr_error::Result<NamedEventLogWriter> {
...
impl WriteEventLog {
    ...
    pub async fn flush_files(&mut self) -> bsmr_error::Result<()> {
...
impl SerializeForLog for StreamValueForWrite<'_> {
    ...
    fn serialize_to_protobuf_length_delimited(&self, buf: &mut Vec<u8>) -> bsmr_error::Result<()> {
        ...
        let stream_val = bsmr_cli_proto::CommandProgressForWrite {
...
impl SerializeForLog for OwnedStreamValueForWrite {
    ...
    fn serialize_to_protobuf_length_delimited(&self, buf: &mut Vec<u8>) -> bsmr_error::Result<()> {
        match &self.0 {
            StreamValue::Event(e) => {
                bsmr_cli_proto::CommandProgressForWrite {
    ...
    fn maybe_serialize_user_event(&self, _buf: &mut Vec<u8>) -> bsmr_error::Result<bool> {
        Ok(false)
    }
}
...
mod tests {
    ...
    impl WriteEventLog {
        async fn new_test(log: EventLogPathBuf) -> bsmr_error::Result<Self> {
            Ok(Self {
                ...
                sanitized_argv: Argv {
                    argv: vec!["bsmr".to_owned()],
                    expanded_argv: ExpandedArgv::from_literals(vec!["bsmr".to_owned()]),
                }
                .no_need_to_sanitize(),
    ...
    #[tokio::test]
    async fn test_protobuf_decoding_gzip() -> bsmr_error::Result<()> {
        test_protobuf_decoding(Encoding::PROTO_GZIP).await
    }

    #[tokio::test]
    async fn test_protobuf_decoding_zstd() -> bsmr_error::Result<()> {
        test_protobuf_decoding(Encoding::PROTO_ZSTD).await
    }

    async fn test_protobuf_decoding(encoding: Encoding) -> bsmr_error::Result<()> {
        //Create log dir
    ...
    #[tokio::test]
    async fn test_tick_makes_valid_log_zstd() -> bsmr_error::Result<()> {
        test_tick_makes_valid_log(Encoding::PROTO_ZSTD).await
    }

    async fn test_tick_makes_valid_log(encoding: Encoding) -> bsmr_error::Result<()> {
        if cfg!(windows) {
    ...
    fn test_stream_value_serialize_to_protobuf_length_delimited() {
        ...
        StreamValueForWrite::Event(event.event())
            ...
            .unwrap();
        let expected = bsmr_cli_proto::CommandProgress {
