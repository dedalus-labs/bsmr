...
use bsmr_client_ctx::ide_support::ide_message_stream;
...
pub fn write_dap_message(out: &mut impl Write, msg: &[u8]) -> bsmr_error::Result<()> {
...
fn send_message_to_dap_client(msg: &[u8]) -> bsmr_error::Result<()> {
...
impl StreamingCommand for StarlarkDebugAttachCommand {
    ...
    fn extra_subscribers(&self) -> Vec<Box<dyn EventSubscriber>> {
        ...
        impl ConvertToDap {
            fn write_console(&self, msg: &str) -> bsmr_error::Result<()> {
        ...
        impl EventSubscriber for ConvertToDap {
            ...
            async fn handle_error(&mut self, error: &bsmr_error::Error) -> bsmr_error::Result<()> {
                Ok(self
                    .write_console(&format!("bsmr starlark-attach debugserver error: {error}"))?)
            }
        }
...
impl PartialResultHandler for DapPartialResultHandler {
    type PartialResult = bsmr_cli_proto::DapMessage;

    async fn handle_partial_result(
        &mut self,
        mut _ctx: PartialResultCtx<'_>,
        partial_res: bsmr_cli_proto::DapMessage,
    ) -> bsmr_error::Result<()> {
