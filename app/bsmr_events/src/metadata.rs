...
pub fn collect(daemon: &DaemonId) -> StdBuckHashMap<String, String> {
    ...
    if let Some(rev) = bsmr_build_info::revision() {
        map.insert("bsmr_revision".to_owned(), rev.to_owned());
    }

    if let Some(time) = bsmr_build_info::time_iso8601() {
        map.insert("bsmr_build_time".to_owned(), time.to_owned());
    }

    if let Some(ts) = bsmr_build_info::release_timestamp() {
        map.insert("bsmr_release_timestamp".to_owned(), ts.to_owned());
    }
    ...
    add_env_var(&mut map, "launched_via_wrapper", BSMR_WRAPPER_ENV_VAR);
...
pub fn username() -> bsmr_error::Result<Option<String>> {
    #[cfg(fbcode_build)]
    {
        use bsmr_error::conversion::from_any_with_tag;
        Ok(Some(user::current_username().map_err(|e| {
            from_any_with_tag(e, bsmr_error::ErrorTag::InvalidUsername)
        })?))
    }
    #[cfg(not(fbcode_build))]
    {
        Ok::<Option<String>, bsmr_error::Error>(None)
    }
}
