...
//! liberally duplicated and passed around to the depths of bsmr so that consumers can insert events into it.
...
impl EventDispatcher {
    ...
    pub fn instant_event<E: Into<bsmr_data::instant_event::Data>>(&self, data: E) {
        let instant = bsmr_data::InstantEvent {
            data: Some(data.into()),
        };
    ...
    pub fn console_message(&self, message: String) {
        self.instant_event(bsmr_data::ConsoleMessage { message })
    }

    pub fn console_warning(&self, message: String) {
        self.instant_event(bsmr_data::ConsoleWarning { message })
    }

    pub fn streaming_output(&self, message: String) {
        self.instant_event(bsmr_data::StdoutStreamingOutput { message })
    }

    /// Like `instant_event`, but bypasses internal buffering. See [`EventSink::send_now`].
    pub async fn instant_event_send_now<E: Into<bsmr_data::instant_event::Data>>(&self, data: E) {
        let instant = bsmr_data::InstantEvent {
            data: Some(data.into()),
        };
    ...
    pub fn command_result(&self, data: bsmr_cli_proto::CommandResult) {
...
impl Span {
    ...
    fn send(&mut self, data: bsmr_data::span_end_event::Data) {
        self.sent = true;
        ...
        self.dispatcher.event_with_span_id(
            SpanEndEvent {
                ...
                data: Some(data),
                stats: Some(bsmr_data::SpanStats {
...
impl Drop for Span {
    ...
    fn drop(&mut self) {
        if !self.sent {
            self.send(bsmr_data::SpanCancelled {}.into())
        }
    }
}
...
pub fn get_dispatcher() -> EventDispatcher {
    let enforce_event_dispatcher_set = bsmr_env!("ENFORCE_DISPATCHER_SET", bool).unwrap();
    ...
    match get_dispatcher_opt() {
        ...
        None => {
            if enforce_event_dispatcher_set {
                ...
                // let _ignored = soft_error!(bsmr_error::bsmr_error!([], "Task local event dispatcher not set."));
...
/// Emits an InstantEvent annotated with the current trace ID
pub fn instant_event<E: Into<bsmr_data::instant_event::Data>>(data: E) {
    get_dispatcher().instant_event(data)
}
