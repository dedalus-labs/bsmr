...
//!
//! Events and event streams for Bessemer.
//!
//! The `Event` enum is the set of events that Bessemer can produce. Events can be produced both during the course of
...
/// An event that can be produced by Bessemer. Events are points in time with additional metadata attached to them,
...
impl BuckEvent {
    ...
    pub fn trace_id(&self) -> bsmr_error::Result<TraceId> {
    ...
    pub fn command_start(&self) -> bsmr_error::Result<Option<&bsmr_data::CommandStart>> {
...
pub fn init_late_bindings() {
    bsmr_core::event::EVENT_DISPATCH.init(&dispatch::EventDispatcherLateBinding);
}
