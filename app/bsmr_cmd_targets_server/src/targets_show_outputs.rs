...
use bsmr_cli_proto::TargetsShowOutputsResponse;
use bsmr_cli_proto::targets_show_outputs_response::TargetPaths;
...
pub async fn targets_show_outputs_command(
    ...
    req: TargetsRequest,
) -> bsmr_error::Result<TargetsShowOutputsResponse> {
    run_server_command(
...
impl ServerCommandTemplate for TargetsShowOutputsServerCommand {
    ...
    type Response = bsmr_cli_proto::TargetsShowOutputsResponse;
...
) -> bsmr_error::Result<Vec<TargetsArtifacts>> {
...
async fn retrieve_artifacts_for_provider_label(
    ...
    global_cfg_options: &GlobalCfgOptions,
) -> bsmr_error::Result<TargetsArtifacts> {
