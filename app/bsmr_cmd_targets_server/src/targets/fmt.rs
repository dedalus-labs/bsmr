...
use bsmr_build_api::configure_targets::ConfiguredTargetsWithErrors;
use bsmr_cli_proto::ConfiguredTargetsRequest;
use bsmr_cli_proto::HasClientContext;
use bsmr_cli_proto::TargetsRequest;
use bsmr_cli_proto::configured_targets_request::OutputFormat as ConfiguredOutputFormat;
...
use bsmr_node::attrs::hacks::value_to_json;
use bsmr_node::attrs::inspect_options::AttrInspectOptions;
use bsmr_node::nodes::attributes::DEPS;
use bsmr_node::nodes::attributes::INPUTS;
use bsmr_node::nodes::attributes::ONCALL;
use bsmr_node::nodes::attributes::PACKAGE;
use bsmr_node::nodes::attributes::PACKAGE_VALUES;
use bsmr_node::nodes::attributes::TARGET_CALL_STACK;
use bsmr_node::nodes::attributes::TARGET_HASH;
use bsmr_node::nodes::attributes::TYPE;
...
fn package_error_to_stderr(package: PackageLabel, error: &bsmr_error::Error, stderr: &mut String) {
    writeln!(stderr, "Error parsing {package}\n{error:?}").unwrap();
}

fn target_error_to_stderr(error: &bsmr_error::Error, stderr: &mut String) {
...
pub trait ConfiguredTargetFormatter: Send + Sync {
    ...
    fn target_error(&self, error: &bsmr_error::Error, stdout: &mut String, stderr: &mut String) {
...
pub(crate) struct Stats {
    ...
    error_tags: BTreeSet<bsmr_error::ErrorTag>,
...
impl Stats {
    ...
    pub(crate) fn add_error(&mut self, e: &bsmr_error::Error) {
        self.error_tags.extend(e.tags());
        self.errors += 1;
    }

    pub(crate) fn to_error(&self) -> Option<bsmr_error::Error> {
        if self.errors == 0 {
            return None;
        }
        // Simpler error so that we don't print long errors twice (when exiting bsmr)
        ...
        let mut e = bsmr_error::Error::from(TargetsError::FailedToParse(self.errors, package_str));
...
) -> bsmr_error::Result<Arc<dyn TargetFormatter>> {
...
) -> bsmr_error::Result<ConfiguredOutputHandler> {
