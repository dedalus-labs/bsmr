...
//! Server-side implementation of `bsmr targets --streaming` command.
...
use bsmr_common::pattern::package_roots::find_package_roots_stream;
...
fn write_str(outputter: &mut dyn Write, s: &mut String) -> bsmr_error::Result<()> {
...
impl PreparePackageResult {
    ...
    fn record_error(&mut self, error: &bsmr_error::Error, formatter: &dyn TargetFormatter) {
...
) -> impl Stream<Item = bsmr_error::Result<(PackageLabel, PackageSpec<T>)>> + '_ {
        ...
        PackageSpec::Targets(targets) => {
            if keep_going {
                ...
                let targets = targets.into_try_map(|(target, TargetPatternExtra)| {
                    bsmr_error::Ok(result.resolve_target(target.as_ref())?.to_owned())
                })?;
