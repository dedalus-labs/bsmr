...
use bsmr_build_api::configure_targets::get_compatible_targets;
use bsmr_common::file_ops::dice::DiceFileComputations;
use bsmr_common::file_ops::error::FileReadErrorContext;
use bsmr_common::file_ops::metadata::PathMetadata;
use bsmr_common::file_ops::metadata::PathMetadataOrRedirection;
...
use bsmr_query::query::environment::QueryTargetDepsSuccessors;
...
// `StarlarkHasherSmallPromote` in `bsmr_build_api/.../provider/callable.rs`.
...
trait FileHasher: Send + Sync {
    ...
    async fn hash_path(&self, path: &CellPath) -> bsmr_error::Result<Vec<u8>>;
}
...
impl FileHasher for PathsAndContentsHasher {
    async fn hash_path(&self, cell_path: &CellPath) -> bsmr_error::Result<Vec<u8>> {
        #[async_recursion]
        async fn hash_item(
...
/// This trait is purposely defined here instead of in bsmr_node crate
...
pub(crate) trait TargetHashingTargetNode: QueryTarget {
    ...
    ) -> bsmr_error::Result<TargetSet<Self>>;
}
