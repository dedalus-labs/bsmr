...
use bsmr_event_observer::event_observer::EventObserver;
use bsmr_event_observer::event_observer::EventObserverExtra;
...
/// bsmr daemon info is printed to stderr if there are no other updates available
...
macro_rules! echo {
    () => {
        {
            // patternlint-disable-next-line bsmr-cli-simpleconsole-echo
...
fn echo_system_warning_exponential(warning: &HealthCheckType, msg: &str) -> bsmr_error::Result<()> {
...
pub struct SimpleConsole<E> {
    ...
    action_errors: Vec<bsmr_data::ActionError>,
    ...
    E: EventObserverExtra,
{
    ...
    fn print_stats_while_waiting(&mut self) -> bsmr_error::Result<()> {
        ...
        if is_snapshot_stale {
            ...
            if let Some((_, snapshot)) = &snapshots.last {
                if let Some(bsmr_rss) = snapshot.bsmr_rss {
                    parts.push(format!("RSS: {}", HumanizedBytes::new(bsmr_rss)));
                }
            }
    ...
    fn print_action_error(&mut self, error: &bsmr_data::ActionError) -> bsmr_error::Result<()> {
    ...
    async fn handle_event_inner(&mut self, event: &BuckEvent) -> bsmr_error::Result<()> {
        match unpack_event(event)? {
            ...
            bsmr_event_observer::unpack_event::UnpackedBuckEvent::Instant(_, _, data) => match data
            {
                ...
                bsmr_data::instant_event::Data::TagEvent(tags) => {
    ...
    async fn handle_test_discovery(
        ...
        if let Some(data) = &test_info.data {
            match data {
                bsmr_data::test_discovery::Data::Session(bsmr_data::TestSessionInfo {
                    info,
                    ..
                }) => {
                ...
                bsmr_data::test_discovery::Data::Tests(..) => {}
            }
        }
    ...
    pub(crate) async fn handle_stderr(&mut self, stderr: &str) -> bsmr_error::Result<()> {
        echo!("{}", stderr)?;
        self.notify_printed();
        Ok(())
    }
    ...
    fn echo_health_check_warning(&self, report: &DisplayReport) -> bsmr_error::Result<()> {
    ...
    E: EventObserverExtra,
{
    ...
    async fn tick(&mut self, _: &Tick) -> bsmr_error::Result<()> {
        if self.verbosity.print_status()
