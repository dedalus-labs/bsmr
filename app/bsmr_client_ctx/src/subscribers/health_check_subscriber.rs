...
use bsmr_health_check::interface::HealthCheckEvent;
...
impl HealthCheckSubscriber {
    ...
    async fn handle_event(&mut self, event: &Arc<BuckEvent>) -> bsmr_error::Result<()> {
        ...
        let health_check_event = match event.data() {
            SpanStart(start) => match &start.data {
                Some(bsmr_data::span_start_event::Data::Command(command)) => {
                    Some(HealthCheckEvent::HealthCheckContextEvent(
                        HealthCheckContextEvent::CommandStart(bsmr_data::CommandStartWithTraceId {
            ...
            SpanEnd(end) => {
                use bsmr_data::span_end_event::Data::*;
                match end
                    .data
                    .as_ref()
            ...
            Instant(instant) => {
                    ...
                    .ok_or_else(|| internal_error!("Missing `data` in `Instant`"))?
                {
                    ...
                    Snapshot(_snapshot) => {
                        ...
                        let snapshot_data = bsmr_health_check::interface::HealthCheckSnapshotData {
            ...
            (health_check_event, &mut self.event_sender)
        {
            ...
            if send_test_threshold {
                if let Ok(val) = std::env::var("BSMR_TEST_SLOW_BUILD_CHECK") {
    ...
    pub fn close_client_connection_with_error_report(&mut self, error: &str) {
        ...
        let _ignored = soft_error!(
            ...
            bsmr_error::bsmr_error!(bsmr_error::ErrorTag::HealthCheck, "{}", error)
        );
    }
}
...
mod tests {
    ...
    use bsmr_health_check::report::DisplayReport;
    use bsmr_health_check::report::HealthIssue;
    use bsmr_health_check::report::Message;
    use bsmr_health_check::report::Severity;
    ...
    fn event_for_excess_cache_miss() -> bsmr_data::buck_event::Data {
        ...
        bsmr_data::buck_event::Data::SpanEnd(bsmr_data::SpanEndEvent {
            ...
            ..bsmr_data::SpanEndEvent::default()
        })
    }

    #[tokio::test]
    async fn test_health_check_subscriber() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_event_limit_exceeded() -> bsmr_error::Result<()> {
        let (events_tx, mut events_rx) = mpsc::channel::<HealthCheckEvent>(EVENT_CHANNEL_SIZE);
    ...
    #[tokio::test]
    async fn test_slow_client() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_dropped_client() -> bsmr_error::Result<()> {
        let (events_tx, events_rx_guard) = mpsc::channel::<HealthCheckEvent>(EVENT_CHANNEL_SIZE);
