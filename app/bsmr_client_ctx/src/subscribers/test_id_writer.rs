...
impl EventSubscriber for TestIdWriter {
    async fn handle_events(&mut self, events: &[Arc<BuckEvent>]) -> bsmr_error::Result<()> {
        ...
        for event in events {
            ...
            {
                if let Some(bsmr_data::test_discovery::Data::Session(session)) = &discovery.data {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_writes_session_id() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_writes_only_once() -> bsmr_error::Result<()> {
        let dir = tempfile::tempdir()?;
        let path = AbsPathBuf::new(dir.path().join("test_id.txt"))?;
        let mut writer = TestIdWriter::new(path.clone());
    ...
    #[tokio::test]
    async fn test_ignores_non_matching_events() -> bsmr_error::Result<()> {
        let dir = tempfile::tempdir()?;
        let path = AbsPathBuf::new(dir.path().join("test_id.txt"))?;
        let mut writer = TestIdWriter::new(path.clone());
    ...
    #[tokio::test]
    async fn test_finds_session_in_mixed_batch() -> bsmr_error::Result<()> {
        let dir = tempfile::tempdir()?;
        let path = AbsPathBuf::new(dir.path().join("test_id.txt"))?;
        let mut writer = TestIdWriter::new(path.clone());
    ...
    #[tokio::test]
    async fn test_writes_first_session_in_batch() -> bsmr_error::Result<()> {
        let dir = tempfile::tempdir()?;
        let path = AbsPathBuf::new(dir.path().join("test_id.txt"))?;
        let mut writer = TestIdWriter::new(path.clone());
    ...
    #[tokio::test]
    async fn test_errors_when_test_session_id_missing() -> bsmr_error::Result<()> {
        let dir = tempfile::tempdir()?;
        let path = AbsPathBuf::new(dir.path().join("test_id.txt"))?;
        let mut writer = TestIdWriter::new(path.clone());
    ...
    #[tokio::test]
    async fn test_errors_when_test_session_id_empty() -> bsmr_error::Result<()> {
        let dir = tempfile::tempdir()?;
        let path = AbsPathBuf::new(dir.path().join("test_id.txt"))?;
        let mut writer = TestIdWriter::new(path.clone());
