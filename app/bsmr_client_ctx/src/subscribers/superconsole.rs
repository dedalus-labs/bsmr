...
use bsmr_event_observer::display::display_file_watcher_end;
use bsmr_event_observer::event_observer::DebugEventObserverExtra;
use bsmr_event_observer::session_info::SessionInfo;
use bsmr_event_observer::unpack_event::VisitorError;
...
impl StatefulSuperConsole {
    ...
    /// Construct a console suitable for use by the Bessemer CLI. We use non-blocking output here
    ...
    pub fn render_result_errors(result: &bsmr_cli_proto::CommandResult) -> Lines {
        let mut lines = Lines::new();
        if let bsmr_cli_proto::CommandResult {
            result: Some(bsmr_cli_proto::command_result::Result::Error(e)),
        } = result
        {
    ...
    fn finalize(&mut self) -> bsmr_error::Result<()> {
        let mut res = Ok(());
...
impl SuperConsoleState {
    pub fn new(
        ...
        config: SuperConsoleConfig,
        health_check_reports_receiver: Option<Receiver<Vec<DisplayReport>>>,
    ) -> bsmr_error::Result<SuperConsoleState> {
...
impl StatefulSuperConsoleImpl {
    ...
    async fn handle_inner_event(&mut self, event: &BuckEvent) -> bsmr_error::Result<()> {
        match unpack_event(event)? {
            ...
            bsmr_event_observer::unpack_event::UnpackedBuckEvent::Instant(_, _, data) => match data
            {
                ...
                bsmr_data::instant_event::Data::ConsoleWarning(message) => {
                ...
                bsmr_data::instant_event::Data::ConsolePreferences(preferences) => {
                    self.handle_console_preferences(preferences).await
                }
                bsmr_data::instant_event::Data::ActionError(error) => {
                    self.handle_action_error(error).await
                }
                bsmr_data::instant_event::Data::StreamingOutput(message) => {
    ...
    async fn handle_stderr(&mut self, msg: &str) -> bsmr_error::Result<()> {
        self.super_console
    ...
    async fn handle_console_message(
        &mut self,
        message: &bsmr_data::ConsoleMessage,
    ) -> bsmr_error::Result<()> {
    ...
    async fn handle_streaming_output(
        &mut self,
        message: &bsmr_data::StdoutStreamingOutput,
    ) -> bsmr_error::Result<()> {
        self.super_console
    ...
    async fn handle_console_warning(
        &mut self,
        message: &bsmr_data::ConsoleWarning,
    ) -> bsmr_error::Result<()> {
        let style = ContentStyle {
    ...
    async fn handle_console_preferences(
        &mut self,
        prefs: &bsmr_data::ConsolePreferences,
    ) -> bsmr_error::Result<()> {
    ...
    async fn tick(&mut self, tick: &Tick) -> bsmr_error::Result<()> {
    ...
    fn finalize(
        ...
        SuperConsoleState,
        Option<superconsole::Error<bsmr_error::Error>>,
    ) {
        let err = self
            .super_console
...
impl EventSubscriber for StatefulSuperConsole {
    ...
    async fn handle_output(&mut self, raw_output: &[u8]) -> bsmr_error::Result<()> {
        self.finalize()?;
        match self {
    ...
    async fn handle_error(&mut self, _error: &bsmr_error::Error) -> bsmr_error::Result<()> {
        self.finalize()?;
        Ok(())
    }
}
    ...
    if let Some(command_kind) = command_failed.command_kind.as_ref() {
        ...
        match command_kind.command.as_ref() {
                ...
                let command = if verbosity.print_failure_full_command() {
                    ...
                    match truncate(command) {
                        ...
                        Some(short) => Cow::Owned(format!(
                            "{short} (run `bsmr log what-failed` to get the full command)"
                        )),
                    }
                };
            ...
            Some(Command::RemoteCommand(remote_command)) => {
                let help_message = if bsmr_core::is_open_source() {
...
mod tests {
    ...
    use bsmr_data::LoadBuildFileEnd;
    use bsmr_data::LoadBuildFileStart;
    ...
    #[tokio::test]
    async fn test_transfer_state_to_simpleconsole() -> bsmr_error::Result<()> {
        ...
        console
            .handle_command_result(&CommandResult {
                result: Some(bsmr_cli_proto::command_result::Result::GenericResponse(
                    GenericResponse {},
                )),
            })
            .await
    ...
    #[tokio::test]
    async fn test_default_layout() -> bsmr_error::Result<()> {
        let trace_id = TraceId::new();
        ...
        console
            .handle_event(&Arc::new(BuckEvent::new(
                ...
                bsmr_data::InstantEvent {
                    data: Some(
                        bsmr_data::RemoteExecutionSessionCreated {
        ...
        console
            .handle_command_result(&bsmr_cli_proto::CommandResult { result: None })
            .await?;
    ...
    #[test]
    fn test_session_info() -> bsmr_error::Result<()> {
        let info = SessionInfo {
            trace_id: TraceId::null(),
            test_session: Some(bsmr_data::TestSessionInfo {
    ...
    #[tokio::test]
    async fn test_tailer_stderr() -> bsmr_error::Result<()> {
        let trace_id = TraceId::new();
        let tick = Tick::now();
