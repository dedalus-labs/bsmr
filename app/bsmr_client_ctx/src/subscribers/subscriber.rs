...
pub trait EventSubscriber: Send {
    ...
    /// stdout.
    async fn handle_output(&mut self, _raw_output: &[u8]) -> bsmr_error::Result<()> {
        Ok(())
    }
    ...
    async fn handle_command_result(
        &mut self,
        _result: &bsmr_cli_proto::CommandResult,
    ) -> bsmr_error::Result<()> {
        Ok(())
    }
    ...
    async fn tick(&mut self, _tick: &Tick) -> bsmr_error::Result<()> {
        Ok(())
    }
    ...
    fn handle_daemon_started(&mut self, _reason: bsmr_data::DaemonWasStartedReason) {}
    ...
    async fn finalize(self: Box<Self>) -> bsmr_error::Result<()> {
        Ok(())
    }
}
