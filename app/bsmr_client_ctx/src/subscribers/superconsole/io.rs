...
fn do_render(
    ...
    if let Some(bsmr_rss) = snapshot.bsmr_rss {
        ...
        // bsmr_rss is only available on Linux. On other platforms, bsmr keeps track of bsmr_max_rss so show that instead.
        parts.push(format!(
            "Max RSS = {}",
            HumanizedBytes::new(snapshot.bsmr_max_rss)
        ));
    }

    // We prefer to display malloc_bytes_active instead of malloc_bytes_allocated
    // because it represents active pages which is more than allocated and better reflects actual memory use of bsmr.
