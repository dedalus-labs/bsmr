...
pub(crate) async fn check_daemon_oom_killed(
    cgroup_path_of_bsmr_daemon: &str,
    daemon_disconnect_time: SystemTime,
) -> bsmr_error::Result<bool> {
    let cgroup_path_of_bsmr_daemon =
        match cgroup_path_of_bsmr_daemon.strip_prefix("/sys/fs/cgroup/") {
            Some(rel) => rel,
            None => {
    ...
    let child = match bsmr_util::process::async_background_command("dmesg")
    ...
    let matcher = BsmrCgroupMatcher::new(cgroup_path_of_bsmr_daemon);
...
/// FNV prime, used as the polynomial base in `BsmrCgroupMatcher`.
...
/// Pre-computed view of a bsmr daemon cgroup path used to test dmesg lines
/// against many candidate killed cgroups without re-splitting or re-hashing
/// `bsmr_cgroup` each time.
struct BsmrCgroupMatcher<'a> {
...
impl<'a> BsmrCgroupMatcher<'a> {
    fn new(bsmr_cgroup: &'a str) -> Self {
        let components: Vec<&'a str> = bsmr_cgroup.split('/').collect();
    ...
    /// this bsmr cgroup (either exactly, as a parent, as a child, or as an
    ...
    /// components equals a prefix of this bsmr cgroup's path components.
    /// This is the case when oomd from the physical host kills `killed`
    /// while the bsmr daemon (or one of its ancestors) is observed inside a
    ...
    fn prefix_matches_suffix(&self, killed: &[&str]) -> bool {
        ...
        // (suffix length cannot exceed the bsmr prefix it is compared to).
...
/// Returns the cgroup path, e.g. `user.slice/.../bsmr_daemon....scope`.
...
/// Returns the killed cgroup path without leading `/`, e.g. `system.slice/bsmr_daemon....scope`.
...
#[cfg(test)]
mod tests {
    use super::BsmrCgroupMatcher;

    #[test]
    fn test_dmesg_line_matches_oom_kill_cgroup() {
        let matches = |line: &str, bsmr_cgroup: &str| {
            BsmrCgroupMatcher::new(bsmr_cgroup).dmesg_line_matches_oom_kill(line)
        };

        let line = "[Thu Jan  1 00:00:00 2025] oom-kill:constraint=CONSTRAINT_MEMCG,task_memcg=/user.slice/user-190155.slice/user@190155.service/bsmr.slice/bsmr_daemon.fbsource.v2.1ac02af6_d3c8_4eb1_bb72_c351ba823628.scope,task=bsmr,pid=1234,uid=1000";

        assert!(matches(
            line,
            "user.slice/user-190155.slice/user@190155.service/bsmr.slice/bsmr_daemon.fbsource.v2.1ac02af6_d3c8_4eb1_bb72_c351ba823628.scope"
        ));
        assert!(matches(
            line,
            "user.slice/user-190155.slice/user@190155.service/bsmr.slice"
        ));
        assert!(matches(
            line,
            "user.slice/user-190155.slice/user@190155.service/bsmr.slice/bsmr_daemon.fbsource.v2.1ac02af6_d3c8_4eb1_bb72_c351ba823628.scope/child"
        ));

        // Sibling cgroup that shares a prefix should not match
        assert!(!matches(
            line,
            "user.slice/user-190155.slice/user@190155.service/bsmr.slice/bsmr_daemon.fbsource.v2.1ac02af6_d3c8_4eb1_bb72_c351ba823628.scope_sibling"
        ));

        // oomd kill line format
        let oomd_line = "[1813654.116643] oomd kill: 81.28 80.05 42.78 user.slice/user-190155.slice/user@190155.service/bsmr.slice/bsmr_daemon.fbsource.v2.1ac02af6_d3c8_4eb1_bb72_c351ba823628.scope 137490391040 ruleset:[protection against low swap] detectorgroup:[free swap goes below 5%] killCommand: build";
        ...
        assert!(matches(
            oomd_line,
            "user.slice/user-190155.slice/user@190155.service/bsmr.slice/bsmr_daemon.fbsource.v2.1ac02af6_d3c8_4eb1_bb72_c351ba823628.scope/daemon"
        ));
    ...
    fn test_dmesg_line_matches_oom_kill_cgroup_namespace_prefix() {
        ...
        // Negative: matched_cgroup tail looks like `bsmr_cgroup` but not at a
        // `/` boundary — for instance, host path ends with `xtask` while
        // bsmr_cgroup starts with `task`. Must not match.
        let bad_boundary_line = "[1813654.116643] oomd kill: 81.28 80.05 42.78 workload.slice/workload-tw.slice/sometask/user.slice/user-29230.slice/user@29230.service/bsmr.slice 137490391040 ruleset:[protection against low swap] detectorgroup:[free swap goes below 5%] killCommand: build";
    ...
    fn test_dmesg_line_matches_systemd_oomd_kill_cgroup() {
        ...
        let line = "[858172.042162] Killed /user.slice/user-190155.slice/user@190155.service/bsmr.slice/bsmr_daemon.fbsource.v2.1ac02af6_d3c8_4eb1_bb72_c351ba823628.scope due to memory pressure for /user.slice being 76.55% > 70.00% for > 3min with reclaim activity";
