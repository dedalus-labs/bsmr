...
enum ArgExpansionError {
    ...
    StdinReadError { source: bsmr_error::Error },
}
...
pub fn log_relative_path_from_cell_root(requested_path: &str) -> bsmr_error::Result<()> {
...
fn resolve_and_expand_argfile(
    ...
    let flagfile = resolve_flagfile(path, context, cwd)
        .with_bsmr_error_context(|| format!("Error resolving flagfile `{path}`"))?;
...
fn resolve_flagfile(
    ...
    let resolved_path = match path_part.split_once("//") {
        Some((cell_alias, cell_relative_path)) => context
            ...
            .bsmr_error_context("Error resolving cell path")?
            .into_abs_path_buf(),
        None => {
...
mod tests {
    ...
    use bsmr_common::argv::FlagfileArgSource;
    ...
    #[test]
    fn test_resolve_argfile_kind_with_project() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_expand_argfile_content() -> bsmr_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
        let cwd =
    ...
    #[test]
    fn test_comment() -> bsmr_error::Result<()> {
        let content = indoc! {"
            --# Usage: bsmr build @mode/mrustc ...
            --#
            --# Generated by tools/build/bsmr/gen_modes.py
