...
use bsmr_error::classify::ErrorTagExtra;
use bsmr_error::classify::best_error;
...
///
/// We can easily turn a bsmr_error::Result (or bsmr_error::Error, or even a message) into a ExitResult,
...
impl ExitResult {
    ...
    pub fn get_error(&self) -> Option<bsmr_error::Error> {
    ...
    pub fn bail(msg: impl Display) -> Self {
        Self::err(bsmr_error::bsmr_error!(
            bsmr_error::ErrorTag::Bail,
            "Command failed: {}",
            msg
        ))
    }
    ...
    pub fn err(err: bsmr_error::Error) -> Self {
        Self {
    ...
    pub fn err_with_exit_code(err: bsmr_error::Error, exit_code: ExitCode) -> Self {
        Self {
    ...
    pub fn retry_command_with_full_binary() -> bsmr_error::Result<Option<Self>> {
...
/// We can produce a ExitResult from a `bsmr_error::Result` for convenience.
impl From<bsmr_error::Result<()>> for ExitResult {
    fn from(e: bsmr_error::Result<()>) -> Self {
        match e {
            Ok(()) => Self::success(),
            Err(e) => Self::err(e),
        }
...
impl From<bsmr_error::Result<ExitCode>> for ExitResult {
    fn from(e: bsmr_error::Result<ExitCode>) -> Self {
        match e {
...
impl From<bsmr_error::Error> for ExitResult {
    fn from(e: bsmr_error::Error) -> Self {
        Self::err(e)
    }
}

impl FromResidual<bsmr_error::Error> for ExitResult {
    #[track_caller]
    fn from_residual(residual: bsmr_error::Error) -> ExitResult {
        Self::err(residual)
    }
}

impl<E: Into<::bsmr_error::Error>> FromResidual<Result<Infallible, E>> for ExitResult {
    #[track_caller]
    fn from_residual(residual: Result<Infallible, E>) -> ExitResult {
        match residual {
            // E -> bsmr_error::Error -> ExitResult
...
impl ExitResultVariant {
    pub fn report(self) -> ! {
        ...
        let mut exit_code = match self {
            Self::Status(v) => v,
            Self::Exec(args) => {
                // Terminate by exec-ing a new process - usually because of `bsmr run`.
                //
        ...
        // So the result is non-reproducible crash of the bsmr client.
...
#[derive(bsmr_error::Error, derivative::Derivative)]
...
pub enum ClientIoError {
    ...
    /// An easy way to trigger this is `bsmr audit config | head`
    #[bsmr(tag = IoClientBrokenPipe)]
    #[bsmr(environment)]
    BrokenPipe(io::Error),
    #[bsmr(tier0)]
    OtherIo(io::Error),
...
impl From<bsmr_error::Error> for ClientIoError {
...
#[cfg(windows)]
fn do_exec(command: &mut Command) -> bsmr_error::Error {
...
fn execv(args: ExecArgs) -> ! {
    // patternlint-disable-next-line bsmr-no-command-new
