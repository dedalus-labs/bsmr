...
fn stdout() -> bsmr_error::Result<io::Stdout> {
...
pub fn _print(fmt: Arguments) -> bsmr_error::Result<()> {
    stdout()?
        .lock()
        .write_fmt(fmt)
...
pub fn _eprint(fmt: Arguments) -> bsmr_error::Result<()> {
    io::stderr()
        .lock()
        .write_fmt(fmt)
        .map_err(|e| ClientIoError::from(e).into())
...
pub fn print_bytes(bytes: &[u8]) -> bsmr_error::Result<()> {
    stdout()?
        .lock()
        .write_all(bytes)
        .map_err(|e| ClientIoError::from(e).into())
...
pub fn eprint_line(line: &Line) -> bsmr_error::Result<()> {
    let line = line.render();
    crate::eprintln!("{}", line)
}

pub fn flush() -> bsmr_error::Result<()> {
    stdout()?.flush().map_err(|e| ClientIoError::from(e).into())
}

fn stdout_to_file(stdout: &Stdout) -> bsmr_error::Result<File> {
    #[cfg(not(windows))]
    {
        use std::os::fd::AsFd;
...
pub async fn print_with_writer<E, F>(f: F) -> bsmr_error::Result<()>
where
    E: Into<bsmr_error::Error>,
