...
pub struct ImmediateConfigContext<'a> {
    ...
    // us to have a shared reference to the underlying `bsmr_error::Error` which
...
impl<'a> ImmediateConfigContext<'a> {
    ...
    pub fn daemon_startup_config(&self) -> bsmr_error::Result<&DaemonStartupConfig> {
        Ok(&self.data()?.daemon_startup_config)
    }

    pub fn allow_daemon_start_unsandboxed_via_wrapper(&self) -> bsmr_error::Result<bool> {
        #[cfg(fbcode_build)]
        {
    ...
    pub fn resolve_alias_to_path_in_cwd(&self, alias: &str) -> bsmr_error::Result<CellRootPathBuf> {
        let data = self.data()?;
        let cell = data.cwd_cell_alias_resolver.resolve(alias)?;
        Ok(data.cell_resolver.get(cell)?.path().to_buf())
    }

    fn data(&self) -> bsmr_error::Result<&ImmediateConfigContextData> {
        self.data
            .get_or_try_init(|| {
                ...
                bsmr_error::Ok(ImmediateConfigContextData {
    ...
    pub(crate) fn resolve_argfile_kind(
        ...
        flag: Option<&str>,
    ) -> Result<bsmr_common::argv::ArgFileKind, bsmr_error::Error> {
