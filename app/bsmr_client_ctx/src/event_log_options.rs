...
use bsmr_event_log::utils::Encoding;
...
impl EventLogOptions {
    pub async fn get(&self, ctx: &ClientCommandContext<'_>) -> bsmr_error::Result<EventLogPathBuf> {
    ...
    async fn download_remote_id(
        ...
        let log_file_name = FileNameBuf::try_from(format!(
            "{}{}",
            trace_id,
            // TODO(nga): hardcoded default, should at least use the same default bsmr uses,
