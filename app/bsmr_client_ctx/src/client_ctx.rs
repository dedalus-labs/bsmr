...
use bsmr_cli_proto::client_context::ExitWhen as GrpcExitWhen;
use bsmr_cli_proto::client_context::HostArchOverride as GrpcHostArchOverride;
use bsmr_cli_proto::client_context::HostPlatformOverride as GrpcHostPlatformOverride;
use bsmr_cli_proto::client_context::PreemptibleWhen as GrpcPreemptibleWhen;
...
pub struct ClientCommandContext<'a> {
    ...
    pub(crate) start_in_process_daemon:
        Option<Box<dyn FnOnce() -> bsmr_error::Result<()> + Send + Sync>>,
    pub(crate) argv: Argv,
...
impl<'a> ClientCommandContext<'a> {
    pub fn new(
        ...
        start_in_process_daemon: Option<Box<dyn FnOnce() -> bsmr_error::Result<()> + Send + Sync>>,
        argv: Argv,
        trace_id: TraceId,
        stdin: &'a mut Stdin,
    ...
    pub fn paths(&self) -> bsmr_error::Result<&InvocationPaths> {
        match &self.paths {
    ...
    pub fn maybe_paths(&self) -> bsmr_error::Result<Option<&InvocationPaths>> {
        match &self.paths {
    ...
    pub fn empty_client_context(&self, command_name: &str) -> bsmr_error::Result<ClientContext> {
        ...
        Ok(ClientContext {
            ...
            argfiles: Vec::new(),
            bsmr_hard_error: bsmr_hard_error_env()?.unwrap_or_default().to_owned(),
            bsmr_show_soft_errors: bsmr_show_soft_errors_env()?.unwrap_or_default().to_owned(),
            ...
            agent_context: self
                .agent_context
                .iter()
                .map(|e| bsmr_data::AgentContextEntry {
                    key: e.key.clone(),
                    value: e.value.clone(),
                })
                .collect(),
    ...
    pub fn log_download_method(&self) -> bsmr_error::Result<LogDownloadMethod> {
        Ok(self
            .immediate_config
