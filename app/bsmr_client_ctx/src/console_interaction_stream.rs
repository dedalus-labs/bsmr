...
mod interactive_terminal {
    ...
    impl InteractiveTerminal {
        pub fn enable() -> bsmr_error::Result<Option<Self>> {
...
mod interactive_terminal {
    ...
    fn get_console_mode(handle: HANDLE) -> bsmr_error::Result<u32> {
        let mut mode: u32 = 0;
    ...
    fn set_console_mode(handle: HANDLE, mode: u32) -> bsmr_error::Result<()> {
...
pub trait SuperConsoleInteraction: Send + Sync {
    async fn toggle(&mut self) -> bsmr_error::Result<Option<SuperConsoleToggle>>;
}
...
impl SuperConsoleInteraction for ConsoleInteractionStream<'_> {
    async fn toggle(&mut self) -> bsmr_error::Result<Option<SuperConsoleToggle>> {
        match self.stdin.read_u8().await {
            ...
            Err(e) => Err(bsmr_error::Error::from(e).context("Error reading char from console")),
        }
    }
}
