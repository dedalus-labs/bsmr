...
enum BuckdCommunicationError {
    ...
    #[error(
        "The Bessemer daemon was shut down while executing your command. This happened because: {0}"
    )]
...
pub trait PartialResultHandler {
    type PartialResult: TryFrom<
            bsmr_cli_proto::partial_result::PartialResult,
            Error = bsmr_cli_proto::partial_result::PartialResult,
        >;
...
impl PartialResultCtx<'_> {
    pub async fn stdout(&mut self, bytes: &[u8]) -> bsmr_error::Result<()> {
        self.inner
...
/// Manages incoming event streams from the daemon for the bsmr client and
...
impl<'a> DaemonEventsCtx<'a> {
    ...
    async fn handle_stream_next<Handler>(
        ...
        next: Option<Vec<bsmr_error::Result<StreamValue>>>,
        shutdown: &mut Option<bsmr_data::DaemonShutdown>,
        last_yield: &mut Instant,
    ) -> bsmr_error::Result<ControlFlow<Box<CommandResult>, ()>>
    where
    ...
    async fn dispatch_tailer_event(&mut self, event: FileTailerEvent) -> bsmr_error::Result<()> {
        match event {
            FileTailerEvent::Stdout(out) | FileTailerEvent::Stderr(out) => {
                ...
                // But if daemon does, it should not be used as standard output of bsmr.
    ...
    where
        S: Stream<Item = bsmr_error::Result<StreamValue>>,
        Handler: PartialResultHandler,
    {
        ...
        let command_result: bsmr_error::Result<CommandResult> = try {
            loop {
                tokio::select! {
                    next = stream.next() => {
    ...
    async fn handle_error_owned(&mut self, error: bsmr_error::Error) -> bsmr_error::Error {
        let result: Result<(), bsmr_error::Error> = self
            .inner
    ...
    pub async fn flush(&mut self) -> bsmr_error::Result<()> {
...
fn convert_result<R: TryFrom<command_result::Result, Error = command_result::Result>>(
    value: CommandResult,
) -> bsmr_error::Result<CommandOutcome<R>> {
    match value.result {
...
pub struct EventsCtx {
    ...
    // which is always available. This differs from `daemon_cgroup_path` in bsmr_resource_control
...
impl EventsCtx {
    ...
    pub(crate) async fn is_daemon_oom_killed(&self) -> bsmr_error::Result<bool> {
        #[cfg(target_os = "linux")]
        {
            let Some(path) = self.cgroup_path_of_bsmr_daemon.as_deref() else {
                return Ok(false);
            };
    ...
    async fn handle_tailer_stderr(&mut self, stderr: &[u8]) -> bsmr_error::Result<()> {
        ...
        let events = events.into_map(|mut event| {
            ...
            if let bsmr_data::buck_event::Data::Instant(instant_event) = event.data_mut() {
                match &mut instant_event.data {
                    ...
                    Some(bsmr_data::instant_event::Data::DaemonShutdown(msg)) => {
    ...
    where
        Fut: Future<Output = bsmr_error::Result<()>> + 'b,
    {
    ...
    pub(crate) fn handle_daemon_started(&mut self, reason: bsmr_data::DaemonWasStartedReason) {
    ...
    pub(crate) async fn eprintln(&mut self, message: &str) -> bsmr_error::Result<()> {
