...
impl AgentContextEntry {
    pub fn to_proto(&self) -> bsmr_data::AgentContextEntry {
        bsmr_data::AgentContextEntry {
            key: self.key.clone(),
    ...
    pub fn from_env() -> bsmr_error::Result<Vec<Self>> {
        let Some(meta) = bsmr_env!("CODING_AGENT_METADATA")? else {
            return Ok(vec![]);
        };
...
fn parse_agent_env_metadata(meta: &str) -> Vec<AgentContextEntry> {
    ...
    let mut result = vec![AgentContextEntry {
        key: bsmr_data::AgentContextEntry::KEY_ID.to_owned(),
        value: find(bsmr_data::AgentContextEntry::KEY_ID)
            .unwrap_or_else(|| "unknown_agent".to_owned()),
    }];
    if let Some(inv_id) = find(bsmr_data::AgentContextEntry::KEY_INVOCATION_ID) {
        result.push(AgentContextEntry {
            key: bsmr_data::AgentContextEntry::KEY_INVOCATION_ID.to_owned(),
            value: inv_id,
        });
    }
    result
...
pub fn parse_agent_context(value: &str) -> bsmr_error::Result<AgentContextEntry> {
...
mod tests {
    ...
    fn test_env_metadata_claude_code() {
        ...
        assert_eq!(result[0].key, bsmr_data::AgentContextEntry::KEY_ID);
        assert_eq!(result[0].value, "claude_code");
        assert_eq!(
            result[1].key,
            bsmr_data::AgentContextEntry::KEY_INVOCATION_ID
        );
        assert_eq!(
            result[1].value,
