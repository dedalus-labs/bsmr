...
impl StartupDeadline {
    pub fn duration_from_now(duration: Duration) -> bsmr_error::Result<StartupDeadline> {
        Ok(StartupDeadline {
    ...
    pub(crate) fn down_deadline(&self) -> bsmr_error::Result<StartupDeadline> {
    ...
    pub(crate) fn rem_duration(&self, op: &str) -> bsmr_error::Result<Duration> {
        self.deadline
    ...
    pub(crate) async fn down<R, Fut, F>(&self, op: &str, f: F) -> bsmr_error::Result<R>
    where
        F: FnOnce(StartupDeadline) -> Fut,
        Fut: Future<Output = bsmr_error::Result<R>>,
    {
    ...
    pub(crate) async fn run<R, Fut>(&self, op: &str, f: Fut) -> bsmr_error::Result<R>
    where
        Fut: Future<Output = bsmr_error::Result<R>>,
    {
    ...
    pub(crate) fn min(&self, other: Duration) -> bsmr_error::Result<StartupDeadline> {
    ...
    pub(crate) fn half(&self) -> bsmr_error::Result<StartupDeadline> {
        let now = Instant::now();
