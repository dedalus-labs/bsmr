...
impl FinalConsole {
    ...
    fn stderr_colored_ln(&self, message: &str, color: Color) -> bsmr_error::Result<()> {
        self.stderr_colored(message, color)?;
        crate::eprintln!()?;
        Ok(())
    }

    fn stderr_colored(&self, message: &str, color: Color) -> bsmr_error::Result<()> {
        if self.is_tty {
