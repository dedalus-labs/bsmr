...
use bsmr_cli_proto::representative_config_flag::Source as RepresentativeConfigFlagSource;
use bsmr_common::argv::ExpandedArgSource;
use bsmr_common::argv::ExpandedArgv;
...
pub struct CommonBuildConfigurationOptions {
    ...
    ///
    /// Normally, when you run two commands - from different terminals, say - bsmr will attempt
    /// to run them in parallel. However, if the two commands are based on different state, that
    /// is they either have different configs or different filesystem states, bsmr cannot run them
    ...
    pub fn config_overrides(
        ...
        fn with_indices<'a, T>(
            ...
            if indices.len() != collection.len() {
                return Err(bsmr_error::Error::from(IndicesLengthMismatchError {
...
impl CommonStarlarkOptions {
    ...
    ) -> Option<bsmr_cli_proto::client_context::ProfilePatternOptions> {
        self.profile_patterns.as_ref().map(|v| {
            bsmr_cli_proto::client_context::ProfilePatternOptions {
...
impl<'a> BuckArgMatches<'a> {
    ...
    pub fn get_representative_config_flags(&self) -> Vec<String> {
        bsmr_common::argv::get_representative_config_flags(self.expanded_argv)
    }

    pub fn get_representative_config_flags_by_source(&self) -> Vec<RepresentativeConfigFlag> {
        use bsmr_common::argv::ConfigFlagValue;
        use bsmr_common::argv::get_flagfile_for_logging;
        use bsmr_common::argv::parse_config_flags;
...
mod tests {
    use bsmr_cli_proto::RepresentativeConfigFlag;
    use bsmr_cli_proto::representative_config_flag::Source as RepresentativeConfigFlagSource;
    use bsmr_common::argv::ArgFileKind;
    use bsmr_common::argv::ArgFilePath;
    use bsmr_common::argv::ExpandedArgvBuilder;
    ...
    #[test]
    fn test_by_source_inline_flags() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_by_source_argfile_collapsing() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_config_overrides_with_default_opts_and_unregistered_args() -> bsmr_error::Result<()> {
