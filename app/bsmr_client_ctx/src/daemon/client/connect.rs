...
use bsmr_resource_control::spawn_daemon::create_daemon_spawn_command;
use bsmr_util::truncate::truncate;
use bsmr_wrapper_common::kill::process_exists;
...
impl ConstraintUnsatisfiedReason {
    pub(crate) fn to_daemon_was_started_reason(&self) -> bsmr_data::DaemonWasStartedReason {
        match self {
            ConstraintUnsatisfiedReason::Version => {
                bsmr_data::DaemonWasStartedReason::ConstraintMismatchVersion
            }
            ConstraintUnsatisfiedReason::UserVersion => {
                bsmr_data::DaemonWasStartedReason::ConstraintMismatchUserVersion
            }
            ConstraintUnsatisfiedReason::StartupConfig => {
                bsmr_data::DaemonWasStartedReason::ConstraintMismatchStartupConfig
            }
            ConstraintUnsatisfiedReason::RejectDaemonId => {
                bsmr_data::DaemonWasStartedReason::ConstraintRejectDaemonId
            }
            ConstraintUnsatisfiedReason::TraceIo => {
                bsmr_data::DaemonWasStartedReason::ConstraintMismatchTraceIo
            }
            ConstraintUnsatisfiedReason::SqliteIdentity => {
                bsmr_data::DaemonWasStartedReason::ConstraintMismatchSqliteIdentity
            }
        }
    }
}
...
impl DaemonConstraintsRequest {
    ...
    fn satisfied(
        &self,
        daemon: &bsmr_cli_proto::DaemonConstraints,
...
impl<'a> BuckdLifecycle<'a> {
    ...
    async fn start_server(&self, options: &BuckdConnectDaemonOptions) -> bsmr_error::Result<()> {
        ...
        daemon_env_vars.push((
            ...
            OsStr::new(bsmr_env!("BSMR_LIB_BACKTRACE")?.unwrap_or("0")),
        ));
...
impl BootstrapBuckdClient {
    ...
    pub(crate) async fn kill(&mut self, reason: &str) -> bsmr_error::Result<Pid> {
...
fn explain_failed_to_connect_reason(reason: bsmr_data::DaemonWasStartedReason) -> &'static str {
    match reason {
...
async fn establish_connection_inner(
            ...
            Ok(Some(buckd_info)) => {
                match try_connect_existing(&buckd_info, &deadline, &lifecycle_lock).await {
                    Ok(channel) => {
                        ...
                        events_ctx
                            .eprintln(&format!(
                                "bsmr daemon constraint mismatch: {reason}; killing daemon..."
                            ))
                            .await?;
                    ...
                    Err(reason) => {
                        events_ctx
                            .eprintln(&format!(
                                "Could not connect to bsmr daemon ({}), killing daemon..",
    ...
    deadline
        .down(
            &format!(
                "starting new bsmr daemon for reason: {}",
...
async fn start_new_buckd_and_connect(
    ...
    events_ctx.eprintln("Connected to new bsmr daemon.").await?;
    ...
    match tokio::time::timeout(rem_duration, buckd_info.create_channel()).await {
        ...
        Ok(Err(_)) => {
            ...
            if !buckd_process_exists {
                ...
                Err(bsmr_data::DaemonWasStartedReason::CouldNotConnectToDaemon)
            }
        }
        Err(e) => {
            let _assert_type: tokio::time::error::Elapsed = e;
            Err(bsmr_data::DaemonWasStartedReason::TimedOutConnectingToDaemon)
        }
    }
}
...
impl<'a> BuckdProcessInfo<'a> {
    ...
    pub async fn hard_kill(&self) -> bsmr_error::Result<()> {
        kill::hard_kill(&self.info).await
    }

    pub fn pid(&self) -> bsmr_error::Result<Pid> {
        Pid::from_i64(self.info.pid)
    }
}
...
async fn get_constraints(
    ...
    let status = DaemonEventsCtx::without_tailers(&mut events_ctx)
        .unpack_oneshot({
            client.status(tonic::Request::new(bsmr_cli_proto::StatusRequest {
                snapshot: false,
    ...
    let status: bsmr_cli_proto::StatusResponse = match status {
        CommandOutcome::Success(r) => Ok(r),
        CommandOutcome::Failure(_) => Err(bsmr_error!(
...
enum BuckdConnectError {
    ...
    BuckDaemonStartupFailed {
        ...
        stderr: String,
        #[source]
        exit_status_error: bsmr_error::Error,
    },
...
async fn daemon_connect_error(
    ...
    let error_report: Result<bsmr_data::ErrorReport, RetryError<bsmr_error::Error>> = retrying(
    ...
    let error = if let Ok(error_report) = error_report {
        ...
        let daemon_error: bsmr_error::Error = error_report.into();
...
fn is_nested_invocation(
    bsmr_daemon_uuid: Option<&String>,
    daemon: &bsmr_cli_proto::DaemonConstraints,
) -> bool {
    bsmr_daemon_uuid == Some(&daemon.daemon_id)
}
...
mod tests {
    ...
    fn constraints(trace_io_enabled: bool) -> bsmr_cli_proto::DaemonConstraints {
        bsmr_cli_proto::DaemonConstraints {
            version: "version".to_owned(),
            user_version: Some("test".to_owned()),
            daemon_id: "foo".to_owned(),
            extra: Some(bsmr_cli_proto::ExtraDaemonConstraints {
                trace_io_enabled,
    ...
    fn test_reject_daemon() {
        ...
        let daemon = bsmr_cli_proto::DaemonConstraints {
            version: "foo".to_owned(),
            user_version: None,
