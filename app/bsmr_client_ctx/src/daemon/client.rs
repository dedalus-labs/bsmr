...
impl BuckdClientConnector {
    ...
    pub fn daemon_constraints(&self) -> &bsmr_cli_proto::DaemonConstraints {
...
pub(crate) fn tonic_status_to_error(status: tonic::Status) -> bsmr_error::Error {
    ...
    bsmr_error::Error::from(status).tag(tags)
}

/// Translates a tonic streaming response into a stream of StreamValues, the set of things that can flow across the gRPC
/// event stream.
fn grpc_to_stream(
    response: bsmr_error::Result<tonic::Response<tonic::Streaming<MultiCommandProgress>>>,
) -> impl Stream<Item = bsmr_error::Result<StreamValue>> {
    ...
    let stream = stream
        ...
        .map_ok(|e| stream::iter(e.messages.into_iter().map(bsmr_error::Ok)))
        .try_flatten();
...
impl BuckdClient {
    ...
    pub async fn status<'a>(
    ...
    ) -> bsmr_error::Result<StatusResponse> {
        let outcome = events_ctx
            ...
            .await;
        // TODO(nmj): We have a number of things that wish to use status() and return an bsmr_error::Result,
        // for now we'll just turn a "CommandMessage" into a error, but that's really not what we
        // want long term.
        match outcome? {
            CommandOutcome::Success(r) => Ok(r),
            CommandOutcome::Failure(_) => Err(bsmr_error::bsmr_error!(
                bsmr_error::ErrorTag::DaemonStatus,
...
impl TryFrom<bsmr_cli_proto::partial_result::PartialResult> for NoPartialResult {
    type Error = bsmr_cli_proto::partial_result::PartialResult;

    fn try_from(v: bsmr_cli_proto::partial_result::PartialResult) -> Result<Self, Self::Error> {
        Err(v)
    }
}
...
impl FlushingBuckdClient<'_> {
    ...
    stream_method!(bxl, BxlRequest, BxlResponse, bsmr_cli_proto::StdoutBytes);
    ...
    pub async fn new_generic(
        &mut self,
        context: bsmr_cli_proto::ClientContext,
        ...
        let command_outcome: CommandOutcome<bsmr_cli_proto::NewGenericResponseMessage> = self
