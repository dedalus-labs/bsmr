...
pub(crate) trait DebugServer {
    ...
    fn attach(&mut self, x: dap::AttachRequestArguments) -> bsmr_error::Result<()>;
    ...
    fn disconnect(&mut self, x: dap::DisconnectArguments) -> bsmr_error::Result<()>;

    /// See <https://microsoft.github.io/debug-adapter-protocol/specification#Requests_Source>
    fn source(&mut self, x: dap::SourceArguments) -> bsmr_error::Result<dap::SourceResponseBody>;
}
...
pub(crate) fn err_response(req: &dap::Request, err: &bsmr_error::Error) -> dap::Response {
    dap::Response {
...
pub(crate) fn dispatch(
    ...
    r: &dap::Request,
) -> bsmr_error::Result<dap::Response> {
    fn arg<T: for<'a> Deserialize<'a>>(r: &dap::Request) -> bsmr_error::Result<T> {
    ...
    fn ret_some<T: Serialize>(
        r: &dap::Request,
        v: bsmr_error::Result<T>,
    ) -> bsmr_error::Result<dap::Response> {
        ret(r, v.map(|v| Some(v)))
    }

    fn ret_none(r: &dap::Request, v: bsmr_error::Result<()>) -> bsmr_error::Result<dap::Response> {
        ret::<()>(r, v.map(|_| None))
    }
    ...
    match r.command.as_str() {
        ...
        _ => Err(bsmr_error::bsmr_error!(
            bsmr_error::ErrorTag::Input,
            "Bessemer debugserver didn't recognize command: {}",
            r.command
        )),
    }
}
