...
/// Messages from the debugger server to its client (the cli `bsmr starlark debug-attach` which
/// then forwards them along through its stdout).
#[derive(Debug)]
pub(crate) enum ToClientMessage {
    Event(dap::Event),
    Response(dap::Response),
    Shutdown(bsmr_error::Result<()>),
}

impl ToClientMessage {
    fn pretty_string(&self) -> bsmr_error::Result<String> {
        match self {
...
pub async fn run_dap_server_command(
    ...
    partial_result_dispatcher: PartialResultDispatcher<bsmr_cli_proto::DapMessage>,
    req: StreamingRequestHandler<bsmr_cli_proto::DapRequest>,
) -> bsmr_error::Result<bsmr_cli_proto::DapResponse> {
    let start_event = ctx
        .command_start_event(bsmr_data::StarlarkDebugAttachCommandStart {}.into())
        .await?;
    span_async(start_event, async move {
        let result = run_dap_server(ctx, partial_result_dispatcher, req).await;
        let end_event = command_end(&result, bsmr_data::StarlarkDebugAttachCommandEnd {});
        (result, end_event)
    })
    .await
}
...
async fn run_dap_server(
    ctx: &dyn ServerCommandContextTrait,
    mut partial_result_dispatcher: PartialResultDispatcher<bsmr_cli_proto::DapMessage>,
    mut req: StreamingRequestHandler<bsmr_cli_proto::DapRequest>,
) -> bsmr_error::Result<bsmr_cli_proto::DapResponse> {
        ...
        select! {
            request = req.next() => {
                let request = match request {
                    ...
                    None => {
                        // client disconnected.
                        break bsmr_cli_proto::DapResponse {};
                    }
                };
...
fn handle_outgoing_message(
    seq: &mut u32,
    partial_result_dispatcher: &mut PartialResultDispatcher<bsmr_cli_proto::DapMessage>,
    message: ToClientMessage,
) -> bsmr_error::Result<Option<bsmr_cli_proto::DapResponse>> {
    ...
    let (dap_json, response) = match message {
        ...
        ToClientMessage::Shutdown(res) => {
            ...
            (
                ...
                Some(bsmr_cli_proto::DapResponse {}),
            )
        }
    };

    partial_result_dispatcher.emit(bsmr_cli_proto::DapMessage { dap_json });
    Ok(response)
}
