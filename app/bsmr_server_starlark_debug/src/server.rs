...
impl BuckStarlarkDebuggerServer {
    pub(crate) fn new(
        ...
        Self {
            to_state,
            eval_semaphore: Arc::new(Semaphore::new(bsmr_util::threads::available_parallelism())),
    ...
    pub(crate) fn send_request(&self, req: dap::Request) -> bsmr_error::Result<()> {
    ...
    pub(crate) fn detach(&self) -> bsmr_error::Result<()> {
...
impl VariableId {
    ...
    pub fn new(frame_id: u32, thread_id: u32, variable_id: u32) -> bsmr_error::Result<Self> {
        if thread_id > Self::MAX_THREAD_ID {
            return Err(bsmr_error::bsmr_error!(
                bsmr_error::ErrorTag::StarlarkServer,
...
impl DebugServer for ServerState {
    ...
    ) -> bsmr_error::Result<Option<serde_json::Value>> {
    ...
    fn set_breakpoints(
        ...
        let resolved = resolve_breakpoints(&x, &self.get_ast(&project_relative)?)
            .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::StarlarkServer))?;
    ...
    fn attach(&mut self, _x: dap::AttachRequestArguments) -> bsmr_error::Result<()> {
        Ok(())
    }

    fn threads(&mut self) -> bsmr_error::Result<dap::ThreadsResponseBody> {
    ...
    fn configuration_done(&mut self) -> bsmr_error::Result<()> {
        Ok(())
    }
    ...
    ) -> bsmr_error::Result<dap::StackTraceResponseBody> {
    ...
    fn scopes(&mut self, x: dap::ScopesArguments) -> bsmr_error::Result<dap::ScopesResponseBody> {
    ...
    ) -> bsmr_error::Result<dap::VariablesResponseBody> {
    ...
    fn source(&mut self, _x: dap::SourceArguments) -> bsmr_error::Result<dap::SourceResponseBody> {
        Err(StarlarkDebuggerError::Unimplemented.into())
    }

    fn continue_(&mut self, x: ContinueArguments) -> bsmr_error::Result<dap::ContinueResponseBody> {
        let hook = self.find_hook_by_pseudo_thread(x.thread_id)?;
        hook.adapter
            .continue_()
            .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::StarlarkServer))?;
    ...
    fn next(&mut self, x: dap::NextArguments) -> bsmr_error::Result<()> {
        let hook = self.find_hook_by_pseudo_thread(x.thread_id)?;
        hook.adapter
            .step(StepKind::Over)
            .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::StarlarkServer))?;
    ...
    fn step_in(&mut self, x: dap::StepInArguments) -> bsmr_error::Result<()> {
        let hook = self.find_hook_by_pseudo_thread(x.thread_id)?;
        hook.adapter
            .step(StepKind::Into)
            .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::StarlarkServer))?;
    ...
    fn step_out(&mut self, x: dap::StepOutArguments) -> bsmr_error::Result<()> {
        let hook = self.find_hook_by_pseudo_thread(x.thread_id)?;
        hook.adapter
            .step(StepKind::Out)
            .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::StarlarkServer))?;
    ...
    ) -> bsmr_error::Result<dap::EvaluateResponseBody> {
    ...
    fn disconnect(&mut self, _x: dap::DisconnectArguments) -> bsmr_error::Result<()> {
        Ok(())
    }
}
...
impl ServerState {
    ...
    fn get_snapshot(&self) -> bsmr_data::DebugAdapterSnapshot {
        ...
        for hook_state in self.current_hooks.values() {
            if let Some(v) = &hook_state.stopped_at {
                let handle_snapshot = current_handles
                    ...
                    .or_insert_with(|| bsmr_data::DebugAdapterCommandSnapshot {
                        stopped_evals: Vec::new(),
                    });
                handle_snapshot
                    .stopped_evals
                    .push(bsmr_data::DebugAdapterStoppedEval {
        ...
        bsmr_data::DebugAdapterSnapshot {
    ...
    fn handle_message(&mut self, msg: ServerMessage) -> bsmr_error::Result<bool> {
        match msg {
            ServerMessage::NewHook {
                ...
                response_channel.send(resp).map_err(|_| {
                    bsmr_error::bsmr_error!(bsmr_error::ErrorTag::StarlarkServer, "channel closed")
                })?;
            }
    ...
    fn drop_hook(&mut self, hook_id: HookId) -> bsmr_error::Result<()> {
    ...
    fn eval_stopped(&mut self, hook_id: HookId) -> bsmr_error::Result<()> {
    ...
    fn find_hook_by_pseudo_thread(&self, thread_id: i64) -> bsmr_error::Result<&HookState> {
    ...
    fn get_ast(&self, source: &ProjectRelativePath) -> bsmr_error::Result<AstModule> {
