...
mod unix {
    ...
    enum StoppedWaiting {
        Success,
        UnexpectedError(bsmr_error::Error),
    }
    ...
    fn check_result(pid: nix::unistd::Pid, result: nix::Result<()>) -> ControlFlow<StoppedWaiting> {
        match result {
            ...
            Err(e) => ControlFlow::Break(StoppedWaiting::UnexpectedError(bsmr_error::bsmr_error!(
...
mod tests {
    ...
    #[tokio::test]
    async fn test_graceful_termination() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_sigkill_when_graceful_termination_fails() -> bsmr_error::Result<()> {
        let dir = tempfile::tempdir()?;
