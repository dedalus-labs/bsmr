...
use crate::invocation_roots::home_bsmr_dir;

/// `~/.bsmr/tmp` after old files removed.
///
/// We use this directory when we need tmp dir with short file names (to connect to unix socket).
pub fn home_temp_dir() -> bsmr_error::Result<&'static AbsNormPath> {
    ...
    fn find_dir() -> bsmr_error::Result<AbsNormPathBuf> {
        let home_bsmr_dir = home_bsmr_dir()?;
        let tmp_dir = home_bsmr_dir.join(FileName::new("tmp")?);
