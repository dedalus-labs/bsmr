...
#[cfg(unix)]
async fn get_channel_uds_no_symlink(connect_to: &Path) -> bsmr_error::Result<Channel> {
...
#[cfg(windows)]
pub async fn get_channel_uds(_unix_filename: &Path, _chg_dir: bool) -> bsmr_error::Result<Channel> {
    Err(bsmr_error::bsmr_error!(
        bsmr_error::ErrorTag::WindowsUnsupported,
        "Unix domain sockets are not supported on Windows",
    ))
}

pub async fn get_channel_tcp(socket_addr: Ipv4Addr, port: u16) -> bsmr_error::Result<Channel> {
...
mod tests {
    ...
    async fn test_retrying_error_forever() {
        ...
        let future = retrying(
            ...
            Duration::from_millis(1),
            Duration::from_millis(1),
            || async { Err(bsmr_error::bsmr_error!(bsmr_error::ErrorTag::Input, "test")) },
        );
        let result: Result<(), RetryError<bsmr_error::Error>> = future.await;
