...
#[derive(bsmr_error::Error, Debug, PartialEq, Eq)]
#[bsmr(tag = Input)]
enum SqliteDbError {
    ...
    VersionMismatch {
        expected: StdBsmrHashMap<String, String>,
        found: StdBsmrHashMap<String, String>,
        path: AbsNormPathBuf,
    },
...
pub trait SqliteTable {
    ...
    fn create_table(&self) -> bsmr_error::Result<()>;
}
...
pub trait SqliteDb {
    ...
    fn new(tables: SqliteTables<Self::TableType>) -> bsmr_error::Result<Self>
    where
        Self: Sized;

    /// Open and configure the database tables
    fn open_tables(path: &AbsNormPath) -> bsmr_error::Result<SqliteTables<Self::TableType>>;
    ...
    fn get_sqlite_db(
        ...
        mut current_instance_metadata: StdBsmrHashMap<String, String>,
...
pub struct SqliteTables<T: SqliteTable> {
    ...
    /// versions this bsmr binary expects. If the versions don't match, we throw
    ...
    /// Table for logging metadata associated with the bsmr that created the db.
...
impl<T: SqliteTable> SqliteTables<T> {
    ...
    pub fn get_identity(&self) -> bsmr_error::Result<SqliteIdentity> {
        let identity = self
            .created_by_table
            .get(IDENTITY_KEY)
            .bsmr_error_context("Error reading creation metadata")?
            .map(SqliteIdentity)
    ...
    pub fn create_all_tables(&self) -> bsmr_error::Result<()> {
    ...
    pub fn create_connection(path: &AbsNormPath) -> bsmr_error::Result<Arc<Mutex<Connection>>> {
...
mod tests {
    ...
    impl TestSqliteTable {
        ...
        fn insert(&self, value: &str) -> bsmr_error::Result<()> {
            let sql = "INSERT INTO test_data (value) VALUES (?1)";
            self.connection
                .lock()
                .execute(sql, [value])
                .bsmr_error_context("Failed to insert test data")?;
            Ok(())
        }

        fn read_all(&self) -> bsmr_error::Result<TestState> {
    ...
    impl SqliteTable for TestSqliteTable {
        fn create_table(&self) -> bsmr_error::Result<()> {
            ...
            self.connection
                .lock()
                .execute(sql, [])
                .bsmr_error_context("Failed to create test table")?;
            Ok(())
        }
    }
    ...
    impl TestSqliteDb {
        fn insert_test_data(&mut self, value: &str) -> bsmr_error::Result<()> {
            self.tables.domain_table.insert(value)
        }
    }

    fn create_test_versions() -> StdBsmrHashMap<String, String> {
        StdBsmrHashMap::from([
    ...
    fn create_test_metadata() -> StdBsmrHashMap<String, String> {
        StdBsmrHashMap::from([
    ...
    #[test]
    fn test_create_new_sqlite_db() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_load_existing_sqlite_db() -> bsmr_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let db_dir = fs
            .path()
            .resolve(ProjectRelativePath::unchecked_new("test_db_dir"));
    ...
    #[test]
    fn test_version_mismatch_error() -> bsmr_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let db_dir = fs
            .path()
            .resolve(ProjectRelativePath::unchecked_new("test_db_dir"));

        let v1_versions = StdBsmrHashMap::from([("schema_version".to_owned(), "1".to_owned())]);
        let v2_versions = StdBsmrHashMap::from([("schema_version".to_owned(), "2".to_owned())]);
        let metadata = create_test_metadata();
    ...
    #[test]
    fn test_rejected_identity() -> bsmr_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let db_dir = fs
            .path()
            .resolve(ProjectRelativePath::unchecked_new("test_db_dir"));
    ...
    #[test]
    fn test_path_does_not_exist_error() -> bsmr_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
    ...
    #[test]
    fn test_sqlite_tables_functionality() -> bsmr_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let db_path = fs
            .path()
        ...
        let test_versions = StdBsmrHashMap::from([
        ...
        let test_metadata = StdBsmrHashMap::from([
            (
