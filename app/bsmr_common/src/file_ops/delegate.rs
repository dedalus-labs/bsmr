...
use bsmr_core::cells::unchecked_cell_rel_path::UncheckedCellRelativePath;
...
impl Key for FileOpsKey {
    type Value = bsmr_error::Result<FileOpsValue>;
    async fn compute(
        &self,
...
) -> bsmr_error::Result<FileOpsDelegateWithIgnores> {
    Ok(dice
        .compute(&FileOpsKey {
            cell,
            check_ignores,
