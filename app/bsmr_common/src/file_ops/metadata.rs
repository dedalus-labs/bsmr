...
impl FileDigest {
    ...
    pub fn from_file(file: &AbsPath, config: FileDigestConfig) -> bsmr_error::Result<Self> {
        if !bsmr_env!("BSMR_DISABLE_FILE_ATTR", bool)? {
    ...
    fn from_file_disk(file: &AbsPath, config: FileDigestConfig) -> bsmr_error::Result<Self> {
