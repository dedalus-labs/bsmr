...
pub enum ResourceControlStatus {
    ...
    /// bsmr errors it out and the command returns with an error exit code.
    Required,
}
        ...
        Ok(Self {
            ...
            macos_qos_class: {
                let from_config = config
                    ...
                    .map(ToOwned::to_owned);
                if bsmr_env!("BSMR_DISABLE_MACOS_QOS", bool)? {
...
mod tests {
    ...
    #[test]
    fn test_daemon_idle_timeout_s_default() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_daemon_idle_timeout_s_configured() -> bsmr_error::Result<()> {
        let config = parse(
            &[(
                "config",
                indoc!(
