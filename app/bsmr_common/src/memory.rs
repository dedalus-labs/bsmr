...
//! Utilities for interacting with the jemalloc heap used by bsmr.
//!
...
mod imp {
    ...
    pub fn write_heap_to_file(filename: &str) -> bsmr_error::Result<()> {
        ...
        let prof_enabled: bool = memory::mallctl_read("opt.prof")
            .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::Mallctl))?;
        if !prof_enabled {
    ...
    pub fn allocator_stats(options: &str) -> bsmr_error::Result<String> {
        allocator_stats::malloc_stats(options)
            .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::MallocStats))
    }
    ...
    pub fn enable_background_threads() -> bsmr_error::Result<()> {
        if memory::is_using_jemalloc() {
    ...
    pub fn purge_jemalloc() -> bsmr_error::Result<()> {
        if !memory::is_using_jemalloc() {
            return Ok(());
        }
...
mod imp {
    pub fn write_heap_to_file(_filename: &str) -> bsmr_error::Result<()> {
    ...
    pub fn allocator_stats(_: &str) -> bsmr_error::Result<String> {
        Err(bsmr_error::bsmr_error!(
            bsmr_error::ErrorTag::Unimplemented,
