...
pub fn find_package_roots_stream(
    ctx: &DiceTransaction,
    paths: Vec<CellPath>,
) -> impl Stream<Item = bsmr_error::Result<PackageLabel>> + '_ {
...
pub async fn collect_package_roots<E>(
    ...
    paths: Vec<CellPath>,
    mut collector: impl FnMut(bsmr_error::Result<PackageLabel>) -> Result<(), E>,
) -> Result<(), E> {
    ...
    while let Some((path, listing)) = queue.next().await {
        let (buildfile_candidates, listing) = {
            let r = async {
                ...
                bsmr_error::Ok((buildfiles, listing?.included))
            }
            .await;
