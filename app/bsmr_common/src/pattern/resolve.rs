...
use bsmr_core::pattern::pattern::display_precise_pattern;
            ...
            let spec = match spec {
                PackageSpec::Targets(targets) => {
                    PackageSpec::Targets(targets.into_try_map(|(target_name, extra)| {
                        ...
                        bsmr_error::Ok((target_name, extra))
                    })?)
                }
...
impl ResolveTargetPatterns {
    ...
    ) -> bsmr_error::Result<ResolvedPattern<P>> {
...
mod tests {
    ...
    impl TestPatternResolver {
        fn new(cells: &[(&str, &str)], files: &[&str]) -> bsmr_error::Result<Self> {
            let resolver = {
        ...
        async fn resolve<T>(&self, patterns: &[&str]) -> bsmr_error::Result<ResolvedPattern<T>>
        where
            T: PatternType,
        {
        ...
        async fn resolve_with_modifiers<T>(
            &self,
            patterns: &[&str],
        ) -> bsmr_error::Result<ResolvedPattern<T>>
        where
            T: PatternType,
        {
    ...
    #[tokio::test]
    async fn test_simple_specs_targets() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_simple_specs_providers() -> bsmr_error::Result<()> {
        let tester = TestPatternResolver::new(&[("root", ""), ("child", "child/cell")], &[])?;
        tester
                            ...
                            ProvidersPatternExtra {
                                providers: ProvidersName::NonDefault(triomphe::Arc::new(
                                    NonDefaultProvidersName::Named(
                                        bsmr_util::arc_str::ArcSlice::new([ProviderName::new(
                                            "my-label".to_owned(),
                                        )
                                        .unwrap()]),
                                    ),
    ...
    #[tokio::test]
    async fn test_simple_specs_providers_with_modifiers() -> bsmr_error::Result<()> {
        let tester = TestPatternResolver::new(&[("root", ""), ("child", "child/cell")], &[])?;
        ...
        tester
            ...
            .assert_eq(&[(
                ...
                PackageSpec::Targets(vec![(
                    ...
                    ProvidersPatternExtra {
                        providers: ProvidersName::NonDefault(triomphe::Arc::new(
                            NonDefaultProvidersName::Named(bsmr_util::arc_str::ArcSlice::new([
