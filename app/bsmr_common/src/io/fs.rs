...
impl FsIoProvider {
    ...
    ) -> bsmr_error::Result<RawPathMetadata<ProjectRelativePathBuf>> {
        let fs = self.fs.dupe();
...
) -> bsmr_error::Result<Option<RawPathMetadata<ForwardRelativePathBuf>>> {
...
fn convert_metadata(
    ...
    file_digest_config: FileDigestConfig,
) -> bsmr_error::Result<RawPathMetadata<ForwardRelativePathBuf>> {
...
impl ExactPathMetadata {
    fn from_exact_path(curr: &PathAndAbsPath) -> bsmr_error::Result<Self> {
...
mod tests {
    ...
    #[test]
    fn test_read_not_symlink() -> bsmr_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_symlink() -> bsmr_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_symlink_in_dir() -> bsmr_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_symlink_remaining_path() -> bsmr_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_symlink_out_of_project() -> bsmr_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_unchecked_symlink() -> bsmr_error::Result<()> {
        let t = TempDir::new()?;
        let root = AbsPath::new(t.path())?;
