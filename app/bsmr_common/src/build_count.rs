...
impl BuildCountManager {
    ...
    pub fn new(base_dir: AbsNormPathBuf) -> bsmr_error::Result<Self> {
    ...
    async fn ensure_dir(&self) -> bsmr_error::Result<()> {
        async_fs_util::create_dir_all(&self.base_dir).await
    }

    async fn read(&self) -> bsmr_error::Result<Option<BuildCountMap>> {
    ...
    async fn read_mergebase(&self, merge_base: &str) -> bsmr_error::Result<BuildCountMap> {
    ...
    async fn write(&self, build_count: &BuildCountMap) -> bsmr_error::Result<()> {
        async_fs_util::write(&self.file_path, &serde_json::to_vec(build_count)?)
            .await
            .categorize_internal()
    }

    async fn lock_with_timeout(&self) -> bsmr_error::Result<FileLockGuard> {
        ...
        client_utils::retrying(
            ...
            Self::LOCK_TIMEOUT,
            || async { bsmr_error::Ok(fs4::fs_std::FileExt::try_lock_exclusive(fileref)?) },
        )
        .await?;
...
mod tests {
    ...
    fn make_patterns(targets: Vec<&'static str>) -> ParsedTargetPatterns {
        ParsedTargetPatterns {
            target_patterns: targets.into_map(|v| bsmr_data::TargetPattern {
                value: v.to_owned(),
            }),
        }
    }
    ...
    #[tokio::test]
    async fn test_read_no_such_file() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_read_normal_file() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_read_illegal_file_contents() -> bsmr_error::Result<()> {
        let temp_dir = tempfile::tempdir()?;
        let bcm = BuildCountManager::new(temp_dir.path().to_path_buf().try_into()?)?;
    ...
    #[tokio::test]
    async fn test_increment_normal_input() -> bsmr_error::Result<()> {
        let temp_dir = tempfile::tempdir()?;
    ...
    #[tokio::test]
    async fn test_increment_on_failure() -> bsmr_error::Result<()> {
        let temp_dir = tempfile::tempdir()?;
        let file_name = "some_file";
        tokio::fs::write(temp_dir.path().join(file_name), "{\"//some:target\":[1,1]}").await?;
        let target_patterns = make_patterns(vec!["//some:target", "//some/other:target"]);
    ...
    #[tokio::test]
    async fn test_increment_empty_input() -> bsmr_error::Result<()> {
        let temp_dir = tempfile::tempdir()?;
        let file_name = "some_file";
    ...
    #[tokio::test]
    async fn test_min_count_no_increment() -> bsmr_error::Result<()> {
        let temp_dir = tempfile::tempdir()?;
        let file_name = "some_file";
        let target_patterns = make_patterns(vec!["//some:target", "//some/other:target"]);
        let bcm = BuildCountManager::new(temp_dir.path().to_path_buf().try_into()?)?;
    ...
    #[tokio::test]
    async fn test_rebase() -> bsmr_error::Result<()> {
        let temp_dir = tempfile::tempdir()?;
