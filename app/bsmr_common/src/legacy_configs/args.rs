...
fn resolve_config_flag_arg(
    ...
    raw_arg: &str,
) -> bsmr_error::Result<ResolvedConfigFlag> {
...
) -> bsmr_error::Result<ResolvedConfigFile> {
...
pub(crate) async fn resolve_config_args(
    args: &[ConfigOverride],
    file_ops: &mut dyn ConfigParserFileOps,
) -> bsmr_error::Result<Vec<ResolvedLegacyConfigArg>> {
        ...
        .map(|arg| {
            let data = match arg {
                ResolvedLegacyConfigArg::Flag(resolved_config_flag) => {
                    ConfigValue(bsmr_data::ConfigValue {
...
mod tests {
    ...
    #[test]
    fn test_argument_pair() -> bsmr_error::Result<()> {
        // Valid Formats
