...
impl LegacyConfigParser {
    ...
    pub(crate) fn to_proto_external_config_values(
        &self,
        is_cli: bool,
    ) -> Vec<bsmr_data::ConfigValue> {
        self.values
            .iter()
            .flat_map(|(k, v)| {
                v.values.iter().map(|(key, value)| bsmr_data::ConfigValue {
                    section: k.to_owned(),
                    key: key.to_owned(),
...
impl<'p> LegacyConfigFileParser<'p> {
    ...
    fn push_file(&mut self, line: usize, path: &ConfigPath) -> bsmr_error::Result<()> {
    ...
    fn parse_section_marker(line: &str) -> bsmr_error::Result<Option<&str>> {
