...
impl ConfigPath {
    ...
    pub(crate) fn join_to_parent_normalized(&self, rel: &RelativePath) -> bsmr_error::Result<Self> {
        match self {
...
pub trait ConfigParserFileOps: Send + Sync {
    ...
    async fn read_dir(&mut self, path: &ConfigPath) -> bsmr_error::Result<Vec<ConfigDirEntry>>;
}
...
mod tests {
    ...
    #[test]
    fn dir_with_file() -> bsmr_error::Result<()> {
        let mut v = vec![];
    ...
    #[test]
    fn empty_dir() -> bsmr_error::Result<()> {
        let mut v = vec![];
        let dir = tempfile::tempdir()?;
        let dir = AbsPath::new(dir.path())?;
    ...
    #[test]
    fn non_existent_dir() -> bsmr_error::Result<()> {
        let mut v = vec![];
        let dir = tempfile::tempdir()?;
    ...
    #[test]
    fn dir_in_dir() -> bsmr_error::Result<()> {
        let mut v = vec![];
        let dir = tempfile::tempdir()?;
        let dir = AbsPath::new(dir.path())?;
    ...
    #[test]
    fn dir_with_file_in_dir() -> bsmr_error::Result<()> {
        let mut v = vec![];
        let dir = tempfile::tempdir()?;
        let dir = AbsPath::new(dir.path())?;
