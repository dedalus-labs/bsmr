...
//!
//! Defines utilities to obtain the basic paths for bsmr client and the daemon.
...
impl InvocationPaths {
    pub fn daemon_dir(&self) -> bsmr_error::Result<DaemonDir> {
        #[cfg(windows)]
        ...
        let root_relative: Cow<ForwardRelativePath> = self
            ...
            .project_root
            .root()
            .strip_prefix(bsmr_fs::paths::abs_norm_path::AbsNormPath::new("/")?)?;
