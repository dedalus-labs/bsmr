...
use bsmr_util::cycle_detector::CycleDescriptor;
use bsmr_util::cycle_detector::LazyCycleDetector;
use bsmr_util::cycle_detector::LazyCycleDetectorGuard;
...
pub struct CycleGuardResult<R, E>(bsmr_error::Result<Result<R, E>>);

impl<R, E> CycleGuardResult<R, E> {
    /// Converts the result from GuardThis into an bsmr_error::Result.
    ///
...
impl<D: CycleAdapterDescriptor> CycleGuard<D> {
    pub fn new(ctx: &DiceComputations<'_>) -> bsmr_error::Result<Self> {
    ...
    pub async fn guard_this<R: Send, Fut: Future<Output = R> + Send>(
        ...
        let res: bsmr_error::Result<Result<R, D::Error>> = (|| async move {
            match &self.0 {
