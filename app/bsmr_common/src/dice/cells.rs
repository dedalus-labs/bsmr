...
pub trait HasCellResolver {
    async fn get_cell_resolver(&mut self) -> bsmr_error::Result<CellResolver>;

    async fn is_cell_resolver_key_set(&mut self) -> bsmr_error::Result<bool>;
...
pub trait SetCellResolver {
    fn set_cell_resolver(&mut self, cell_resolver: CellResolver) -> bsmr_error::Result<()>;

    fn set_none_cell_resolver(&mut self) -> bsmr_error::Result<()>;
}
...
impl Key for CellAliasResolverKey {
    type Value = bsmr_error::Result<CellAliasResolver>;
