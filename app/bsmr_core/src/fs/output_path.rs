...
/// of a `Package`. The `OutputPath` refers to only the outputs of rules,
...
impl BuildArtifactPath {
    ...
    pub fn is_content_based_path(&self) -> bool {
        self.0.path_resolution_method == OutputPathKind::ContentHash
    }

    pub fn is_configuration_based_path(&self) -> bool {
        self.0.path_resolution_method == OutputPathKind::Configuration
    }
}

#[derive(Clone, Debug, Display, Eq, PartialEq)]
#[display("tmp/({})/{}", owner, path.as_str())]
pub struct ScratchOutputPath {
...
impl ScratchOutputPath {
    ...
    pub fn new(
        ...
        const MAKE_SENSIBLE_PREFIX: &str = "_bsmr_";
...
pub struct OutputPathResolver {
    output_isolation: ProjectRelativePathBuf,
}

impl OutputPathResolver {
    /// creates a 'OutputPathResolver' that will resolve outputs to the provided bsmr-out root.
    /// If not set, output defaults to "bsmr-out/default"
    pub fn new(output_isolation: ProjectRelativePathBuf) -> Self {
        OutputPathResolver { output_isolation }
    }

    /// Returns the bsmr-out root.
    pub fn root(&self) -> &ProjectRelativePath {
        &self.output_isolation
    }

    /// Resolves a 'OutputPath' into a 'ProjectRelativePath' based on the base
    ...
    /// Same as `resolve_gen`, except it also uses OutputPathKind::Configuration
    ...
    pub fn resolve_external_cell_source(
        ...
        ProjectRelativePathBuf::from(ForwardRelativePathBuf::concat([
            self.output_isolation.as_forward_relative_path(),
    ...
    /// Resolve a test path
    pub fn resolve_test(&self, path: &TestOutputPath) -> ProjectRelativePathBuf {
...
mod tests {
    ...
    use crate::fs::output_path::ScratchOutputPath;
    ...
    #[test]
    fn build_path_resolves() -> bsmr_error::Result<()> {
        ...
        let output_path_resolver = OutputPathResolver::new(ProjectRelativePathBuf::unchecked_new(
            "base/bsmr-out/default".into(),
        ));
    ...
    #[test]
    fn bsmr_output_path_resolves() -> bsmr_error::Result<()> {
        let path_resolver = OutputPathResolver::new(ProjectRelativePathBuf::unchecked_new(
            "base/bsmr-out/default".into(),
        ));
        ...
        let expected_gen_path = Regex::new(
            "base/bsmr-out/default/art/foo/[0-9a-f]{16}/baz-package/__target-name__/faz.file",
        )?;
        assert!(
        ...
        let expected_gen_content_based_path = Regex::new(
            "base/bsmr-out/default/art/foo/baz-package/__target-name__/0000000000000000/faz.file",
        )?;
        assert!(
        ...
        let expected_scratch_path =
            Regex::new("base/bsmr-out/default/tmp/foo/[0-9a-f]{16}/category/blah.file")?;
        assert!(
    ...
    #[test]
    fn bsmr_target_output_path_resolves() -> bsmr_error::Result<()> {
        let path_resolver =
            OutputPathResolver::new(ProjectRelativePathBuf::unchecked_new("bsmr-out".into()));
        ...
        let expected_scratch_path =
            Regex::new("bsmr-out/tmp/foo/[0-9a-f]{16}/category/_bsmr_[0-9a-f]{16}")?;
        assert!(
            expected_scratch_path.is_match(resolved_scratch_path.as_str()),
            "{expected_scratch_path}.is_match({resolved_scratch_path})"
        );
