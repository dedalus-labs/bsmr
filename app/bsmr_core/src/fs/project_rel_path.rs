...
use bsmr_fs::paths::IntoFileNameBufIterator;
use bsmr_fs::paths::file_name::FileName;
use bsmr_fs::paths::file_name::FileNameBuf;
use bsmr_fs::paths::fmt::quoted_display;
use bsmr_fs::paths::forward_rel_path::ForwardRelativePath;
use bsmr_fs::paths::forward_rel_path::ForwardRelativePathBuf;
use bsmr_fs::paths::forward_rel_path::ForwardRelativePathIter;
use bsmr_fs::paths::relative_path::RelativePath;
use bsmr_fs::paths::relative_path::RelativePathBuf;
use bsmr_util::arc_str::StringInside;
...
impl ProjectRelativePath {
    ...
    /// assert!(ProjectRelativePath::new(Path::new("normalize/../bar")).is_err());
    /// ```
    pub fn new<P: ?Sized + AsRef<Path>>(p: &P) -> bsmr_error::Result<&ProjectRelativePath> {
    ...
    /// # bsmr_error::Ok(())
    /// ```
    pub fn strip_prefix<P>(&self, base: P) -> bsmr_error::Result<&ForwardRelativePath>
    where
    ...
    pub fn strip_suffix<P>(&self, suffix: P) -> bsmr_error::Result<&ProjectRelativePath>
    where
...
impl ProjectRelativePathBuf {
    ...
    pub fn push_normalized<P: AsRef<RelativePath>>(&mut self, path: P) -> bsmr_error::Result<()> {
...
impl<'a> TryFrom<&'a str> for &'a ProjectRelativePath {
    ...
    /// assert!(<&ProjectRelativePath>::try_from("normalize/../bar").is_err());
    /// ```
    fn try_from(s: &'a str) -> bsmr_error::Result<&'a ProjectRelativePath> {
...
impl<'a> TryFrom<&'a RelativePath> for &'a ProjectRelativePath {
    ...
    /// );
    /// ```
    fn try_from(s: &'a RelativePath) -> bsmr_error::Result<&'a ProjectRelativePath> {
...
impl TryFrom<String> for ProjectRelativePathBuf {
    ...
    /// assert!(ProjectRelativePathBuf::try_from("normalize/../bar".to_owned()).is_err());
    /// ```
    fn try_from(s: String) -> bsmr_error::Result<ProjectRelativePathBuf> {
...
impl TryFrom<RelativePathBuf> for ProjectRelativePathBuf {
    ...
    /// assert!(ProjectRelativePathBuf::try_from(RelativePathBuf::from("normalize/../bar")).is_err());
    /// ```
    fn try_from(p: RelativePathBuf) -> bsmr_error::Result<ProjectRelativePathBuf> {
        Ok(ProjectRelativePathBuf::from(
...
impl TryFrom<PathBuf> for ProjectRelativePathBuf {
    ...
    /// assert!(ProjectRelativePathBuf::try_from(PathBuf::from("normalize/../bar")).is_err());
    /// ```
    fn try_from(p: PathBuf) -> bsmr_error::Result<ProjectRelativePathBuf> {
...
mod tests {
    ...
    #[test]
    fn path_display_is_readable() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn path_is_comparable() -> bsmr_error::Result<()> {
