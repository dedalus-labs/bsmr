...
mod tests {
    ...
    #[tokio::test]
    async fn test() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_get_or_try_init() -> bsmr_error::Result<()> {
        let cell1 = AsyncOnceCell::new();

        assert_eq!(
            &43,
            cell1
                .get_or_try_init(async { bsmr_error::Ok(43) })
                .await
                .unwrap()
        );
        assert_eq!(
            &43,
            cell1
                .get_or_try_init(async { bsmr_error::Ok(55) })
                .await
                .unwrap()
        );
        ...
        let cell2 = AsyncOnceCell::new();
        cell2
            .get_or_try_init(async { Err(bsmr_error!(bsmr_error::ErrorTag::TestOnly, "foo")) })
            .await
            .unwrap_err();
        assert_eq!(
            &44,
            cell2
                .get_or_try_init(async { bsmr_error::Ok(44) })
                .await
                .unwrap()
        );
        assert_eq!(
            &44,
            cell2
                .get_or_try_init(async { bsmr_error::Ok(56) })
                .await
                .unwrap()
        );
