...
impl CellAliasResolver {
    ...
    pub fn resolve(&self, alias: &str) -> bsmr_error::Result<CellName> {
        if alias.is_empty() {
            return Ok(self.current);
        }
        self.aliases.get(alias).duped().ok_or_else(|| {
            bsmr_error::Error::from(CellError::UnknownCellAlias(
...
impl CellResolver {
    pub fn new(
        ...
        root_cell_alias_resolver: CellAliasResolver,
    ) -> bsmr_error::Result<CellResolver> {
    ...
    pub fn get(&self, cell: CellName) -> bsmr_error::Result<&CellInstance> {
        self.0.cells.get(&cell).ok_or_else(|| {
            bsmr_error::Error::from(CellError::UnknownCellName(
                cell,
...
mod tests {
    ...
    #[test]
    fn test_of_names_and_paths() -> bsmr_error::Result<()> {
        use crate::fs::project_rel_path::ProjectRelativePathBuf;
    ...
    #[test]
    fn test_cells() -> bsmr_error::Result<()> {
