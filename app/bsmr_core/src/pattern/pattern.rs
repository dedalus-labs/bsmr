...
pub(crate) fn split_providers_name(s: &str) -> bsmr_error::Result<(&str, ProvidersName)> {
    if let Some((t, flavors)) = split1_opt_ascii(s, AsciiChar::new('#')) {
        ...
        Ok((
            ...
            ProvidersName::NonDefault(triomphe::Arc::new(NonDefaultProvidersName::Named(
                bsmr_util::arc_str::ArcSlice::from_iter(names),
            ))),
        ))
    } else {
...
impl ParsedPattern<TargetPatternExtra> {
    ...
    pub fn as_target_label(self, original: &str) -> bsmr_error::Result<TargetLabel> {
...
impl ParsedPattern<ProvidersPatternExtra> {
    ...
    pub fn as_providers_label(self, original: &str) -> bsmr_error::Result<ProvidersLabel> {
...
impl<T: PatternType> ParsedPattern<T> {
    pub fn try_map<U: PatternType>(
        self,
        f: impl FnOnce(T) -> bsmr_error::Result<U>,
    ) -> bsmr_error::Result<ParsedPattern<U>> {
        match self {
    ...
    pub fn as_literal(self, original: &str) -> bsmr_error::Result<(TargetLabel, T)> {
...
impl<'a, T: PatternType> PatternParts<'a, T> {
    fn try_map<U: PatternType, F: FnOnce(T) -> bsmr_error::Result<U>>(
        self,
        f: F,
    ) -> bsmr_error::Result<PatternParts<'a, U>> {
        let PatternParts {
            cell_alias,
            pattern,
        } = self;
...
impl<'a, T: PatternType> PatternDataOrAmbiguous<'a, T> {
    fn try_map<U: PatternType>(
        self,
        f: impl FnOnce(T) -> bsmr_error::Result<U>,
    ) -> bsmr_error::Result<PatternDataOrAmbiguous<'a, U>> {
        match self {
    ...
    T: PatternType,
{
    ...
    /// `foo/bar:bar`.
    pub fn infer_target(self) -> bsmr_error::Result<PatternData<'a, T>> {
        match self {
    ...
    pub fn reject_ambiguity(self) -> bsmr_error::Result<PatternData<'a, T>> {
        match self {
            Self::PatternData(d) => Ok(d),
...
impl<'a, T: PatternType> PatternData<'a, T> {
    fn try_map<U: PatternType>(
        self,
        f: impl FnOnce(T) -> bsmr_error::Result<U>,
    ) -> bsmr_error::Result<PatternData<'a, U>> {
        match self {
...
pub fn maybe_split_cell_alias_and_relative_path(
    pattern: &str,
) -> bsmr_error::Result<Option<(CellAlias, &ForwardRelativePath)>> {
...
fn lex_provider_pattern(
    pattern: &str,
    strip_package_trailing_slash: bool,
) -> bsmr_error::Result<PatternParts<'_, ProvidersPatternExtra>> {
...
fn lex_configuration_predicate(pattern: &str) -> bsmr_error::Result<ConfigurationPredicate> {
    let pattern = pattern
        .strip_prefix('(')
...
pub fn lex_configured_providers_pattern(
    pattern: &str,
    strip_package_trailing_slash: bool,
) -> bsmr_error::Result<PatternParts<'_, ConfiguredProvidersPatternExtra>> {
...
pub fn lex_target_pattern<T: PatternType>(
    pattern: &str,
    strip_package_trailing_slash: bool,
) -> bsmr_error::Result<PatternParts<'_, T>> {
...
fn normalize_package(
    package: &str,
    strip_package_trailing_slash: bool,
) -> bsmr_error::Result<&RelativePath> {
...
impl<'a> TargetParsingRel<'a> {
    ...
    fn join_relative(&self, path: &RelativePath) -> bsmr_error::Result<CellPath> {
        match self {
            ...
            TargetParsingRel::RequireAbsolute(_) => Err(bsmr_error::Error::from(
...
fn resolve_target_alias<T>(
    ...
    target_alias_resolver: &dyn TargetAliasResolver,
    lex: &PatternParts<T>,
) -> bsmr_error::Result<Option<ParsedPatternWithModifiers<T>>>
where
    T: PatternType,
{
    // Imported from Buck1
...
mod tests {
        ...
        ParsedPattern::Target(
            ...
            ProvidersPatternExtra {
                providers: providers.map_or(ProvidersName::Default, |n| {
                    ProvidersName::NonDefault(triomphe::Arc::new(NonDefaultProvidersName::Named(
                        bsmr_util::arc_str::ArcSlice::from_iter(
    ...
    fn fails<R>(x: bsmr_error::Result<R>, msgs: &[&str]) {
        match x {
            Err(e) => {
                let s = format!("{e:#}");
                for msg in msgs {
    ...
    impl TargetAliasResolver for NoAliases {
        fn get<'a>(&'a self, _name: &str) -> bsmr_error::Result<Option<&'a str>> {
            Ok(None)
        }
    }
    ...
    #[test]
    fn parse_relative_pattern() -> bsmr_error::Result<()> {
        let package = CellPath::new(
            CellName::testing_new("root"),
            CellRelativePath::unchecked_new("package/path").to_owned(),
        );
    ...
    #[test]
    fn test_relaxed() -> bsmr_error::Result<()> {
        let package = CellPath::new(
            CellName::testing_new("root"),
    ...
    #[test]
    fn test_parsed_opt_absolute() -> bsmr_error::Result<()> {
        let package = CellPath::new(
            CellName::testing_new("root"),
            CellRelativePath::unchecked_new("package/path").to_owned(),
        );
    ...
    #[test]
    fn parse_providers_pattern() -> bsmr_error::Result<()> {
        assert_eq!(
    ...
    #[test]
    fn parse_providers_pattern_with_alias() -> bsmr_error::Result<()> {
        let package = CellPath::new(
            CellName::testing_new("root"),
            CellRelativePath::unchecked_new("package").to_owned(),
        );
    ...
    #[test]
    fn test_parse_configured_providers_pattern() -> bsmr_error::Result<()> {
        assert_eq!(
    ...
    #[test]
    fn parsed_pattern_contains() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_relative_pattern_with_parent() -> bsmr_error::Result<()> {
        let package = CellPath::new(
            CellName::testing_new("root"),
            CellRelativePath::unchecked_new("package/path").to_owned(),
        );
    ...
    #[test]
    fn test_parsed_pattern_with_modifiers_relaxed() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn test_configured_parsed_pattern_with_modifiers() -> bsmr_error::Result<()> {
        let resolver = resolver();
        let alias_resolver = alias_resolver();
        let package = CellPath::new(
            CellName::testing_new("root"),
