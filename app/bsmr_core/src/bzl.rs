...
enum ImportPathError {
    ...
    Invalid(CellPath),
    #[error("Import path must have suffix `.bsmr`, `.bzl`, `.json`, or `.toml`: `{0}`")]
    Suffix(CellPath),
}

/// Path of a Starlark module.
...
impl ImportPath {
    ...
    pub fn new_same_cell(path: CellPath) -> bsmr_error::Result<Self> {
...
mod tests {
    ...
    #[test]
    fn accepts_bsmr_imports() {
        assert!(ImportPath::new_same_cell(CellPath::testing_new("root//defs/rules.bsmr")).is_ok());
    }
}
