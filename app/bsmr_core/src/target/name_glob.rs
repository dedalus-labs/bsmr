...
impl TargetNameGlob {
    pub fn try_new(pattern: String) -> bsmr_error::Result<Self> {
    ...
    fn validate(pattern: &str) -> bsmr_error::Result<()> {
        if pattern.is_empty() {
    ...
    pub fn matches(&self, name: &TargetNameRef) -> bsmr_error::Result<bool> {
