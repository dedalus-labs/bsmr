...
use bsmr_util::arc_str::ThinArcStr;
use dupe::Dupe;
use pagable::Pagable;
...
impl TargetName {
    #[inline]
    pub fn new(name: &str) -> bsmr_error::Result<Self> {
    ...
    fn bad_name_error(name: &str) -> bsmr_error::Error {
    ...
    fn verify(name: &str) -> bsmr_error::Result<()> {
        if name.is_empty()
            || !name
                .chars()
...
impl TargetNameRef {
    #[inline]
    pub fn new(name: &str) -> bsmr_error::Result<&TargetNameRef> {
