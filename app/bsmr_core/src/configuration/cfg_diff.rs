...
pub fn cfg_diff(a: &ConfigurationData, b: &ConfigurationData) -> Result<(), String> {
    ...
    impl DiffPrinter {
        ...
        fn print_label_result_line(&mut self, sign: char, label: bsmr_error::Result<&str>) {
            match label {
        ...
        fn print_cfg_data_result_line(
            &mut self,
            sign: char,
            data: bsmr_error::Result<&ConfigurationDataData>,
        ) {
            match data {
        ...
        fn diff_label_result(&mut self, a: bsmr_error::Result<&str>, b: bsmr_error::Result<&str>) {
            match (a, b) {
                (Ok(a), Ok(b)) => {
                    if a != b {
        ...
        fn diff_cfg_data_result(
            &mut self,
            a: bsmr_error::Result<&ConfigurationDataData>,
            b: bsmr_error::Result<&ConfigurationDataData>,
        ) {
            match (a, b) {
