...
impl<T> MaybeCompatible<T> {
    ...
    pub fn require_compatible(self) -> bsmr_error::Result<T> {
        match self {
...
impl<T> From<bsmr_error::Error> for ResultMaybeCompatible<T> {
...
impl<T> ResultMaybeCompatible<T> {
    ...
    pub fn map_err(
        self,
        func: impl FnOnce(bsmr_error::Error) -> bsmr_error::Error,
    ...
    pub fn try_map<U>(
        self,
        func: impl FnOnce(T) -> Result<U, bsmr_error::Error>,
        ...
        F: FnOnce() -> String,
    {
        match self {
            ...
            Self::Err(e) => {
                bsmr_error::Result::<!>::Err(e.into()).with_internal_error(f)?;
            }
        }
    }
    ...
    #[track_caller]
    pub fn tag(self, tag: bsmr_error::ErrorTag) -> ResultMaybeCompatible<T> {
        match self {
    ...
    pub fn require_compatible(self) -> Result<T, bsmr_error::Error> {
        match self {
    ...
    pub fn ok(self) -> Result<MaybeCompatible<T>, bsmr_error::Error> {
        match self {
...
impl<T, E: Into<bsmr_error::Error>> std::ops::FromResidual<Result<std::convert::Infallible, E>>
    for ResultMaybeCompatible<T>
{
...
impl IncompatiblePlatformReason {
    pub fn to_err(&self) -> bsmr_error::Error {
        match self.cause {
    ...
    pub fn to_soft_err(&self) -> bsmr_error::Error {
        match &self.cause {
            IncompatiblePlatformReasonCause::UnsatisfiedConfig(_)
