...
fn emit_configuration_instant_event(cfg: &ConfigurationData) -> bsmr_error::Result<()> {
    let constraints: Vec<bsmr_data::Constraint> = cfg
        .data()?
        .constraints
        .iter()
        .map(|(k, v)| bsmr_data::Constraint {
            setting: k.to_string(),
            value: v.to_string(),
        })
        .collect();

    // Sometimes this isn't going to be init'd in tests (oss or bsmr), let's
    ...
    if let Ok(event_dispatch) = EVENT_DISPATCH.get() {
        event_dispatch.emit_instant_event_for_data(
            bsmr_data::ConfigurationCreated {
                cfg: Some(bsmr_data::ConfigurationWithConstraints {
...
impl ConfigurationData {
    ...
    pub fn lookup_bound(cfg: BoundConfigurationId) -> bsmr_error::Result<Self> {
    ...
    pub fn get_constraint_value(
        &self,
        key: &ConstraintKey,
    ) -> bsmr_error::Result<Option<&ConstraintValue>> {
        Ok(self.data()?.constraints.get(key))
    }

    pub fn label(&self) -> bsmr_error::Result<&str> {
    ...
    pub fn data(&self) -> bsmr_error::Result<&ConfigurationDataData> {
        match &self.0.configuration_platform {
            ConfigurationPlatform::Builtin(BuiltinPlatform::UnspecifiedExec) => {
...
impl ToProtoMessage for ConfigurationData {
    type Message = bsmr_data::Configuration;

    fn as_proto(&self) -> Self::Message {
        bsmr_data::Configuration {
...
mod tests {
    ...
    #[test]
    fn test_stable_output_hash() -> bsmr_error::Result<()> {
