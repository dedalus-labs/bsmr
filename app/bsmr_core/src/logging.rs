...
pub trait LogConfigurationReloadHandle: Send + Sync + 'static {
    fn update_log_filter(&self, format: &str) -> bsmr_error::Result<()>;
}
...
impl LogConfigurationReloadHandle for NoopLogConfigurationReloadHandle {
    fn update_log_filter(&self, _filter: &str) -> bsmr_error::Result<()> {
        Ok(())
    }
}
...
pub fn init_tracing_for_writer<W>(
    writer: W,
) -> bsmr_error::Result<Arc<dyn LogConfigurationReloadHandle>>
where
