...
impl PackageLabel {
    #[inline]
    pub fn new(cell: CellName, path: &CellRelativePath) -> bsmr_error::Result<Self> {
        PackageLabel::from_cell_path(CellPathRef::new(cell, path))
    }

    #[inline]
    pub fn from_cell_path(path: CellPathRef) -> bsmr_error::Result<Self> {
    ...
    pub fn join(&self, path: &ForwardRelativePath) -> bsmr_error::Result<Self> {
        if path.is_empty() {
            Ok(self.dupe())
        } else {
    ...
    pub fn parent(&self) -> bsmr_error::Result<Option<PackageLabel>> {
