...
impl ExecutionPlatformResolutionBase {
    ...
    fn platform(&self) -> bsmr_error::Result<&ExecutionPlatform> {
        match &self.platform {
            Some(v) => Ok(v),
    ...
    fn executor_config(&self) -> bsmr_error::Result<&Arc<CommandExecutorConfig>> {
...
impl ExecutionPlatformResolutionPartial {
    ...
    pub fn platform(&self) -> bsmr_error::Result<&ExecutionPlatform> {
        self.0.platform()
    }
    ...
    pub fn executor_config(&self) -> bsmr_error::Result<&Arc<CommandExecutorConfig>> {
...
impl ExecutionPlatformResolution {
    ...
    // TODO(cjhopman): Should this be an bsmr_error::Result and never return an invalid configuration?
    #[inline]
    ...
    pub fn cfg_for_exec_dep(&self, target: &TargetLabel) -> bsmr_error::Result<ConfigurationData> {
        match self {
            Self::Unspecified => {
