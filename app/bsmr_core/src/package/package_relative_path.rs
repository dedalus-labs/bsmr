...
impl PackageRelativePath {
    ...
    /// ```
    #[inline]
    pub fn new<P: ?Sized + AsRef<Path>>(p: &P) -> bsmr_error::Result<&PackageRelativePath> {
    ...
    /// ```
    #[inline]
    pub fn strip_prefix<'a, P>(&'a self, base: &'a P) -> bsmr_error::Result<&'a ForwardRelativePath>
    where
...
impl<'a> TryFrom<&'a str> for &'a PackageRelativePath {
    ...
    /// ```
    #[inline]
    fn try_from(s: &'a str) -> bsmr_error::Result<&'a PackageRelativePath> {
...
impl<'a> TryFrom<&'a RelativePath> for &'a PackageRelativePath {
    ...
    /// ```
    #[inline]
    fn try_from(s: &'a RelativePath) -> bsmr_error::Result<&'a PackageRelativePath> {
        Ok(PackageRelativePath::ref_cast(ForwardRelativePath::new(
            s.as_str(),
        )?))
    }
...
impl TryFrom<String> for PackageRelativePathBuf {
    ...
    /// ```
    #[inline]
    fn try_from(s: String) -> bsmr_error::Result<PackageRelativePathBuf> {
...
impl TryFrom<RelativePathBuf> for PackageRelativePathBuf {
    ...
    /// ```
    #[inline]
    fn try_from(p: RelativePathBuf) -> bsmr_error::Result<PackageRelativePathBuf> {
        Ok(PackageRelativePathBuf::from(
...
impl TryFrom<PathBuf> for PackageRelativePathBuf {
    ...
    /// ```
    #[inline]
    fn try_from(p: PathBuf) -> bsmr_error::Result<PackageRelativePathBuf> {
        Ok(PackageRelativePathBuf::from(
            ForwardRelativePathBuf::try_from(p)?,
        ))
    }
...
mod tests {
    ...
    #[test]
    fn paths_work_in_maps() -> bsmr_error::Result<()> {
