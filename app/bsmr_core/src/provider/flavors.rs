...
pub fn map_flavors(flavors: &str, full_target: &str) -> bsmr_error::Result<ProvidersName> {
    ...
    if let Some(index) = flavors_parts.iter().position(|x| is_platform_flavor(x)) {
        ...
        soft_error!(
            "platform_flavor",
            bsmr_error::bsmr_error!(bsmr_error::ErrorTag::Input, "Platform flavor found in target: {}", full_target),
            deprecation: true,
            quiet: true,
            error_on_oss: true
        )?;
