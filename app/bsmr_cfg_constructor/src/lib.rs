...
) -> bsmr_error::Result<(Vec<String>, Value<'v>)> {
...
async fn analyze_constraints(
    ...
    refs: Vec<String>,
) -> bsmr_error::Result<SmallMap<String, FrozenProviderCollectionValue>> {
...
fn eval_post_constraint_analysis<'v>(
    ...
    eval.with_evaluator(|eval| -> bsmr_error::Result<ConfigurationData> {
...
impl CfgConstructorImpl for CfgConstructor {
    ...
    ) -> Pin<Box<dyn Future<Output = bsmr_error::Result<ConfigurationData>> + Send + 'a>> {
