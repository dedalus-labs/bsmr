...
use bsmr_interpreter_for_build::interpreter::build_context::PerFileTypeContext;
use bsmr_interpreter_for_build::interpreter::package_file_extra::MAKE_CFG_CONSTRUCTOR;
use bsmr_interpreter_for_build::interpreter::package_file_extra::PackageFileExtra;
...
) -> bsmr_error::Result<Arc<dyn CfgConstructorImpl>> {
...
pub(crate) fn register_set_cfg_constructor(globals: &mut GlobalsBuilder) {
    ...
    fn set_cfg_constructor<'v>(
        ...
        let ctx = match &build_context.additional {
            ...
            _ => {
                return Err(
                    bsmr_error::Error::from(RegisterCfgConstructorError::NotPackageRoot).into(),
                );
            }
        };
        if ctx.path.dir()
        ...
        if package_file_extra.cfg_constructor.get().is_some() {
            return Err(
                bsmr_error::Error::from(RegisterCfgConstructorError::AlreadyRegistered).into(),
            );
        }
...
pub(crate) fn init_registration() {
    ...
    REGISTER_BSMR_CFG_CONSTRUCTOR_GLOBALS.init(register_set_cfg_constructor);
}
