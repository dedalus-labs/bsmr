...
use bsmr_execute_local::status_decoder::MiniperfStatusDecoder;
use bsmr_forkserver_proto::CommandRequest;
use bsmr_forkserver_proto::RequestEvent;
use bsmr_forkserver_proto::SetLogFilterRequest;
use bsmr_forkserver_proto::SetLogFilterResponse;
use bsmr_forkserver_proto::forkserver_server::Forkserver;
...
type RunStream =
    Pin<Box<dyn Stream<Item = Result<bsmr_forkserver_proto::CommandEvent, Status>> + Send>>;

struct ValidatedCommand {
    exe: PathBuf,
    argv: Vec<Vec<u8>>,
    env: Vec<bsmr_forkserver_proto::EnvDirective>,
    cwd: PathBuf,
    ...
    network_access: Option<bsmr_data::NetworkAccess>,
}

impl ValidatedCommand {
    fn try_from(cmd_request: CommandRequest) -> bsmr_error::Result<Self> {
        let CommandRequest {
            exe,
            argv,
            env,
        ...
        Ok(ValidatedCommand {
            ...
            network_access: network_access
                .map(bsmr_data::NetworkAccess::try_from)
                .transpose()
...
impl UnixForkserverService {
    ...
    ) -> bsmr_error::Result<CommandRequest> {
    ...
    fn configure_environment(
        ...
        env_directives: &[bsmr_forkserver_proto::EnvDirective],
    ) -> bsmr_error::Result<()> {
        use bsmr_forkserver_proto::env_directive::Data;
    ...
    ) -> bsmr_error::Result<(Command, Option<AbsNormPathBuf>)> {
    ...
    async fn create_command_stream(
        cmd: Command,
        validated_cmd: ValidatedCommand,
        cancellation: impl futures::Future<Output = bsmr_error::Result<GatherOutputStatus>>
        + Send
        + Unpin
        + 'static,
        miniperf_output: Option<AbsNormPathBuf>,
        freeze_rx: impl ActionFreezeEventReceiver,
    ) -> bsmr_error::Result<RunStream> {
...
#[cfg(target_os = "linux")]
fn is_restricted_network_access(network_access: bsmr_data::NetworkAccess) -> bool {
    match network_access {
        bsmr_data::NetworkAccess::All => false,
        bsmr_data::NetworkAccess::None
        | bsmr_data::NetworkAccess::Loopback
        | bsmr_data::NetworkAccess::Strict
        | bsmr_data::NetworkAccess::Private => true,
    }
}
            ...
            tokio::spawn(async move {
                ...
                while let Some(m) = stream.next().await {
                    let err = match m {
                        Ok(m) => match m.data {
                            Some(d) => match d {
                                bsmr_forkserver_proto::request_event::Data::CommandRequest(_) => {
                                    bsmr_error::internal_error!("Non-leading command request")
                                }
                                bsmr_forkserver_proto::request_event::Data::CancelRequest(_) => {
                                    drop(cancel_tx.send(Ok(GatherOutputStatus::Cancelled)));
                                    break;
                                }
                                bsmr_forkserver_proto::request_event::Data::FreezeRequest(_) => {
                                    drop(freeze_tx.send(ActionFreezeEvent::Freeze));
                                    continue;
                                }
                                bsmr_forkserver_proto::request_event::Data::UnfreezeRequest(_) => {
...
impl MiniperfContainer {
    fn new(forkserver_state_dir: &AbsNormPath) -> bsmr_error::Result<Option<Self>> {
        let miniperf_bin: Option<&'static [u8]>;

        #[cfg(all(fbcode_build, target_os = "linux"))]
        {
            miniperf_bin = Some(bsmr_miniperf_data::get());
        }
