...
use bsmr_health_check_proto::health_check_client::HealthCheckClient;
use bsmr_util::properly_reaped_child::ProperlyReapedChild;
use bsmr_util::properly_reaped_child::reap_on_drop_command;
use dupe::Dupe;
...
const CLI_NAME: &str = "bsmr-health-check";
const CLI_INFO_FILE: &str = "bsmr-health-check-server-info";
...
pub(crate) struct HealthCheckRpcClient {
    ...
    connection: AsyncOnceCell<bsmr_error::Result<HealthCheckServerConnection>>,
...
impl HealthCheckRpcClient {
    ...
    async fn connection(&self) -> bsmr_error::Result<&HealthCheckServerConnection> {
    ...
    fn get_cli_path() -> bsmr_error::Result<String> {
        if let Ok(overridden_path) = std::env::var("BSMR_HEALTH_CHECK_CLI_PATH") {
    ...
    async fn get_state_info_file_path(&self) -> bsmr_error::Result<AbsNormPathBuf> {
        ...
        match std::env::var("BSMR_HEALTH_CHECK_STATE_INFO_PATH") {
...
impl HealthCheckService for HealthCheckRpcClient {
    async fn update_context(&mut self, event: HealthCheckContextEvent) -> bsmr_error::Result<()> {
        let rpc_event: bsmr_health_check_proto::HealthCheckContextEvent = event.try_into()?;
        self.rpc_client()
            .await?
    ...
    async fn run_checks(
        ...
        let snapshot: bsmr_health_check_proto::HealthCheckSnapshotData = snapshot.try_into()?;
...
mod tests {
    ...
    async fn temp_state_info(
        temp_dir: &TempDir,
    ) -> bsmr_error::Result<(AbsNormPathBuf, AbsNormPathBuf)> {
    ...
    #[tokio::test]
    async fn test_server_state_reset() -> bsmr_error::Result<()> {
