...
/// Trait to generalize a bsmr health check.
/// Refer <https://fburl.com/bsmr_health_checks> for details on adding a new health check.
...
pub trait HealthCheck: Send + Sync {
    ...
    ) -> bsmr_error::Result<Option<Report>>;
...
pub struct HealthCheckContext {
    ...
    pub command_data: Option<bsmr_data::command_start::Data>,
    ...
    pub parsed_target_patterns: Option<bsmr_data::ParsedTargetPatterns>,
    ...
    pub experiment_configurations: Option<bsmr_data::SystemInfo>,
...
/// This struct contains timing metrics extracted from bsmr_data::Snapshot.
#[derive(Dupe, Clone)]
...
pub enum HealthCheckEvent {
    ...
    // This snapshot passes a subset of the bsmr_data::Snapshot data to health checks.
...
pub enum HealthCheckContextEvent {
    CommandStart(bsmr_data::CommandStartWithTraceId),
    ParsedTargetPatterns(bsmr_data::ParsedTargetPatterns),
    BranchedFromRevision(String),
    /// Sent only once and communicates if bsmr is experiencing excess cache misses.
    HasExcessCacheMisses(),
    /// Configuration about the health checks.
    ExperimentConfigurations(bsmr_data::SystemInfo),
    /// Test override for slow build threshold (in seconds).
    /// Set via BSMR_TEST_SLOW_BUILD_CHECK env var on the client.
