...
async fn wait_for_watchman(watchman_sock: &Path) -> bsmr_error::Result<()> {
    ...
    loop {
        ...
        match connector.connect().await {
            Ok(..) => return Ok(()),
            Err(e) if i >= 100 => return Err(bsmr_error!(bsmr_error::ErrorTag::Tier0, "{}", e)),
            _ => {
...
impl WatchmanInstance {
    pub async fn shutdown(&mut self) -> bsmr_error::Result<()> {
        let child = self
            .child
            .as_mut()
...
async fn spawn_watchman(watchman_dir: &Path) -> bsmr_error::Result<WatchmanInstance> {
...
#[tokio::test]
async fn test_syncable_query() -> bsmr_error::Result<()> {
