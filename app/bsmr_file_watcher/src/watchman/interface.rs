...
impl WatchmanQueryProcessor {
    async fn process_events_impl(
        ...
        base_stats: bsmr_data::FileWatcherStats,
            ...
            match ev {
                ChangeEvent::Watchman(ev) => match (&ev.kind, &ev.event) {
                    (WatchmanKind::File, typ) => {
                        log_kind = bsmr_data::FileWatcherKind::File;
                        match typ {
                            WatchmanEventType::Modify => {
                                handler.file_contents_changed(cell_path);
                                log_event = bsmr_data::FileWatcherEventType::Modify;
                            }
                            WatchmanEventType::Create => {
                                handler.file_added_or_removed(cell_path);
                                log_event = bsmr_data::FileWatcherEventType::Create;
                            }
                            WatchmanEventType::Delete => {
                                handler.file_added_or_removed(cell_path);
                                log_event = bsmr_data::FileWatcherEventType::Delete;
                            }
                        }
                    }
                    (WatchmanKind::Directory, typ) => {
                        log_kind = bsmr_data::FileWatcherKind::Directory;
                        match typ {
                            WatchmanEventType::Modify => {
                    ...
                    (WatchmanKind::Symlink, typ) => {
                        log_kind = bsmr_data::FileWatcherKind::Symlink;
                        match typ {
                            WatchmanEventType::Modify => {
                                handler.file_contents_changed(cell_path);
                                log_event = bsmr_data::FileWatcherEventType::Modify;
...
impl SyncableQueryProcessor for WatchmanQueryProcessor {
    type Output = bsmr_data::FileWatcherStats;
    ...
    ) -> bsmr_error::Result<(Self::Output, DiceTransactionUpdater)> {
...
pub(crate) struct WatchmanFileWatcher {
    #[allocative(skip)]
    query: SyncableQuery<bsmr_data::FileWatcherStats, DiceTransactionUpdater>,
}
    ...
    async fn sync(
        ...
        span_async(
            bsmr_data::FileWatcherStart {
                provider: bsmr_data::FileWatcherProvider::Watchman as i32,
            },
            async {
