    ...
    async fn sync(
        ...
        span_async(
            bsmr_data::FileWatcherStart {
                provider: bsmr_data::FileWatcherProvider::FsHashCrawler as i32,
            },
            async {
...
impl FsSnapshot {
    ...
    fn get_updates(&self, new_snapshot: &FsSnapshot) -> bsmr_error::Result<Vec<FsEvent>> {
...
fn file_hash(path: &Path) -> bsmr_error::Result<Hash> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_fs_snapshot() -> bsmr_error::Result<()> {
        ...
        let get_path = |path| -> bsmr_error::Result<(AbsPathBuf, CellPath)> {
