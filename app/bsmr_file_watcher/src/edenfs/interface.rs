...
use bsmr_data::FileWatcherEventType as Type;
...
pub(crate) enum EdenFsWatcherError {
    #[bsmr(tag = IoNotConnected)]
    #[error("Failed to connect to EdenFS {0}")]
    EdenConnectionError(bsmr_error::Error),
    #[bsmr(tag = Input)]
...
impl EdenFsFileWatcher {
                        ...
                        soft_error!(
                            ...
                            bsmr_error::bsmr_error!(
                                ...
                                "EdenFS reported SmallChangeNotification::renamed directory: '{}' -> '{}'. \
                                 ...
                                 EdenFS Thrift API has changed and the bsmr code needs to be updated.",
    ...
    async fn skip_change(&self, change: &ChangeNotification) -> bsmr_error::Result<()> {
    ...
    async fn process_sapling_status(
            ...
            .buck_error_context("Failed to get Sapling status.")?
        {
            ...
            SaplingGetStatusResult::Normal(status) => {
                ...
                let results: bsmr_error::Result<Vec<_>> = status
                    .into_iter()
    ...
    async fn on_large_or_unknown_change(
        &self,
        dice: DiceTransactionUpdater,
    ) -> bsmr_error::Result<(FileWatcherStats, FileChangeTracker, DiceTransactionUpdater)> {
        ...
        let base_stats = self.base_file_watcher_stats().await?;
        let mut base_stats = bsmr_data::FileWatcherStats {
            fresh_instance: true,
    ...
    async fn update_mergebase(&self, to: &str) -> bsmr_error::Result<bool> {
    ...
    async fn base_file_watcher_stats(&self) -> bsmr_error::Result<bsmr_data::FileWatcherStats> {
    ...
    async fn sync(
        ...
        span_async(
            bsmr_data::FileWatcherStart {
                provider: bsmr_data::FileWatcherProvider::EdenFs as i32,
            },
            async {
