...
pub const DOT_BSMRCONFIG_D: &str = ".bsmrconfig.d";
...
/// Extract the isolation dir from a bsmr process's command line.
///
/// bsmr daemons and clients are launched with the global `--isolation-dir <name>`
...
/// Find all bsmr processes in the system.
fn find_bsmr_processes(who_is_asking: WhoIsAsking) -> Vec<ProcessInfo> {
    ...
    let mut bsmr_processes = Vec::new();
    for (pid, process) in system.processes() {
        ...
        if is_bsmr_exe(exe, who_is_asking) && !current_parents.contains(pid) {
            ...
            bsmr_processes.push(ProcessInfo {
                pid,
...
/// Kills all running Bessemer processes, except this process's hierarchy. Returns whether it
/// succeeded without errors.
pub fn killall(who_is_asking: WhoIsAsking, write: impl Fn(String)) -> bool {
    let bsmr_processes = find_bsmr_processes(who_is_asking);

    if bsmr_processes.is_empty() {
        write("No bsmr processes found".to_owned());
        return true;
    }
    ...
    impl<F: Fn(String)> Printer<F> {
        ...
        fn failed_to_kill(&mut self, process: &ProcessInfo, error: bsmr_error::Error) {
            ...
            for process in processes_still_alive {
                printer.failed_to_kill(
                    ...
                    bsmr_error::bsmr_error!(
                        bsmr_error::ErrorTag::DaemonWontDieFromKill,
