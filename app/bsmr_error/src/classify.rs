...
pub enum Tier {
    ...
    Environment,
    // Unexpected errors in bsmr or core dependencies.
...
impl ErrorLike for bsmr_data::ErrorReport {
    fn best_tag(&self) -> Option<ErrorTag> {
    ...
    fn error_rank(self: &bsmr_data::ErrorReport) -> u32 {
...
pub fn best_error<'a>(
    tags: impl IntoIterator<Item = &'a bsmr_data::ErrorReport>,
) -> Option<&'a bsmr_data::ErrorReport> {
...
// Bessemer is the fallback/default source area, use the first non-bsmr source area.
pub fn source_area(tags: impl IntoIterator<Item = ErrorTag>) -> ErrorSourceArea {
    tags.into_iter()
        .find_map(|tag| {
            let area = tag.source_area();
            if area != ErrorSourceArea::Bessemer {
                Some(area)
            } else {
                None
            }
        })
        .unwrap_or(ErrorSourceArea::Bessemer)
}
