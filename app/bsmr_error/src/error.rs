...
/// entirely replaced it in the Bessemer codebase. It has `From` impls from many common error types.
...
impl Error {
    ...
    pub fn new(
        ...
        let bsmr_error = crate::Error(Arc::new(ErrorKind::Root(Box::new(error_root))));
        bsmr_error.tag([error_tag])
    }
    ...
    pub fn action_error(&self) -> Option<&bsmr_data::ActionError> {
