...
pub use bsmr_data::error::ErrorTag;
...
/// `thiserror` in `bsmr/app` will be replaced with this macro.
///
/// Currently, the only distinction from `thiserror::Error` is that an additional impl of
/// `AnyError` is generated for the type, which makes some of the interactions with `bsmr_error` more
...
/// struct MyError;
///
/// let e = bsmr_error::Error::from(MyError);
/// assert_eq!(&format!("{}", e), "My error type");
/// ```
#[doc(inline)]
pub use bsmr_error_derive::Error;
