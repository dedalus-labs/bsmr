...
//! Starlark::Error <-> bsmr_error::Error conversion implementation.
...
fn error_with_starlark_context(
    ...
    let mut bsmr_error = e.clone();

    loop {
        match bsmr_error.0.as_ref() {
            ErrorKind::Root(root) => {
            ...
            ErrorKind::WithContext(context_value, inner) => {
                match context_value {
                    ...
                    ContextValue::StarlarkError(_) => {
                        return bsmr_error.context_for_starlark_backtrace(starlark_context);
                    }
...
mod tests {
    ...
    use crate::bsmr_error;
    use crate::context_value::StarlarkContext;
    use crate::starlark_error::error_with_starlark_context;

    #[derive(bsmr_error::Error, Debug, Allocative)]
    #[error("FullMetadataError")]
    #[bsmr(tag = crate::ErrorTag::Tier0)]
    ...
    fn test_roundtrip_starlark() {
        // Tests bsmr_error->starlark->bsmr_error
    ...
    fn test_pop_base_error_from_context() {
        ...
        let e = bsmr_error!(crate::ErrorTag::StarlarkError, "{}", base_error);
