...
impl StderrOutputWriter {
    pub fn new(context: &dyn ServerCommandContextTrait) -> bsmr_error::Result<Self> {
        Ok(Self {
    ...
    fn get_chunk_size() -> bsmr_error::Result<usize> {
        // protobuf recommends each message should be under 1MB
        const DEFAULT_CHUNK_SIZE: usize = 1024 * 1024;
        bsmr_env!("BSMR_DEBUG_RAWOUTPUT_CHUNK_SIZE", type=usize, default=DEFAULT_CHUNK_SIZE)
    }
...
impl Write for StderrOutputWriter {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        if len > 0 {
            ...
            let raw_output = bsmr_data::ConsoleMessage {
                message: s.to_owned(),
            };
