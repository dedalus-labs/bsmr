...
use bsmr_data::CommandCriticalEnd;
use bsmr_data::CommandCriticalStart;
use bsmr_data::DiceCriticalSectionEnd;
use bsmr_data::DiceCriticalSectionStart;
    ...
    pub fn process_current_command(
        ...
        }) = self.data.as_ref()
        {
            if *current_external_and_local_configs != *external_configs {
                event_dispatcher.instant_event(bsmr_data::PreviousCommandWithMismatchedConfig {
...
pub trait ServerCommandContextTrait: Send + Sync {
    ...
    ) -> bsmr_error::Result<DiceAccessor<'a>>;
...
pub trait ServerCommandDiceContext {
    async fn with_dice_ctx<'v, F, Fut, R>(&'v self, exec: F) -> bsmr_error::Result<R>
    where
                            ...
                            |mut dice, early_command_timing| async move {
                                ...
                                events
                                    .span_async(
                                        ...
                                        async move {
                                            let res = bsmr_build_signals::env::scope(
                                                build_signals,
                                                self.events().dupe(),
