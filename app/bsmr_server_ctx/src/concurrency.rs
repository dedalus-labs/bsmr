...
use bsmr_build_signals::env::EXCLUSIVE_COMMAND_WAIT;
...
use bsmr_data::CommandPreempted;
use bsmr_data::DiceBlockConcurrentCommandEnd;
use bsmr_data::DiceBlockConcurrentCommandStart;
use bsmr_data::DiceEqualityCheck;
use bsmr_data::DiceSynchronizeSectionEnd;
use bsmr_data::DiceSynchronizeSectionStart;
...
impl CommandData {
    fn format_argv(&self) -> String {
        ...
        // Skip the "/path/to/bsmr" part so we can just emit "bsmr" for the start of the cmd
        iter.next();

        let cmd = format!("bsmr {}", iter.join(" "));
        truncate(&cmd, 500)
    }

    fn notify_tainted(&self) {
        self.dispatcher.instant_event(bsmr_data::TagEvent {
        ...
        let (mut transaction, tainted) = loop {
            match &data.dice_status {
                DiceStatus::Cleanup { future, epoch } => {
                    ...
                    event_dispatcher
                        .span_async(
                            bsmr_data::DiceCleanupStart { epoch: epoch as _ },
                            async move { (future.await, bsmr_data::DiceCleanupEnd {}) },
                        )
                        .await;
                ...
                DiceStatus::Available { active } => {
                    ...
                    let transaction = async {
                        ...
                        let transaction = event_dispatcher
                            .span_async(bsmr_data::DiceStateUpdateStart {}, async {
                                (
                                    async {
                                        let transaction =
                                            transaction.commit_with_data(user_data).await;
                                        bsmr_error::Ok(transaction)
                                    }
                                    .await,
                                    bsmr_data::DiceStateUpdateEnd {},
                                )
                            })
                            .await?;
                        bsmr_error::Ok(transaction)
...
mod tests {
    ...
    #[tokio::test]
    async fn parallel_invocation_different_traceid_blocks() -> bsmr_error::Result<()> {
        let dice = make_default_dice().await;
    ...
    #[tokio::test]
    async fn parallel_invocation_exit_when_different_state() -> bsmr_error::Result<()> {
        let dice = make_default_dice().await;
        ...
        let fut3_error: bsmr_error::Error = fut3_result.unwrap_err();
        assert!(
            fut3_error
                .tags()
                .contains(&bsmr_error::ErrorTag::DaemonIsBusy),
        );
    ...
    #[tokio::test]
    async fn parallel_invocation_exit_when_preemptible() -> bsmr_error::Result<()> {
        let dice = make_default_dice().await;
        ...
        let fut1_error: bsmr_error::Error = fut1_result.unwrap_err();
        assert!(
            fut1_error
                .tags()
                .contains(&bsmr_error::ErrorTag::DaemonPreempted),
        );
    ...
    #[tokio::test]
    async fn test_cleanup_stage() -> bsmr_error::Result<()> {
    ...
    async fn wait_for_exclusive_span_start(
        source: &mut ChannelEventSource,
        cmd: Option<&str>,
    ) -> bsmr_error::Result<Option<SpanId>> {
    ...
    #[tokio::test]
    #[allow(clippy::await_holding_lock)] // Intentional: testing exclusive access
    async fn exclusive_command_lock() -> bsmr_error::Result<()> {
        let dice = make_default_dice().await;
        let concurrency = ConcurrencyHandler::new(dice.dupe());
    ...
    #[tokio::test]
    async fn test_thundering_herd() -> bsmr_error::Result<()> {
        let dice = make_default_dice().await;
        ...
        bsmr_util::future::try_join_all(tasks).await?;
    ...
    #[tokio::test]
    async fn test_updates_are_synchronized() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_exit_when_not_idle_with_same_state() -> bsmr_error::Result<()> {
        let dice = make_default_dice().await;
        let concurrency = ConcurrencyHandler::new(dice.dupe());
        ...
        let fut2 = tokio::spawn(bsmr_util::async_move_clone!(concurrency, {
            concurrency
                .enter(
                    null_sink_with_trace(traces2),
                    &NoChanges,
        ...
        let fut2_error: bsmr_error::Error = fut2_result.unwrap_err();
        assert!(
            fut2_error
                .tags()
                .contains(&bsmr_error::ErrorTag::DaemonIsBusy),
    ...
    #[tokio::test]
    async fn test_exit_when_not_idle_with_different_state() -> bsmr_error::Result<()> {
        let dice = make_default_dice().await;
        let concurrency = ConcurrencyHandler::new(dice.dupe());
    ...
    #[tokio::test]
    async fn test_multiple_exit_when_not_idle_commands_with_same_state() -> bsmr_error::Result<()> {
        let dice = make_default_dice().await;
        let concurrency = ConcurrencyHandler::new(dice.dupe());
        ...
        let fut3 = tokio::spawn(bsmr_util::async_move_clone!(concurrency, {
            concurrency
                .enter(
                    null_sink_with_trace(traces3),
                    &NoChanges,
    ...
    #[tokio::test]
    async fn test_exit_when_not_idle_with_preemptible_command() -> bsmr_error::Result<()> {
        let dice = make_default_dice().await;
        let concurrency = ConcurrencyHandler::new(dice.dupe());
    ...
    #[tokio::test]
    async fn test_exit_when_not_idle_gets_preempted() -> bsmr_error::Result<()> {
        let dice = make_default_dice().await;
        let concurrency = ConcurrencyHandler::new(dice.dupe());
        ...
        let fut1 = tokio::spawn({
            ...
            async move {
                ...
                if let Err(ref e) = result {
                    let error: bsmr_error::Error = e.clone();
                    if error
                        .tags()
                        .contains(&bsmr_error::ErrorTag::DaemonPreempted)
                    {
    ...
    #[tokio::test]
    async fn test_enter_duration_parameter_populated() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_enter_duration_parameter_zero_for_non_exclusive() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_file_watcher_sync_duration_captured() -> bsmr_error::Result<()> {
