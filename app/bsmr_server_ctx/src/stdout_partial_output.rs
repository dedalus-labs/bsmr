...
impl<'a> StdoutPartialOutput<'a> {
    pub fn new(
        dispatcher: &'a mut PartialResultDispatcher<bsmr_cli_proto::StdoutBytes>,
...
struct WriterWrapper<'a> {
    inner: &'a mut PartialResultDispatcher<bsmr_cli_proto::StdoutBytes>,
    cancellation: CancellationPoller,
}
...
impl Write for WriterWrapper<'_> {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        self.inner.emit(bsmr_cli_proto::StdoutBytes {
            data: buf.to_owned(),
        });
