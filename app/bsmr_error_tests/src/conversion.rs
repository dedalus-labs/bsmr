...
use bsmr_util::golden_test_helper::golden_test_template;
use bsmr_util::golden_test_helper::trim_rust_backtrace;
...
fn starlark_conversion_helper() -> starlark::Error {
    ...
    fn fail3() -> bsmr_error::Result<()> {
        fail2()
            .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::Input).context("rust failure"))
    }
...
fn test_format_starlark_stacktrace_with_later_context() {
    ...
    let test_context = bsmr_error::Error::from(e)
...
fn test_starlark_multiple_stacktrace() {
    ...
    fn outer_module(builder: &mut GlobalsBuilder) {
        fn outer_rust_failure() -> starlark::Result<NoneType> {
            let e: bsmr_error::Error = starlark_conversion_helper().into();
            Err(e.into())
        }
    }
...
outer_fail()"#,
    ...
    golden_test_template(
        ...
        trim_rust_backtrace(&format!("{:?}", bsmr_error::Error::from(e))),
    );
}
