...
use bsmr_build_api::analysis::calculation::EVAL_ANALYSIS_QUERY;
use bsmr_build_api::analysis::calculation::RULE_ANALYSIS_CALCULATION;
use bsmr_build_api::analysis::calculation::RuleAnalysisCalculation;
use bsmr_build_api::analysis::calculation::RuleAnalysisCalculationImpl;
...
use bsmr_query::query::syntax::simple::eval::label_indexed::LabelIndexedSet;
...
impl Key for AnalysisKey {
    type Value = bsmr_error::Result<MaybeCompatible<AnalysisResult>>;
    async fn compute(
        &self,
...
pub async fn get_dep_analysis<'v>(
    ...
    ctx: &mut DiceComputations<'_>,
) -> bsmr_error::Result<Vec<(&'v ConfiguredTargetLabel, AnalysisResult)>> {
...
pub async fn get_rule_spec(
    ctx: &mut DiceComputations<'_>,
    func: &StarlarkRuleType,
) -> bsmr_error::Result<impl RuleSpec + use<>> {
...
async fn get_analysis_result_inner(
    ...
    let ((res, now, split_instants), spans): ((bsmr_error::Result<_>, _, _), _) =
            ...
            RuleType::Starlark(func) => {
                ...
                let (res, spans) = async_record_root_spans(async {
                    ...
                    let (result, split_instants) = span_async(start_event, async {
                        ...
                        let result: bsmr_error::Result<_> = try {
                            let (result, split) = span_async_simple(
                                bsmr_data::AnalysisStageStart {
                                    stage: Some(
                                        bsmr_data::analysis_stage_start::Stage::EvaluateRule(()),
                                    ),
                                },
                                run_analysis(
                                    ctx,
                    ...
                    bsmr_error::Ok((result, split_instants))
                })
                .await;
...
fn make_analysis_profile(res: &AnalysisResult) -> bsmr_error::Result<bsmr_data::AnalysisProfile> {
    let heap = res.providers()?.owner();

    Ok(bsmr_data::AnalysisProfile {
    ...
    let profile_datas = ctx
        .try_compute_join(all_deps.iter(), |ctx, node| {
            async move {
                ...
                bsmr_error::Ok(result.profile_data)
            }
            .boxed()
        })
        .await?;
