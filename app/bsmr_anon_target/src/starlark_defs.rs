...
use bsmr_build_api::interpreter::rule_defs::context::ANALYSIS_ACTIONS_METHODS_ANON_TARGET;
...
use bsmr_interpreter_for_build::rule::FrozenArtifactPromiseMappings;
...
impl<'v> StarlarkAnonTarget<'v> {
    ...
    ) -> bsmr_error::Result<StarlarkAnonTarget<'v>> {
...
fn anon_target_methods(builder: &mut MethodsBuilder) {
    ...
    fn artifact<'v>(
        ...
        match this.artifacts.get(name) {
            ...
            None => {
                let bsmr_error: bsmr_error::Error =
...
pub(crate) fn init_register_anon_target_types() {
    REGISTER_BSMR_ANON_TARGETS_GLOBALS.init(register_anon_target_types);
}
