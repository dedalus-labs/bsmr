...
use bsmr_analysis::analysis::calculation::AnalysisSplitInstants;
use bsmr_analysis::analysis::calculation::AnalysisWithExtraData;
use bsmr_analysis::analysis::calculation::AnonTargetSplitData;
use bsmr_analysis::analysis::calculation::get_rule_spec;
use bsmr_analysis::analysis::env::RuleSpec;
use bsmr_analysis::analysis::env::transitive_validations;
use bsmr_artifact::artifact::artifact_type::Artifact;
use bsmr_build_api::analysis::AnalysisResult;
use bsmr_build_api::analysis::anon_promises_dyn::AnonPromisesDyn;
use bsmr_build_api::analysis::anon_promises_dyn::RunAnonPromisesAccessorPair;
use bsmr_build_api::analysis::anon_targets_registry::ANON_TARGET_REGISTRY_NEW;
use bsmr_build_api::analysis::anon_targets_registry::AnonTargetsRegistryDyn;
...
use bsmr_build_api::artifact_groups::promise::PromiseArtifact;
use bsmr_build_api::artifact_groups::promise::PromiseArtifactId;
use bsmr_build_api::artifact_groups::promise::PromiseArtifactResolveError;
use bsmr_build_api::build::detailed_aggregated_metrics::dice::HasDetailedAggregatedMetrics;
use bsmr_build_api::deferred::calculation::DeferredHolder;
use bsmr_build_api::deferred::calculation::EVAL_ANON_TARGET;
use bsmr_build_api::deferred::calculation::GET_PROMISED_ARTIFACT;
use bsmr_build_api::interpreter::rule_defs::context::AnalysisContext;
use bsmr_build_api::interpreter::rule_defs::plugins::AnalysisPlugins;
...
use bsmr_configured::execution::find_execution_platform_by_configuration;
...
use bsmr_core::pattern::pattern::PatternData;
use bsmr_core::pattern::pattern::lex_target_pattern;
...
use bsmr_interpreter::starlark_promise::StarlarkPromise;
use bsmr_interpreter::types::configured_providers_label::StarlarkConfiguredProvidersLabel;
use bsmr_interpreter_for_build::attrs::coerce::arc_str_interner::ArcStrInterner;
use bsmr_interpreter_for_build::rule::FrozenStarlarkRuleCallable;
...
impl Key for AnonTargetKey {
    type Value = bsmr_error::Result<AnalysisResult>;
...
impl AnonTargetKey {
    fn downcast(key: Arc<dyn BaseDeferredKeyDyn>) -> bsmr_error::Result<Self> {
        Ok(AnonTargetKey(
            key.into_any()
                .downcast()
                .ok()
    ...
    fn parse_target_label(x: &str) -> bsmr_error::Result<TargetLabel> {
        let err = || {
            format!(
    ...
    fn create_name(rule_name: &str) -> bsmr_error::Result<TargetLabel> {
    ...
    fn coerce_name(x: Value) -> bsmr_error::Result<TargetLabel> {
    ...
    fn coerce_to_anon_target_attr(
        ...
        x: Value,
        ctx: &AnonAttrCtx,
    ) -> bsmr_error::Result<AnonTargetAttr> {
    ...
    ) -> BoxFuture<'a, bsmr_error::Result<(AnalysisResult, Option<AnalysisSplitInstants>)>> {
...
impl<'v> AnonTargetsRegistry<'v> {
    ...
    ) -> bsmr_error::Result<&'v mut AnonTargetsRegistry<'v>> {
    ...
    pub(crate) fn anon_target_key(
        ...
        attributes: UnpackDictEntries<&'v str, Value<'v>>,
        owner_key: &BaseDeferredKey,
    ) -> bsmr_error::Result<AnonTargetKey> {
    ...
    pub(crate) fn register_artifact(
        ...
        id: usize,
    ) -> bsmr_error::Result<PromiseArtifact> {
...
impl<'v> AnonTargetsRegistryDyn<'v> for AnonTargetsRegistry<'v> {
    ...
    */
    ...
    fn assert_no_promises(&self) -> bsmr_error::Result<()> {
