...
enum ProviderCollectionError {
    ...
    #[error(
        "Cannot handle flavor `{flavor}` on target `{target}`. Most flavors are unsupported in Bessemer."
    )]
...
/// pagable-only (`bsmr_core` cannot depend on `starlark`), so the
...
impl<'v, V: ValueLike<'v>> ProviderCollectionGen<V> {
    ...
    fn try_from_value_impl(
        mut value: Value<'v>,
    ) -> bsmr_error::Result<SmallMap<CollectionKey, Value<'v>>> {
    ...
    pub fn try_from_value(value: Value<'v>) -> bsmr_error::Result<ProviderCollection<'v>> {
    ...
    fn get_impl(
        ...
        index: Value<'v>,
        op: GetOp,
    ) -> bsmr_error::Result<Either<Value<'v>, Arc<ProviderId>>> {
    ...
    pub(crate) fn get(
        &self,
        index: Value<'v>,
    ) -> bsmr_error::Result<NoneOr<ValueOfUnchecked<'v, AbstractProvider>>> {
    ...
    fn at(&self, index: Value<'v>, _heap: Heap<'v>) -> starlark::Result<Value<'v>> {
        match self.get_impl(index, GetOp::At)? {
            ...
            Either::Right(provider_id) => {
                Err(bsmr_error::Error::from(ProviderCollectionError::AtNotFound(
...
impl FrozenProviderCollection {
    pub fn default_info<'a>(
        &'a self,
    ) -> bsmr_error::Result<FrozenValueTyped<'a, FrozenDefaultInfo>> {
...
impl FrozenProviderCollectionValue {
    ...
    pub fn try_from_value(value: OwnedFrozenValue) -> bsmr_error::Result<Self> {
        Ok(Self {
    ...
    ) -> bsmr_error::Result<FrozenProviderCollectionValueRef<'f>> {
...
impl<'f> FrozenProviderCollectionValueRef<'f> {
    ...
    pub fn lookup_inner(
        ...
        match label.name() {
            ProvidersName::Default => bsmr_error::Ok(self),
            ...
            let collection = frozen
                ...
                .ok_or_else(|| {
                    bsmr_error::bsmr_error!(
                        bsmr_error::ErrorTag::StarlarkError,
