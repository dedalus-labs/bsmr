...
use bsmr_test_api::data::TestListingEntry;
use bsmr_test_api::data::TestResultEntry;
...
/// Provider that signals that a rule can be tested using Bessemer's internal test
...
impl FrozenInternalRunnerTestInfo {
    ...
    pub fn parse_test_listing_output(
        &self,
        listing_content: &str,
    ) -> bsmr_error::Result<Vec<TestListingEntry>> {
        let callback = self.parse_test_listing();

        // patternlint-disable-next-line bsmr-no-starlark-module: calling frozen callback
        Module::with_temp_heap(|env| {
            ...
            let result = eval
                ...
                .map_err(|e| bsmr_error!(bsmr_error::ErrorTag::Input, "{}", e))?;
            ...
            list.iter()
                .map(|item| {
                    ...
                    let get_required_str = |key: &str| -> bsmr_error::Result<String> {
    ...
    pub fn parse_test_result_output(
        ...
        stderr: &str,
        exit_code: i32,
    ) -> bsmr_error::Result<Vec<TestResultEntry>> {
        ...
        Module::with_temp_heap(|env| {
            ...
            list.iter()
                .map(|item| {
                    ...
                    let get_opt_str = |key: &str| -> bsmr_error::Result<Option<String>> {
                        match dict.get_str(key) {
                            Some(v) => {
                                if v.is_none() {
                                    Ok(None)
    ...
    V: ValueLike<'v>,
{
    ...
    let provided_local_resources =
        iter_local_resources(info.local_resources.get().to_value())
            .collect::<bsmr_error::Result<IndexMap<&str, Option<&ConfiguredProvidersLabel>>>>()?;
