...
pub struct DefaultInfoGen<V: ValueLifetimeless> {
    ...
    /// subtarget, such as when doing `bsmr build cell//foo:bar[baz]`. Just like any
    ...
    /// e.g.: `bsmr build cell//foo:bar[baz][qux]`.
...
fn validate_default_info(info: &FrozenDefaultInfo) -> bsmr_error::Result<()> {
...
impl FrozenDefaultInfo {
    ...
    fn get_sub_target_providers_impl(
        &self,
        name: &str,
    ) -> bsmr_error::Result<Option<FrozenValueTyped<'static, FrozenProviderCollection>>> {
    ...
    fn default_outputs_impl(
        &self,
    ) -> bsmr_error::Result<impl Iterator<Item = bsmr_error::Result<StarlarkArtifact>> + '_> {
    ...
    fn sub_targets_impl(
        ...
        impl Iterator<
            Item = bsmr_error::Result<(&str, FrozenValueTyped<'static, FrozenProviderCollection>)>,
        > + '_,
    > {
    ...
    fn for_each_in_list(
        &self,
        value: FrozenValue,
        mut processor: impl FnMut(Value) -> bsmr_error::Result<()>,
    ) -> bsmr_error::Result<()> {
    ...
    fn DefaultInfo<'v>(
        ...
        let valid_default_outputs: ValueOfUnchecked<ListType<ValueIsInputArtifactAnnotation>> =
            match (default_outputs.into_option(), default_output.into_option()) {
                ...
                (Some(_), Some(_)) => {
                    return Err(
                        bsmr_error::Error::from(DefaultOutputError::ConflictingArguments).into(),
                    );
                }
            };
        ...
        let valid_sub_targets = sub_targets
            ...
            .collect::<bsmr_error::Result<Vec<(StringValue<'v>, _)>>>()?;
