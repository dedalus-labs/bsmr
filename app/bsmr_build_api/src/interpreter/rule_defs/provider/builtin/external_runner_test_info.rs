...
/// Legacy-compatible API for tests.
...
impl FrozenExternalRunnerTestInfo {
    ...
    pub fn use_project_relative_paths(&self) -> bool {
        NoneOr::<bool>::unpack_value(self.use_project_relative_paths.get().to_value())
            ...
            .unwrap()
            .into_option()
            .unwrap_or_else(bsmr_core::is_open_source)
    }
    ...
    pub fn local_resources(&self) -> BsmrIndexMap<&str, Option<&ConfiguredProvidersLabel>> {
...
pub(super) fn iter_value<'v>(
    value: Value<'v>,
) -> bsmr_error::Result<impl Iterator<Item = Value<'v>> + 'v> {
...
pub(super) fn iter_test_command<'v>(
    command: Value<'v>,
) -> impl Iterator<Item = bsmr_error::Result<TestCommandMember<'v>>> {
    if command.is_none() {
    ...
    Either::Right(iterable.map(|item| {
        ...
        let arglike = ValueAsCommandLineLike::unpack_value_err(item)
            .with_bsmr_error_context(|| format!("Invalid item in `command`: {item}"))?
            .0;
...
pub(super) fn iter_test_env<'v>(
    env: Value<'v>,
) -> impl Iterator<Item = bsmr_error::Result<(&'v str, &'v dyn CommandLineArgLike<'v>)>> {
    if env.is_none() {
        return Either::Left(Either::Left(empty()));
    }
    ...
    let env = match DictRef::from_value(env) {
        Some(env) => env,
        None => {
            return Either::Left(Either::Right(once(Err(bsmr_error!(
...
pub(super) fn iter_opt_str_list<'v>(
    list: Value<'v>,
    name: &'static str,
) -> impl Iterator<Item = bsmr_error::Result<&'v str>> {
    if list.is_none() {
        return Either::Left(Either::Left(empty()));
    }
...
) -> impl Iterator<Item = bsmr_error::Result<(&'v str, &'v StarlarkCommandExecutorConfig)>> {
...
) -> impl Iterator<Item = bsmr_error::Result<(&'v str, Option<&'v ConfiguredProvidersLabel>)>> {
...
pub(super) fn unpack_opt_executor<'v>(
    executor: Value<'v>,
) -> bsmr_error::Result<Option<&'v StarlarkCommandExecutorConfig>> {
    if executor.is_none() {
        return Ok(None);
    }
...
pub(super) fn unpack_opt_worker<'v>(
    worker: Value<'v>,
) -> bsmr_error::Result<Option<&'v WorkerInfo<'v>>> {
    if worker.is_none() {
        return Ok(None);
    }
...
pub(super) fn check_all<I, T>(it: I) -> bsmr_error::Result<()>
where
    I: IntoIterator<Item = bsmr_error::Result<T>>,
{
    for e in it {
        e?;
    }
    ...
    V: ValueLike<'v>,
{
    ...
    let provided_local_resources = iter_local_resources(info.local_resources.get().to_value())
        .collect::<bsmr_error::Result<
        BsmrIndexMap<&str, Option<&ConfiguredProvidersLabel>>,
    >>()?;
