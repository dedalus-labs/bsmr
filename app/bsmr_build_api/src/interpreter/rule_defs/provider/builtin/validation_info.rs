...
/// `bsmr build`, `bsmr test`), Bessemer schedules every `ValidationSpec`
...
/// they only have to finish before Bessemer reports success.
///
...
fn validate_validation_info<'v, V>(info: &ValidationInfoGen<V>) -> bsmr_error::Result<()>
where
    V: ValueLike<'v>,
{
    let values = ListRef::from_value(info.validations.get().to_value())
        .ok_or(bsmr_error::Error::from(
