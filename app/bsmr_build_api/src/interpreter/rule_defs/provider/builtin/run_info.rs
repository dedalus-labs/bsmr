...
/// provides `RunInfo`, it can be executed with `bsmr run <target>`. The provider wraps the
...
/// ```bash
/// bsmr run <target>                    # Run with the args specified in RunInfo
/// bsmr run <target> -- [extra args]    # Pass additional args after '--' to the command
/// ```
/// ## Examples
///
