...
use bsmr_core::error::validate_logview_category;
...
/// ```
///
/// Then register a target of this rule in root .buckconfig under `bsmr.dep_only_incompatible_info`
    ...
    fn DepOnlyIncompatibleInfo<'v>(
        ...
        for (category, value) in custom_soft_errors.entries {
            let category_str = category.to_value().unpack_str().ok_or_else(|| {
                starlark::Error::from(bsmr_error::internal_error!(
...
impl FrozenDepOnlyIncompatibleInfo {
    ...
    ) -> bsmr_error::Result<DepOnlyIncompatibleCustomSoftErrors> {
        let value_to_target_pattern =
            |target_pattern: Value<'_>| -> bsmr_error::Result<ParsedPattern<TargetPatternExtra>> {
