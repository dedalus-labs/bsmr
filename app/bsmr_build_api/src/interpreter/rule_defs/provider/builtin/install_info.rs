...
impl<'v, V: ValueLike<'v>> InstallInfoGen<V> {
    pub fn get_installer(&self) -> bsmr_error::Result<ConfiguredProvidersLabel> {
    ...
    fn get_files_iter<'a>(
        files: &'a DictRef<'v>,
    ) -> impl Iterator<Item = bsmr_error::Result<(&'v str, ValueAsInputArtifactLike<'v>)>> + 'a
    {
        files.iter().map(|(k, v)| {
            let k = k
                .unpack_str()
    ...
    pub fn get_files(&self) -> bsmr_error::Result<SmallMap<&'v str, Artifact>> {
...
fn validate_install_info<'v, V>(info: &InstallInfoGen<V>) -> bsmr_error::Result<()>
where
    V: ValueLike<'v>,
{
