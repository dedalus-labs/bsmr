...
fn create_callable_function_signature(
    ...
    ret_ty: Ty,
) -> bsmr_error::Result<(ParametersSpec<FrozenValue>, TyCallable)> {
...
impl ProviderCallableLike for UserProviderCallable {
    fn id(&self) -> bsmr_error::Result<&Arc<ProviderId>> {
        self.callable
            .get()
            .map(|x| &x.id)
...
impl<'v> StarlarkValue<'v> for UserProviderCallable {
    ...
    fn export_as(
        ...
        self.callable.get_or_try_init(|| {
            ...
            let ty_provider_type_instance_id =
                TypeInstanceId::from_identity(BsmrTypeIdDomain::UserProvider, &*provider_id);
            ...
            bsmr_error::Ok(UserProviderCallableNamed {
                id: provider_id.dupe(),
                signature,
                data: eval
                    .frozen_heap()
    ...
    fn invoke(
        ...
        match self.callable.get() {
            ...
            None => Err(bsmr_error::Error::from(ProviderCallableError::NotBound).into()),
        }
    }
...
fn provider_field_parse_type<'v>(
    ty: Value<'v>,
    eval: &mut Evaluator<'v, '_, '_>,
) -> bsmr_error::Result<TypeCompiled<FrozenValue>> {
        ...
        let fields = match fields {
            Either::Left(fields) => {
                ...
                if new_fields.len() != fields.items.len() {
                    return Err(
                        bsmr_error::Error::from(ProviderCallableError::NonUniqueFields(
                            fields.items,
                        ))
                        .into(),
                    );
