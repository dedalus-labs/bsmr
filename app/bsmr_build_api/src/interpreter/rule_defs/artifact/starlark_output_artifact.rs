...
///
/// When you create custom actions with `ctx.actions.run()`, you need to tell Bessemer which artifacts
...
/// - If you forget to bind a declared artifact, Bessemer will raise an error
#[starlark_module]
...
impl<'v, V: ValueLike<'v>> CommandLineArgLike<'v> for StarlarkOutputArtifactGen<V> {
    ...
    fn add_to_command_line(&self, fmt: &mut CommandLineBuilder<'v, '_>) -> bsmr_error::Result<()> {
        match self.unpack() {
            Either::Left(_) => Err(bsmr_error::internal_error!(
