...
impl StarlarkPromiseArtifact {
    ...
    fn short_path_err(&self) -> bsmr_error::Result<&ForwardRelativePath> {
        self.short_path
            .as_deref()
            .ok_or_else(|| PromiseArtifactError::NoShortPathPromised(self.clone()).into())
    }

    fn file_name_err(&self) -> bsmr_error::Result<&FileName> {
...
impl<'v> StarlarkArtifactLike<'v> for StarlarkPromiseArtifact {
    ...
    ) -> bsmr_error::Result<StringValue<'v>> {
        match self.artifact.get() {
    ...
    fn is_source(&'v self) -> bsmr_error::Result<bool> {
        Ok(false)
    }

    fn owner(&'v self) -> bsmr_error::Result<Option<BaseDeferredKey>> {
        match self.artifact.get() {
...
impl<'v> StarlarkInputArtifactLike<'v> for StarlarkPromiseArtifact {
    fn get_bound_artifact(&self) -> bsmr_error::Result<Artifact> {
        match self.artifact.get() {
            Some(v) => Ok(v.dupe()),
    ...
    fn as_output_error(&self) -> bsmr_error::Error {
    ...
    fn get_artifact_group(&self) -> bsmr_error::Result<ArtifactGroup> {
        Ok(self.as_artifact())
    }

    fn as_output(&'v self, _this: Value<'v>) -> bsmr_error::Result<StarlarkOutputArtifact<'v>> {
    ...
    fn project(
        ...
        hide_prefix: bool,
    ) -> bsmr_error::Result<EitherStarlarkInputArtifact<'v>> {
