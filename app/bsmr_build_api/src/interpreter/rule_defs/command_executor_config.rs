...
use bsmr_core::execution_types::executor_config::CacheUploadBehavior;
...
use bsmr_core::execution_types::executor_config::ExecutorNetworkAccess;
use bsmr_core::execution_types::executor_config::HybridExecutionLevel;
use bsmr_core::execution_types::executor_config::ImagePackageIdentifier;
...
use bsmr_core::execution_types::executor_config::ReGang;
...
use bsmr_core::execution_types::executor_config::RemoteEnabledExecutor;
use bsmr_core::execution_types::executor_config::RemoteEnabledExecutorOptions;
use bsmr_core::execution_types::executor_config::RemoteExecutionPolicy;
use bsmr_core::execution_types::executor_config::RemoteExecutorCafFbpkg;
        ...
        let command_executor_config = {
            ...
            let re_properties = if remote_execution_properties.is_none() {
                ...
                let re_properties = DictRef::from_value(remote_execution_properties.to_value())
                    .ok_or_else(|| {
                        bsmr_error::Error::from(CommandExecutorConfigErrors::RePropertiesNotADict(
            ...
            let re_dependencies = remote_execution_dependencies
                ...
                .collect::<bsmr_error::Result<Vec<RemoteExecutorDependency>>>()?;

            let re_gang_workers = remote_execution_gang_workers
                .into_iter()
                .map(ReGangWorker::parse)
                .collect::<bsmr_error::Result<Vec<ReGangWorker>>>()?;
            ...
            if extra_params.gang.is_some() && !re_gang_workers.is_empty() {
                return Err(bsmr_error::Error::from(
            ...
            let remote_cache_default = if bsmr_core::is_open_source() {
                remote_enabled
            } else {
                true
            };
            ...
            let executor = match (local_options, remote_options, remote_cache_enabled) {
                (local, Some(remote), remote_cache_enabled) => {
                    ...
                    Executor::RemoteEnabled(RemoteEnabledExecutorOptions {
                        executor,
                        re_properties: re_properties.ok_or(bsmr_error::Error::from(
                            CommandExecutorConfigErrors::MissingField(
                                "remote_execution_properties",
                            ),
                        ))?,
                        re_use_case: re_use_case.ok_or(bsmr_error::Error::from(
                ...
                (Some(local), None, true) => {
                    Executor::RemoteEnabled(RemoteEnabledExecutorOptions {
                        ...
                        re_use_case: re_use_case
                            .unwrap_or_else(RemoteExecutorUseCase::bsmr_default),
                        re_action_key,
                        cache_upload_behavior,
...
pub fn parse_custom_re_image(
    ...
    value: Value,
) -> bsmr_error::Result<Option<Box<RemoteExecutorCustomImage>>> {
    if value.is_none() {
        return Ok(None);
    }

    fn dict_ref<'v>(field_name: &'static str, value: Value<'v>) -> bsmr_error::Result<DictRef<'v>> {
    ...
    fn get_value<'v>(dict_ref: &DictRef<'v>, name: &'static str) -> bsmr_error::Result<Value<'v>> {
    ...
    fn get_string<'v>(dict_ref: &DictRef<'v>, name: &'static str) -> bsmr_error::Result<String> {
...
fn parse_remote_execution_policy(
    policy: Option<Value>,
) -> bsmr_error::Result<RemoteExecutionPolicy> {
    match policy {
        None => Ok(RemoteExecutionPolicy::default()),
        Some(policy) => {
            let re_policy_dict = DictRef::from_value(policy.to_value()).ok_or_else(|| {
                bsmr_error::Error::from(CommandExecutorConfigErrors::RePolicyNotADict(
...
) -> bsmr_error::Result<Vec<RemoteExecutorCafFbpkg>> {
    match caf_fbpkgs {
        None => Ok(vec![]),
        Some(caf_fbpkgs) => {
            let re_caf_fbpkgs_list =
                ListRef::from_value(caf_fbpkgs.to_value()).ok_or_else(|| {
                    bsmr_error::Error::from(CommandExecutorConfigErrors::ReCafFbpkgsNotAList(
            ...
            Ok(re_caf_fbpkgs_list
                ...
                .map(|caf_fbpkg| match DictRef::from_value(caf_fbpkg) {
                    ...
                    None => Err(bsmr_error::Error::from(
                ...
                .collect::<bsmr_error::Result<Vec<RemoteExecutorCafFbpkg>>>()?)
        }
    }
}
...
) -> bsmr_error::Result<Arc<MetaInternalExtraParams>> {
...
fn parse_remote_execution_gang<'v>(
...
) -> bsmr_error::Result<Option<ReGang>> {
    ...
    let capabilities_dict = DictRef::from_value(capabilities_value).ok_or_else(|| {
        bsmr_error::Error::from(CommandExecutorConfigErrors::ReGangCapabilitiesNotADict(
    ...
    let num_of_workers: i32 = num_of_workers_value.unpack_i32().ok_or_else(|| {
        bsmr_error::Error::from(CommandExecutorConfigErrors::ReGangNumOfWorkersNotAnInt(
    ...
    let num_sub_groups = gang
        ...
        .map(|v| {
            v.unpack_i32().ok_or_else(|| {
                bsmr_error::Error::from(CommandExecutorConfigErrors::ReGangNumSubGroupsNotAnInt(
                    v.to_repr(),
                ))
            })
        })
    ...
    let resource_units = gang
        ...
        .map(|v| {
            v.unpack_i32().map(i64::from).ok_or_else(|| {
                bsmr_error::Error::from(CommandExecutorConfigErrors::ReGangResourceUnitsNotAnInt(
                    v.to_repr(),
                ))
            })
        })
