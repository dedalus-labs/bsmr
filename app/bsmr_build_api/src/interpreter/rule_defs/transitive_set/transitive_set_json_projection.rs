...
impl<'v, V: ValueLike<'v>> TransitiveSetJsonProjectionGen<V> {
    fn projection_name(&self) -> bsmr_error::Result<&'v str> {
    ...
    pub(crate) fn to_projection_key_wrapper(
        &self,
    ) -> bsmr_error::Result<TransitiveSetProjectionWrapper> {
...
impl<'v, V: ValueLike<'v>> TransitiveSetJsonProjectionGen<V> {
    pub fn iter_values<'a>(&'a self) -> bsmr_error::Result<Box<dyn Iterator<Item = Value<'v>> + 'a>>
    where
        'v: 'a,
    {
        let set = TransitiveSet::from_value(self.transitive_set.get().to_value())
