...
impl ProjectionBitSet {
    ...
    pub fn from_bools(bools: &[bool]) -> bsmr_error::Result<Self> {
    ...
    pub fn get(self, index: usize) -> bsmr_error::Result<bool> {
...
impl<'v, V: ValueLike<'v>> TransitiveSetGen<V> {
    ...
    pub fn projection_name(&'v self, projection: usize) -> bsmr_error::Result<&'v str> {
    ...
    pub fn get_projection_value(&self, projection: usize) -> bsmr_error::Result<Option<V>> {
        match &self.node {
            None => Ok(None),
            Some(node) => Ok(Some(
                *node
    ...
    TransitiveSetGen<V>: StarlarkValue<'v> + TransitiveSetLike<'v>,
{
    ...
    pub fn iter_values<'a>(
        &'a self,
        ordering: TransitiveSetOrdering,
    ) -> bsmr_error::Result<Box<dyn Iterator<Item = Value<'v>> + 'a>>
    where
        'v: 'a,
    {
        Ok(Box::new(
        ...
        let node = value.try_map(|value| {
            let projections = def
                ...
                .map(|(name, spec)| {
                    ...
                    bsmr_error::Ok(projected_value)
                })
                .collect::<Result<Box<[_]>, _>>()?;

            bsmr_error::Ok(NodeGen { value, projections })
        })?;
...
fn transitive_set_methods(builder: &mut MethodsBuilder) {
    ...
    fn reduce<'v>(
        ...
        let index = match def.operations().reductions.get_index_of(reduction) {
            ...
            None => {
                return Err(
                    bsmr_error::Error::from(TransitiveSetError::ReductionDoesNotExist {
