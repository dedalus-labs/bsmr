...
/// produced, is parsed by Bessemer to decide pass/fail. Group one or more
...
pub struct StarlarkValidationSpecGen<V: ValueLifetimeless> {
    ...
    /// produces it runs, Bessemer reads the file as UTF-8 JSON and expects
    /// the following shape:
    ///
    /// ```json
    /// {
    ...
    ///
    /// Bessemer surfaces three distinct errors if the file does not conform:
...
fn validate_validation_spec<'v, V>(spec: &StarlarkValidationSpecGen<V>) -> bsmr_error::Result<()>
where
    V: ValueLike<'v>,
{
...
fn validation_spec_methods(builder: &mut MethodsBuilder) {
    ...
    /// runs, Bessemer reads the file as UTF-8 JSON and uses its contents to
    /// decide pass/fail.
    ///
    /// Expected shape:
    ///
    ...
    /// by Bessemer — both at the top level (alongside `version` / `data`) and
