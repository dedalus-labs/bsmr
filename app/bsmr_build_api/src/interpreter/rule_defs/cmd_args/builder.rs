...
impl<'v, 'a> CommandLineBuilder<'v, 'a> {
    ...
    pub fn push_artifact(&mut self, artifact: &Artifact) -> bsmr_error::Result<()> {
        self.write_project_path(
            artifact.resolve_path(self.fs.fs(), self.artifact_path_mapping.get(artifact))?,
        )
    }

    pub fn push_output_artifact(&mut self, artifact: &Artifact) -> bsmr_error::Result<()> {
    ...
    pub(crate) fn push_next_write_to_file_macro_path(&mut self) -> bsmr_error::Result<()> {
        ...
        let p = write_to_file_macro_paths
            .pop()
            .ok_or_else(|| bsmr_error::internal_error!("Inconsistent number of macro artifacts"))?;
        self.write_project_path(p)
    }

    pub fn push_cell_path(&mut self, path: CellPathRef) -> bsmr_error::Result<()> {
        self.write_project_path(self.fs.fs().resolve_cell_path(path)?)
    }

    pub fn push_project_path(&mut self, path: ProjectRelativePathBuf) -> bsmr_error::Result<()> {
        self.write_project_path(path)
    }

    fn write_project_path(&mut self, mut path: ProjectRelativePathBuf) -> bsmr_error::Result<()> {
        let ArtifactOptions {
            relative_to,
            absolute_prefix,
            absolute_suffix,
