...
pub struct ConfiguredBuildTargetResultGen<T> {
    ...
    pub graph_properties: Option<bsmr_error::Result<MaybeCompatible<GraphPropertiesValues>>>,
    pub errors: Vec<Timed<bsmr_error::Error>>,
}
...
impl<T> ConfiguredBuildTargetResultGen<T> {
    ...
    pub fn timed_out(&self) -> bool {
        self.errors
            .iter()
            .any(|e| e.inner.has_tag(bsmr_error::ErrorTag::BuildDeadlineExpired))
    }
}

pub type ConfiguredBuildTargetResult =
    ConfiguredBuildTargetResultGen<bsmr_error::Result<ProviderArtifacts>>;
...
pub struct BuildTargetResultBuilder {
    res: HashMap<
        ...
        Option<ConfiguredBuildTargetResultGen<(usize, bsmr_error::Result<ProviderArtifacts>)>>,
    >,
    configured_to_pattern_modifiers: HashMap<ConfiguredProvidersLabel, Vec<Modifiers>>,
    other_errors: BTreeMap<Option<ProvidersLabel>, Vec<bsmr_error::Error>>,
    build_failed: bool,
...
impl BuildTargetResultBuilder {
    ...
    pub fn event(&mut self, event: BuildEvent) -> bsmr_error::Result<FailFastState> {
        ...
        match variant {
            ...
            ConfiguredBuildEventVariant::Timeout => {
                ...
                results.errors.push(Timed {
                    inner: bsmr_error::Error::from(BuildDeadlineExpired),
                    elapsed,
                });
                // TODO(cjhopman): Why don't we break here?
                self.build_failed = true;
...
pub struct BuildTargetResult {
    ...
    pub other_errors: BTreeMap<Option<ProvidersLabel>, Vec<bsmr_error::Error>>,
...
pub enum ConfiguredBuildEventExecutionVariant {
    BuildOutput {
        output: bsmr_error::Result<ProviderArtifacts>,
...
pub enum ConfiguredBuildEventVariant {
    ...
    GraphProperties {
        graph_properties: bsmr_error::Result<MaybeCompatible<GraphPropertiesValues>>,
    },
    Error {
