...
impl ArtifactGroup {
    ...
    ) -> bsmr_error::Result<ResolvedArtifactGroup<'_>> {
        Ok(match self {
    ...
    pub fn is_eligible_for_dedupe(
        ...
        if is_artifact_group_eligible_for_dedupe {
            return bsmr_data::EligibleForDedupe::Eligible;
        }
        ...
        let artifact_group_owner = match self {
            ...
            ArtifactGroup::Promise(_) => return bsmr_data::EligibleForDedupe::IneligibleInput,
        };
        ...
        match artifact_group_owner {
            BaseDeferredKey::TargetLabel(a) => {
                if a.cfg().is_marked_as_exec_platform() {
                    if target_platform.is_some_and(|tp| tp == a.cfg()) {
                        bsmr_data::EligibleForDedupe::ExecutionPlatformUnknownEligibility
                    } else {
            ...
            BaseDeferredKey::AnonTarget(_) => bsmr_data::EligibleForDedupe::Eligible,
            BaseDeferredKey::BxlLabel(_) => bsmr_data::EligibleForDedupe::IneligibleInput,
        }
    }
}
