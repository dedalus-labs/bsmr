...
use bsmr_node::load_patterns::load_patterns_with_modifiers;
                ...
                let local_cfg_options = match package_with_modifiers.modifiers.as_slice() {
                    Some(modifiers) => {
                        if !global_cfg_options.cli_modifiers.is_empty() {
                            let error = bsmr_error::Error::from(
...
pub async fn get_compatible_targets(
    ...
    global_cfg_options: &GlobalCfgOptions,
    keep_going: bool,
) -> bsmr_error::Result<ConfiguredTargetsWithErrors> {
