...
use bsmr_data::ActionSubError;
use bsmr_data::CommandExecution;
...
fn action_error_context_methods(builder: &mut MethodsBuilder) {
    ...
    /// should not go here - bsmr tries to categorize these types of errors automatically.
    ...
    fn parse_with_errorformat<'v>(
        ...
        let error_lines = bsmr_errorformat::split_lines(&error);
        let error_entries = bsmr_errorformat::parse_error_format(errorformats.items, error_lines)
...
impl StarlarkActionSubError {
    pub(crate) fn from_errorformat_entry(entry: bsmr_errorformat::Entry, category: String) -> Self {
