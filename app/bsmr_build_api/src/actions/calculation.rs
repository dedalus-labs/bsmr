...
use bsmr_data::ActionErrorDiagnostics;
...
async fn build_action_no_redirect(
    ...
    let start_event = bsmr_data::ActionExecutionStart {
...
async fn build_action_inner(
    ...
    let is_expected_eligible_for_dedupe = match action.is_expected_eligible_for_dedupe() {
        Some(v) => {
            if v {
                bsmr_data::ExpectedEligibleForDedupe::ExpectedEligible
            } else {
                bsmr_data::ExpectedEligibleForDedupe::ExpectedIneligible
            }
        }
        None => bsmr_data::ExpectedEligibleForDedupe::UnknownEligibility,
    };
    ...
    let action_name = bsmr_data::ActionName {
        category: action.category().as_str().to_owned(),
        identifier: action.identifier().unwrap_or("").to_owned(),
    };
    ...
    let mut bsmr_revision = None;
    let mut bsmr_build_time = None;
    ...
    let error_diagnostics = match execute_result {
        Ok((outputs, meta)) => {
            ...
            if matches!(
                ...
                bsmr_data::ActionExecutionKind::Local
                    ...
                    | bsmr_data::ActionExecutionKind::LocalDepFile
                    | bsmr_data::ActionExecutionKind::LocalActionCache
            ) {
                hostname = bsmr_events::metadata::hostname();
            }
        ...
        Err(e) => {
            ...
            // so as to reduce Scribe traffic and log it in bsmr_action_errors
            bsmr_revision = bsmr_build_info::revision().map(|s| s.to_owned());
            bsmr_build_time = bsmr_build_info::time_iso8601().map(|s| s.to_owned());
            hostname = bsmr_events::metadata::hostname();
            ...
            let outputs = match &e {
                ExecuteError::CommandExecutionError { action_outputs, .. } => {
                    cache_upload_result = Some(bsmr_data::UploadResult::ActionNotSuccessful);
                    Some(action_outputs)
                }
                _ => None,
            };
            ...
            action_result = Err(bsmr_error::Error::from(e)
    ...
    let invalidation_info = if executor.invalidation_tracking_enabled() {
        fn to_proto(
        ...
        ) -> Option<bsmr_data::command_invalidation_info::InvalidationSource> {
            match invalidation_path {
                ...
                dice::DiceTrackedInvalidationPath::Invalidated(_) => {
                    Some(bsmr_data::command_invalidation_info::InvalidationSource {})
                }
            }
        }
        let invalidation_paths = ctx.get_invalidation_paths();
        Some(bsmr_data::CommandInvalidationInfo {
    ...
    let execution_kind = execution_kind.unwrap_or(bsmr_data::ActionExecutionKind::NotSet);
    let cache_upload_result =
        cache_upload_result.unwrap_or(bsmr_data::UploadResult::NonCommandAction);
    let dep_file_cache_upload_result =
        dep_file_cache_upload_result.unwrap_or(bsmr_data::UploadResult::NotAttempted);
    ...
    (
        ...
        Box::new(bsmr_data::ActionExecutionEnd {
            key: Some(action_key),
            kind: action.kind().into(),
...
fn is_action_eligible_for_dedupe(
    ...
    if !action.all_outputs_are_content_based() {
        return bsmr_data::EligibleForDedupe::IneligibleOutput;
    }

    for (ag, _agv) in inputs.iter() {
        let eligibility = ag.is_eligible_for_dedupe(target_platform);
        if eligibility != bsmr_data::EligibleForDedupe::Eligible {
            return eligibility;
        }
    }

    bsmr_data::EligibleForDedupe::Eligible
}

fn check_infra_error_patterns(
    last_command: Option<&bsmr_data::CommandExecution>,
) -> Option<bsmr_error::ErrorTag> {
...
fn try_run_error_handler(
    action: Arc<RegisteredAction>,
    last_command: Option<&bsmr_data::CommandExecution>,
    artifact_fs: bsmr_error::Result<ArtifactFs>,
    outputs: Option<&ActionOutputs>,
) -> Option<ActionErrorDiagnostics> {
    use bsmr_data::action_error_diagnostics::Data;

    fn create_error(
        e: bsmr_error::Error,
    ) -> (
        Option<ActionErrorDiagnostics>,
        bsmr_data::ActionErrorHandlerExecutionEnd,
    ) {
        (
    ...
    match action.action.error_handler() {
        Some(error_handler) => {
            ...
            dispatcher
                .clone()
                .span(bsmr_data::ActionErrorHandlerExecutionStart {}, || {
                    // patternlint-disable-next-line bsmr-no-starlark-module: FIXME(JakobDegen): Wrong
                    Module::with_temp_heap(|env| {
                        ...
                        let data = match error_handler_result {
                            ...
                            Err(e) => {
                                let e = bsmr_error::Error::from(e).context("Error handler failed");
...
pub struct ActionExtraData {
    ...
    pub invalidation_info: Option<bsmr_data::CommandInvalidationInfo>,
    /// RE platform name if the action ran remotely.
    pub re_platform_name: Option<String>,
}

struct ActionExecutionData {
    action_result: bsmr_error::Result<ActionOutputs>,
...
impl ActionCalculation {
    pub async fn get_action(
    ...
    ) -> bsmr_error::Result<Arc<RegisteredAction>> {
        ...
        match deferred_holder.lookup_action(action_key)? {
            ...
            ActionLookup::Deferred(action_key) => {
                ...
                ) -> BoxFuture<'a, bsmr_error::Result<Arc<RegisteredAction>>> {
...
impl Key for BuildKey {
    type Value = bsmr_error::Result<ActionOutputs>;
...
) -> bsmr_data::CommandExecution {
    let details = command_details(report, allow_omit_details).await;

    let status = match &report.status {
        CommandExecutionStatus::Success { .. } => bsmr_data::command_execution::Success {}.into(),
        CommandExecutionStatus::Cancelled { .. } => {
            bsmr_data::command_execution::Cancelled {}.into()
        }
        CommandExecutionStatus::Failure { .. } => bsmr_data::command_execution::Failure {}.into(),
        CommandExecutionStatus::WorkerFailure { .. } => {
            bsmr_data::command_execution::WorkerFailure {}.into()
        }
        CommandExecutionStatus::TimedOut { duration, .. } => {
            bsmr_data::command_execution::Timeout {
                duration: (*duration).try_into().ok(),
            }
            .into()
        }
        CommandExecutionStatus::Error { stage, error, .. } => bsmr_data::command_execution::Error {
...
pub async fn command_details(
    ...
    allow_omit_details: bool,
) -> bsmr_data::CommandExecutionDetails {
