...
use bsmr_directory::directory::directory_hasher::NoDigest;
...
impl<'v> ActionsRegistry<'v> {
    ...
    pub fn finalize(
        ...
        impl FnOnce(&AnalysisValueFetcher) -> bsmr_error::Result<RecordedActions> + use<>,
    > {
        ...
        Ok(move |analysis_value_fetcher: &AnalysisValueFetcher| {
            // Bessemer has an invariant that pairs of categories and identifiers are unique throughout a build. That
...
impl RecordedActions {
    ...
    pub fn lookup(&self, key: &ActionKey) -> bsmr_error::Result<ActionLookup> {
        self.actions
