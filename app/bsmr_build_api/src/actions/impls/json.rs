...
struct BsmrErrorResultOfSerializedValue<'a, 'v> {
    result: bsmr_error::Result<SerializeValue<'a, 'v>>,
}

impl<'a, 'v> Serialize for BsmrErrorResultOfSerializedValue<'a, 'v> {
...
impl<'a, 'v> SerializeValue<'a, 'v> {
    fn with_value(&self, x: Value<'v>) -> BsmrErrorResultOfSerializedValue<'a, 'v> {
        BsmrErrorResultOfSerializedValue {
...
fn err<R, E: serde::ser::Error>(res: bsmr_error::Result<R>) -> Result<R, E> {
    match res {
        Ok(v) => Ok(v),
        Err(e) => Err(serde::ser::Error::custom(format!("{e:#}"))),
    }
