...
impl AnalysisData {
    pub fn providers(&self) -> bsmr_error::Result<FrozenProviderCollectionValue> {
...
impl ActionQueryNodeRef {
    pub fn require_action(&self) -> bsmr_error::Result<&ActionKey> {
        match self {
            Self::Analysis(a) => Err(bsmr_error::bsmr_error!(
...
pub static FIND_MATCHING_ACTION: LateBinding<
    for<'c> fn(
        ...
        Box<dyn Future<Output = bsmr_error::Result<Option<ActionQueryNode>>> + Send + 'c>,
    >,
> = LateBinding::new("FIND_MATCHING_ACTION");

/// Hook to link printer in `bsmr_server_commands` from `bsmr_cmd_audit_server`.
...
/// Use of "configured_attr_to_value" in `bsmr_transition` from `bsmr_analysis`.
