...
use bsmr_artifact::artifact::artifact_dump::DirectoryInfo;
use bsmr_artifact::artifact::artifact_dump::ExternalSymlinkInfo;
...
use bsmr_core::pattern::pattern::TargetLabelWithModifiers;
...
pub struct BuildReportCollector<'a> {
    ...
    error_cause_cache: HashMap<bsmr_error::UniqueRootId, usize>,
...
#[error(transparent)]
#[bsmr(tag = BuildReport)]
pub struct BuildReportGenerationError(bsmr_error::Error);

impl From<bsmr_error::Error> for BuildReportGenerationError {
...
impl<'a> BuildReportCollector<'a> {
    ...
    fn assemble_build_report(
        ...
        // Determine error category using existing Bessemer error classification
    ...
    ) -> bsmr_error::Result<BuildReportEntry> {
    ...
    fn collect_results_for_configured<'b>(
        ...
        artifact_size_cardinalities: &HashMap<ConfiguredProvidersLabel, f64>,
        all_error_reports: &mut Vec<ErrorReport>,
    ) -> bsmr_error::Result<ConfiguredBuildReportEntry> {
    ...
    fn convert_error_list(
        ...
        for e in errors {
            ...
            let message = if self.truncate_error_content {
                bsmr_util::truncate::truncate(&message, MAX_ERROR_CONTENT_BYTES)
            } else {
                message
            };
            let error_tags = e
...
pub async fn build_report_opts<'a>(
    ...
    graph_properties_opts: GraphPropertiesOptions,
) -> bsmr_error::Result<BuildReportOpts> {
