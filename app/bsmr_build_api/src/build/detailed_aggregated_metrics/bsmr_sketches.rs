...
use bsmr_sketches::ArtifactCountSketch;
use bsmr_sketches::ArtifactSizeSketch;
use bsmr_sketches::DependencyGraphSketch;
use bsmr_sketches::MemoryUsageSketch;
...
pub(crate) fn compute_artifact_path_sketches(
    ...
    sketch_size: bool,
    sketch_count: bool,
) -> bsmr_error::Result<ArtifactPathSketches> {
...
impl Key for AnalysisGraphPropertiesKey {
    type Value = bsmr_error::Result<MaybeCompatible<AnalysisHeapSketches>>;
...
mod tests {
    ...
    use crate::artifact_groups::ArtifactGroup;
    use crate::build::detailed_aggregated_metrics::bsmr_sketches::compute_artifact_path_sketches_impl;
    use crate::build::sketch_impl::Sketcher;
    use crate::deferred::calculation::DeferredHolder;
    ...
    impl Action for MockAction {
        ...
        async fn execute(
            ...
            _waiting_data: bsmr_build_signals::env::WaitingData,
    ...
    #[test]
    fn test_artifact_path_sketches_dedups_paths() -> bsmr_error::Result<()> {
