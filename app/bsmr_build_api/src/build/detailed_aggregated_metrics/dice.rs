...
use bsmr_data::ComputeDetailedAggregatedMetricsEnd;
use bsmr_data::ComputeDetailedAggregatedMetricsStart;
...
use crate::build::detailed_aggregated_metrics::bsmr_sketches::ArtifactPathSketches;
use crate::build::detailed_aggregated_metrics::bsmr_sketches::compute_artifact_path_sketches_for_target;
...
pub trait HasDetailedAggregatedMetrics {
    ...
    fn top_level_target(&self, spec: TopLevelTargetSpec) -> bsmr_error::Result<()>;
    fn take_per_build_events(&self) -> bsmr_error::Result<PerBuildEvents>;
    ...
    ) -> impl Future<Output = bsmr_error::Result<DetailedAggregatedMetrics>> + Send;
    fn compute_action_graph_sketch(
        &self,
        events: &PerBuildEvents,
    ) -> impl Future<Output = bsmr_error::Result<ActionGraphSketchResult>> + Send;
    fn compute_artifact_path_sketch(
        ...
        sketch_count: bool,
        sketch_size: bool,
    ) -> impl Future<Output = bsmr_error::Result<ArtifactPathSketchResult>> + Send;
}
...
impl HasDetailedAggregatedMetrics for DiceComputations<'_> {
    ...
    async fn compute_detailed_metrics(
        &self,
        events: PerBuildEvents,
    ) -> bsmr_error::Result<DetailedAggregatedMetrics> {
        span_async_simple(
    ...
    async fn compute_action_graph_sketch(
        &self,
        events: &PerBuildEvents,
    ) -> bsmr_error::Result<ActionGraphSketchResult> {
        get_detailed_aggregated_metrics_handle(self)?
    ...
    async fn compute_artifact_path_sketch(
        ...
        sketch_count: bool,
        sketch_size: bool,
    ) -> bsmr_error::Result<ArtifactPathSketchResult> {
            ...
            .compute_join(targets, |ctx, spec| {
                ...
                async move {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            let _ignored = bsmr_core::soft_error!(
...
) -> bsmr_error::Result<&'a DetailedAggregatedMetricsHandle> {
    ctx.global_data()
...
fn get_per_build_events_holder<'a>(
    ctx: &'a DiceComputations<'_>,
) -> bsmr_error::Result<&'a DetailedAggregatedMetricsPerBuildEventsHolder> {
