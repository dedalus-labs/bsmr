...
use bsmr_sketches::ActionGraphSketch;
use dupe::Dupe;
...
pub struct PerBuildEvents {
    pub executed_actions: bsmr_hash::BuckHashSet<ActionKey>,
...
impl ToProtoMessage for DetailedAggregatedMetrics {
    type Message = bsmr_data::DetailedAggregatedMetrics;

    fn as_proto(&self) -> Self::Message {
        bsmr_data::DetailedAggregatedMetrics {
...
impl ToProtoMessage for AggregatedBuildMetrics {
    type Message = bsmr_data::AggregatedBuildMetrics;

    fn as_proto(&self) -> Self::Message {
        bsmr_data::AggregatedBuildMetrics {
...
pub struct TopLevelTargetAggregatedData {
    ...
    /// Bessemer daemon, so use this to compare durations but don't add it to a
...
impl ToProtoMessage for TopLevelTargetAggregatedData {
    type Message = bsmr_data::TopLevelTargetMetrics;

    fn as_proto(&self) -> Self::Message {
        bsmr_data::TopLevelTargetMetrics {
    ...
    fn aggregate_execution(&mut self, factor: f64, ev: &ActionExecutionMetrics, when: BuiltWhen) {
        ...
        if let BuiltWhen::ThisBuild = when {
            ...
            match ev.execution_kind {
                ...
                bsmr_data::ActionExecutionKind::Remote
                ...
                bsmr_data::ActionExecutionKind::ActionCache
                | bsmr_data::ActionExecutionKind::RemoteDepFileCache => {
                    self.remote_cache_hits += factor;
                }
                bsmr_data::ActionExecutionKind::NotSet
                | bsmr_data::ActionExecutionKind::Simple
                | bsmr_data::ActionExecutionKind::Deferred
...
impl ToProtoMessage for AllTargetsAggregatedData {
    type Message = bsmr_data::AllTargetsBuildMetrics;

    fn as_proto(&self) -> Self::Message {
        bsmr_data::AllTargetsBuildMetrics {
            action_graph_size: self.action_graph_size,
            metrics: Some(self.metrics.as_proto()),
