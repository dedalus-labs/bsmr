...
pub(crate) enum DetailedAggregatedMetricsEvent {
    ...
    ComputeMetrics(
        PerBuildEvents,
        tokio::sync::oneshot::Sender<bsmr_error::Result<DetailedAggregatedMetrics>>,
    ),
    ComputeActionGraphSketch(
        Vec<TopLevelTargetSpec>,
        tokio::sync::oneshot::Sender<bsmr_error::Result<ActionGraphSketchResult>>,
    ),
...
impl DetailedAggregatedMetricsEventHandler {
    ...
    fn send(&self, event: DetailedAggregatedMetricsEvent) -> bsmr_error::Result<()> {
...
impl DetailedAggregatedMetricsHandle {
    ...
    pub fn action_executed(&self, ev: ActionExecutionMetrics) -> bsmr_error::Result<()> {
    ...
    pub fn analysis_started(&self, key: &DeferredHolderKey) -> bsmr_error::Result<()> {
        match self.tracker.get() {
            Some(tracker) => {
...
impl DetailedAggregatedMetricsPerBuildEventsHolder {
    ...
    pub(crate) fn take_events(&self) -> bsmr_error::Result<PerBuildEvents> {
