...
impl<'a> Graph<'a> {
    ...
    fn lookup_node(&self, key: Key) -> bsmr_error::Result<Node> {
        match key {
    ...
    fn lookup_artifact(&self, artifact: &ArtifactGroup) -> bsmr_error::Result<Option<Key>> {
        match artifact {
    ...
    fn visit_deps(&self, node: &Node, mut visit_dep: impl FnMut(Key)) -> bsmr_error::Result<()> {
        match node {
            Node::Action(action) => {
            ...
            Node::TransitiveSetProjection(proj_node) => {
                ...
                struct Visitor<'a, 'b, F: FnMut(Key)>(F, &'a Graph<'b>, bsmr_error::Result<()>);
    ...
    fn traverse(
        ...
        mut visit: impl FnMut(&Self, &Node, &mut TraversalState) -> bsmr_error::Result<()>,
