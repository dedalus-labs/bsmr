...
use bsmr_event_observer::display::get_action_error_reason;
use serde::Serialize;
...
impl BuildReportActionError {
    pub(crate) fn new<'a>(
        ...
        let error_diagnostics = if opts.exclude_action_error_diagnostics {
            ...
            error.error_diagnostics.clone().map(|error_diagnostics| {
                match error_diagnostics.data.unwrap() {
                    ...
                    bsmr_data::action_error_diagnostics::Data::HandlerInvocationError(
                        invocation_failure,
        ...
        let (reason, stderr, stdout) = if opts.truncate_error_content {
            (
                bsmr_util::truncate::truncate(&reason, MAX_ERROR_CONTENT_BYTES),
                bsmr_util::truncate::truncate(&stderr, MAX_ERROR_CONTENT_BYTES),
                bsmr_util::truncate::truncate(&stdout, MAX_ERROR_CONTENT_BYTES),
            )
        } else {
