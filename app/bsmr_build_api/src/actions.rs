...
pub trait Action: PagableTagged + Allocative + Debug + Send + Sync + 'static {
    ...
    /// A machine-readable category for this action, intended to be used when analyzing actions outside of bsmr itself.
    ///
    ...
    /// rule implementation; however, bsmr enforces some restrictions on category names.
    fn category(&self) -> CategoryRef<'_>;

    /// A machine-readable identifier for this action. Required (but as of now, not yet enforced) to be unique within
    /// a category within a single invocation of a rule. Like categories, identifiers are also user-specified and bsmr
