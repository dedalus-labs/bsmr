...
impl<'v> AnalysisRegistry<'v> {
    ...
    ) -> bsmr_error::Result<AnalysisRegistry<'v>> {
    ...
    ) -> bsmr_error::Result<(ArtifactDeclaration<'v>, OutputArtifact<'v>)> {
        let declaration_location = eval.call_stack_top_location();
        let heap = eval.heap();
    ...
    pub fn assert_no_promises(&self) -> bsmr_error::Result<()> {
    ...
    pub fn finalize(
        ...
        impl FnOnce(&FrozenModule) -> bsmr_error::Result<RecordedAnalysisValues> + use<>,
    > {
...
pub struct FrozenAnalysisValueStorage {
    ...
    // `ThinBoxSlice` lives in `bsmr_util` (cannot depend on `starlark`),
...
impl<'v> AnalysisValueStorage<'v> {
    ...
    fn write_to_module(self, module: &Module<'v>) -> bsmr_error::Result<()> {
    ...
    pub(crate) fn register_transitive_set<
        F: FnOnce(TransitiveSetKey) -> bsmr_error::Result<ValueTyped<'v, TransitiveSet<'v>>>,
    >(
        &mut self,
        func: F,
    ) -> bsmr_error::Result<ValueTyped<'v, TransitiveSet<'v>>> {
...
impl AnalysisValueFetcher {
    fn extra_value(
        &self,
    ) -> bsmr_error::Result<Option<(&FrozenAnalysisValueStorage, &FrozenHeapRef)>> {
    ...
    pub fn get_action_data(
        &self,
        id: &ActionKey,
    ) -> bsmr_error::Result<(Option<OwnedFrozenValue>, Option<OwnedFrozenValue>)> {
...
impl RecordedAnalysisValues {
            ...
            analysis_storage: Some(
                unsafe {
                    OwnedFrozenValue::new(
                        heap.into_ref_named(BsmrTestHeapName::frozen_heap_name()),
                        value,
                    )
                }
                .downcast()
    ...
    pub fn lookup_action(&self, key: &ActionKey) -> bsmr_error::Result<ActionLookup> {
        if key.holder_key() != &self.self_key {
            return Err(internal_error!(
    ...
    pub fn analysis_storage(
        &self,
    ) -> bsmr_error::Result<OwnedRefFrozenRef<'_, FrozenAnalysisValueStorage>> {
        Ok(self
            .analysis_storage
            .as_ref()
    ...
    pub fn provider_collection(&self) -> bsmr_error::Result<FrozenProviderCollectionValueRef<'_>> {
    ...
    pub(crate) fn retained_memory(&self) -> bsmr_error::Result<usize> {
        Ok(self
            .analysis_storage
            .as_ref()
            .ok_or_else(|| internal_error!("missing analysis storage"))?
