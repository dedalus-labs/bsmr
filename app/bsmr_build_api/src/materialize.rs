...
async fn materialize_artifact_group(
    ...
    if let MaterializationContext::Materialize { force } = materialization_context {
        ...
        bsmr_util::future::try_join_all(materialize_futs).await?;
    }
...
async fn ensure_uploaded(
    ...
    for (artifact, value) in values.iter() {
        ...
        bsmr_execute::directory::insert_artifact(&mut dir, path, value)?;
    }
    ...
    let re_use_case = ctx
        ...
        .ok()
        .flatten()
        .map_or_else(RemoteExecutorUseCase::bsmr_default, |v| {
