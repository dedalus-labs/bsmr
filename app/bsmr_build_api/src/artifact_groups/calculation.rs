...
use bsmr_artifact::artifact::artifact_type::ArtifactKind;
...
) -> impl Future<Output = bsmr_error::Result<EnsureArtifactGroupReady>> + use<'a> {
    match input {
...
impl EnsureArtifactGroupReady {
    ...
    fn unpack_single(self) -> bsmr_error::Result<ArtifactValue> {
        match self {
...
async fn dir_artifact_value(
    ...
    // `PathMetadataKey` of every file inside that directory, blowing up Bessemer's memory use.
    ...
    impl Key for DirArtifactValueKey {
        type Value = bsmr_error::Result<ArtifactValue>;
            ...
            let entry_values = ctx
                .try_compute_join(files.iter(), |ctx, x| {
                    async move {
                        ...
                        .await?;
                        bsmr_error::Ok((x.file_name.clone(), value))
                    }
                    .boxed()
                })
                .await?;
...
enum ProjectedArtifactError {
    ...
    #[bsmr(tag = bsmr_error::ErrorTag::ProjectMissingPath)]
...
impl Key for EnsureTransitiveSetProjectionKey {
    type Value = bsmr_error::Result<ArtifactGroupValues>;
