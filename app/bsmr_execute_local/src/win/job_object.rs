...
impl JobObject {
    ...
    pub(crate) fn assign_process(&self, process: HANDLE) -> bsmr_error::Result<()> {
        result_bool(unsafe { AssignProcessToJobObject(self.job_handle.handle(), process) })
    }

    pub(crate) async fn terminate(&self, exit_code: u32) -> bsmr_error::Result<()> {
    ...
    async fn wait(&self) -> bsmr_error::Result<()> {
...
fn set_job_limits(job: &WinapiHandle) -> bsmr_error::Result<()> {
