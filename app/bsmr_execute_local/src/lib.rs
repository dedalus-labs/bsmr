...
use bsmr_resource_control::action_scene::ActionCgroupResult;
...
pub struct CollectedExecutionStats {
    ...
    pub userspace_events: Option<bsmr_data::CpuCounter>,
    pub kernel_events: Option<bsmr_data::CpuCounter>,
}
...
struct CommandEventStream<Status, Stdio> {
    exit: Option<bsmr_error::Result<(GatherOutputStatus, Vec<OrphanProcessInfo>)>>,
...
where
    Status: Future<Output = bsmr_error::Result<(GatherOutputStatus, Vec<OrphanProcessInfo>)>>,
    Stdio: Stream<Item = bsmr_error::Result<StdioEvent>> + InterruptNotifiable,
{
    type Item = bsmr_error::Result<CommandEvent>;
...
) -> bsmr_error::Result<GatherOutputStatus> {
    match timeout {
        Some(t) => {
...
) -> bsmr_error::Result<impl Stream<Item = bsmr_error::Result<CommandEvent>>>
where
    T: Future<Output = bsmr_error::Result<GatherOutputStatus>> + Send + Unpin,
{
    ...
    let process_details = if retry_on_txt_busy {
        ...
        cmd.spawn().map_err(|x| bsmr_error::Error::from(x.0))
    };
    ...
    let status = async move {
        ...
        let execute = async {
            ...
            bsmr_error::Ok(match futures::future::select(status, cancellation).await {
...
pub async fn decode_command_event_stream<S>(stream: S) -> bsmr_error::Result<CommandResult>
where
...
pub(crate) trait KillProcess {
    async fn kill(self, process: &mut ProcessGroup) -> bsmr_error::Result<()>;
}
...
impl KillProcess for DefaultKillProcess {
    async fn kill(self, process_group: &mut ProcessGroup) -> bsmr_error::Result<()> {
...
async fn spawn_retry_txt_busy<F, D>(
    ...
    mut delay: F,
) -> bsmr_error::Result<ProcessGroup>
where
    F: FnMut() -> D,
    D: Future<Output = ()>,
{
...
mod tests {
    ...
    #[tokio::test]
    async fn test_gather_output() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_gather_does_not_wait_for_children() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_gather_output_timeout() -> bsmr_error::Result<()> {
        let now = Instant::now();
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_spawn_retry_txt_busy() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_spawn_retry_other_error() -> bsmr_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
    ...
    #[tokio::test]
    async fn test_kill_terminates_process_group() -> bsmr_error::Result<()> {
        use sysinfo::Pid;
        use sysinfo::System;
    ...
    #[tokio::test]
    async fn test_stream_command_events_ends() -> bsmr_error::Result<()> {
        let mut cmd = if cfg!(windows) {
            background_command("powershell")
        } else {
            background_command("sh")
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_signal_exit_code() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn timeout_kills_before_dropping_decoder() -> bsmr_error::Result<()> {
        struct Kill {
        ...
        impl StatusDecoder for Decoder {
            async fn decode_status(self, _status: ExitStatus) -> bsmr_error::Result<DecodedStatus> {
                panic!("Should not be called in this test since we timeout")
            }

            async fn cancel(self) -> bsmr_error::Result<()> {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_no_stdio_stream_command_events() -> bsmr_error::Result<()> {
        let mut cmd = background_command("sh");
        cmd.args(["-c", "echo hello"]);
