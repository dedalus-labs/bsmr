...
pub struct CommandExecutionMetadata {
    ...
    /// Additional stats.
    pub execution_stats: Option<bsmr_data::CommandExecutionStats>,
...
impl CommandExecutionMetadata {
    ...
    pub fn to_proto(&self) -> bsmr_data::CommandExecutionMetadata {
        let metadata = self.dupe();
        bsmr_data::CommandExecutionMetadata {
...
pub struct CommandExecutionResult {
    ...
    /// Why dep file information for this action did or did not get uploaded to cache, by Bessemer.
...
impl CommandExecutionResult {
    ...
    pub fn resolve_outputs<'a>(
        &'a self,
        fs: &'a ArtifactFs,
    ) -> impl Iterator<Item = bsmr_error::Result<(ResolvedCommandExecutionOutput, &'a ArtifactValue)>> + 'a
    {
...
pub struct CommandExecutionReport {
    ...
    pub inline_environment_metadata: bsmr_data::InlineCommandExecutionEnvironmentMetadata,
}
...
impl CommandExecutionReport {
    pub async fn to_command_execution_proto(
        ...
        let status = match &self.status {
            CommandExecutionStatus::Success { .. } => {
                bsmr_data::command_execution::Success {}.into()
            }
            CommandExecutionStatus::Cancelled { .. } => {
                bsmr_data::command_execution::Cancelled {}.into()
            }
            CommandExecutionStatus::Failure { .. } => {
                bsmr_data::command_execution::Failure {}.into()
            }
            ...
            CommandExecutionStatus::Error { stage, error, .. } => {
                bsmr_data::command_execution::Error {
...
mod tests {
    ...
    fn make_simple_report() -> CommandExecutionReport {
        ...
        let status = CommandExecutionStatus::Success {
            execution_kind: CommandExecutionKind::Local {
                ...
                command: vec!["fake_bsmr".to_owned()],
                env: {
        ...
        let timing = CommandExecutionMetadata {
            ...
            execution_stats: Some(bsmr_data::CommandExecutionStats {
                cpu_instructions_user: Some(4),
                cpu_instructions_kernel: Some(5),
                userspace_events: Some(bsmr_data::CpuCounter {
                    count: 4,
                    time_enabled: 100,
                    time_running: 100,
                }),
                kernel_events: Some(bsmr_data::CpuCounter {
                    count: 10,
                    time_enabled: 50,
                    time_running: 100,
                }),
        ...
        CommandExecutionReport {
            ...
            inline_environment_metadata: bsmr_data::InlineCommandExecutionEnvironmentMetadata {
                sandcastle_instance_id: Some(123),
            },
        }
    }

    fn make_simple_proto() -> bsmr_data::CommandExecution {
        // The field values correspond to what `make_simple_report()` builds.
        let command_execution_kind = bsmr_data::CommandExecutionKind {
            command: Some(bsmr_data::command_execution_kind::Command::LocalCommand(
                bsmr_data::LocalCommand {
                    argv: vec!["fake_bsmr".to_owned()],
                    env: vec![bsmr_data::EnvironmentEntry {
        ...
        let command_execution_stats = bsmr_data::CommandExecutionStats {
            cpu_instructions_user: Some(4),
            cpu_instructions_kernel: Some(5),
            userspace_events: Some(bsmr_data::CpuCounter {
                count: 4,
        ...
        let command_execution_metadata = bsmr_data::CommandExecutionMetadata {
        ...
        let command_execution_details = bsmr_data::CommandExecutionDetails {
        ...
        bsmr_data::CommandExecution {
            ...
            status: Some(bsmr_data::command_execution::Status::Success(
                bsmr_data::command_execution::Success {},
            )),
            inline_environment_metadata: Some(
                bsmr_data::InlineCommandExecutionEnvironmentMetadata {
                    sandcastle_instance_id: Some(123),
                },
            ),
        }
    ...
    async fn test_to_command_execution_proto_omit_command_details() {
        ...
        expected_proto
            ...
            .unwrap()
            .command = Some(
            bsmr_data::command_execution_kind::Command::OmittedLocalCommand(
                bsmr_data::OmittedLocalCommand {
                    action_digest: format!("{}:{}", "0".repeat(64), "123"),
                },
            ),
        );
