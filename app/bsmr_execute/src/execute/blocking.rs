...
impl dyn BlockingExecutor {
    pub async fn execute_io_inline<F, T>(&self, f: F) -> bsmr_error::Result<T>
    where
        F: FnOnce() -> bsmr_error::Result<T> + Send,
        T: Send,
    {
        let mut res = None;
        ...
        .await
        .tag(bsmr_error::ErrorTag::IoBlockingExecutor)?;
...
struct ThreadPoolIoRequest {
    ...
    sender: oneshot::Sender<bsmr_error::Result<()>>,
}
...
impl BuckBlockingExecutor {
    ...
    pub fn default_concurrency(fs: ProjectRoot) -> bsmr_error::Result<Self> {
        let io_threads = bsmr_env!("BSMR_IO_THREADS", type=usize, default=4)?;
        let io_semaphore = bsmr_env!("BSMR_IO_SEMAPHORE", type=usize, default=bsmr_util::threads::available_parallelism())?;
...
impl DirectIoExecutor {
    pub fn new(project_fs: ProjectRoot) -> bsmr_error::Result<Self> {
        Ok(Self { project_fs })
    }
}
