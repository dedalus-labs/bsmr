...
use bsmr_data::ReQueueAcquiringDependencies;
...
use bsmr_events::schedule_type::SandcastleScheduleType;
...
#[bsmr(tag = MaterializationCancelled)]
...
impl RemoteExecutionClient {
    pub async fn new(re_config: &RemoteExecutionConfig) -> bsmr_error::Result<Self> {
        if bsmr_env!("BSMR_TEST_FAIL_CONNECT", bool, applicability = testing)? {
    ...
    pub async fn new_retry(re_config: &RemoteExecutionConfig) -> bsmr_error::Result<Self> {
    ...
    ) -> bsmr_error::Result<Option<ActionResultResponse>> {
        self.data
            .action_cache
            .op(self
                .data
    ...
    pub async fn execute<'a>(
        ...
        re_gang_workers: &[bsmr_core::execution_types::executor_config::ReGangWorker],
        use_case: RemoteExecutorUseCase,
        ...
        priority: Option<i32>,
    ) -> bsmr_error::Result<ExecuteResponseOrCancelled> {
        self.data
            .executes
    ...
    ) -> bsmr_error::Result<GetDigestsTtlResponse> {
        self.data
            .get_digests_ttl
    ...
    pub async fn get_digest_expirations(
        ...
        digests: Vec<TDigest>,
        metadata: RemoteExecutionMetadata,
    ) -> bsmr_error::Result<Vec<(TDigest, DateTime<Utc>)>> {
        let now = Utc::now();
    ...
    ) -> bsmr_error::Result<WriteActionResultResponse> {
        self.data
    ...
    pub fn get_experiment_name(&self) -> bsmr_error::Result<Option<String>> {
        self.data
            .client
            .client()
            .get_experiment_name()
            .map_err(|e| from_any_with_tag(e, bsmr_error::ErrorTag::ReExperimentName))
    }
...
fn anticipated_queue_duration(
    ...
    if let Some(duration) = bsmr_env!(
        "BSMR_TEST_RE_QUEUE_ESTIMATE_S",
        type=u64,
        applicability = testing
    )
    // Stringify the error because we can't deal with bsmr_errors here
...
fn trace_action_digest(
    ...
    ttl_secs: Option<i64>,
    use_case: RemoteExecutorUseCase,
    event: bsmr_data::action_digest_trace::ActionDigestTraceEvent,
    ...
    if let Some(weight) = should_sample_action_digest(digest) {
        ...
        let metadata = bsmr_events::metadata::collect(dispatcher.daemon_id());
        let schedule_type = SandcastleScheduleType::new()
            .ok()
            .and_then(|s| s.as_str().map(|s| s.to_owned()));
        dispatcher.instant_event(bsmr_data::ActionDigestTrace {
            metadata,
...
impl RemoteExecutionClientImpl {
    async fn new(re_config: &RemoteExecutionConfig) -> bsmr_error::Result<Self> {
        let op_name = "REClientBuilder";
        tracing::info!("Creating a new RE client");

        let res: bsmr_error::Result<Self> = try {
            let download_concurrency =
                bsmr_env!("BSMR_RE_DOWNLOAD_CONCURRENCY", type=usize, default=256)?;
            ...
            let client = {
                ...
                // bsmr makes find_missing calls for the same blobs
                ...
                if let Some(external_casd_address) = &static_metadata.shared_casd_address {
                    use bsmr_re_configuration::CASdAddress;
                    use bsmr_re_configuration::CopyPolicy as BsmrCopyPolicy;
                    use remote_execution::RemoteCASdAddress;
                    use remote_execution::RemoteCacheConfig;
                    use remote_execution::RemoteCacheSyncConfig;

                    let policies: bsmr_error::Result<(RemoteFetchPolicy, RemoteFetchPolicy)> =
                    ...
                    let remote_cache_config = {
                        let mut remote_cache_config = RemoteCacheConfig {
                            ...
                            sync_copy_policy: match static_metadata
                                ...
                                .clone()
                                .unwrap_or(BsmrCopyPolicy::Copy)
                            {
                                BsmrCopyPolicy::Copy => CopyPolicy::FULL_COPY,
                                BsmrCopyPolicy::Reflink => CopyPolicy::SOFT_COPY,
                                BsmrCopyPolicy::Hybrid => CopyPolicy::HYBRID_COPY,
                            },
                            ..Default::default()
                        };
                    ...
                    .unwrap_or(
                        if static_metadata.use_zippy_rich_client && cfg!(target_os = "linux") {
                            if static_metadata.shared_casd_address.is_some() {
                                "remote_execution/features/client_bsmr_pc"
                            } else {
                                "remote_execution/features/client_bsmr"
                            }
                        } else {
                            "remote_execution/features/client_bsmr_alternative"
                        },
                    )
                    .to_owned();
    ...
    async fn action_cache(
        ...
        trace_action_digest(
            ...
            use_case,
            if res.is_some() {
                bsmr_data::action_digest_trace::ActionDigestTraceEvent::CacheHit
            } else {
                bsmr_data::action_digest_trace::ActionDigestTraceEvent::CacheMiss
            },
            None,
            None,
            None,
    ...
    async fn execute_impl(
        ...
        use bsmr_data::ReAfterAction;
        use bsmr_data::ReBeforeAction;
        use bsmr_data::ReExecute;
        use bsmr_data::ReQueue;
        use bsmr_data::ReUnknown;
        use bsmr_data::ReWorkerDownload;
        use bsmr_data::ReWorkerUpload;
            ...
            if !log_stream_emitted {
                if let Some(ref meta) = operation_metadata {
                    if !meta.stdout_stream_name.is_empty() || !meta.stderr_stream_name.is_empty() {
                        ...
                        get_dispatcher().instant_event(bsmr_data::ReLogStreamAvailable {
    ...
    pub async fn execute<'a>(
        ...
        if bsmr_env!("BSMR_TEST_FAIL_RE_EXECUTE", bool, applicability = testing)? {
            return Err(test_re_error("Injected error", TCode::FAILED_PRECONDITION));
        }

        if bsmr_env!(
            "BSMR_TEST_FAIL_RE_RESOURCE_EXHAUSTED",
            bool,
            applicability = testing
        )? {
        ...
        let request = ExecuteRequest {
            ...
            gang: if let Some(gang) = meta_internal_extra_params.gang.as_ref() {
                ...
                let locality = gang.locality.map(|l| {
                    use bsmr_core::execution_types::executor_config::ReGangLocality;
                    match l {
        ...
        let trace = match &res {
            Ok(ExecuteResponseOrCancelled::Response(r)) => {
                ...
                let event = if was_not_actually_executed {
                    ...
                    bsmr_data::action_digest_trace::ActionDigestTraceEvent::RemoteExecution
                };
    ...
    async fn download_typed_blobs<T: Message + Default>(
        ...
        digests: Vec<TDigest>,
        use_case: RemoteExecutorUseCase,
    ) -> bsmr_error::Result<(Vec<T>, TLocalCacheStats)> {
        if digests.is_empty() {
        ...
        if blobs.len() != expected_blobs {
            return Err(bsmr_error!(
                bsmr_error::ErrorTag::CasBlobCountMismatch,
    ...
    pub async fn download_blob(
        ...
        digest: &TDigest,
        use_case: RemoteExecutorUseCase,
    ) -> bsmr_error::Result<(Vec<u8>, TLocalCacheStats)> {
    ...
    async fn materialize_files(
        ...
        use_case: RemoteExecutorUseCase,
        priority_control: DynamicPriorityHandle,
    ) -> bsmr_error::Result<TLocalCacheStats> {
        if bsmr_env!("BSMR_TEST_FAIL_RE_DOWNLOADS", bool, applicability = testing)? {
        ...
        let futs = chunks(files, self.download_chunk_size).map(|chunk| {
            ...
            async move {
                ...
                bsmr_error::Ok(ChunkDownloadResult::Downloaded(response.local_cache_stats))
            }
        });

        let results: Vec<ChunkDownloadResult> = bsmr_util::future::try_join_all(futs).await?;
    ...
    async fn write_action_result(
        ...
        trace_action_digest(
            ...
            use_case,
            bsmr_data::action_digest_trace::ActionDigestTraceEvent::CacheUpload,
