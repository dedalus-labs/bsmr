...
impl Uploader {
    async fn find_missing<'a>(
        ...
        let now = Utc::now();
        let ttl_wanted = if bsmr_core::is_open_source() {
            1
        } else {
            600i64
        };
        ...
        let digests_and_ttls_iterator = if deduplicate_get_digests_ttl_calls {
            ...
            {
                type Item = bsmr_error::Result<(&'a TrackedFileDigest, i64)>;

                fn next(&mut self) -> Option<bsmr_error::Result<(&'a TrackedFileDigest, i64)>> {
    ...
    pub async fn upload(
        ...
        if !upload_files.is_empty() || !upload_blobs.is_empty() {
            ...
            .await
            .map_err(|e| {
                if e.tags().contains(&bsmr_error::ErrorTag::ReInvalidArgument) {
                    bsmr_error::bsmr_error!(
                        bsmr_error::ErrorTag::ReInvalidArgument,
...
fn process_get_digest_ttls_response<T>(
    ...
    digest_config: DigestConfig,
) -> bsmr_error::Result<impl Iterator<Item = bsmr_error::Result<(T, i64)>>>
where
    T: Borrow<TrackedFileDigest> + Ord,
{
    ...
    let mut digest_ttls = digest_ttls.into_try_map(|d| {
        bsmr_error::Ok((
            FileDigest::from_re(&d.digest, digest_config).map_err(bsmr_error::Error::from)?,
            d.ttl,
        ))
    })?;
    digest_ttls.sort();
