...
impl RemoteExecutionConfig {
    async fn connect_now(&self) -> bsmr_error::Result<RemoteExecutionClient> {
...
struct LazyRemoteExecutionClient {
    client: AsyncOnceCell<bsmr_error::Result<RemoteExecutionClient>>,
...
impl LazyRemoteExecutionClient {
    ...
    async fn get(&self) -> bsmr_error::Result<&RemoteExecutionClient> {
    ...
    async fn init(&self) -> bsmr_error::Result<RemoteExecutionClient> {
...
impl ReGetSessionId for ReConnectionManager {
    async fn get_session_id(&self) -> bsmr_error::Result<String> {
...
impl UnconfiguredRemoteExecutionClient {
    ...
    fn lock(&self) -> bsmr_error::Result<Arc<Arc<LazyRemoteExecutionClient>>> {
        self.data
            .upgrade()
    ...
    pub async fn get_session_id(&self) -> bsmr_error::Result<String> {
...
impl ManagedRemoteExecutionClient {
    ...
    pub async fn download_blob(&self, digest: &TDigest) -> bsmr_error::Result<Vec<u8>> {
        self.lock()?
            .get()
            .await?
    ...
    pub async fn upload_blob(&self, blob: InlinedBlobWithDigest) -> bsmr_error::Result<TDigest> {
        self.lock()?
            .get()
            .await?
    ...
    pub async fn write_action_result(
        ...
        if bsmr_env!(
            "BSMR_TEST_SKIP_ACTION_CACHE_WRITE",
            bool,
