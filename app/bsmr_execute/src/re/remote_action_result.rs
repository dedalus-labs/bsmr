...
pub struct ActionCacheResult(pub ActionResultResponse, pub bsmr_data::CacheType);
...
impl RemoteActionResult for ActionCacheResult {
    ...
    fn execution_kind(&self, details: RemoteCommandExecutionDetails) -> CommandExecutionKind {
        match self.1 {
            bsmr_data::CacheType::ActionCache => CommandExecutionKind::ActionCache { details },
...
) -> bsmr_error::Result<bsmr_data::CommandExecutionStats> {
    Ok({
        ...
        bsmr_data::CommandExecutionStats {
...
) -> bsmr_error::Result<Option<MiniperfCounter>> {
