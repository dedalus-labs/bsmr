...
fn do_normalize_permissions(path: &AbsNormPathBuf) -> bsmr_error::Result<()> {
...
async fn build_dir_from_disk(
    ...
    blocking_executor: &dyn BlockingExecutor,
    project_root: &AbsNormPath,
) -> bsmr_error::Result<(ActionDirectoryBuilder, HashingInfo)> {
...
fn build_file_metadata(
    disk_path: AbsNormPathBuf,
    digest_config: FileDigestConfig,
) -> impl Future<Output = bsmr_error::Result<(FileMetadata, HashingInfo)>> {
    ...
    let io_task = move || {
        ...
        bsmr_error::Ok((disk_path.executable(), hashing_info, digest))
    };
