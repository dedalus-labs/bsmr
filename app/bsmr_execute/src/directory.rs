...
use bsmr_directory::directory::builder_lazy::DirectoryBuilderLike;
use bsmr_directory::directory::builder_lazy::LazyDirectoryBuilder;
...
use bsmr_directory::directory::shared_directory::SharedDirectory;
use bsmr_directory::directory::shared_directory::SharedDirectoryInternable;
...
pub fn new_symlink<T: AsRef<Path>>(target: T) -> bsmr_error::Result<ActionDirectoryMember> {
...
pub fn extract_artifact_value(
    ...
    digest_config: DigestConfig,
) -> bsmr_error::Result<Option<ArtifactValue>> {
...
mod tests {
    ...
    #[test]
    fn directory_relativized() -> bsmr_error::Result<()> {
    ...
    fn build_test_dir() -> bsmr_error::Result<ActionDirectoryBuilder> {
        let digest_config = DigestConfig::testing_default();
    ...
    #[test]
    fn test_extract_no_deps() -> bsmr_error::Result<()> {
        let digest_config = DigestConfig::testing_default();
    ...
    #[test]
    fn test_extract_has_deps_dir() -> bsmr_error::Result<()> {
        let digest_config = DigestConfig::testing_default();
    ...
    #[test]
    fn test_extract_has_deps_leaf() -> bsmr_error::Result<()> {
        let digest_config = DigestConfig::testing_default();
    ...
    #[test]
    fn test_extract_cycle() -> bsmr_error::Result<()> {
        let digest_config = DigestConfig::testing_default();
    ...
    #[test]
    fn test_re_tree_roundtrip() -> bsmr_error::Result<()> {
        let digest_config = DigestConfig::testing_default();
    ...
    ///
    /// ```sh
    /// bsmr run upstream//remote_execution/rust/recli:recli -- \
    ...
    /// ```
    #[test]
    fn test_re_tree_compatibility() -> bsmr_error::Result<()> {
        let digest_config = DigestConfig::testing_default();
    ...
    // `normalize_target_for_re_*` in `bsmr_common::external_symlink`, which together
    // ensure both arms store POSIX-style targets when uploaded to CAS.
    fn test_unnormalized_symlinks() -> bsmr_error::Result<()> {
        if !cfg!(windows) {
            return Ok(());
        }
    ...
    #[test]
    fn external_symlink_serializer_preserves_target_str_verbatim() -> bsmr_error::Result<()> {
        let digest_config = DigestConfig::testing_default();
    ...
    #[test]
    fn test_rename_and_serialize() -> bsmr_error::Result<()> {
        let digest_config = DigestConfig::testing_default();
