...
use bsmr_http::retries::HttpError;
use bsmr_http::retries::HttpErrorForRetry;
use bsmr_http::retries::IntoBsmrError;
...
impl IntoBsmrError for HttpHeadError {
    fn into_bsmr_error(self) -> bsmr_error::Error {
        bsmr_error::Error::from(self)
    }
}

impl IntoBsmrError for HttpDownloadError {
    fn into_bsmr_error(self) -> bsmr_error::Error {
        bsmr_error::Error::from(self)
    }
}

pub async fn http_head(client: &HttpClient, url: &str) -> bsmr_error::Result<Response<()>> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_copy_and_hash_ok() -> bsmr_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_copy_and_hash_invalid_primary_hash() -> bsmr_error::Result<()> {
        assert_matches!(
    ...
    #[tokio::test]
    async fn test_copy_and_hash_invalid_secondary_hash() -> bsmr_error::Result<()> {
        assert_matches!(
