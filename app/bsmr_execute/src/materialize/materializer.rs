...
use bsmr_directory::directory::walk::ordered_entry_walk;
...
pub struct CasNotFoundError {
    ...
    #[source]
    pub error: Arc<bsmr_error::Error>,
}
...
pub trait Materializer: Allocative + Send + Sync + 'static {
    ...
    ) -> bsmr_error::Result<DeclareMatchOutcome>;
    ...
    async fn has_artifact_at(&self, path: ProjectRelativePathBuf) -> bsmr_error::Result<bool>;
    ...
    async fn invalidate(&self, path: ProjectRelativePathBuf) -> bsmr_error::Result<()> {
        self.invalidate_many(vec![path]).await
    }

    /// Declare an artifact at `path` exists. This will overwrite any pre-existing materialization
    /// methods for this file and indicate that no materialization is necessary.
    async fn invalidate_many(&self, paths: Vec<ProjectRelativePathBuf>) -> bsmr_error::Result<()>;
    ...
    ) -> bsmr_error::Result<BoxStream<'static, Result<(), MaterializationError>>>;
    ...
    /// Bessemer significantly slower, after we stop deferring local copies of
    /// artifacts that are already on disk.
    async fn get_materialized_file_paths(
        &self,
        file_paths: Vec<ProjectRelativePathBuf>,
    ) -> bsmr_error::Result<Vec<Result<ProjectRelativePathBuf, ArtifactNotMaterializedReason>>>;
    ...
    /// time.
    fn add_snapshot_stats(&self, _snapshot: &mut bsmr_data::Snapshot) {}
    ...
    ) -> bsmr_error::Result<Option<ProjectRelativePathBuf>> {
    ...
    ) -> bsmr_error::Result<Box<dyn EagerMaterializationGuard>> {
        Ok(Box::new(()))
    }
}
...
impl dyn Materializer {
    ...
    fn check_declared_external_symlink(&self, value: &ArtifactValue) -> bsmr_error::Result<()> {
...
impl MaterializationMethod {
    pub fn try_new_from_config_value(config_value: Option<&str>) -> bsmr_error::Result<Self> {
        match config_value {
...
pub trait DeferredMaterializerExtensions: Send + Sync {
    fn iterate(&self) -> bsmr_error::Result<BoxStream<'static, DeferredMaterializerIterItem>>;

    fn list_subscriptions(&self) -> bsmr_error::Result<BoxStream<'static, ProjectRelativePathBuf>>;

    async fn allocative(&self) -> bsmr_error::Result<allocative::FlameGraphOutput>;

    /// Obtain a list of files that don't match their in-memory representation. This may not catch
    /// all discrepancies.
    fn fsck(
        &self,
    ) -> bsmr_error::Result<BoxStream<'static, (ProjectRelativePathBuf, bsmr_error::Error)>>;

    async fn refresh_ttls(&self, min_ttl: i64) -> bsmr_error::Result<()>;

    async fn get_ttl_refresh_log(&self) -> bsmr_error::Result<String>;
    ...
    async fn test_iter(&self, count: usize) -> bsmr_error::Result<String>;
    async fn flush_all_access_times(&self) -> bsmr_error::Result<String>;

    /// Create a new DeferredMaterializerSubscription.
    async fn create_subscription(
        &self,
    ) -> bsmr_error::Result<Box<dyn DeferredMaterializerSubscription>>;
}
