...
impl Materializer for NoDiskMaterializer {
    ...
    async fn has_artifact_at(&self, _path: ProjectRelativePathBuf) -> bsmr_error::Result<bool> {
        // This materializer does not keep track of state
        Ok(false)
    }
    ...
    async fn invalidate_many(&self, _paths: Vec<ProjectRelativePathBuf>) -> bsmr_error::Result<()> {
        Ok(())
    }
