...
use bsmr_cmd_audit_client::config::AuditConfigCommand;
use bsmr_cmd_audit_client::config::LocationStyle;
...
use bsmr_server_ctx::stdout_partial_output::StdoutPartialOutput;
use gazebo::prelude::*;
use serde_json::json;
...
impl<'a> Match<'a> {
    fn parse(resolver: &CellAliasResolver, spec: &'a str) -> bsmr_error::Result<Self> {
...
impl<'a> Matches<'a> {
    fn parse(resolver: &CellAliasResolver, specs: &'a [String]) -> bsmr_error::Result<Self> {
        Ok(Self {
...
trait CellConfigRenderer {
    fn render_cell_header(&mut self, cell: CellName) -> bsmr_error::Result<()>;
    fn render_section_header(&mut self, section: &str) -> bsmr_error::Result<()>;
...
impl CellConfigRenderer for JsonCellConfigRenderer<'_> {
    fn render_cell_header(&mut self, _cell: CellName) -> bsmr_error::Result<()> {
        Ok(())
    }

    fn render_section_header(&mut self, _section: &str) -> bsmr_error::Result<()> {
        Ok(())
    }
