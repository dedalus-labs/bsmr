...
#[bsmr(tag = InvalidBuckOutPath)]
enum BuckOutPathParserError {
    #[error("Path is probably a buck-out generated by Buck1: `{0}`")]
...
pub(crate) struct BuckOutPathTypeCommon {
    ...
    /// `bsmr-out/v2/art/cell/<CONFIG_HASH>/path/to/__target_name__/target`, it would be `cell/path/to/__target_name__/target`.
...
fn validate_buck_out_and_isolation_prefix<'v>(
    ...
    match iter.next() {
        Some(output_root) => {
            if output_root != BSMR_OUTPUT_ROOT {
                ...
                if output_root.as_str().ends_with("buck-out") {
                    return Err(
...
) -> bsmr_error::Result<BuckOutPathData> {
...
impl BuckOutPathParser {
    ...
    // Validates and parses the bsmr-out path, returning the `BuckOutPathType`. Assumes
    // that the inputted path is not a symlink.
    pub(crate) fn parse(&self, output_path: &str) -> bsmr_error::Result<BuckOutPathType> {
    ...
    ) -> bsmr_error::Result<BuckOutPathType> {
        ...
        match iter.next() {
            Some(part) => {
                let result = match part.as_str() {
                    ...
                    _ => Err(bsmr_error!(
                        bsmr_error::ErrorTag::InvalidBuckOutPath,
...
mod tests {
    ...
    fn test_validation() -> bsmr_error::Result<()> {
        ...
        let malformed_path2 = "bsmr-out/v2/invalid_buck_prefix/blah/blah/blah/blah";
        let malformed_path3 = "bsmr-out/v2/art/bar/no/target/name/found";
        let malformed_path4 = "bsmr-out/v2/art/bar/path/to/target/__but_no_artifacts__";
        ...
        let cell_does_not_exist =
            "bsmr-out/v2/art/nonexistent_cell/cfg_hash/path/to/target/__target_name__/output";

        let res = buck_out_parser.parse(cell_does_not_exist);
        assert!(res.err().unwrap().to_string().contains("Malformed"));

        let no_artifacts_after_target_name =
            &format!("bsmr-out/v2/art/bar/{config_hash}/path/to/target/__target_name__");
    ...
    fn test_target_output() -> bsmr_error::Result<()> {
        ...
        let rule_path = format!(
            "bsmr-out/v2/art/bar/{expected_config_hash}/path/to/target/__target_name__/output"
        );
    ...
    fn test_target_content_based_output() -> bsmr_error::Result<()> {
        ...
        let rule_path = format!(
            "bsmr-out/v2/art/bar/path/to/target/__target_name__/{content_based_hash}/output"
        );
    ...
    fn test_target_output_with_slashes() -> bsmr_error::Result<()> {
        ...
        let rule_path_target_label_with_slashes = format!(
            "bsmr-out/v2/art/bar/{expected_config_hash}/path/to/target/__target_name_start/target_name_end__/output"
        );
    ...
    fn test_target_output_with_eq_sign() -> bsmr_error::Result<()> {
        ...
        let rule_path_with_equal_sign = format!(
            "bsmr-out/v2/art/bar/{expected_config_hash}/path/to/target/__target_name_eqsb_out__/output"
        );
    ...
    fn test_tmp_output() -> bsmr_error::Result<()> {
        ...
        let tmp_path = format!(
            "bsmr-out/v2/tmp/bar/{expected_config_hash}/path/to/target/__target_name__/output"
        );
    ...
    fn test_tmp_content_based_output() -> bsmr_error::Result<()> {
        ...
        let tmp_path = format!(
            "bsmr-out/v2/tmp/bar/path/to/target/__target_name__/{content_based_hash}/output"
        );
    ...
    fn test_test_output() -> bsmr_error::Result<()> {
        ...
        let test_path =
            format!("bsmr-out/v2/test/bar/{expected_config_hash}/path/to/target/test/output");
    ...
    fn test_anon_output() -> bsmr_error::Result<()> {
        ...
        let anon_path = format!(
            "bsmr-out/v2/art-anon/bar/{expected_config_hash}/path/to/target/anon_hash/__target_name__/output"
        );
    ...
    fn test_anon_content_based_output() -> bsmr_error::Result<()> {
        ...
        let anon_path = format!(
            "bsmr-out/v2/art-anon/bar/path/to/target/anon_hash/__target_name__/{content_based_hash}/output"
        );
    ...
    fn test_bxl_output() -> bsmr_error::Result<()> {
        ...
        let path = format!(
            "bsmr-out/v2/art-bxl/bar/{expected_config_hash}/path/to/function.bxl/__function_name__/output"
        );
    ...
    fn test_bxl_content_based_output() -> bsmr_error::Result<()> {
        ...
        let path = format!(
            "bsmr-out/v2/art-bxl/bar/path/to/function.bxl/__function_name__/{content_based_hash}/output"
        );
    ...
    fn test_empty_package_path() -> bsmr_error::Result<()> {
        ...
        let target_path =
            format!("bsmr-out/v2/art/bar/{expected_config_hash}/__target_name__/output");
