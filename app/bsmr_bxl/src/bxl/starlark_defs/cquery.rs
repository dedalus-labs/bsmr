...
use bsmr_build_api::query::bxl::BxlCqueryFunctions;
use bsmr_build_api::query::bxl::NEW_BXL_CQUERY_FUNCTIONS;
...
pub(crate) async fn get_cquery_env(
    ctx: &BxlContext<'_>,
    global_cfg_options_override: &GlobalCfgOptions,
) -> bsmr_error::Result<Box<dyn BxlCqueryFunctions>> {
...
impl<'v> StarlarkCQueryCtx<'v> {
    ...
    ) -> bsmr_error::Result<StarlarkCQueryCtx<'v>> {
        Ok(Self {
            ctx,
