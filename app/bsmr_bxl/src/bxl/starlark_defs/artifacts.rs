...
pub(crate) async fn visit_artifact_path_without_associated_deduped(
    ags: &[ArtifactGroup],
    abs: bool,
    mut visitor: impl FnMut(ArtifactPath, bool) -> bsmr_error::Result<()>,
...
impl<'v> EnsuredArtifactGroup<'v> {
    ...
    pub(crate) async fn visit_artifact_path_without_associated_deduped(
        &self,
        visitor: impl FnMut(ArtifactPath, bool) -> bsmr_error::Result<()>,
        ctx: &mut DiceComputations<'_>,
    ) -> bsmr_error::Result<()> {
...
impl<'v> ArtifactArg<'v> {
    pub(crate) fn into_ensured_artifact(self) -> bsmr_error::Result<EnsuredArtifact> {
        match self {
...
impl LazyBuildArtifact {
    ...
    pub(crate) async fn build_artifacts(
        &self,
        ctx: &mut DiceComputations<'_>,
    ) -> bsmr_error::Result<Vec<ActionOutputs>> {
        let res = ctx
