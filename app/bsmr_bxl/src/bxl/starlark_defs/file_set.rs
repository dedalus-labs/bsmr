...
impl<'a> FileSetExpr<'a> {
    pub(crate) async fn get(self, bxl: &BxlContext<'_>) -> bsmr_error::Result<Cow<'a, FileSet>> {
        let set = match self {
...
impl OwnedFileSetExpr {
    ...
    ) -> bsmr_error::Result<Cow<'a, FileSet>> {
        let set = match self {
...
impl StarlarkReadDirSet {
    fn children(&self) -> bsmr_error::Result<Vec<CellPath>> {
