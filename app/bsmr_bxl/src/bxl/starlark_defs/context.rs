...
impl<'v> BxlContextType<'v> {
    fn unpack_root(&self) -> bsmr_error::Result<&'v RootBxlContextData<'_>> {
        match &self {
            BxlContextType::Root(root) => Ok(root),
            BxlContextType::Dynamic(_) => Err(bsmr_error!(
                bsmr_error::ErrorTag::Input,
                "Expected root BXL context type"
            )),
            BxlContextType::AnonTarget => Err(bsmr_error!(
                bsmr_error::ErrorTag::Input,
                "Expected root BXL context type"
            )),
        }
...
impl BxlContextCoreData {
    ...
    /// Note, unlike bsmr command line UI, we resolve targets and literals
    /// against the cell root instead of user working dir.
    pub(crate) fn working_dir(&self) -> bsmr_error::Result<ProjectRelativePathBuf> {
        let cell = self.cell_resolver().get(self.cell_name())?;
        Ok(cell.path().as_project_relative_path().to_owned())
    }

    pub(crate) fn parse_query_file_literal(&self, literal: &str) -> bsmr_error::Result<CellPath> {
        parse_query_file_literal(
            literal,
            self.cell_alias_resolver(),
            self.cell_resolver(),
            // NOTE(nga): we pass cell root as working directory here,
            //   which is inconsistent with the rest of bsmr:
            //   The same query `owner(foo.h)` is resolved using
            //   current directory in `bsmr query`, but relative to cell root in BXL.
            self.cell_root_abs(),
            self.project_root(),
        )
    }
    ...
    ) -> bsmr_error::Result<Option<TargetLabel>> {
    ...
    ) -> bsmr_error::Result<GlobalCfgOptions> {
...
impl<'v> BxlContext<'v> {
    ...
    ) -> bsmr_error::Result<(AnalysisRegistry<'v>, OutputStreamOutcome)> {
    ...
    pub(crate) fn take_state_dynamic(&self) -> bsmr_error::Result<AnalysisRegistry<'v>> {
        self.take_state_dynamic_or_anon_impl()
    }

    /// Must take an `AnalysisContext` which has never had `take_state` called on it before.
    pub(crate) fn take_state_anon(&self) -> bsmr_error::Result<AnalysisRegistry<'v>> {
        self.take_state_dynamic_or_anon_impl()
    }
}

pub(crate) trait ErrorPrinter {
    fn print_to_error_stream(&self, msg: String) -> bsmr_error::Result<()>;
}
