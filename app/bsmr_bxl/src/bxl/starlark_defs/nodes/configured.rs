...
use bsmr_analysis::analysis::env::RuleAnalysisAttrResolutionContext;
use bsmr_analysis::analysis::env::get_deps_from_analysis_results;
use bsmr_analysis::attrs::resolve::configured_attr::ConfiguredAttrExt;
use bsmr_artifact::artifact::artifact_type::Artifact;
use bsmr_artifact::artifact::source_artifact::SourceArtifact;
use bsmr_build_api::actions::query::PackageLabelOption;
use bsmr_build_api::analysis::AnalysisResult;
use bsmr_build_api::bxl::unconfigured_attribute::StarlarkCoercedAttr;
use bsmr_build_api::interpreter::rule_defs::artifact::starlark_artifact::StarlarkArtifact;
use bsmr_build_api::interpreter::rule_defs::provider::builtin::configuration_info::ConfigurationInfo;
...
use bsmr_node::attrs::attr_type::query::QueryAttr;
use bsmr_node::attrs::attr_type::transition_dep::CoercedTransitionDep;
...
fn attr_with_stripped_cfg(attr: &ConfiguredAttr) -> bsmr_error::Result<CoercedAttr> {
    Ok(match attr {
        ...
        ConfiguredAttr::SplitTransitionDep(dep) => {
            ...
            CoercedAttr::Dep(
                deps.iter()
                    .next()
                    .ok_or_else(|| {
                        bsmr_error::internal_error!("deps is empty after checking length")
                    })?
                    .dupe(),
            )
        }
            ...
            let unconfigured_parts = match &arg.string_with_macros {
                ...
                StringWithMacros::ManyParts(parts) => StringWithMacros::ManyParts(
                    parts
                        .iter()
                        .map(|part| match part {
                            bsmr_node::attrs::attr_type::arg::StringWithMacrosPart::String(s) => {
                                bsmr_node::attrs::attr_type::arg::StringWithMacrosPart::String(
                                    s.clone(),
                                )
                            }
                            bsmr_node::attrs::attr_type::arg::StringWithMacrosPart::Macro(
                                write_to_file,
                                macr,
                            ) => bsmr_node::attrs::attr_type::arg::StringWithMacrosPart::Macro(
                                *write_to_file,
                                match macr {
                                    bsmr_node::attrs::attr_type::arg::MacroBase::Location {
                                        label,
                                        dep_kind,
                                    } => bsmr_node::attrs::attr_type::arg::MacroBase::Location {
                                    ...
                                    bsmr_node::attrs::attr_type::arg::MacroBase::Exe {
                                        label,
                                        exec_dep,
                                    } => bsmr_node::attrs::attr_type::arg::MacroBase::Exe {
                                        label: label.unconfigured(),
                                    ...
                                    bsmr_node::attrs::attr_type::arg::MacroBase::UserUnkeyedPlaceholder(
                                        var_name,
                                    ) => {
                                        bsmr_node::attrs::attr_type::arg::MacroBase::UserUnkeyedPlaceholder(
                                            var_name.clone(),
                                    ...
                                    bsmr_node::attrs::attr_type::arg::MacroBase::UserKeyedPlaceholder(
                                        box_value,
                                    ) => {
                                    ...
                                    bsmr_node::attrs::attr_type::arg::MacroBase::Query(
                                        query_macro,
                                    ) => {
                                        bsmr_node::attrs::attr_type::arg::MacroBase::Query(
                                            Box::new(
                                                bsmr_node::attrs::attr_type::query::QueryMacroBase {
                                                    expansion_type: query_macro
                                                        .expansion_type
                                                        .clone(),
                                                    query: bsmr_node::attrs::attr_type::query::QueryAttrBase {
                                                        query: query_macro.query.query.clone(),
                                                        resolved_literals: bsmr_node::attrs::attr_type::query::ResolvedQueryLiterals(
                                                            query_macro
                                                                .query
                                                                .resolved_literals
                                                                .0
                                    ...
                                    bsmr_node::attrs::attr_type::arg::MacroBase::Source(path) => {
                                        bsmr_node::attrs::attr_type::arg::MacroBase::Source(
                                            path.clone(),
                                        )
                                    }
                                    bsmr_node::attrs::attr_type::arg::MacroBase::UnrecognizedMacro(
                                        macr,
                                    ) => {
                                        bsmr_node::attrs::attr_type::arg::MacroBase::UnrecognizedMacro(
                                            macr.clone(),
...
fn configured_target_node_value_methods(builder: &mut MethodsBuilder) {
    ...
    fn resolved_attrs_eager<'v>(
        ...
        let dep_analysis: bsmr_error::Result<Vec<(&ConfiguredTargetLabel, AnalysisResult)>> = ctx
            .via_dice(eval, |ctx| {
    ...
    fn sources(this: &StarlarkConfiguredTargetNode) -> starlark::Result<Vec<StarlarkArtifact>> {
        ...
        impl ConfiguredAttrTraversal for InputsCollector {
            fn dep(&mut self, _dep: &ConfiguredProvidersLabel) -> bsmr_error::Result<()> {
                Ok(())
            }

            fn input(&mut self, path: SourcePathRef) -> bsmr_error::Result<()> {
                self.inputs
    ...
    fn deps<'v>(
        this: &'v StarlarkConfiguredTargetNode,
        // ) -> bsmr_error::Result<Vec<StarlarkConfiguredTargetNode>> {
