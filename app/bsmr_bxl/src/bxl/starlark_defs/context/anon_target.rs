...
use bsmr_analysis::analysis::calculation::get_loaded_module;
use bsmr_analysis::analysis::env::get_rule_impl;
use bsmr_analysis::analysis::env::promise_artifact_mappings;
use bsmr_analysis::analysis::env::transitive_validations;
use bsmr_build_api::analysis::AnalysisResult;
use bsmr_build_api::analysis::anon_promises_dyn::RunAnonPromisesAccessor;
...
use bsmr_interpreter_for_build::attrs::StarlarkAttribute;
use bsmr_interpreter_for_build::rule::StarlarkRuleCallable;
...
async fn eval_bxl_for_anon_target_inner(
    ...
    BsmrStarlarkModule::with_profiling(|env| {
        ...
        let (bxl_ctx, list_res) = reentrant_eval.with_evaluator(|eval| {
            ...
            let list_res = tokio::task::block_in_place(|| -> bsmr_error::Result<Value<'_>> {
    ...
    for BxlAnonPromisesAccessor<'me, 'v, 'a, 'e>
{
    fn with_evaluator(
        &mut self,
        closure: &mut dyn FnMut(&mut Evaluator<'v, 'a, 'e>) -> bsmr_error::Result<()>,
    ) -> bsmr_error::Result<()> {
        closure(self.0)
    }
