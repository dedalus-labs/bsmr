...
pub(crate) fn bxl_context_methods(builder: &mut MethodsBuilder) {
    ...
    /// bsmr even when the script is cached.
    ///
    ...
    fn output<'v>(
        ...
        let output_stream = this
            ...
            .unpack_root()
            .map_err(|_| {
                bsmr_error::Error::from(BxlContextError::Unsupported("output".to_owned()))
            })?
            .output_stream;
        Ok(output_stream)
    }
    ...
    fn root<'v>(
        ...
        let _root_type = this.context_type.unpack_root().map_err(|_| {
            bsmr_error::Error::from(BxlContextError::Unsupported("root".to_owned()))
        })?;
    ...
    > {
        ...
        let res: bsmr_error::Result<_> = this.via_dice(eval, |dice| {
            dice.via(|dice| {
    ...
    fn cli_args<'v>(
        ...
        let cli_args = this
            ...
            .unpack_root()
            .map_err(|_| {
                bsmr_error::Error::from(BxlContextError::Unsupported("cli_args".to_owned()))
            })?
            .cli_args;
