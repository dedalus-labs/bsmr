...
) -> bsmr_error::Result<BxlExecutionResolution> {
...
impl<'v> BxlActions<'v> {
    ...
    ) -> bsmr_error::Result<BxlActions<'v>> {
    ...
    fn is_anon_target_or_dyn_action(&self) -> bsmr_error::Result<bool> {
...
async fn alloc_deps<'v>(
    ...
    frozen_heap: &FrozenHeap,
    ctx: &mut DiceComputations<'_>,
) -> bsmr_error::Result<ValueOfUnchecked<'v, DictType<StarlarkProvidersLabel, Dependency<'v>>>> {
    ...
    let deps: Vec<(StarlarkProvidersLabel, Dependency)> = analysis_results
        ...
        .map(|(configured, analysis_result)| {
            ...
            bsmr_error::Ok((starlark_label, dependency))
        })
        .collect::<Result<_, _>>()?;
...
fn bxl_actions_methods(builder: &mut MethodsBuilder) {
    ...
    ) -> starlark::Result<ValueOfUnchecked<'v, DictType<StarlarkProvidersLabel, Dependency<'v>>>>
    {
        if this.is_anon_target_or_dyn_action()? {
            soft_error!(
                ...
                bsmr_error!(bsmr_error::ErrorTag::Input, "Anon target or dynamic action accesses bxl.Actions.exec_deps."),
                quiet: true,
                error_on_oss: true
            )?;
        }
    ...
    ) -> starlark::Result<ValueOfUnchecked<'v, DictType<StarlarkProvidersLabel, Dependency<'v>>>>
    {
        if this.is_anon_target_or_dyn_action()? {
            soft_error!(
                "bxl_acessing_exec_platform",
                bsmr_error!(bsmr_error::ErrorTag::Input, "Anon target or dynamic action accesses bxl.Actions.toolchains."),
                quiet: true,
                error_on_oss: true
            )?;
        }
