...
use bsmr_build_api::bxl::build_result::BxlBuildResult;
...
impl EnsuredArtifactOrGroup {
    pub(crate) fn into_artifact_groups(self) -> bsmr_error::Result<Vec<ArtifactGroup>> {
        match self {
...
impl OutputStreamState {
    ...
    pub(crate) fn take_state(&self) -> bsmr_error::Result<OutputStreamOutcome> {
...
fn output_stream_methods(builder: &mut MethodsBuilder) {
    ...
    /// of a bxl script, which will be displayed to stdout by bsmr even when the script is cached.
    ...
    /// stdout by bsmr even when the script is cached.
    ///
    /// Prints that are not result of the bxl should be printed via stderr via the stdlib `print`
    /// and `pprint`.
    ///
    ...
    /// It will be displayed to stdout by bsmr even when the script is cached.
    /// Accepts an optional separator that defaults to " ".
    ///
        ...
        match artifacts {
            ...
            EnsureMultipleArtifactsArg::EnsuredArtifactArgs(list) => {
                let artifacts: Vec<EnsuredArtifact> = list.items.into_try_map(|artifact| {
                    ...
                    Ok::<EnsuredArtifact, bsmr_error::Error>(artifact)
                })?;
...
) -> bsmr_error::Result<StarlarkCommandLineInputs> {
...
) -> bsmr_error::Result<Vec<EnsuredArtifact>> {
