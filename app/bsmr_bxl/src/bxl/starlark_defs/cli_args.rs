...
impl CliArgType {
    fn coerce_value<'v>(&self, value: Value<'v>) -> bsmr_error::Result<CliArgValue> {
        Ok(match self {
    ...
    ) -> BoxFuture<'a, bsmr_error::Result<Option<CliArgValue>>> {
        async move {
            Ok(match self {
                CliArgType::Bool => clap.value_of().map_or(Ok(None), |x| {
                    let r: bsmr_error::Result<_> = try {
                        CliArgValue::Bool(x.parse::<bool>().map_err(bsmr_error::Error::from)?)
                    };
                    r.map(Some)
                })?,
                ...
                CliArgType::Float => clap.value_of().map_or(Ok(None), |x| {
                    let r: bsmr_error::Result<_> = try {
                        CliArgValue::Float({
                            x.parse::<f64>().map_err(bsmr_error::Error::from)?;
                            x.to_owned()
                        })
                    };
                    r.map(Some)
...
pub(crate) fn cli_args_module(registry: &mut GlobalsBuilder) {
    ...
    /// CLI usage:
    /// ```text
    /// bsmr bxl //my.bxl:target -- --name foo
    /// ```
    fn string<'v>(
        default: Option<Value<'v>>,
    ...
    /// ```text
    /// bsmr bxl //my.bxl:target -- --numbers 1 2 3
    /// bsmr bxl //my.bxl:target -- --numbers 1 --numbers 2 --numbers 3
    /// ```
    ///
    ...
    /// CLI usage:
    /// ```text
    /// bsmr bxl //my.bxl:target -- --verbose true
    /// ```
    fn bool<'v>(
    ...
    /// CLI usage:
    /// ```text
    /// bsmr bxl //my.bxl:target -- --count 42
    /// ```
    fn int<'v>(
        default: Option<Value<'v>>,
        #[starlark(default = "")] doc: &str,
    ...
    /// CLI usage:
    /// ```text
    /// bsmr bxl //my.bxl:target -- --threshold 3.14
    /// ```
    fn float<'v>(
        default: Option<Value<'v>>,
        #[starlark(default = "")] doc: &str,
    ...
    /// ```text
    /// bsmr bxl //my.bxl:target -- --message "hello"
    /// ```
    fn option<'v>(
        inner: &CliArgs,
        #[starlark(default = "")] doc: &str,
    ...
    /// CLI usage:
    /// ```text
    /// bsmr bxl //my.bxl:target -- --mode debug
    /// ```
    fn r#enum<'v>(
    ...
    /// CLI usage:
    /// ```text
    /// bsmr bxl //my.bxl:target -- --target cell//package:rule
    /// ```
    fn target_label<'v>(
        #[starlark(default = "")] doc: &str,
        #[starlark(require = named)] short: Option<Value<'v>>,
    ...
    /// CLI usage:
    /// ```text
    /// bsmr bxl //my.bxl:target -- --target cell//package:rule?cell//config:platform
    /// ```
    ...
    /// CLI usage:
    /// ```text
    /// bsmr bxl //my.bxl:target -- --provider cell//package:rule[subtarget]
    /// ```
    fn sub_target<'v>(
        #[starlark(default = "")] doc: &str,
        #[starlark(require = named)] short: Option<Value<'v>>,
    ...
    /// ```text
    /// bsmr bxl //my.bxl:target -- --targets cell//package/...
    /// ```
    fn target_expr<'v>(
        #[starlark(default = "")] doc: &str,
        #[starlark(require = named)] short: Option<Value<'v>>,
    ...
    /// CLI usage (accepts patterns like `cell//foo:bar`, `cell//foo:`, or `cell//foo/...`):
    /// ```text
    /// bsmr bxl //my.bxl:target -- --targets cell//package:rule?cell//config:platform
    /// ```
    ...
    /// CLI usage:
    /// ```text
    /// bsmr bxl //my.bxl:target -- --targets cell//package:rule[subtarget]
    /// ```
    ...
    /// CLI usage:
    /// ```text
    /// bsmr bxl //my.bxl:target -- --config '{"key": "value", "count": 3}'
    /// ```
    fn json<'v>(
        #[starlark(default = "")] doc: &str,
        #[starlark(require = named)] short: Option<Value<'v>>,
    ...
    /// CLI usage:
    /// ```text
    /// bsmr bxl //my.bxl:target -- --config path/to/config.json
    /// ```
    fn json_file<'v>(
        #[starlark(default = "")] doc: &str,
        #[starlark(require = named)] short: Option<Value<'v>>,
...
mod tests {
    ...
    #[test]
    fn print_cli_arg_list() -> bsmr_error::Result<()> {
        let args = vec![
    ...
    #[test]
    fn print_cli_arg_json() -> bsmr_error::Result<()> {
    ...
    #[test]
    fn coerce_starlark() -> bsmr_error::Result<()> {
        Heap::temp(|heap| {
            assert_eq!(
