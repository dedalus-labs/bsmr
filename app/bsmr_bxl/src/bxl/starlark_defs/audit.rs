...
use bsmr_build_api::audit_cell::audit_cell;
use bsmr_build_api::audit_output::AuditOutputResult;
use bsmr_build_api::audit_output::audit_output;
...
fn audit_methods(builder: &mut MethodsBuilder) {
    ...
    ///     target_platform = "foo"
    ///     result = ctx.audit().output("bsmr-out/default/art/fbcode/some_cfg_hash/path/to/__target__/artifact", target_platform)
        ...
        Ok(this.ctx.via_dice(eval, |ctx| {
            ctx.via(|ctx| {
                async move {
                    ...
                    match output {
                        None => Ok(NoneOr::None),
                        Some(result) => bsmr_error::Ok(NoneOr::Other(match result {
