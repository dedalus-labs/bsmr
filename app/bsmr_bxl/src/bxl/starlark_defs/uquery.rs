...
use bsmr_build_api::query::bxl::BxlUqueryFunctions;
use bsmr_build_api::query::bxl::NEW_BXL_UQUERY_FUNCTIONS;
...
) -> bsmr_error::Result<Box<dyn BxlUqueryFunctions>> {
...
impl<'v> StarlarkUQueryCtx<'v> {
    pub(crate) fn new(ctx: ValueTyped<'v, BxlContext<'v>>) -> bsmr_error::Result<Self> {
        Ok(Self { ctx })
    }
}
        ...
        Ok(this.ctx.via_dice(eval, |dice| {
            dice.via(|dice| {
                async {
                    let filter = filter
                        .into_option()
                        .try_map(bsmr_query_parser::parse_expr)?;
