...
enum BxlResultError {
    ...
    UnwrapOnError(bsmr_error::Error),
...
impl StarlarkError {
    pub(crate) fn new(err: bsmr_error::Error) -> Self {
        Self { err }
    }
}
...
pub(crate) enum StarlarkResultGen<T> {
    Ok(T),
    Err(#[freeze(identity)] bsmr_error::Error),
}
...
impl<T> StarlarkResultGen<T> {
    pub(crate) fn from_result(res: bsmr_error::Result<T>) -> Self {
        match res {
            Ok(val) => Self::Ok(val),
...
impl<'v, V: ValueLike<'v>> StarlarkResultGen<V> {
    fn unwrap(&self) -> bsmr_error::Result<Value<'v>> {
        match self {
    ...
    fn unwrap_err(&self) -> bsmr_error::Result<StarlarkError> {
        match self {
