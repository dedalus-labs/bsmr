...
use bsmr_build_api::interpreter::rule_defs::artifact::starlark_artifact_like::ValueAsInputArtifactLikeUnpack;
...
use bsmr_interpreter::types::package_path::StarlarkPackagePath;
...
#[derive(Debug, bsmr_error::Error, Clone)]
#[bsmr(tag = Input)]
...
impl<'v> PackagePathArg<'v> {
    fn pkg(&self, cell_alias_resolver: &CellAliasResolver) -> bsmr_error::Result<PackageLabel> {
        match self {
...
pub(crate) fn register_read_package_value_function(builder: &mut GlobalsBuilder) {
    ...
    fn read_package_value<'v>(
        ...
        let metadata_key = MetadataKeyRef::new(key).map_err(bsmr_error::Error::from)?;
...
pub(crate) fn register_read_package_visibility_functions(builder: &mut GlobalsBuilder) {
    ...
    /// given package path.
    ///
    /// Returns the same JSON-shaped value as `bsmr audit package-values`: a list of
