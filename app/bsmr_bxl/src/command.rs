...
use bsmr_build_api::build::build_report::BuildReportOpts;
use bsmr_build_api::build::build_report::write_bxl_build_report;
use bsmr_build_api::bxl::result::BxlResult;
use bsmr_build_api::bxl::result::PendingStreamingOutput;
...
use bsmr_cli_proto::BxlResponse;
...
use bsmr_interpreter::parse_import::parse_import_with_config;
...
impl BxlServerCommand {
    ...
    ) -> bsmr_error::Result<bsmr_cli_proto::BxlResponse> {
    ...
    ) -> bsmr_error::Result<BxlCommandContext<'a>> {
    ...
    async fn materialize_collected_artifacts(
        ...
        let mut errors: Vec<bsmr_error::Error> = Vec::new();
    ...
    async fn write_build_report(
        ...
        dice_ctx: &mut DiceTransaction,
        server_ctx: &dyn ServerCommandContextTrait,
        ensured_artifact_errors: Vec<bsmr_error::Error>,
        ...
        let serialized_build_report = if bxl_opts.unstable_print_build_report {
            ...
            let build_report_opts = BuildReportOpts {
                // These are all deprecated for `bsmr build`, so don't need to support them
