...
/// found in the `BsmrResourcesError`.
///
...
pub fn get<S>(name: S) -> Result<PathBuf, BsmrResourcesError>
where
    S: AsRef<str>,
{
    ...
    let (manifest_path, manifest) = MANIFEST.get_or_try_init(|| {
        let manifest_path = match env::current_exe() {
            ...
            Err(source) => {
                return Err(BsmrResourcesError::NoCurrentExe { source });
            }
        };
        ...
        let data = match fs::read(&manifest_path) {
            Ok(x) => x,
            Err(source) => {
                return Err(BsmrResourcesError::ReadFailed {
                    manifest_path,
                    source,
                });
            }
        ...
        let manifest = match ResourcesMap::new(base_dir).deserialize(deserializer) {
            Ok(x) => x,
            Err(source) => {
                return Err(BsmrResourcesError::ParsingFailed {
                    manifest_path,
                    source,
                });
            }
    ...
    if let Some(resource_path) = manifest.get(name.as_ref()) {
        dunce::canonicalize(resource_path).map_err(|source| BsmrResourcesError::BadResourcePath {
        ...
        Err(BsmrResourcesError::NoSuchResource {
            name: name.as_ref().to_owned(),
            manifest_path: manifest_path.clone(),
        })
    }
