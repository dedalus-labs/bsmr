...
transformations of [Bessemer](https://github.com/dedalus-labs/bsmr). It is
intended to offer a generic computation API that can be used beyond Bessemer,
so that any kind of incremental computation can run on DICE. All computations
are executed in parallel on DICE via tokio executors. Duplicate requests to the
same computations are deduplicated.
