...
Starlark (e.g. Bessemer) may also perform additional types of checking. In all
...
- Some degree of compatibility with Python, which allows types as expressions in
  the same places Bessemer allows them (but with different meaning and different
  checking).
