...
class ResultType(AutoName):
    """Enum for result types of bsmr test"""
...
class BsmrResult(Result):
    """
    Represents a bsmr process that has finished running and succeeded.
    If the bsmr process failed, proceeds to raise BsmrException
    """
    ...
    def __init__(
        ...
        self.bsmr_build_id = bsmr_build_id
        self.bsmr_args = bsmr_args
...
class BsmrException(Exception, BsmrResult):
    """Represents a Bsmr process that has finished running and failed."""
    ...
    def __init__(
        ...
        BsmrResult.__init__(
            self, process, stdout, stderr, bsmr_build_id, invocation_record_path
        )
    ...
    def get_exit_code(self) -> ExitCode:
        """Returns the exit code of a Bsmr Result when it exits"""
...
class BuildReport:
    """
    A parsed JSON representation of bsmr v2 build output on stdout.
    Build report is invoked on v2 builds by passing --build-report flag.
    Does not support bsmr v1.
...
class TargetsResult(BsmrResult):
    """Represents a Bsmr process of a targets command that has finished running"""

    def __init__(self, base: BsmrResult) -> None:
    ...
    def get_target_list(self) -> List[str]:
        ...
        """
        assert "--json-lines" in self.bsmr_args, (
            "Must add --json-lines arg to get targets"
        )
        targets = []
        for line in self.stdout.splitlines():
            js = json.loads(line)
            targets.append(js["bsmr.package"] + ":" + js["name"])
    ...
    def get_target_to_build_output(self) -> Dict[str, str]:
        ...
        target_to_output = {}
        assert (
            "--show-output" in self.bsmr_args or "--show-full-output" in self.bsmr_args
...
class BuildResult(BsmrResult):
    """Represents a Bsmr process of a build command that has finished running"""
    ...
    def get_target_to_build_output(self) -> Dict[str, str]:
        ...
        show_output = self.stdout.strip().splitlines()
        if "--build-report=-" in self.bsmr_args:
    ...
    def get_build_report(self) -> BuildReport:
        """
        Returns a BuildReport object for a bsmr v2 build invoked with --build-report.
...
class TestResult(BsmrResult):
    """Represents a Bsmr process  of a test command that has finished running"""

    def __init__(self, base: BsmrResult, test_output_file: Path) -> None:
        self.__dict__.update(base.__dict__)
        self.test_root = (
...
class AuditConfigResult(BsmrResult):
    """Represents a Bsmr process of an audit config command that has finished running"""

    def __init__(self, base: BsmrResult) -> None:
        self.__dict__.update(base.__dict__)

    def get_json(self) -> Dict[str, str]:
        """Returns a dict of the json sent back by bsmr"""
        assert "--style=json" in self.bsmr_args or "--style json" in self.bsmr_args, (
