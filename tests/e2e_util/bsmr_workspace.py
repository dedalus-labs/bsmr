...
async def bsmr_fixture(  # noqa C901 : "too complex"
    marker: BsmrTestMarker,
) -> AsyncGenerator[Bsmr, None]:
    """Returns a Bsmr for testing"""
    ...
    if platform.system() != "Windows":
        env["BSMRD_STARTUP_TIMEOUT"] = "120"
        env["BSMRD_STARTUP_INIT_TIMEOUT"] = "120"
    ...
    # Inform bsmr of the test timeout
    ...
    # Python tests that run Bsmr, we clear out Python test environment
    # variables so that if we run a Python test via Bsmr 2, they won't
    # interfere.
    ...
    keep_temp = os.environ.get("BSMR_E2E_KEEP_TEMP") == "1"
    ...
    # Create a temporary file to store all lines of extra bsmr config values.
    extra_config_lines = []
    ...
    try:
        ...
        if marker.inplace:
            ...
            if is_windows:
                ...
                isolation_prefix = ".bsmr_e2e" + isolation_prefix
        ...
        if isolation_prefix is not None:
            bsmr.set_isolation_prefix(isolation_prefix)
...
async def bsmr(request: pytest.FixtureRequest) -> AsyncIterator[Bsmr]:
    marker = request.node.get_closest_marker("bsmr_test")
    if marker is None:
        raise Exception(
            "Test method must be decorated with @bsmr_test() to use the bsmr fixture."
        )
    marker = marker.args[0]
    async with bsmr_fixture(marker) as bsmr:
        yield bsmr
...
def nobsmrd(fn: Callable[..., Any]) -> Callable[..., Any]:
    """Disables bsmr daemon"""

    def wrapped(fn: Callable[..., Any], bsmr: Bsmr, *args: Any, **kwargs: Any) -> Any:
        bsmr.set_bsmrd(True)
        return fn(bsmr, *args, **kwargs)
...
def _maybe_setup_prelude_and_ovr_config(path: Path) -> None:
    ...
    with Path(path, ".bsmr").open("a") as f:
        ...
        print("# Following lines are added by bsmr_workspace.py", file=f)
...
BsmrTestFn = Callable[..., Awaitable[None]]
...
def bsmr_test(
    ...
    """
    Defines a bsmr test. This is a must have decorator on all test case functions.
    ...
    Parameters:
        ...
        extra_bsmr_config:
            A optional dict of extra bsmr config to add to the test.
    ...
    if os.environ.get("BSMR_E2E_TEST_FLAVOR") == "isolated":
        ...
        if inplace is None:
            raise Exception("`inplace` must be set for `bsmr_test()`")

    # Set up arguments to use for the bsmr fixture.
...
def env(key: str, value: str) -> Callable[..., Any]:
    ...
    For example, @env("BSMR_LOG", "info")
    """

    def inner_decorator(fn: BsmrTestFn) -> Callable[..., Any]:
        async def wrapped(
            fn: BsmrTestFn, bsmr: Bsmr, *args: Any, **kwargs: Any
        ) -> None:
            bsmr.set_env(key, value)
            return await fn(bsmr, *args, **kwargs)
...
def windows_cmd_option(key: WindowsCmdOption, value: bool) -> Callable[..., Any]:
    ...
    def inner_decorator(fn: BsmrTestFn) -> Callable[..., Any]:
        async def wrapped(
            ...
            bsmr.set_windows_cmd_option(key, value)
            return await fn(bsmr, *args, **kwargs)
