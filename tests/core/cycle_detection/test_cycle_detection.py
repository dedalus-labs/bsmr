...
# It's better to fail a test than to hit our test timeout. When cycle detection is not working, bsmr will just hang. So wrap these in a timeout.
...
@bsmr_test()
async def test_detect_load_cycle(bsmr: Bsmr) -> None:
...
@bsmr_test()
async def test_detect_configured_graph_cycles(bsmr: Bsmr) -> None:
    failure = await expect_cycle(
        bsmr.cquery(
            "//:top",
            "-c",
...
@bsmr_test()
async def test_detect_configured_graph_cycles_on_recompute(bsmr: Bsmr) -> None:
    await bsmr.cquery("//:top")
...
@bsmr_test()
async def test_detect_configured_graph_cycles_2(bsmr: Bsmr) -> None:
    failure = await expect_cycle(
        bsmr.cquery(
            "//:top",
            "-c",
...
@bsmr_test()
async def test_more_recompute_cases(bsmr: Bsmr) -> None:
    await bsmr.cquery("//:top")
