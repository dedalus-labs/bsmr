# ===----------------------------------------------------------------------===
# Upstream-Source: facebook/buck2@1560aca2002865cd73d7cafb22c705cfb640b2bc
# Modifications Copyright (c) 2026 Dedalus Labs, Inc. and its contributors
# SPDX-License-Identifier: Apache-2.0
# ===----------------------------------------------------------------------===

# Copyright (c) Meta Platforms, Inc. and affiliates.
#
# This source code is dual-licensed under either the MIT license found in the
# LICENSE-MIT file in the root directory of this source tree or the Apache
# License, Version 2.0 found in the LICENSE-APACHE file in the root directory
# of this source tree. You may select, at your option, one of the
# above-listed licenses.

# pyre-strict


import typing

from bsmr.tests.e2e_util.api.bsmr import Bsmr
from bsmr.tests.e2e_util.api.bsmr_result import BsmrResult
from bsmr.tests.e2e_util.bsmr_workspace import bsmr_test


def check_rule_type_names(
    res: BsmrResult, expected_rule_type_names: typing.List[str]
) -> None:
    record = res.invocation_record()
    assert record["target_rule_type_names"] == expected_rule_type_names


@bsmr_test(write_invocation_record=True)
async def test_test_single_dep_touch(bsmr: Bsmr) -> None:
    res = await bsmr.test("//:rule1")
    check_rule_type_names(res, ["one"])


@bsmr_test(write_invocation_record=True)
async def test_test_two_out_of_order(bsmr: Bsmr) -> None:
    res = await bsmr.test("//:rule2", "//:rule1")
    check_rule_type_names(res, ["one", "two"])


@bsmr_test(write_invocation_record=True)
async def test_test_all_in_target(bsmr: Bsmr) -> None:
    res = await bsmr.test("//:")
    # Includes `not_a_test` because the recorder receives a
    # `TargetRuleTypeName` event for every CLI-resolved top-level target,
    # not only the ones that resolve to a test rule (T269576064).
    check_rule_type_names(res, ["not_a_test", "one", "two"])


@bsmr_test(write_invocation_record=True)
async def test_test_all_recursive(bsmr: Bsmr) -> None:
    res = await bsmr.test("//...")
    check_rule_type_names(res, ["not_a_test", "one", "two"])


@bsmr_test(write_invocation_record=True)
async def test_test_non_test_rule_logs_actual_rule_type(bsmr: Bsmr) -> None:
    # `bsmr test` against a non-test rule logs that rule's actual type in
    # `target_rule_type_names`.
    res = await bsmr.test("//:not_a_test_target")
    check_rule_type_names(res, ["not_a_test"])
