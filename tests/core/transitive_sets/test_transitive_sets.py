...
@bsmr_test()
async def test_transitive_sets(bsmr: Bsmr) -> None:
    rule = "//:bar"
    report = await bsmr.build(rule)
...
@bsmr_test()
async def test_transitive_set_deduplication(bsmr: Bsmr) -> None:
    await bsmr.build("//:test_duplication")


@bsmr_test()
async def test_project_as_args_with_optional_args(bsmr: Bsmr) -> None:
...
@bsmr_test()
async def test_project_as_json_with_optional_args(bsmr: Bsmr) -> None:
