...
def _write_bsmrconfig_local(bsmr: Bsmr, contents: str) -> None:
...
@bsmr_test()
async def test_agent_host_guard_disabled(bsmr: Bsmr) -> None:
...
@bsmr_test()
async def test_agent_host_guard_denied(bsmr: Bsmr) -> None:
    ...
    golden(
        output=_sanitize_denial(result.stderr, str(bsmr.cwd)),
        rel_path="golden/denied.golden.stderr",
    )


@bsmr_test()
async def test_agent_host_guard_denied_with_context(bsmr: Bsmr) -> None:
    _write_bsmrconfig_local(
        bsmr,
...
@bsmr_test()
async def test_agent_host_guard_denied_custom_isolation_dir(bsmr: Bsmr) -> None:
    ...
    bsmr.set_isolation_prefix("custom_iso")
    _write_bsmrconfig_local(
        bsmr,
        "[bsmr]\nagent_hostname_fail_v2_glob=*\n",
    )
...
@bsmr_test()
async def test_agent_host_guard_non_agent_cgroup(bsmr: Bsmr) -> None:
...
@bsmr_test()
async def test_agent_host_guard_hostname_no_match(bsmr: Bsmr) -> None:
