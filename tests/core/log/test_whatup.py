...
#
# ```
# > bsmr build //:long_build --local-only --no-remote-cache
# > cp $(bsmr log path) buildlog.meta-only.pb.zst
# ```
#
...
@bsmr_test()
async def test_whatup_command(bsmr: Bsmr) -> None:
    log = (await bsmr.log("show", LOG_SOURCE)).stdout.strip()
    ...
    ext = await bsmr.log("whatup", log_file.name)
    golden(
        output=ext.stderr,
...
@bsmr_test()
async def test_whatup_after_command(bsmr: Bsmr) -> None:
    # Get event log
    log = (await bsmr.log("show", LOG_SOURCE)).stdout.strip()
    elapsed = [0, 0]
    ...
    ext = await bsmr.log("whatup", LOG_SOURCE, "--after", str(action_start))
    golden(
        output=ext.stderr,
