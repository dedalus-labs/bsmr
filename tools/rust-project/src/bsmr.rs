...
pub(crate) fn to_project_json(
    ...
    let project_root = bsmr.resolve_project_root()?;
    ...
    for target in &targets_vec {
        ...
        // Bsmr generates __srcs directories that mirror the source layout,
...
impl Bsmr {
    ...
    ///
    /// Care should be taken to ensure that bsmr is invoked with the same set
        ...
        S: AsRef<OsStr>,
    {
        ...
        cmd.args([
            ...
            // Bsmr owner() queries stop at the innermost BUILD.bsmr file unless
            // package_boundary_exceptions is set.
            //
            // This is arguably a bug in bsmr, because it's possible for a parent BUILD.bsmr
            // file to own a file in a subdirectory that has its own BUILD.bsmr file.
            //
            // Bsmr probably didn't intend to allow this pattern: it doesn't work when you
    ...
    /// Invoke `bsmr` with the given subcommands.
    ///
    /// This method should only be used with bsmr commands that do not accept
    /// configuration options, such as `root`. [`Bsmr::command`] should be preferred.
    ...
    /// Return the absolute path of the current Bsmr project root.
    pub(crate) fn resolve_project_root(&self) -> Result<PathBuf, anyhow::Error> {
        ...
        if enabled!(Level::TRACE) {
            trace!(%stdout, "got root from bsmr");
        }
    ...
    pub(crate) fn check_saved_file(
        ...
        // unrelated bsmr project (e.g. www).
        ...
        for _ in 0..5 {
            ...
            let new_aliases =
                match self.query_aliased_targets(&alias_destinations, universe_targets) {
                    ...
                    Err(_) => {
                        warn!("bsmr cquery failed, falling back to best-effort uquery");
    ...
    fn query_aliased_dependencies(
        ...
        if enabled!(Level::TRACE) {
            for (target, info) in &raw {
                trace!(%target, ?info, "parsed target from bsmr");
            }
        }
        Ok(raw)
    }
    ...
    pub(crate) fn query_owners(
        ...
        info!(kind = "progress", ?input, "finding relevant bsmr targets");
    ...
    match output {
        Ok(Output {
            ...
            for line in stderr_str.lines() {
                if let Some(pos) = line.find("Bsmr UI") {
