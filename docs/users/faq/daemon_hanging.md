...
Let's look at how to troubleshoot when bsmr hangs, i.e. it just sits there
saying "Jobs: In progress: 0, ..." but it’s not finishing...

When bsmr hangs, there are two possibilities: It’s either hanging doing
...
To figure out which hang you have on your hands, just look at how much CPU bsmr
is using when the hang occurs using your favorite activity monitor (e.g. `top`,
`htop`). Remember that you can find the bsmr daemon’s PID using `bsmr status`.
...
a busy hang (bsmr is doing “something”) which are usually easier to debug.
...
[here](https://fb.workplace.com/groups/bsmrusers/permalink/3232782826978076/))</FbInternalOnly>
with the following stack trace:

```
#01  0x0000000005b1ec26 in <bsmr_build_api::artifact_groups::artifact_group_values::TransitiveSetIterator<bsmr_build_api::artifact_groups::artifact_group_values::ArtifactGroupValues, (bsmr_build_api::actions::artifact::Artifact, bsmr_execute::artifact_value::ArtifactValue), bsmr_build_api::artifact_groups::artifact_group_values::ArtifactValueIdentity> as core::iter::traits::iterator::Iterator>::next () from ...
#02  0x0000000005b23998 in <bsmr_build_api::artifact_groups::artifact_group_values::TransitiveSetIterator<bsmr_build_api::artifact_groups::artifact_group_values::ArtifactGroupValues, (bsmr_build_api::actions::artifact::Artifact, bsmr_execute::artifact_value::ArtifactValue), bsmr_build_api::artifact_groups::artifact_group_values::ArtifactValueIdentity> as itertools::Itertools>::exactly_one () from ...
#03  0x00000000059dbb2c in bsmr_server_commands::commands::build::create_unhashed_outputs () from ...
#04  0x0000000005c3c677 in <core::future::from_generator::GenFuture<<bsmr_server_commands::commands::build::BuildServerCommand as bsmr_server_ctx::template::ServerCommandTemplate>::command::{closure#0}> as core::future::future::Future>::poll () from ...
#05  0x00000000054c58a3 in <core::future::from_generator::GenFuture<<alloc::boxed::Box<dyn bsmr_server_ctx::ctx::ServerCommandContextTrait> as bsmr_server_ctx::ctx::ServerCommandDiceContext>::with_dice_ctx<bsmr_server_ctx::template::run_server_command<bsmr_server_commands::commands::build::BuildServerCommand>::{closure#0}::{closure#0}::{closure#0}, core::pin::Pin<alloc::boxed::Box<dyn core::future::future::Future<Output = core::result::Result<cli_proto::BuildResponse, anyhow::Error>> + core::marker::Send>>, cli_proto::BuildResponse>::{closure#0}> as core::future::future::Future>::poll () from ...
#06  0x00000000054c7ae3 in <core::future::from_generator::GenFuture<bsmr_server_ctx::template::run_server_command<bsmr_server_commands::commands::build::BuildServerCommand>::{closure#0}::{closure#0}> as core::future::future::Future>::poll () from ...
#07  0x0000000005370df8 in <bsmr_events::dispatch::Span>::call_in_span::<core::task::poll::Poll<(core::result::Result<cli_proto::BuildResponse, anyhow::Error>, bsmr_data::CommandEnd)>, <bsmr_events::dispatch::EventDispatcher>::span_async<bsmr_data::CommandStart, bsmr_data::CommandEnd, core::future::from_generator::GenFuture<bsmr_server_ctx::template::run_server_command<bsmr_server_commands::commands::build::BuildServerCommand>::{closure#0}::{closure#0}>, core::result::Result<cli_proto::BuildResponse, anyhow::Error>>::{closure#0}::{closure#0}::{closure#0}> () from ...
#08  0x00000000054f7288 in <core::future::from_generator::GenFuture<<cli::commands::daemon::BsmrdServerDependenciesImpl as bsmr_server::daemon::server::BsmrdServerDependencies>::build::{closure#0}> as core::future::future::Future>::poll () from...
 ...
```
...
you’ve found your culprit: there is indeed a bsmr bug and we’re spending ages
...
**Cycle in dependencies**: If bsmr seems to be doing nothing (e.g. CPU usage is
0%), one of the reasons could be a cycle in your dependencies, which may cause
bsmr to hang (bsmr does implement a form of cycle detection, but it
unfortunately has false negatives). You can confirm this by running legacy, which
