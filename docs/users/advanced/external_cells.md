...
Normally, bsmr requires source files to be checked into the repo. However, this
...
To help support these use cases, bsmr has a concept of "external cells."
...
Bessemer currently supports three external cell origins: `bundled`, `git`, and
`disabled`.

### The `bundled` origin

The bundled origin can only be used with the `prelude` cell, and provides access
to a copy of the prelude that is bundled as part of the bsmr binary. This is
...
- The `cells` bsmrconfig section of external cells is ignored. This is done to
  ...
  possible that running `bsmr expand-external-cell` may not produce a working
