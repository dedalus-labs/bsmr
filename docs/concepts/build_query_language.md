---
id: bsmr_query_language
title: Bsmr Query Language
---
<!-- ===----------------------------------------------------------------------=== -->
...
Bessemer's query language provides a powerful way to inspect and analyze the build
graph. The query language is shared across different query commands
(`bsmr uquery`, `bsmr cquery`, and `bsmr aquery`), though each command
operates on different graph representations and supports different sets of
operators.

## Query Parameters

The most common parameter for a Bsmr query operator is an expression that
...
- The set of targets returned by another Bsmr query operator
...
In addition to target parameters, some Bsmr query operators take string
...
when Bessemer doesn't require it.

You should always use quotes when writing scripts that construct `bsmr query`
expressions from user-supplied values.

Note that argument quoting for `bsmr query` is in addition to any quoting that
...
```sh
bsmr uquery "'//foo:bar=wiz'"
```

## Algebraic Set Operations

Bessemer's query language supports algebraic set operations for combining query
results.
...
```sh
bsmr uquery "deps('//foo:bar') intersect deps('//baz:lib')"
```

and

```sh
bsmr uquery "deps('//foo:bar') ^ deps('//baz:lib')"
```
...
If you want to invoke `bsmr query` on a list of targets, then `set()` is a way
to group this list in a query.

**Example:**

The following command line returns the target `main` in the build file in the
root of the Bessemer project and all the targets from the build file in the
`myclass` subdirectory of the root:

```sh
bsmr uquery "set( '//:main' '//myclass:' )"
```

**Example:**

The following command line returns the merged set (union) of dependencies for
the targets `main` and `subs` in the build file in the root of the Bessemer
project:

```sh
bsmr uquery "deps( set( '//:main' '//:subs' ) )"
```
...
```sh
bsmr cquery "testsof(deps(set('target1' 'target2' 'target3')))"
```
...
entire set of targets, Bessemer's query commands provide a way to repeat a query
...
```sh
bsmr cquery "testsof(deps( %s ))" target1 target2 target3
```
...
```sh
bsmr uquery "owner( %s )" main.cpp myclass.cpp myclass.h
```
...
```sh
bsmr cquery "testsof(deps(%s))" @/path/to/args-file
```
...
```sh
bsmr cquery "testsof(deps(%Ss))" @/path/to/args-file
```
...
```sh
bsmr cquery "testsof(deps(set('//foo:bar' '//foo:baz')))"
```

## Query Environments

Bessemer provides different query environments that operate on different graph
representations:
...
- [Bessemer Cheat Sheet](../users/cheatsheet.md) for practical query examples
