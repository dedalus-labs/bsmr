...
Bessemer daemon can save significant time as it enables Bsmr to share cache between
...
While it runs, the Bsmr daemon process monitors the project's file system for
changes. The Bsmr daemon excludes from monitoring any subtrees of the project
...
## Killing or disabling the Bsmr daemon

The Bsmr daemon process is killed if `bsmr clean` or `bsmr kill` commands are
