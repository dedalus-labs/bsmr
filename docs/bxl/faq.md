...
The entire BXL script is represented as a single node on the DICE graph (Bessemer’s
...
uquery, then uses the result to do a cquery and then a build, if Bessemer detects
...
- `ctx.output.print()` writes items to stdout by bsmr even when the script is
  cached. Items written to the output stream are considered to be the results of
  a BXL script, which will be displayed to stdout by bsmr even when the script
  is cached.
...
finishes executing, and Bessemer core performs some additional actions after the
BXL script.
