...
%% @format
-module(shell_bsmr_module_search).
-moduledoc """
...
find_module(Module) ->
    ...
    of
        [found] ->
            available;
        _ ->
            case shell_bsmr_utils:get_app_env(search_module) of
                {ok, Mod} ->
...
-spec find_module_source(module()) ->
    ...
    | {error, not_found | {ambiguous, [file:filename_all()]}}.
find_module_source(Module) ->
    Root = shell_bsmr_utils:cell_root(),
    io:format("use ~ts as root", [Root]),
    {ok, Output} = shell_bsmr_utils:run_command([
        "find",
        Root,
        "-type",
        "d",
    ...
    of
        ...
        Candidates ->
            ...
            {ok, RawOutput} = shell_bsmr_utils:bsmr_query(
                ~|owner(\\"%s\\")|,
                [~"--json"],
                Candidates
            ),
