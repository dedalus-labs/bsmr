...
%% @format
-module(shell_bsmr_utils).
-moduledoc """
Documentation for shell_bsmr_utils, ways to use
...
-export([
    ...
    cell_root/0,
    rebuild_modules/1,
    bsmr_build_targets/1,
    bsmr_query/1, bsmr_query/2, bsmr_query/3,
...
root(Type) ->
    case run_command(no_stderr([~"bsmr", ~"root", ~"--kind", atom_to_binary(Type)]), [{at_root, false}]) of
        {ok, Output} ->
...
rebuild_modules(Modules) ->
    ...
    {ok, RawQueryResult} = bsmr_query(~"owner(\%s)", RelSources),
    ...
    case Targets of
        [[]] ->
            ...
            error;
        _ ->
            bsmr_build_targets(Targets)
    end.

-spec bsmr_build_targets(Targets) -> ok | error when
...
-spec bsmr_query(Query) -> {ok, binary()} | error when
    Query :: string() | binary().
bsmr_query(Query) ->
    bsmr_query(Query, []).

-spec bsmr_query(Query, Args) -> {ok, binary()} | error when
    Query :: string() | binary(),
    Args :: [string() | binary()].
...
-spec get_bsmr_args_from_env() -> [binary()].
get_bsmr_args_from_env() ->
    case get_app_env(bsmr_args) of
        undefined -> [];
        {ok, BsmrArgs} when is_list(BsmrArgs) -> BsmrArgs
    end.
...
-spec get_app_env(Key) -> {ok, Value} | undefined when
    ...
    Value :: dynamic().
get_app_env(Key) ->
    _ = application:load(bsmr_shell_utils),
    % elp:ignore W0011 (application_get_env)
    application:get_env(bsmr_shell_utils, Key).
