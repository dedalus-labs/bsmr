...
// Bsmrer is a mockable interface of the bsmr functionality this package uses
type Bsmrer interface {
	Root(ctx context.Context) (string, error)
	BXL(ctx context.Context, label string, args []string) ([]byte, error)
}

type bsmrShell struct {
	cmder       Commander
	bsmrOptions []string // common flags for run/build/test/bxl commands
}
...
func (b *bsmrShell) run(ctx context.Context, args []string) ([]byte, error) {
...
func (b *bsmrShell) Root(ctx context.Context) (string, error) {
...
func (b *bsmrShell) BXL(ctx context.Context, label string, args []string) ([]byte, error) {
	data, err := b.run(ctx, slices.Concat([]string{"bxl"}, b.bsmrOptions, []string{label, "--"}, args))
...
func retriveActionErrors(bsmrStderr []byte) string {
	logPrefix := regexp.MustCompile(`^\[\S+\]\s*`)
	scanner := bufio.NewScanner(bytes.NewReader(bsmrStderr))
	sb := strings.Builder{}
