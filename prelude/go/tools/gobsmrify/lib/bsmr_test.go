...
func TestBsmrTargetNormalise(t *testing.T) {
	tests := []struct {
		name                     string
		target                   *BsmrTarget
...
func TestBsmrTargetsAddPackage(t *testing.T) {
	tests := []struct {
		name           string
		pkg            *Package
		bsmrOS         string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			targets := make(BsmrTargets)
			targets.AddPackage(tt.pkg, tt.bsmrOS, tt.bsmrArch)
			...
			// Check platform deps
			osDeps, exists := target.PlatformDeps[tt.bsmrOS]
			if !exists {
				t.Fatalf("OS deps not found for %s", tt.bsmrOS)
			}

			archDeps, exists := osDeps.ArchDeps[tt.bsmrArch]
			if !exists {
				t.Fatalf("Arch deps not found for %s", tt.bsmrArch)
			}
			...
			// Check TargetCompatibleWith
			archList, exists := target.TargetCompatibleWith[tt.bsmrOS]
			if !exists {
				t.Fatalf("TargetCompatibleWith not found for %s", tt.bsmrOS)
			}

			if !slices.Contains(archList, tt.bsmrArch) {
				t.Errorf("Arch %s not found in TargetCompatibleWith for OS %s", tt.bsmrArch, tt.bsmrOS)
			}
		})
	}
}
