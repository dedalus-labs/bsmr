...
type CollectPackagesResult struct {
	BsmrTargets BsmrTargets
	Modules     map[string]*Module
}

// CollectPackages queries go list for all platforms and collects packages into BsmrTargets and unique Modules
func CollectPackages(cfg *Config, thirdPartyDir, rootModuleName string) (*CollectPackagesResult, error) {
	...
	for _, p := range cfg.Platforms {
		...
		go func() {
			...
			for pkg := range pkgChan {
				pkgCount++
				results <- &result{pkg: pkg, bsmrOS: p.BsmrOS, bsmrArch: p.BsmrArch}
			}
	...
	for {
		...
		case res, ok := <-results:
			...
			bsmrTargets.AddPackage(res.pkg, res.bsmrOS, res.bsmrArch)
	...
	slog.Info("Packages collected", "count", len(bsmrTargets))
