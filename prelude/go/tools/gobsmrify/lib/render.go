...
func RenderBsmrFiles(cfg *Config, thirdPartyDir string, bsmrTargets BsmrTargets) error {
	tmpl1 := template.New("bsmr")
	...
	for _, target := range bsmrTargets {
		wg.Add(1)
		go func() {
			defer wg.Done()
			...
			bsmrFile := filepath.Join(thirdPartyDir, "vendor", target.ImportPath, "BUILD.bsmr")
			f, err := os.Create(bsmrFile)
			if err != nil {
			...
			if err := tmpl1.Execute(f, TemplateData{Config: cfg.Bsmr, Target: target}); err != nil {
	...
	for err := range errors {
		slog.Error("RenderBsmrFiles", "err", err)
		lastErr = err
	}
