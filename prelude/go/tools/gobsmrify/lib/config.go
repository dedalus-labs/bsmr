...
// BsmrConfig option to generate BUILD.bsmr files
type BsmrConfig struct {
	Preambule             string `json:"preambule"`                // added to the top of each BUILD.bsmr file, e.g. "# Generated by gobsmrify"
...
// PlatformConfig mapping between goos/goarch and bsmr os/arch
type PlatformConfig struct {
	GoOS     string `json:"go_os"`
	GoArch   string `json:"go_arch"`
	BsmrOS   string `json:"bsmr_os"`
	BsmrArch string `json:"bsmr_arch"`
}

// Config is the configuration for bsmrify
type Config struct {
	Bsmr        BsmrConfig       `json:"bsmr"`         // options for BUILD.bsmr files
...
func ensureDefaultValues(c *Config) {
	if c.Bsmr.GoBinaryRule == "" {
		c.Bsmr.GoBinaryRule = "go_binary"
	}
	if c.Bsmr.GoLibraryRule == "" {
		c.Bsmr.GoLibraryRule = "go_library"
	}
	if c.Bsmr.DepsAttr == "" {
		c.Bsmr.DepsAttr = "deps"
	}
	if c.Bsmr.DepsTargetLabelPrefix == "" {
		slog.Info("bsmr.deps_target_label_prefix is not set, dependencies will be formatted as raw import paths")
	}
}
