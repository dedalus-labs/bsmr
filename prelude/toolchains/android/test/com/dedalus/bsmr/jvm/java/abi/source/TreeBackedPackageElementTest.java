...
public class TreeBackedPackageElementTest extends CompilerTreeApiParameterizedTest {
  ...
  public void testCanExtendPackageFromDependencies() throws IOException {
    withClasspath(
        ImmutableMap.of(
            "com/example/bsmr/List.java",
            Joiner.on('\n').join("package com.example.bsmr;", "public class List { }")));

    compile(Joiner.on('\n').join("package com.example.bsmr;", "class Foo { }"));

    PackageElement bsmrPackage = elements.getPackageElement("com.example.bsmr");
    TypeElement listType = elements.getTypeElement("com.example.bsmr.List");
    TypeElement fooType = elements.getTypeElement("com.example.bsmr.Foo");

    assertPackageContains(bsmrPackage, listType);
    assertPackageContains(bsmrPackage, fooType);
  }
