        ...
        (annotations, roundEnv) -> {
          assertTrue(
              Boolean.parseBoolean(
                  processingEnv
                      .getOptions()
                      .getOrDefault("com.dedalus.bsmr.java.generating_abi", "false")));
          return false;
        });
  }
        ...
        (annotations, roundEnv) -> {
          if (!roundEnv.processingOver()) {
            TreeBackedTypeElement annotationType =
                (TreeBackedTypeElement) elements.getTypeElement("com.example.bsmr.FooAnno");
        ...
        (annotations, roundEnv) -> {
          if (!roundEnv.processingOver()) {
            ...
            TreeBackedTypeElement fooType =
                (TreeBackedTypeElement) elements.getTypeElement("com.example.bsmr.Foo");
        ...
        (annotations, roundEnv) -> {
          if (!roundEnv.processingOver()) {
            messager.printMessage(
                Diagnostic.Kind.ERROR, "Foo", elements.getTypeElement("com.example.bsmr.Foo"));
          }
          return false;
        });
        ...
        Collections.singletonList(
            new Processor() {
              ...
              public Set<String> getSupportedAnnotationTypes() {
                return Collections.singleton("com.example.bsmr.FooAnno");
              }
