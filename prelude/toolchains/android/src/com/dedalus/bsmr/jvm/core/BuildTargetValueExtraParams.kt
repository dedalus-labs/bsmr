...
package com.dedalus.bsmr.jvm.core
...
data class BuildTargetValueExtraParams(
  ...
  companion object {
    ...
    @JvmStatic
    fun of(buildTargetValue: BuildTargetValue, bsmrOut: RelPath): BuildTargetValueExtraParams {
      // in legacy: cell//base_path:target#flavor vs. in bsmr: cell//base_path:target[flavor]
      val legacyStyle = "(#(.*))"
      val bsmrStyle = "(\\[([^\\]]*)\\])"
      val pattern =
          Pattern.compile(String.format("(.*)//([^:]*):([^#\\[]*)(%s|%s)?", legacyStyle, bsmrStyle))
