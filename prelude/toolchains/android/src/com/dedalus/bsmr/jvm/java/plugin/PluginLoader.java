...
package com.dedalus.bsmr.jvm.java.plugin;
...
/**
 * Loads the Bsmr javac plugin JAR using the {@link ClassLoader} of a particular compiler instance.
 *
...
public final class PluginLoader implements PluginClassLoader {
  ...
  /**
   * Extracts the jar containing the Bsmr javac plugin and returns a URL that can be given to a
  ...
  private static URL extractJavacPluginJar() {
    ...
    if (resourceURL == null) {
      throw new RuntimeException("Could not find javac plugin jar; Bsmr may be corrupted.");
    } else if ("file".equals(resourceURL.getProtocol())) {
      // When Bsmr is running from the repo, the jar is actually already on disk, so no extraction
      // is necessary
      return resourceURL;
    } else {
