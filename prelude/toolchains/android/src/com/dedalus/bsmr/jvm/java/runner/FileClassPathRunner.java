...
package com.dedalus.bsmr.jvm.java.runner;
 ...
 * specified by {@code bsmr.classpath_file}. This is to sidestep commandline length limits.
 *
 * <p>There are 3 modes:
 *
 * <ul>
 *   <li>Java 8 or older launched: We manually add the entries from `bsmr.classpath_file` to the
 *       system classloader.
 *   <li>Java 9 or newer launched, and Bsmr is aware of this: We rely on `@argfile` support in newer
 *       versions of `java` to pass in our long `-classpath` argument. Here, the JVM adds all the
 *       classpath entries itself.
 *   <li>Java 9 or newer launched, and Bsmr isn't aware of this: We create a new classloader and add
 *       entries from `bsmr.classpath_file` manually. This approach has problems, e.g. classloading
 *       for in-process Java compilation does not work for Bsmr's own tests, but is provided to
...
public class FileClassPathRunner {
  public static final String CLASSPATH_FILE_PROPERTY = "bsmr.classpath_file";
  public static final String TESTRUNNER_CLASSES_PROPERTY = "bsmr.testrunner_classes";
  ...
  public static void main(String[] args) throws IOException, ReflectiveOperationException {
    ...
    if (shouldPopulateClassLoader()) {
      ...
      // not support argfiles in `java` invocations, or we're running on Java 9 or later, but Bsmr
