...
class _BsmrlerDepCppComponent(object):
    ...
    def __init__(self, dep_name, dep_cpp_info, public=False):
        ...
        if dep_cpp_info.components:
            self.components = collections.OrderedDict(
                (name, _BsmrlerDepCppComponent(dep_name, component_info))
                for (name, component_info) in dep_cpp_info.components.items()
            )
        else:
            self.components = collections.OrderedDict(
                [(dep_name, _BsmrlerDepCppComponent(dep_name, dep_cpp_info))]
            )
...
class BsmrlerGenerator(Generator):
    @property
    def filename(self):
    ...
    def content(self):
        ...
        for dep_name, dep_cpp_info in self.deps_build_info.dependencies:
            ...
            adapter_dep = _BsmrlerDepCpp(dep_name, dep_cpp_info, public=direct_dep)
            result += adapter_dep.generate()

        return result


class Bsmrler(ConanFile):
    name = "adapter"
    version = "0.1"
    description = """\
Bsmrler - Conan extension for Bessemer
