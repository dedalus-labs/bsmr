...
load(":common.bzl", "OnDuplicateEntry", "bsmr", "prelude_rule", "validate_uri")
    ...
    attrs = (
        # @unsorted-dict-items
        {
            ...
            "platform_exe": attrs.dict(
                ...
                Valid platforms are all values of the [`Platform` enum](https://dev.bsmr.build/javadoc/com/dedalus/bsmr/util/environment/Platform.html) :
            ...
            "args": attrs.list(
                ...
                A string of arguments that is passed to the executable specified by
                 ...
                 Bsmr's `string parameter macros`
                . Only the
    ...
    attrs = (
        # @unsorted-dict-items
        {
            ...
            "mode": attrs.option(
                ...
                default = None,
                doc = """
                How files are referenced internally in bsmr.
...
genrule = prelude_rule(
    ...
    examples = """
        ...
        Note you don't need to prepend execution commands with
         `python`: Bsmr knows how to execute different
        ...
        ```
          bsmr build //:generate_manifest_with_named_outputs
        ```

        ```
          bsmr build //:generate_manifest_with_named_outputs[manifest]
        ```
        ...
        | {
            ...
            "executable": attrs.option(
                ...
                Whether the output of the genrule is itself executable. Marking an output as
                 executable makes `bsmr run` and `$(exe ...)` macro
    ...
    attrs = (
        ...
        | {
            ...
            "out": attrs.option(
                ...
                default = None,
                doc = """
                An optional name to call the downloaded artifact. Bsmr will generate a default name if one is not
    ...
    attrs = (
        # @unsorted-dict-items
        {
            ...
            "size_bytes": attrs.option(
                ...
                default = None,
                doc = """
                Size in bytes of the downloaded artifact. Bsmr verifies this is correct and fails the fetch
            ...
            "type": attrs.option(
                ...
                `executable`
                 ...
                 Executable file. Bsmr will ensure that output has appropriate permissions if applicable.
...
test_suite = prelude_rule(
    ...
    docs = """
        ...
        This rule can then be given to `bsmr test`, and all tests that it depends on will be invoked.
         Note that the test\\_suite() target is not tested itself, it just tells bsmr to run other
    ...
    attrs = (
        # @unsorted-dict-items
        {
            # On legacy query, tests attribute on test_suite is treated as deps, while on bsmr it is not.
            # While bsmr's behavior makes more sense, we want to preserve legacy behavior on test_suite for now to make TD behavior match between legacy and bsmr.
...
worker_tool = prelude_rule(
    ...
    docs = """
        Some external tools have high startup costs. To amortize those costs over the whole build
         ...
         Bsmr then starts the external tool once and reuses it by communicating with it
    ...
    examples = """
        ...
        ```
         ...
         When doing a `bsmr build` on all three of the above `genrules`, Bsmr
        ...
        ```
         ...
         Bsmr then communicates with this process using JSON over `stdin`,
        ...
        ```
         ...
         Bsmr then waits for the tool to reply on `stdout`:
        ...
        ```
         ...
         Then, when building the first `genrule`, Bsmr writes to `stdin`:
        ...
        ```
         ...
         supplied by Bsmr—in this case, `/tmp/1.out` and `/tmp/1.err`.
         Once the job is done, the tool should reply to Bsmr on `stdout` with:
        ...
        ```
         ...
         Once Bsmr hears back from the first genrule's job, it submits the second genrule's job in the
         same fashion and awaits the response. When the build is all finished,
         Bsmr closes the JSON by writing to `stdin`:
        ...
        ```
         ...
         In this example, Bsmr is guaranteed to invoke
    ...
    attrs = (
        # @unsorted-dict-items
        {
            "exe": attrs.option(
                ...
                A `build target` for a rule that outputs
                 an executable, such as an `sh_binary()`.
                 Bsmr runs this executable only once per build.
            """,
            ),
            "args": attrs.one_of(
                attrs.arg(),
            ...
            "max_workers": attrs.option(
                ...
                default = None,
                doc = """
                The maximum number of workers of this type that Bsmr starts. Use `-1` to allow
            ...
            "max_workers_per_thread_percent": attrs.option(
                ...
                default = None,
                doc = """
                The maximum ratio of workers of this type that Bsmr starts per
            ...
            "persistent": attrs.option(
                ...
                default = None,
                doc = """
                If set to true, Bsmr does not restart the tool unless the tool itself changes. This means the
                 tool persists across multiple Bsmr commands without being shut down and may see the same
    ...
    attrs = (
        # @unsorted-dict-items
        {
            ...
            "hardcode_permissions_for_deterministic_output": attrs.option(
                ...
                default = None,
                doc = """
                If set to true, Bsmr hardcodes the permissions in order to ensures that all files have the same
            ...
            "on_duplicate_entry": attrs.enum(
                ...
                default = "overwrite",
                doc = """
                Action performed when Bsmr detects that zip\\_file input contains multiple entries with the same
                 name.
