...
# You can enable this with `-c bsmrconfig.log=<a single key>`, ex. `-c bsmrconfig.log=build.use_limited_hybrid`.
# To print in json, use `-c bsmrconfig.log_json=build.use_limited_hybrid`.
...
# You can also print a record for all keys with `-c bsmrconfig.log_all_in_json=true`. This prints all `read_config` calls
...
def _bsmr_config_log_keys(json: bool) -> set[(str, str)]:
    log_section_and_key = read_root_config("bsmrconfig", "log_json" if json else "log")
    if not log_section_and_key:
        return set()

    # Unfortunately, due to bsmrconfigs allowing `.`, it's possible to have multiple
    # ambiguous section/key for a single bsmrconfig, so check for that here.
    result = set()
...
_BSMRCONFIG_LOG_KEYS = _bsmr_config_log_keys(json = False)
_BSMRCONFIG_LOG_JSON_KEYS = _bsmr_config_log_keys(json = True)
_BSMRCONFIG_LOG_ALL = read_root_config("bsmrconfig", "log_all_in_json") in ("True", "true")
LOG_BSMRCONFIGS = bool(_BSMRCONFIG_LOG_KEYS or _BSMRCONFIG_LOG_JSON_KEYS or _BSMRCONFIG_LOG_ALL)

# optional fields
_BSMRCONFIG_LOG_CALLSTACK = read_root_config("bsmrconfig", "log_callstack") in ("True", "true")
_BSMRCONFIG_LOG_VALUE = read_root_config("bsmrconfig", "log_value") in ("True", "true")

def _log_read_config(read_func, section: str, key: str, default = None):
    value = read_func(section, key, default)

    maybe_value_dict = {"value": value} if _BSMRCONFIG_LOG_VALUE else {}
    maybe_callstack_dict = {"call_stack": call_stack()} if _BSMRCONFIG_LOG_CALLSTACK else {}
    if _BSMRCONFIG_LOG_ALL:
        output = {
            "starlark_log_all_bsmrconfigs": {
                "cell": get_cell_name(),
                "key": key,
                "section": section,
            }
        ...
        # This only prints if bsmrconfig is set
        # buildifier: disable=print
        print(json.encode(output))

    if _BSMRCONFIG_LOG_JSON_KEYS:
        if (section, key) in _BSMRCONFIG_LOG_JSON_KEYS:
            output = {
    ...
    if _BSMRCONFIG_LOG_KEYS:
        if (section, key) in _BSMRCONFIG_LOG_KEYS:
            # This only prints if bsmrconfig is set
...
_read_config = read_config_with_logging if LOG_BSMRCONFIGS else read_config
