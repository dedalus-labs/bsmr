...
# Bessemer Rule Basics - Interactive Tutorial

## Overview

This is an **interactive, step-by-step tutorial** that teaches Bessemer
...
- **`references/concepts.md`** - Deep dive into Bessemer core concepts including:
  - The Bessemer build model (load, configuration, analysis, execution phases)
...
- User asks "how does X work in Bessemer?" → Check `concepts.md`
...
**Goal:** Get the simplest possible Bessemer rule working.
...
output but haven't created an action to produce it. Bessemer is telling us: "You
...
"""
    ...
    # Register the action with Bessemer
    ctx.actions.run(
        cmd_args([
            "/bin/bash",
            "-c",
        ...
        category = "uppercase",  # Shows in bsmr output
    )
...
```bash
bsmr build :hello --show-full-output
...
- **Action**: A command Bessemer will execute (bash script + Python)
- **cmd_args()**: Builds command line, handles artifacts properly
- **artifact.as_output()**: Marks artifact as output (Bessemer creates parent dirs)
- **ctx.actions.run()**: Registers action - doesn't execute yet, just declares
- **Shell redirection**: Use `>` in shell scripts (Bessemer doesn't support
  redirect_stdout)
- **Lazy execution**: Bessemer decides when/if to run based on what's needed
...
1. List available targets with `bsmr targets`
...
A full Bessemer target name has three parts:
...
bsmr uquery :hello --output-attribute=src

# Configured - shows with platform config applied
bsmr cquery :hello --output-attribute=src
```
...
bsmr uquery :hello --output-attribute=output_name

# See the resolved value for your platform
bsmr cquery :hello --output-attribute=output_name
...
- **Resolution**: During configuration phase, Bessemer picks one branch
...
bsmr build :goodbye --show-full-output
...
- **Automatic ordering**: Bessemer builds deps first, ensuring outputs are ready
...
Run a comprehension check (see "Use the Socratic Method Between Major
Steps"), generating the question from what this step covered. Aim it at
how Bessemer knows to build `:hello` before `:goodbye`, and how the rule
...
```bash
bsmr build :hello_macro --show-full-output
...
- Core Bessemer primitives declared with `rule()`
- Run during analysis phase
- Create actions that produce artifacts
- Can be queried with `bsmr cquery`/`uquery`
...
macro itself doesn't appear in `bsmr cquery`/`uquery`. Wrap up only once
they show they understand.
...
**User:** "I want to learn Bessemer"

**Claude:** "Great! I'll guide you through writing your first Bessemer rule. We'll
...
**Claude:** "Perfect! Step 1 is about creating the simplest possible Bessemer
...
2. **Normalize errors**: "This error is expected!", "Let's see what Bessemer is
   telling us"
...
1. **Read the reference materials**: This skill includes detailed documentation:
   - `references/concepts.md` - Deep dive into Bessemer core concepts (build model, targets, artifacts, actions, providers, configurations, build graph)
