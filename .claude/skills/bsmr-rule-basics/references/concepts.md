...
# Bessemer Core Concepts - Deep Dive

This reference provides detailed explanations of Bessemer core concepts for users
who want to go beyond the basic tutorial.

## Table of Contents

1. [The Bessemer Build Model](#the-bsmr-build-model)
...
should be run. Bessemer decides when to actually run them.
...
**Target** is an overloaded term in Bessemer. Be precise:
...
# bsmr uquery //path:app --output-attribute=srcs
...
# bsmr cquery //path:app --output-attribute=srcs
...
Artifacts represent files in Bessemer's build model.
...
Bessemer tracks artifacts symbolically during analysis:

```starlark
output = ctx.actions.declare_output("foo.txt")
# At this point, output is just a promise - file doesn't exist yet!
# Bessemer knows: "If anyone needs foo.txt, run the action that produces it"
```

### Bound vs Unbound Artifacts

- **Bound artifact:** Has an action that produces it
- **Unbound artifact:** Declared but no action produces it (Bessemer error!)
...
ctx.actions.run(
    ...
    category = "process",        # For Bessemer UI
...
Bessemer tracks dependencies automatically:
...
Bessemer caches action results based on:
...
If inputs haven't changed, Bessemer reuses cached outputs.
...
**Used by `bsmr build`:** When you run `bsmr build //target:name`, Bessemer
...
# Access sub-targets
bsmr build //app:main[foo]  # Builds artifacts from sub_targets["foo"]
```
...
**Used by `bsmr run`:** When you run `bsmr run //target:name`, Bessemer executes
...
bsmr run //app:main -- additional_args
...
This is not actual Bessemer syntax - it's a conceptual representation.
...
1. User specifies a target: `bsmr build //app:main`
2. Bessemer applies default configuration (or user-specified one)
...
1. Bessemer walks the configured build graph
...
def my_rule_impl(ctx):
    print("Analysis!")  # Prints during bsmr build (analysis)
...
Bessemer provides tools to inspect the graph:

#### uquery - Unconfigured target graph

```bash
bsmr uquery "deps(//app:main)"        # All dependencies
bsmr uquery "rdeps(//..., //lib:foo)" # Reverse dependencies
```

#### cquery - Configured target graph

```bash
bsmr cquery "deps(//app:main)"                    # With configs applied
bsmr cquery //app:main --output-attribute=srcs   # Show resolved attributes
```

#### aquery - Action graph

```bash
bsmr aquery "deps(//app:main)"  # Show actions that will run
```

### Incremental Builds

Bessemer uses the graph for incrementality:

1. User changes `src/util.cpp`
2. Bessemer identifies affected artifacts (util.o)
...
These concepts work together to make Bessemer fast, correct, and scalable.
